...
web/                         # static site (MapLibre) for GitHub Pages
  css/                       # layout/theme/map/components CSS layers
...
> The Makefile also supports Python validation if available; otherwise falls back to `kgt`.
...
* Time slider uses ISO `start_datetime` / `end_datetime` (or `datetime`) on STAC Items
* CSS layering documented in [`web/css/README.md`](web/css/README.md)
...
* DEM assets auto-patched for size/hash during `make stac` if `.sha256` exists
...
```bash
bash scripts/collections/archaeology_sites.sh deps
bash scripts/collections/archaeology_sites.sh init
...
* Thumbnails → `web/assets/thumbnails/*.png` (attached to STAC items)
...
* **STAC Badges**: `stac-badges.yml` refreshes status badges in `web/badges/`.

---

## 🧯 Troubleshooting

* **Mermaid fails to render** → ensure labels are **quoted** and multiline uses `\n` (this README already does).
* **`kgt` missing** → install (`pip install -e .` for project; `pip install jinja2` if using templates).
* **DEM not found** → pass `DEM=/path/to/dem.tif` to `make terrain` or build the DEM COG first.
* **STAC items unparented** → ensure `stac/collections/<collection>.json` exists.
* **No thumbnails** (arch sites) → run thumbs stage, then attach (or `make arch-sites-render`).
...
5. Public viewer presets (classrooms & fieldwork)
...
* Python: `rasterio`, `rio-cogeo`, `pyproj`, `shapely`, `pystac`, `jsonschema` (opt), `jinja2` (opt), `Pillow`
* GDAL CLI: `gdal_translate`, `gdaldem`, `gdalwarp`, `gdalinfo`
* CLI: `kgt` (viewer config + validation fallback)
