...
notes: [Root README revision candidate; doc_id, owners, created date, and branch-local implementation details require verification against repo metadata, CODEOWNERS, and the active branch.]
[/KFM_META_BLOCK_V2] -->
...
- the documented repo-root shape that still needs branch verification
...
- long-form runbooks that belong under [`./docs/`][docs-dir]
...
This file may describe KFM’s governing posture, visible top-level shape, and documentation contract, but it must not imply that every policy gate, workflow, route, or runtime proof already exists on the active branch.

> [!NOTE]
> Public-facing design posture is documented. Exact merge-blocking checks, GitHub rulesets, environment approvals, deployed service coverage, and runtime enforcement depth still require branch-local verification.
...
|---|---|---|
| Upstream | [`./CONTRIBUTING.md`][contributing] | Root-level contribution contract and review discipline. |
| Upstream | [`.github/README.md`][github-readme] | Current style benchmark for directory-level README quality. |
| Upstream | [`./docs/`][docs-dir] | Long-form architecture, governance, standards, ADRs, and runbooks. |
| Upstream | [`./contracts/`][contracts-dir], [`./schemas/`][schemas-dir], [`./policy/`][policy-dir] | Machine-enforced contract and policy surfaces. |
| Downstream | [`./apps/`][apps-dir] | User-facing apps and governed runtime entrypoints. |
| Downstream | [`./packages/`][packages-dir] | Shared internal modules and reusable core logic. |
| Downstream | [`./data/`][data-dir] | Registry, truth-path artifacts, and governed data-facing structures. |
| Downstream | [`./infra/`][infra-dir] | Bring-up, deployment, runtime, and operations surfaces. |
| Downstream | [`./tests/`][tests-dir], [`./tools/`][tools-dir] | Verification, validation, reproducibility, and support tooling. |
...
### Documented repo-root tree *(verify against the active branch)*
...
| [`./apps/`][apps-dir] | Runnable services and user-facing applications |
| [`./configs/`][configs-dir] | Shared configuration that should not hard-code secrets |
| [`./contracts/`][contracts-dir] | API contracts, vocabularies, and interface surfaces |
| [`./data/`][data-dir] | Data specs, registries, examples, manifests, and governed data artifacts |
| [`./docs/`][docs-dir] | Architecture docs, ADRs, standards, runbooks, and long-form guidance |
| [`./examples/`][examples-dir] | Safe example material and demo assets |
| [`./infra/`][infra-dir] | Deployment, platform, and operational definitions |
| [`./migrations/`][migrations-dir] | Database or structure migration surfaces |
| [`./packages/`][packages-dir] | Shared libraries and internal core modules |
| [`./policy/`][policy-dir] | Policy-as-code, fixtures, and policy tests |
| [`./schemas/`][schemas-dir] | Validation schemas for ingestion, contracts, and publishing |
| [`./scripts/`][scripts-dir] | Automation and helper scripts |
| [`./tests/`][tests-dir] | Unit, integration, policy, and end-to-end tests |
...
- exact branch-local implementation depth of every service, contract family, and policy pack
...
> [!TIP]
> Treat the commands above as the minimum branch-honest inspection loop before documenting any implementation claim as fact.
...
| Deterministic identity | **CONFIRMED doctrine** | Comparable inputs and the same spec produce the same logical identity and `spec_hash`. | Unstable IDs or ambiguous lineage |
| Evidence as interface | **CONFIRMED doctrine** | Evidence must be resolvable and operational, not decorative. | Provenance trapped in disconnected notes |
| Docs as production surface | **CONFIRMED doctrine** | Behavior changes update docs, templates, tests, and runbooks together. | Silent drift between behavior and written procedure |
...
The diagram is not decorative. It encodes the operating promise that every public-facing surface remains downstream of acquisition, policy checks, provenance capture, and evidence resolution.
...
Because KFM doctrine rejects persuasive overclaiming. The root doc should stay sharp, operational, and branch-honest.
...
### Inspection basis used for this rewrite

The following inputs were available during this rewrite:

- the supplied root README draft
- the GitHub README + repo doc rules supplied in the prompt
- the previously generated revised README draft
- attached KFM doctrinal and support materials available in the workspace

A live repo checkout was **not** available in the current workspace. That means nearby README files, current workflows, CODEOWNERS contents, tests, and exact directory contents still need branch-local verification before placeholders are upgraded.
