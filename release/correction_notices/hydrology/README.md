# `release/correction_notices/hydrology/` — Hydrology Correction Notice Lane
...
`release/correction_notices/hydrology/` holds notice drafts for hydrology release corrections.
...
Hydrology notices should make time validity clear when an affected release has a date range, update cadence, or stale-state concern.

## Status & authority

| Field | Value |
|---|---|
| Document type | Hydrology correction notice README |
| Owning root | `release/` |
| Notice lane | `release/correction_notices/hydrology/` |
| Status | Draft |
...
| Required reviewers | Hydrology steward, release steward, correction steward, and data steward. |

## Path status

This README documents the path exactly requested: `release/correction_notices/hydrology/`.

Current-session evidence confirms this README existed as a blank file before this update. Current-session evidence also confirms `release/correction_notices/README.md` currently exists only as a greenfield stub. Treat this hydrology notice lane as self-contained until the parent notice lane is expanded.
...
- Time-validity and update-context notes for hydrology releases.
...
| Notice type | Meaning |
|---|---|
| `INFORMATIONAL` | Clarifies release-facing hydrology material. |
| `CORRECTION` | Explains a bounded correction. |
| `TIME_VALIDITY_UPDATE` | Explains a changed time window or validity note. |
...
- Notice ID
- Notice type
- Affected hydrology release or candidate pointer
- Affected manifest, artifact, claim, or release target
- Hydrology scope
- Time validity or stale-state posture
...
## Notice type
INFORMATIONAL / CORRECTION / TIME_VALIDITY_UPDATE / SUPERSESSION / NO_NOTICE

## Affected hydrology release or candidate
...
- [ ] Expand `release/correction_notices/README.md` so this hydrology lane inherits from a complete parent notice contract.
...
- [ ] Confirm time-validity notice vocabulary.
...
| Next review trigger | First hydrology notice, parent notice-lane expansion, correction record, changelog update, time-validity decision, or notice schema decision |
