...
version: v2.1
status: draft; repository-grounded; ROOT_FULL; canonical-root-confirmed; mixed-maturity; fixture-first release profiles; operational-release-hold; non-publication; non-authoritative
owner: NEEDS VERIFICATION — CODEOWNERS and the active machine projection route /release/ to @bartytime4life; no independent release steward, authenticated runtime release authority, enforced approver separation, or required-review enforcement was verified
created: 2026-07-03
updated: 2026-08-09
...
policy_label: "release-decision-plane; candidate-is-not-release; promotion-is-state-transition; no-payloads; no-policy-source; append-only; audit-bound; cite-or-abstain; correction-aware; withdrawal-aware; rollback-aware; fail-closed"
owning_root: release/
root_class: canonical
responsibility: explain and index release-governance records without becoming a release decision, proof, receipt, policy, schema, data payload, generated output, runtime, or publication authority
truth_posture: >-
  ...
  RollbackCard, ReleaseAliasVerification, and Rego validation surfaces, and
  current workflow holds / PROPOSED release objects and inactive profiles that
  have not crossed governed adoption or operational gates / CONFLICTED root
  policy source, domain-first, singular, generic, and empty-scaffold lanes /
  UNKNOWN production assembly, authenticated release authority, signing
  custody, operational promotion, rollback, invalidation, correction
  propagation, and public parity / NEEDS VERIFICATION named stewards, accepted
  profiles, consumer closure, ruleset coupling, hosted checks, external
  storage, and first governed release
evidence_snapshot:
...
**Quick navigation:** [Purpose](#purpose) · [Authority](#authority-level) · [Status](#status) · [Belongs and exclusions](#what-belongs-and-what-is-prohibited) · [Inputs and outputs](#inputs-outputs-and-permitted-writers) · [Exposure](#public-exposure-and-sensitivity-posture) · [Storage](#mutability-retention-generation-and-physical-storage) · [Validation](#validation) · [Review](#review-burden) · [ADRs](#adrs) · [Direct children](#current-repository-lane-map) · [Last review](#last-reviewed) · [Maturity](#current-maturity-and-readiness) · [Lifecycle](#lifecycle-boundary) · [Workflow boundaries](#workflow-readiness-boundaries) · [Validation slices](#bounded-implemented-validation-slices) · [States](#release-state-model) · [Record contract](#release-record-minimum-contract) · [Template](#minimal-release-root-record) · [Open verification](#open-verification) · [Rollback](#maintenance-correction-and-rollback)
...
Current bounded behavior exercises fixture-only review and promotion-gate semantics, including A–G closure checks and Gate G synthetic identity, authority, self-review, scope, subject, time, obligations, supersession, and hash binding. PASS means the synthetic input met the declared bounded profile.

It does not authenticate EvidenceRefs, reviewers, assignments, signatures, or external state; evaluate an accepted live release policy; bind another workflow result as proof; or emit a release transition. `release/reviews/` contains guidance and an empty Atmosphere scaffold, not accountable review records.
...
- retention posture;
- unresolved holds, obligations, and follow-up.

A missing dependency produces `HELD`, `ABSTAIN`, `DENY`, or `ERROR`; it must not be represented as completion through an empty field, filename, default allow, workflow status, or prose note. Accepted semantic contracts and machine schemas outrank this guidance.
...
Use the accepted lane contract and schema when one exists. The following is an illustrative review template, not an authority source:
...
Before merge, close or abandon the draft pull request and branch. No release state changes, so no operational rollback is needed. Remote branch deletion requires separate authority.

After an authorized merge, use a transparent revert or forward-fix documentation/provenance pull request against the actual merged commit; never rewrite shared history. Reverting this README and generated receipt restores documentation bytes only, not release, policy, schema, workflow, candidate, published artifact, cache, alias, or runtime state.
