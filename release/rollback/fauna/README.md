# `release/rollback/fauna/` — Fauna Rollback Review Lane

<!-- [KFM_META_BLOCK_V2]
doc_id: kfm://doc/release-rollback-fauna-readme
title: release/rollback/fauna/ — Fauna Rollback Review Lane
version: v1
status: draft
policy_label: public
owners:
  ...
  - <docs-steward>
updated: 2026-07-03
tags: [kfm, release, rollback, fauna, sensitivity, geoprivacy, correction, review]
[/KFM_META_BLOCK_V2] -->
...
`release/rollback/fauna/` holds fauna rollback review records.

A rollback review records whether a fauna release, candidate, manifest, correction, notice, public-facing derivative, or release-facing artifact family must be reversed, held, withdrawn, superseded, corrected, generalized, redacted, or repaired.
...
| Field | Value |
|---|---|
| Document type | Fauna rollback review README |
| Owning root | `release/` |
| Lane | `release/rollback/fauna/` |
| Status | Draft |
...
| Required reviewers | Fauna steward, release steward, policy/sensitivity reviewer, data steward, and docs steward. |

## Path status

This README documents the requested path: `release/rollback/fauna/`.

Current-session evidence confirms this README existed as a blank file before this update. Current-session evidence did not find a parent `release/rollback/README.md`. Treat this fauna sublane as draft until the parent rollback lane is created or confirmed.
...
| Scope | State the fauna release family, artifact family, layer family, taxon group, time slice, or public-surface target. |
...
| Policy review | Link policy or sensitivity review when public posture, geoprivacy, protected-resource exposure, or exact-location exposure is involved. |
...
- Fauna rollback review records.
- Fauna rollback-readiness checklists.
- Steward rollback summaries.
- Sensitivity, geoprivacy, validation, evidence, and public-surface review notes.
- Links to fauna candidates, manifests, decisions, evidence records, validation receipts, policy/sensitivity reviews, correction records, notices, changelog entries, and supersession records.
...
| `GEOPRIVACY_REQUIRED` | Public-facing output requires geoprivacy review or transform before release state can stand. |
| `SUPERSESSION_REQUIRED` | A newer governed record should replace the affected state. |
| `HOLD_FOR_REVIEW` | More evidence, validation, policy, sensitivity, geoprivacy, or steward review is required. |
...
- Geoprivacy review or transform pointer, when applicable
...
## Rollback outcome
ROLLBACK_REQUIRED / WITHDRAW_REQUIRED / CORRECTION_REQUIRED / GENERALIZATION_REQUIRED / REDACTION_REQUIRED / GEOPRIVACY_REQUIRED / SUPERSESSION_REQUIRED / HOLD_FOR_REVIEW / NO_ACTION

## Affected record
<release, candidate, manifest, correction, notice, changelog, or artifact pointer>

## Fauna scope
<release family, artifact family, layer family, taxon group, time slice, public-surface target, or N/A>

## Governed support pointers
- Evidence: <path or N/A>
- Validation: <path or N/A>
- Policy or sensitivity review: <path or N/A>
- Geoprivacy review or transform: <path or N/A>
...
## Release-facing effect
<none / held / rollback required / withdrawn / corrected / generalized / redacted / geoprivacy required / superseded / review pending>

## Rollback reason
<short reason grounded in evidence, validation, policy, sensitivity, geoprivacy, or steward review>
...
- [ ] Geoprivacy review or transform is linked when exact-location exposure is involved.
...
- [ ] No data payloads or sensitive wildlife-location material are stored in this lane.
...
```text
<YYYY-MM-DD>_<fauna-scope>_rollback-review.md
```

Examples:

```text
2026-07-03_fauna-range-summary_rollback-review.md
2026-07-03_fauna-public-surface_rollback-review.md
...
- [ ] Create or confirm `release/rollback/README.md` so this fauna lane inherits from a complete parent contract.
- [ ] Confirm whether `release/rollback/` and `release/correction/rollback/` should remain distinct lanes or be reconciled.
- [ ] Confirm CODEOWNERS for `release/rollback/fauna/`.
...
- [ ] Confirm policy, sensitivity, and geoprivacy review pointer formats.
...
| Next review trigger | Parent rollback lane creation, first fauna rollback review, correction, notice, policy/sensitivity review, geoprivacy update, changelog update, or release-state change |
