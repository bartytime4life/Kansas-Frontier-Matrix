...
policy_label: public
related: [../../README.md, ../README.md, ../cli/README.md, ../explorer-web/README.md, ../governed-api/README.md, ../api/README.md, ../api/src/api/README.md, ../api/tests/README.md, ../review-console/README.md, ../ui/README.md, ../../packages/README.md, ../../contracts/README.md, ../../schemas/README.md, ../../policy/README.md, ../../tests/README.md, ../../data/README.md, ../../infra/README.md, ../../pipelines/README.md, ../../.github/CODEOWNERS, ../../.github/workflows/README.md, ../../.github/watchers/README.md]
tags: [kfm, workers]
notes: [doc_id placeholder until registry-backed assignment exists, created/updated dates need git-history verification, public-main tree claims are inspection-bounded and should be rechecked against the target branch before publication]
[/KFM_META_BLOCK_V2] -->
...
| Sibling consumers / neighbors | [`../cli/README.md`](../cli/README.md), [`../explorer-web/README.md`](../explorer-web/README.md), [`../governed-api/README.md`](../governed-api/README.md), [`../api/README.md`](../api/README.md), [`../review-console/README.md`](../review-console/README.md), [`../ui/README.md`](../ui/README.md) |
| Worker-local downstream | **CONFIRMED on inspected public main:** only `README.md` was visible under `apps/workers/`; any child worker lanes remain **PROPOSED / NEEDS VERIFICATION** on the target branch |
...
| Execution split | `apps/workers/` stays separate from source-edge `../../pipelines/` work and from docs-only, emit-oriented watcher material in `../../.github/watchers/` |
...
| Evidence boundary | Public-main path evidence plus attached KFM doctrine; active-branch worker code, schedules, manifests, runtime behavior, and emitted receipts remain **UNKNOWN** unless directly rechecked |

> [!IMPORTANT]
> Treat this file as a **boundary contract first** and an **implementation index second**. It is allowed to say what future worker lanes must respect; it must not imply that unverified worker code already exists.
...
`apps/workers/` is the app-adjacent background execution seam for work that is safer outside request-time paths but still downstream of governed scope.

At this path, “workers” means **app-adjacent background execution**: jobs that move already-governed material forward through bounded, inspectable steps such as validation support, projection/export builds, correction propagation, and other release-backed follow-on work.

This README does **not** claim that those lanes are already implemented here. It states the boundary they must respect if and when they land here.

A practical reading rule:

| Surface | Default meaning |
|---|---|
| `../../pipelines/` | Source-edge and lane-specific execution: fetch, normalize, validate, watch, package, and emit |
| `../../.github/watchers/` | Repo-side, docs-first watcher doctrine and handoff guidance |
| `apps/workers/` | App-adjacent asynchronous follow-on work that starts from governed scope, not raw intake |
...
|---|---|
| Parent boundary | [`../README.md`](../README.md) keeps `apps/` as the runtime-facing surface family and places workers downstream of trust-bearing shell law. |
...
| [`../api/README.md`](../api/README.md) | A parallel `apps/api/` doc surface exists in the current surfaced app family; this README should not collapse that split by assumption. |
| [`../api/src/api/README.md`](../api/src/api/README.md) | Deeper synchronous API enforcement is documented elsewhere; worker code must not absorb request-time duties. |
...
| [`../ui/README.md`](../ui/README.md) | Current app-family docs surface `apps/ui/` as a placeholder child lane; do not infer worker ownership from it without branch proof. |
| `../.codex/` | Current app-family docs surface this as support/audit material, not a runtime app surface. |
...
| [`../../.github/workflows/README.md`](../../.github/workflows/README.md) | Workflow orchestration may eventually schedule or validate workers, but workflow documentation is not itself proof of active worker runtime behavior. |
...
| Lifecycle-zone handles rooted in governed state (`PROCESSED`, `CATALOG`, `PUBLISHED`, proof-pack refs) | **CONFIRMED doctrine / local placement INFERRED** | App-adjacent follow-on work should usually start from governed lifecycle edges rather than silent raw intake. |
...
> [!TIP]
> The easiest review win is a small fixture-backed job spec with a clear trigger, bounded reads, explicit outputs, and one negative test.
...
### Current app-family snapshot

This tree is a **path snapshot**, not a runtime maturity claim.

```text
apps/
├── README.md
├── .codex/                 # support / audit material, not a runtime app surface
├── api/
│   ├── README.md
│   ├── src/
│   │   ├── README.md
...
│   └── README.md
├── ui/
│   └── README.md           # placeholder child lane; durable ownership NEEDS VERIFICATION
└── workers/
    └── README.md
```

> [!NOTE]
> The surfaced documentation keeps both `apps/governed-api/` and `apps/api/` visible. It also surfaces `apps/ui/` as a placeholder child lane and `apps/.codex/` as support/audit material. This README preserves those facts instead of guessing final app topology.

### Worker-local tree

The inspected public-main directory showed only the README at this path.
...
> [!CAUTION]
> Public tree presence proves that a path exists. It does **not** prove active schedulers, emitted receipts, live workflow YAML wiring, catalog promotion, or production runtime behavior.
...
| `apps/workers/README.md` child links | **NEEDS VERIFICATION** | Add only after real child directories land on the target branch |
...
`apps/workers/` may participate in governed flow, but it must stay **downstream of release-safe scope** and **upstream of derived outputs, receipts, and visible correction propagation**. It is not the sovereign truth layer, the source-edge intake lane, the gatehouse watcher lane, or the request-time trust membrane.
...
|---|---|---|
| `apps/workers/` | App-adjacent asynchronous follow-ons after governed scope exists | **CONFIRMED path / README-only public-main depth / worker implementation depth UNKNOWN** |
| `../../pipelines/` | Lane-specific source execution, normalization, validation, packaging, and watcher-style runtime | **CONFIRMED path family / runtime maturity lane-specific** |
| `../../.github/watchers/` | Gatehouse watcher doctrine, emit-only posture, and runtime handoff guidance | **CONFIRMED path family / docs-first posture** |
| `../cli/` | Human-invoked steward/operator commands | **CONFIRMED sibling** |
| `../governed-api/` + `../api/` | Request-time trust mediation and deeper API documentation | **CONFIRMED split doc surface / final authority naming unresolved** |
...
- [ ] This README’s tree has been updated to match the live target branch.
...
Not from the inspected public-main tree. The directory path exists and currently showed `README.md`; deeper worker-local code, schedules, service wiring, or runtime behavior still need active-branch verification.

### Why not just use `pipelines/` instead?

Because `pipelines/` is the execution-family lane for source-edge and lane-local fetch, transform, validate, watch, and emit work. `apps/workers/` should stay narrower: release-backed, app-adjacent asynchronous follow-ons after governed scope already exists.

### Why not put watcher work in `.github/watchers/`?

Because `.github/watchers/` is a gatehouse watcher documentation lane. It is the place to document emit-only watcher posture and handoff boundaries, not to smuggle app runtime code into the gatehouse.
...
Adding child directories or job lanes without updating the tree, execution-surface split, sibling-boundary cues, inputs/exclusions, and “must never” rules together.
...
| Label | Meaning here |
|---|---|
| **CONFIRMED** | Directly supported by inspected public-main repo evidence, surfaced KFM doctrine, or directly visible directory-level documentation |
...
| **UNKNOWN** | Not verified strongly enough to present as current implementation reality |
| **NEEDS VERIFICATION** | Reviewer should inspect the target branch, manifests, tests, schedules, or runtime traces before treating the item as settled |
...
| `apps/ui/` | Placeholder child lane unless the target branch proves durable ownership |
