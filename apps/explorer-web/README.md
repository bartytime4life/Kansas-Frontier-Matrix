...
The same local verification exposed an inherited blank-map layout defect: MapLibre's later container rule overrode the Atlas host positioning and collapsed its height. The stage-scoped Atlas selector now preserves the existing canvas dimensions. Browser checks assert stage, host and canvas alignment in addition to `READY`; the renderer still uses the existing inline synthetic geometry and pinned dependencies. This layout correction adds no external source or rendered-feature evidence authority.
