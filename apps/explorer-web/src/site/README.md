...
The collapsed synthetic trust-state laboratory now loads its fixture bridge and finite status presenter when its disclosure opens. A load failure shows a generic announced error with a reload path and keeps its controls disabled. Its original behavior, including keyboard access and no-network fixture selection, remains covered by browser tests. A local production build moved the main JS asset from 1,246.52 kB (338.98 kB gzip) to 1,238.88 kB (337.47 kB gzip) and emitted a 9.62 kB (3.35 kB gzip) laboratory chunk. In three fresh headless Chrome contexts at 390 × 844, with service workers blocked, 100 ms emulated latency, 500,000 bytes/s download, 250,000 bytes/s upload, and 4× CPU throttling, median first-contentful paint was 1,172 ms both before and after. This local sample is not evidence of a user-visible speedup or a performance budget pass.
