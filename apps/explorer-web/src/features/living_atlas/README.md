...
- an app-local, fixture-only Living Waters frame projection with explicit `AVAILABLE`, `STALE`, `NO_RESULTS`, `UNAVAILABLE`, and `ABSTAIN` states; it is a renderer-neutral seam, not a live hydrology layer;
...
`living-waters-fixture.ts` accepts only the closed `living-waters-fixture-v1` packet identity and all-false source/release governance flags. It projects the canonical synthetic hydrograph for the bounded chart states, suppresses observations for empty, unavailable, or ambiguous joins, and binds the existing watershed slot as `CATALOG_ONLY` with `visible: false` and `rendererBound: false`. It carries the packet reference but no EvidenceBundle reference; the hydrology schema and validator remain authoritative.
