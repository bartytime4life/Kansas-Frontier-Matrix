...
from governed_api.provider import DeterministicSliceProvider, EvidenceResolution
from governed_api.routes.evidence import PINNED_FIXTURE_CITATION
...
class OverrideLayerProvider(DeterministicSliceProvider):
    def list_layers(self):
...
def test_unsupported_valid_identity_combination_fails_closed() -> None:
    ...
    assert response["status"] == "400 Bad Request"
    assert response["payload"]["outcome"] == "ERROR"
    assert response["payload"]["id"] == "kfm:ui:evidence-drawer:error-provider-scope-001"
    assert response["payload"]["reason_code"] == "PROVIDER_SCOPE_ERROR"
...
def test_provider_cannot_widen_the_one_layer_slice() -> None:
    response = _call_app(create_app(OverrideLayerProvider()), "/layers")
