...
def create_app(provider: SliceProvider | None = None):
    ...
    def application(environ, start_response):
        ...
        if (path in ROUTES or path in REQUEST_AWARE_ROUTES) and method != "GET":
        ...
        if method == "GET" and path in REQUEST_AWARE_ROUTES:
            response = REQUEST_AWARE_ROUTES[path](
                environ.get("QUERY_STRING", ""),
                bound_provider,
            )
            return _json_response(start_response, response.status, response.payload)
