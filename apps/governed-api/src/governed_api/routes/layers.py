...
def _project_feature(feature: object) -> tuple[dict, dict]:
    ...
    if feature.feature_id != FEATURE_ID or not _SAFE_IDENTIFIER.fullmatch(feature.feature_id):
        raise ValueError("invalid-provider-result")
    if feature.selection_id != SELECTION_ID or not _SAFE_IDENTIFIER.fullmatch(
...
def _project_layers(records: object) -> list[dict]:
    ...
    if not isinstance(layer, PublicLayer):
        raise ValueError("invalid-provider-result")
    if layer.source_id != SOURCE_ID or not _SAFE_IDENTIFIER.fullmatch(layer.source_id):
        raise ValueError("invalid-provider-result")
