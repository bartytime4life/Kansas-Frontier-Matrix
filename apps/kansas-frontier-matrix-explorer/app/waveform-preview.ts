...
export const buildLocalWaveformPreview = async (input: Readonly<{
  ...
  const gates: WaveformGate[] = [
    ...
    { id: "continuity", label: "Record continuity", state: continuity ? "PASS" : "BLOCK", detail: continuity ? "MiniSEED records are contiguous within the timestamp precision tolerance." : "A gap or overlap was found between MiniSEED records; the preview will not connect it as a false trace." },
