...
test("adds a no-upload KML and GeoJSON inspection preview without admission or renderer effects", async () => {
  ...
  const disguisedGeometryPreview = imports.buildLocalImportPreview({
    ...
    fileSizeBytes: 700,
    inspectedAt: "2026-08-30T18:34:00.000Z",
    supportedBounds: { west: -104.8, south: 34.8, east: -92, north: 42.2 },
  ...
  assert.equal(disguisedGeometryPreview.invalidFeatureCount, 1);
  ...
  const malformedCoordinatePreview = imports.buildLocalImportPreview({
    ...
    fileSizeBytes: 500,
    inspectedAt: "2026-08-30T18:35:00.000Z",
    supportedBounds: { west: -104.8, south: 34.8, east: -92, north: 42.2 },
  ...
  assert.equal(malformedCoordinatePreview.invalidFeatureCount, 1);
