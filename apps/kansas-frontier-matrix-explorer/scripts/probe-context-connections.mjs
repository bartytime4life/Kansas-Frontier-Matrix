...
export function validateOrigin(value) {
  ...
  if (url.username || url.password || url.pathname !== '/' || url.search || url.hash
    || (!hosted && !(local && ['http:', 'https:'].includes(url.protocol)))) {
...
async function probeOne(connection, origin, fetchImpl, timeoutMs) {
  ...
  try {
    const status = await Promise.race([execute(), new Promise((_, reject) => {
    ...
    return { ...base, status: timedOut ? 'TIMEOUT' : error?.message === 'BODY_LIMIT' ? 'BODY_LIMIT' : 'NETWORK_OR_STREAM_ERROR',
...
export async function probeContextConnections({ origin, fetchImpl = globalThis.fetch, timeoutMs = 8000 } = {}) {
  ...
  async function worker() {
    while (cursor < CONNECTIONS.length) {
      const index = cursor++;
      results[index] = await probeOne(CONNECTIONS[index], target, fetchImpl, timeoutMs);
    }
  }
