...
In **Domains + live data → Historical roads → Compare road years**, the Map Workbench accepts up to four small road-line GeoJSON files for browser-local visual study. The 21 local QUARANTINE files contain colored printed linework, not road-only centerlines or change-only segments; each edition can be viewed in a separate color and opacity. The Site does not compute or verify road changes. The files are never uploaded, persisted, added to the Layer Catalog, reported, exported, or released. Only line coordinates reach MapLibre; properties are discarded. When a file declares a source edition or PDF hash, the importer checks those fields against the selected inventory entry; this does not authenticate its geometry. A user-selected edition tag without that metadata remains unverified. A road line drawn in an edition does not establish when the road first existed.

The app's year-by-year Time Sweep remains the governed atlas clock. Road study overlays are independent so that multiple map vintages can be compared at once; the Site does not fill years without a source edition or carry the last mapped road state forward. The 21 derived local files stay outside the Site until explicitly selected in a browser on the operator's machine.
