...
Audit date: 2026-09-17; Site/repository/domain checkpoint refreshed
...
Connection checkpoint: the active Site project is
...
`91aeee1ca163bcb3f007577a541a825f60dcddc2`. The Site and repository remain
...
|---|---|---|---|---|
| P0 | Embedded Explorer shell | Current source validates stored workspaces before render, avoids passive history changes and deliberate context loss, contains renderer mutations, keeps route/global recovery UI, and passes the focused shell-survival checks | Run the remaining WebGL-capable deployed acceptance without weakening the fail-visible unsupported-browser path | Shell remains visible after corrupt-storage restore, delayed data completion, context loss, and more than one second of embedded runtime |
...
- The official context registry exposes sixteen source-specific controls, including NASA GIBS daily VIIRS NOAA-20 thermal-anomaly image context and a separate selectable, exact-UTC-day NOAA-20 vector detection feed. The image uses the provider-default daily frame; the Site does not resolve its exact UTC date. The bounded vector adapter displays Kansas-window acquisition time, confidence, radiative power, brightness, pixel dimensions, processing version, and provider ID per point, without interpreting a hotspot as a confirmed wildfire. The main map can also request a bounded rolling 24-hour NOAA HMS smoke footprint feed, a Kansas-bounded Raspberry Shake AM FDSN station inventory, and USGS 3DEP LiDAR-derived hillshade and slope raster functions. Each source has a visible endpoint, freshness axis, limitation, fallback, opacity control, and historical-frame hold.
...
| Official Kansas context adapters | Census TIGERweb 2020 decennial population/housing baseline, USGS Water Data, USGS Earthquake Hazards, NASA GIBS daily VIIRS NOAA-20 thermal-anomaly image and dated selectable detection points, NOAA HMS smoke, Raspberry Shake FDSN stations, USGS 3DEP LiDAR-derived terrain, NOAA/NWS alerts, NOAA NWM, and NOAA nowCOAST/NWS/OAR MRMS radar | `GAP-P1-012`, `GAP-P1-013`, `GAP-P2-014` | Sixteen fixed connections; bounded current/rolling context, provider-default daily fire imagery, exact-day vector fire detections and attributes, dated smoke intervals, station metadata, and optional LiDAR-derived slope/hillshade | `EXTERNAL_CONTEXT_ONLY`; excluded from KFM reports, exports, admission, release, and EvidenceBundles | `app/live-context.ts`, `app/api/live-context/route.ts`, `app/event-atlas.ts`, `app/api/event-atlas/upstream.ts`, `app/terrain-sources.ts`, `app/source-intelligence.ts`, `app/noaa-radar.ts`, `app/api/noaa-radar/frames/route.ts`, `app/page.tsx` | Fixed allowlists, bounded response budgets, exact fire acquisition dates, exact smoke interval parsing, FDSN header/coordinate validation, dynamic raster registry, temporal holds, build/type/rendered-shell checks | Provider cadence, exact image date, revisions, FDSN latency, station quality, raw-count response, work-unit identity, datum/accuracy, and emergency/life-safety boundaries remain explicit; no fire, smoke, station, raster, model, or alert layer is KFM evidence | Remove the new registry/adapter blocks and retain the prior site-local layers |
