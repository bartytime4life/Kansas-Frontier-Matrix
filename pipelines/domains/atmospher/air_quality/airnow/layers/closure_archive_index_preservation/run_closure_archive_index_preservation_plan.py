...
CAVEATS = [
    ...
    "CLOSURE_ARCHIVE_INDEX_PRESERVATION_NOT_ACTION_EXECUTION",
    "CLOSURE_ARCHIVE_INDEX_PRESERVATION_NOT_PUBLICATION",
]


def _build_packet(out: Path, created_at: str) -> str:
    packet = out / "closure_archive_index_preservation_packet.tar.gz"
    members = sorted([p for p in out.iterdir() if p.is_file() and p.name != packet.name], key=lambda p: p.name)
    ...
    return sha256_bytes(packet.read_bytes())
...
def run_closure_archive_index_preservation_plan(manifest_path, out_dir, created_at):
    ...
    for k in REQUIRED:
        v = li.get(k)
        if not v:
            errs.append(f"MISSING_LAYER31_INPUT:{k}")
            continue
        if not validate_path_safe(v):
    ...
    if not rec or rec.get("object_type_detected") != "AirNowClosureArchiveIndexAuditReceipt":
        errs.append("INVALID_LAYER31_RECEIPT_OBJECT_TYPE")
    if rec:
        ...
        for bk in ["publication_allowed", "public_release_allowed", "index_execution_allowed", "database_write_allowed", "search_service_creation_allowed", "public_catalog_creation_allowed", "closure_execution_allowed", "task_closure_allowed", "audit_closure_allowed", "governance_closure_allowed", "preservation_action_execution_allowed", "preservation_copy_allowed", "preservation_transfer_allowed", "snapshot_export_allowed", "snapshot_copy_allowed", "snapshot_transfer_allowed", "snapshot_publication_allowed", "snapshot_release_allowed", "archive_transfer_allowed", "file_deletion_allowed", "legal_hold_creation_allowed", "official_archive_certification_allowed", "command_execution_allowed", "final_accession_execution_allowed"]:
            if ro.get(bk) not in (False, None):
                errs.append(f"LAYER31_RECEIPT_DENIED_CAPABILITY_REQUIRED:{bk}")
    ...
    for r in rows:
        iid = r["input_id"]
        c = "internal_index_planning_record"
        if any(x in iid for x in ["receipt", "decision", "policy", "caveat", "hash_chain"]):
            c = "internal_audit_core"
        if "non_execution" in iid:
            c = "internal_governance_record"
    ...
    wj(out / "closure_archive_index_preservation_caveat_register.json", {"object_type": "AirNowClosureArchiveIndexPreservationCaveatRegister", "caveats": CAVEATS, "created_at": created_at})
    wj(out / "closure_archive_index_preservation_readiness_summary.json", {"object_type": "AirNowClosureArchiveIndexPreservationReadinessSummary", "readiness_status": "READY" if not errs else "DENIED", "public_release_approved": False, "created_at": created_at})
