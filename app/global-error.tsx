...
export default function GlobalError({ reset }: { error: Error & { digest?: string }; reset?: () => void }) {
  const retry = () => { if (reset) reset(); else window.location.reload(); };
  return <html lang="en"><body style={{ margin: 0, background: "#061416", color: "#eff5ef", fontFamily: "system-ui, sans-serif" }}>
    <main style={{ minHeight: "100dvh", display: "grid", placeItems: "center", padding: 24 }}>
      <section role="alert" style={{ maxWidth: 620, border: "1px solid #8b7650", borderRadius: 18, padding: 28, background: "#0a2022" }}>
        ...
        <button type="button" onClick={retry} style={{ border: 0, borderRadius: 10, padding: "11px 16px", background: "#e0ba6d", color: "#102022", fontWeight: 800 }}>Retry Explorer</button>
      </section>
    </main>
  </body></html>;
}
