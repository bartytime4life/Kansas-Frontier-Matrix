...
export const DATA_SOURCES = [...new Map([...dataSourceChoices, ...EARTH_ENGINE_DATASETS.map((dataset) => ({ id: dataset.id, title: `Earth Engine · ${dataset.title}` }))].map((source) => [source.id, source])).values()];
