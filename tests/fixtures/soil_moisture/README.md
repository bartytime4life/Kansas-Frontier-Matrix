...
tags: [kfm, tests, fixtures, soil-moisture, mesonet]
notes: [Owner is confirmed at the /tests/ scope via current CODEOWNERS coverage; exact leaf subtree, dates, and policy label remain branch-level verification items. This README is intentionally source-bounded and does not assert live fixture inventory beyond the target document itself.]
[/KFM_META_BLOCK_V2] -->

# Soil Moisture Fixtures

Deterministic, public-safe fixture lane for soil-moisture source samples and normalized records used to prove watcher, validation, anomaly, and receipt behavior without treating test data as authoritative truth.
...
> **Repo fit:** child fixture lane for reviewable soil-moisture samples that support governed tests without becoming source truth, policy truth, or release-proof storage  
...
![tree](https://img.shields.io/badge/subtree-NEEDS_VERIFICATION-yellow)
![posture](https://img.shields.io/badge/posture-public--safe%20fixtures-2ea043)

> [!IMPORTANT]
> This README is **source-bounded**. It is written to fit the repo’s current documented `tests/` posture and the current KFM watcher doctrine for soil-moisture work. It does **not** prove that the checked-out branch already contains this exact subtree, fixture inventory, runner wiring, or merge-blocking coverage.
...
| `tests/fixtures/` as a documented current parent family | **DOCUMENTED / NEEDS VERIFICATION ON ACTIVE BRANCH** | Some current repo-facing docs show `fixtures/` in the visible `tests/` tree, but the exact public-tree snapshot is not fully uniform across all surfaced materials. |
...
[Back to top](#soil-moisture-fixtures)
...
| Normalized series fixtures | compact JSON/CSV/GeoParquet-ready rows after field cleanup | Helps prove deterministic normalization and later `spec_hash` stability | **INFERRED / PROPOSED** |
...
| Secrets, API credentials, or consent-sensitive automation helpers | secret manager / host configuration | Sensitive operational material must not live in public fixture paths. |
...
grep -RIn \
  ...
  -e 'spec_hash' \
  -e 'run_receipt' \
  tests contracts schemas policy docs tools 2>/dev/null || true
```
...
2. one **failing** slice named by reason (`unordered_timestamps`, `impossible_vwc_value`, etc.)
3. one tiny note or expected-output fragment if a downstream test needs it
...
### Keep raw and derived fixture roles separate
...
flowchart LR
  ...
  C --> D[Domain checks<br/>depths · timestamps · ranges · stale state]
  D --> E[Policy-aware test use]
  E --> F[Expected decision / handoff]
  F --> G[run_receipt expectation<br/>if needed]
  G --> H[Higher-order tests<br/>policy · reproducibility · validators]

  I[Release proofs / catalogs] -. not owned here .-> H
```
...
| Deterministic shape | Replay, diff, and policy behavior depend on stable fixture content |
...
- [ ] Replace placeholder `doc_id`, dates, and `policy_label` with repo-backed values.
