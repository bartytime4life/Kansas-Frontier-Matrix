...
from tools.generators.governance_health import compile_governance_health_projection as compiler
...
def test_fixture_coverage_states():
    for case in cases():
        assert compiler.compile_projection(case["input"])["coverage_state"] == case["expected_coverage"]


def test_complete_fixture_ratios_are_derived():
    result = compiler.compile_projection(cases()[0]["input"])
...
def test_projection_is_deterministic():
    ...
    assert compiler.compile_projection(source) == compiler.compile_projection(
        json.loads(json.dumps(source))
    )


def test_output_schema_and_fixture_projections_are_valid():
    schema = json.loads(SCHEMA.read_text(encoding="utf-8"))
    Draft202012Validator.check_schema(schema)
    validator = Draft202012Validator(schema)
    for case in cases():
        assert not list(validator.iter_errors(compiler.compile_projection(case["input"])))


def test_output_validation_fails_closed():
    projection = compiler.compile_projection(cases()[0]["input"])
    projection["effects"]["published"] = True
    with pytest.raises(ValueError, match="does not conform"):
        compiler.validate_projection_output(projection)


def test_compiler_calls_output_validation(monkeypatch):
    def reject_output(_projection):
        raise ValueError("forced output rejection")

    monkeypatch.setattr(compiler, "validate_projection_output", reject_output)
    with pytest.raises(ValueError, match="forced output rejection"):
        compiler.compile_projection(cases()[0]["input"])
...
def test_wrong_source_family_fails_closed():
    ...
    with pytest.raises(ValueError):
        compiler.compile_projection(source)
...
def test_projection_never_claims_authority():
    effects = compiler.compile_projection(cases()[0]["input"])["effects"]
    assert effects == {
