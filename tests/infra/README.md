...
type: README
version: v0.3
status: draft; repository-grounded; two-module-inventory; full-static-workflow-binding; static-only; no-network-by-implementation; non-authoritative
...
truth_posture: CONFIRMED two direct test modules, nine source-defined unittest methods, one full static workflow binding, one related security workflow, and no dedicated Make target at the pinned snapshot / UNKNOWN complete infrastructure coverage, required-check status, runtime parity, accountable stewardship, correction propagation, and operational rollback
...
`tests/infra/` contains two modules and nine repository-local, no-network
...
| [`test_docker_security_overrides.py`](test_docker_security_overrides.py) | 6 | Both review-image Dockerfiles, Explorer overlay manifest and lock, and `.dockerignore` under `infra/docker/` | The three exact runtime overrides are pinned; the lock binds `tar` `7.5.22` and integrity metadata; its five checked runtime dependencies have `sha512` integrity values; the Explorer Dockerfile contains the expected npm checksum, replacement paths, exact dependency versions, and fail-closed extraction-API assertion; both images retain six bounded OS package floors and the context is an exact allowlist |
| `README.md` | — | Repository and workflow evidence | Human-maintained routing, execution, interpretation, and maintenance boundary |
| **Total** | **9** | — | Source-defined `unittest` methods; not a coverage percentage or runtime claim |
...
|---|---|---|---|
| [`infra-compose-smoke`](../../.github/workflows/infra-compose-smoke.yml) | Changes under `tests/infra/**`, `infra/compose/**`, `infra/docker/**`, or the workflow itself | Explicitly runs the two-module `tests/infra` static suite with `KFM_NO_NETWORK=1`, renders the Compose file, and builds both review images | Does not start services |
...
The full static suite is explicitly invoked by `infra-compose-smoke`; the local
command above remains the reproducible repository-only check. Do not report an
image build or scan as a substitute for that static-test evidence.
...
| Review-image security module passes | The exact Explorer overlay inputs, both image package floors, and build-context allowlist matched the tested commit | Keep the result scoped to repository inputs; run a separate build and scan when those claims matter |
...
- Dockerfile base, working directory, final user, OS package floor, Explorer
  override archive, checksum, replacement target, dependency version, API
  assertion, or `.dockerignore` allowlist;
