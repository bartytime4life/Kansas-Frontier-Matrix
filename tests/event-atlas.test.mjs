...
test("saved Worker serves the event mixer and navigable cited research without browser globals", async () => {
  ...
  assert.match(report,/Sources and references/);assert.match(report,/45c5ebad66e5a43c9f6f65f9ae8863a510331342/);assert.match(report,/HRRR flow is therefore researched but not activated/);
});
