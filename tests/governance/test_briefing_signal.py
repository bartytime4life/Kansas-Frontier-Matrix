...
VALID_FIXTURES = {
    ...
    FIXTURE_ROOT / "valid/valid_p0_corrective_override.json",
    FIXTURE_ROOT / "valid/valid_p1_source_discovery.json",
    FIXTURE_ROOT / "valid/valid_dependency_hold.json",
    FIXTURE_ROOT / "valid/valid_reject_unsafe.json",
}
INVALID_FIXTURES = {
    ...
    FIXTURE_ROOT / "invalid/invalid_priority_without_reasons.json": {
        ("BRIEFING_SIGNAL_SCHEMA_INVALID", "$.materiality"),
    },
}
...
class BriefingSignalTests(unittest.TestCase):
    ...
    def test_schema_is_closed_and_valid(self):
        ...
        self.assertFalse(schema["properties"]["materiality"]["additionalProperties"])
        self.assertFalse(schema["properties"]["routing"]["additionalProperties"])
        self.assertEqual(schema["x-kfm"]["profile_version"], "1.2.0")
    ...
    def test_declared_identity_reproduces_for_all_valid_inputs(self):
        for path in sorted(VALID_FIXTURES | EXAMPLES):
            ...
            with self.subTest(path=path.name):
                self.assertEqual(
                    candidate["identity"]["signal_digest"],
                    compute_signal_digest(candidate),
                )
                self.assertEqual(
                    candidate["signal_id"],
                    compute_signal_id(candidate),
                )
                self.assertEqual(
                    candidate["event_cluster_id"],
                    compute_event_cluster_id(candidate),
                )
                self.assertEqual(
    ...
    def test_substantive_headline_change_changes_signal_not_cluster(self):
        ...
        self.assertEqual(
            compute_event_cluster_id(changed),
            candidate["event_cluster_id"],
        )
    ...
    def test_identity_token_drift_and_digest_tampering_fail(self):
        ...
        self.assertIn(
            Finding(
                ...
                "$.identity.primary_authority_id",
            ),
            findings,
        )
        self.assertIn(
            Finding("SIGNAL_DIGEST_MISMATCH", "$.identity.signal_digest"),
            findings,
        )
        self.assertIn(
            Finding("EVENT_CLUSTER_ID_MISMATCH", "$.event_cluster_id"),
            findings,
        )
    ...
    def test_examples_remain_non_authoritative_and_issue_idempotent(self):
        for path in sorted(EXAMPLES):
            ...
            with self.subTest(path=path.name):
                ...
                self.assertEqual(
                    payload["deduplication"]["matched_issue_ids"],
                    [1647],
                )
                self.assertEqual(
                    payload["next_action"]["disposition"],
    ...
    def test_hays_candidate_does_not_claim_the_meeting_occurred(self):
        ...
        self.assertEqual(
            payload["candidate_payload"]["candidate_state"],
            "STATUS_UNCONFIRMED",
        )
        self.assertEqual(
            attributes["observed_event_state"],
            "status_unconfirmed",
        )
    ...
    def test_gmd_inventory_does_not_infer_approval_or_non_submission(self):
        ...
        gmd1 = next(
            record for record in records if record["district_id"] == "ks-gmd-1"
        )
        self.assertEqual(
            gmd1["action_plan_index_state"],
            "NOT_LISTED_ON_AUTHORITY_INDEX",
        )
    ...
    def test_parser_rejects_duplicate_nonfinite_and_nonobject_json(self):
        cases = (
            (
                b'{"signal_id":"a","signal_id":"b"}',
                Finding("BRIEFING_SIGNAL_JSON_INVALID", "$"),
            ),
            (
                b'{"score":NaN}',
                Finding("BRIEFING_SIGNAL_JSON_INVALID", "$"),
