...
MODULE_PATH = REPO_ROOT / "tools/validators/cross_domain/soil_hydrology/validate_public_safe_context.py"
SPEC = importlib.util.spec_from_file_location("soil_hydrology_public_safe_context", MODULE_PATH)
...
def test_fail_closed_cases_preserve_finite_outcomes() -> None:
    cases = _cases()
    assert cases["restricted_exact_hydrology_geometry_denied"][0]["decision"]["validator_outcome"] == "DENY"
    assert cases["missing_hydrology_evidence_abstains"][0]["decision"]["status"] == "EVIDENCE_REF_MISSING"
    assert cases["modeled_hydrology_role_requires_review"][0]["decision"]["status"] == "SOURCE_ROLE_REVIEW_REQUIRED"

def test_pair_profile_rejects_public_exact_precision_operational_claim_and_wrong_profile() -> None:
    cases = _cases()
    assert [f.code for f in cases["public_safe_exact_hydrology_geometry_fails_pair_profile"][1].findings] == ["ALLOW_GEOMETRY_NOT_GENERALIZED"]
    assert [f.code for f in cases["operational_warning_reference_fails_pair_profile"][1].findings] == ["OPERATIONAL_OR_CAUSAL_CLAIM_DENIED"]
