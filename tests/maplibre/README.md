...
type: readme
version: v0.5
status: draft; repository-grounded; executable-no-network-coverage; performance-config-contract-validated; runtime-performance-hold
...
related:
  ...
  - ../fixtures/maplibre/perf-envelope/README.md
...
readiness, retired performance harness, performance-envelope machine contract,
and synthetic scalar constraints.
...
The seven test modules define 92 test functions or `unittest` methods in source.
...
| Performance-envelope configuration | Tracked config and positive fixtures `OK`; negative fixtures `EXPECTED_FAIL` | The v1 machine shape is enforceable; threshold authority and observed performance are not proved |
...
Validate the bounded configuration contract independently of browser execution:
...
These commands are deterministic and no-network. They validate configuration,
not a renderer measurement or release decision.
...
All five workflows use read-only repository permissions. Their conclusions and
...
- [`tests/fixtures/maplibre/`](../fixtures/maplibre/) is a local test-support lane;
  ...
  authority.
- [`tests/fixtures/maplibre/perf-envelope/`](../fixtures/maplibre/perf-envelope/)
  contains synthetic configuration examples, not runtime measurements or
...
| Envelope contract test fails | Schema/config drift, fixture polarity, strict parsing, or CLI path binding | That a rejected config proves a runtime performance regression |
...
- **HOLD:** Seven trust-output performance schemas and the aggregate governance
  verifiers remain permissive placeholders; the performance-envelope machine
  schema is closed but not a benchmark or release contract.
...
including the seven direct test modules, the focused envelope schema/validator
and fixtures, five workflows,
...
not treated as current implementation or adopted authority.
