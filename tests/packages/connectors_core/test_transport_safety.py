...
from _support import SRC, profile, request, response, transport


def test_import_profile_and_repr_are_side_effect_free_exact_host_and_secret_safe(
    tmp_path,
    monkeypatch,
):
    ...
    assert not hasattr(connectors_core, "TransportProfile")
    assert not hasattr(connectors_core, "execute_retrieval")
    ...
    with pytest.raises(transport.TransportInputError):
        transport.TransportRequest(
            "GET",
            "https://user:secret@source.example.test/data",
        )
    with pytest.raises(transport.TransportInputError):
        transport.TransportRequest(
            "GET",
            "http://source.example.test/data",
        )
    with pytest.raises(transport.TransportInputError):
        profile(allowed_hosts=frozenset({"*.example.test"}))
...
def test_module_family_has_no_live_client_source_endpoint_or_lifecycle_dependency():
    source = "\n".join(
        ...
        for path in sorted(
            (SRC / "connectors_core").glob("*transport*.py")
        )
    )
    forbidden = (
        "import requests",
