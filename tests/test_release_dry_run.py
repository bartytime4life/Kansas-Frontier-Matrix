...
class ReleaseDryRunTests(unittest.TestCase):
    def test_promotion_dry_run(self):
        self.assertEqual(subprocess.run([sys.executable, "tools/promotion_dry_run.py"]).returncode, 0)

    def test_source_ledger_check(self):
        self.assertEqual(subprocess.run([sys.executable, "tools/check_source_ledger.py"]).returncode, 0)

    def test_dry_run_receipt_written(self):
        subprocess.run([sys.executable, "tools/promotion_dry_run.py"], check=True)
