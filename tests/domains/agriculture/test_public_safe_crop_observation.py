...
FIXTURE_PATH = ROOT / "fixtures/domains/agriculture/public_safe_map_feature/crop_observation.json"


def _module():
    spec = importlib.util.spec_from_file_location("ag_map_validator_crop_observation", VALIDATOR_PATH)
...
def test_crop_observation_fixture_is_generalized_observed_aggregate():
    module = _module()
    ...
    assert candidate["object_family"] == "CropObservation"
    ...
    assert candidate["sensitivity"]["parcel_owner_join"] is False
...
def test_crop_observation_rejects_derived_role_collapse():
    module = _module()
    candidate = json.loads(FIXTURE_PATH.read_text(encoding="utf-8"))
    collapsed = copy.deepcopy(candidate)
    collapsed["semantic_role"] = "DERIVED_CONTEXT"
...
def test_crop_observation_rejects_field_boundary_precision():
    module = _module()
    candidate = json.loads(FIXTURE_PATH.read_text(encoding="utf-8"))
    precise = copy.deepcopy(candidate)
    precise["field_boundary"] = "synthetic-private-boundary"

    result = module.validate_payload(precise)

    assert result.outcome == "DENY"
    assert [(finding.code, finding.path) for finding in result.findings] == [
        ("AG_MAP_HARMFUL_PRECISION_DENIED", "/field_boundary")
    ]
