...
from tools.validators.domains.atmosphere import validate_pm25_trigger_candidate_assessment as module

ROOT = Path(__file__).resolve().parents[3]
CASES = ROOT / "fixtures/contracts/v1/domains/atmosphere/pm25_trigger_candidate_assessment/cases.json"
SCHEMA = ROOT / "schemas/contracts/v1/domains/atmosphere/pm25_trigger_candidate_assessment.schema.json"
VALIDATOR = ROOT / "tools/validators/domains/atmosphere/validate_pm25_trigger_candidate_assessment.py"
...
def test_no_numeric_threshold_or_raw_concentration_is_admitted() -> None:
    schema_text = SCHEMA.read_text(encoding="utf-8")
    for forbidden in ("pm25_value", "threshold_value", "median_value", "aqi_value"):
        assert forbidden not in schema_text
    assert materialize(MANIFEST["cases"][0])["governance"]["numeric_threshold_computed"] is False
