...
def test_valid_manifest_pass(tmp_path):
 r=run_manifest('snapshot_preservation_review_valid_manifest.json',tmp_path)
 assert r['validation_outcome']=='PASS'

def test_missing_receipt_fails(tmp_path):
 r=run_manifest('snapshot_preservation_review_missing_layer20_receipt_manifest.json',tmp_path)
 assert r['validation_outcome']=='FAIL'
