...
def test_no_packet_manifest(tmp_path):
 cp,out=run_manifest("closure_archive_index_preservation_review_valid_no_packet_manifest.json",tmp_path)
 assert cp.returncode==0
 assert not (out/"closure_archive_index_preservation_review_packet.tar.gz").exists()
