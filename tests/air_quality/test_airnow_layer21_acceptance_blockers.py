...
def test_acceptance_and_blockers_written(tmp_path):
 run_manifest('snapshot_preservation_review_valid_manifest.json',tmp_path)
 assert (tmp_path/'snapshot_preservation_acceptance_candidates.jsonl').exists()
 assert (tmp_path/'residual_snapshot_preservation_blockers.jsonl').exists()
