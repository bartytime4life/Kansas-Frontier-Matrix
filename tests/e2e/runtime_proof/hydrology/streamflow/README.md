...
notes: [README-like directory doc for a proposed runtime-proof leaf. Owners, exact related paths, and mounted fixture/test inventory still need branch verification. Revised to align with the hydrology seasonal-tail doctrine, finite runtime outcomes, and the current probes-lane parent/child documentation.]
[/KFM_META_BLOCK_V2] -->
...
| **Repo fit** | child proof leaf under the runtime-proof family; consumes hydrology doctrine and probe outputs but does not own schema, policy, receipt, or publication authority |
...
> [!NOTE]
> This leaf is intentionally downstream of `tools/probes/`. A probe may emit a bounded observational result or event object, but the proof burden for outward finite runtime behavior belongs here, not in `tools/probes/`.
...
| `baseline_kind` | `day_of_year_percentile` | **PROPOSED fixture field aligned to doctrine** |
...
- visible abstention when the approved baseline is missing or unresolved
...
### Alternative more granular landing

```text
tests/e2e/runtime_proof/hydrology/streamflow/
├── README.md
├── fixtures/
│   ├── answer_released_streamflow_current.json
│   ├── abstain_streamflow_evidence_insufficient.json
│   ├── abstain_streamflow_evidence_stale.json
│   ├── deny_streamflow_unreleased_or_policy_blocked.json
│   ├── error_streamflow_resolver_failure.json
│   ├── evidence_bundle/
...
├── test_streamflow_runtime_answer.py
├── test_streamflow_runtime_abstain.py
├── test_streamflow_runtime_deny.py
└── test_streamflow_runtime_error.py
```
...
flowchart LR
  ...
  B -->|deny| DNY[RuntimeResponseEnvelope or equivalent finite DENY]
  B -->|allow or scoped| C[Evidence or event resolution]
  C -->|support resolves| D[Bounded evidence / streamflow event object]
  C -->|missing baseline or insufficient support| ABS[Finite ABSTAIN]
...
| baseline or threshold basis visible when used | Required | Required when abstaining because basis is missing or stale | Optional | Optional |
...
| Hydrology gate | Streamflow-specific checks cover discharge, comparison basis, freshness, and QC. |
