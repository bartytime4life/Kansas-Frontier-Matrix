...
FIXTURE_PATH = (
    ...
    / "context_pr_1791_terminal_divergence.json"
)
...
def test_pr_1791_is_terminal_state_divergence_from_held_projection() -> None:
...
def test_pr_1791_terminal_divergence_not_suppressed_by_non_applicability() -> None:
    context = load(FIXTURE_PATH)
    context.update(
        applicable=False,
        not_applicable_reason="classification cannot suppress untracked terminal divergence",
    )
    result = evaluate(held_state(context), context)
    assert (result.outcome_class, result.reason_code, result.blocks_merge) == (
        "REGRESSION",
