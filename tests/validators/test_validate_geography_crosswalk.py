...
from tools.validators import validate_geography_crosswalk as validator

ROOT = Path(__file__).resolve().parents[2]


class GeographyCrosswalkTests(unittest.TestCase):
    ...
    def test_positive_cases_cover_finite_mapping_relations(self) -> None:
        manifest = validator.load_fixtures()
        passing = [
        ...
        self.assertEqual(
            {"EXACT", "SPLIT", "MERGE", "PARTIAL_OVERLAP", "UNMAPPED"},
            {row["relation"] for item in passing for row in item["mappings"]},
        )
    ...
    def test_fixture_contains_only_digest_feature_identities_and_no_geometry(self) -> None:
        manifest = validator.load_fixtures()
        document = validator.materialize_case(manifest, manifest["cases"][0])
        row = document["mappings"][0]
        self.assertTrue(row["source_feature_id_digest"].startswith("sha256:"))
        self.assertTrue(row["targets"][0]["target_feature_id_digest"].startswith("sha256:"))
        serialized = json.dumps(document, sort_keys=True).lower()
        for forbidden in ("coordinate", "geometry", "county_name", "source_payload"):
    ...
    def test_identity_is_content_addressed_and_replay_stable(self) -> None:
        ...
        changed["mappings"][0]["targets"][0]["weight_millionths"] = 999999
    ...
    def test_serialization_does_not_echo_payload_values(self) -> None:
        manifest = validator.load_fixtures()
        document = validator.materialize_case(manifest, manifest["cases"][0])
        sentinel = "do-not-echo-crosswalk-value"
        document["scope"]["method_profile_ref"] = (
            "kfm://method/" + sentinel + "@sha256:" + "f" * 64
        )
        result = validator.validate_payload(document)
    ...
    def test_cli_fixture_replay_and_parser_error_are_deterministic(self) -> None:
        ...
        self.assertIn('"cases":25', first.stdout)
    ...
    def test_duplicate_nonfinite_symlink_and_oversize_are_errors(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            ...
            for path, code in (
                (duplicate, "CROSSWALK_JSON_DUPLICATE_KEY"),
                (nonfinite, "CROSSWALK_JSON_NONFINITE_NUMBER"),
                (link, "CROSSWALK_INPUT_SYMLINK_DENIED"),
                (oversized, "CROSSWALK_INPUT_TOO_LARGE"),
            ):
