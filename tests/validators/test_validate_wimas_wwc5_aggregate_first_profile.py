...
from tools.validators import validate_wimas_wwc5_aggregate_first_profile as M
class Tests(unittest.TestCase):
 def test_schema(self):M.Draft202012Validator.check_schema(json.loads(M.SCHEMA_PATH.read_text()))
 def test_fixtures(self):
  ok,p=M.run_fixture_suite();self.assertTrue(ok,p);self.assertEqual(p["counts"],{"PASS":1,"ABSTAIN":3,"DENY":7,"ERROR":4})
