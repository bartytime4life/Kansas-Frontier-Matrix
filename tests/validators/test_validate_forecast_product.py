...
VALIDATOR_PATH = REPO_ROOT / "tools/validators/validate_forecast_product.py"
SCHEMA_PATH = REPO_ROOT / "schemas/contracts/v1/common/forecast_product.schema.json"

spec = importlib.util.spec_from_file_location("validate_forecast_product", VALIDATOR_PATH)
...
class ForecastProductTests(unittest.TestCase):
    @classmethod
    ...
    def test_identity_is_stable_across_mapping_order(self) -> None:
        candidate = self.by_name["valid_model_current"][1]
        reordered = {
            key: candidate[key]
        ...
        self.assertEqual(
            validator.expected_forecast_id(reordered),
            candidate["forecast_product_id"],
        )

    def test_forecast_role_and_prediction_support_are_mandatory(self) -> None:
        expected = {
        ...
        valid = self.by_name["valid_model_current"][1]
        self.assertEqual(valid["source_role"], "FORECAST")
        self.assertEqual(valid["support_type"], "PREDICTION")

    def test_model_expert_and_hybrid_methods_are_explicit(self) -> None:
        expected = {
            "valid_model_current": "MODEL",
            "valid_expert_current": "EXPERT",
            "valid_hybrid_corrected": "HYBRID",
        }
        for name, method_kind in expected.items():
            candidate = self.by_name[name][1]
            self.assertEqual(candidate["method"]["kind"], method_kind)
            self.assertTrue(validator.validate_payload(candidate).ok)

        for name, code in (
            ("model_method_missing_model", "METHOD_MODEL_CONTEXT_REQUIRED"),
            ("expert_method_model_conflict", "METHOD_EXPERT_CONTEXT_CONFLICT"),
        ):
            result = validator.validate_payload(self.by_name[name][1])
            self.assertEqual(result.outcome, "DENY")
            self.assertEqual([finding.code for finding in result.findings], [code])

    def test_forecast_time_dimensions_remain_distinct(self) -> None:
        expected = {
            "data_cutoff_after_issuance": "DATA_CUTOFF_AFTER_ISSUANCE",
            "validity_before_issuance": "VALIDITY_BEFORE_ISSUANCE",
    ...
    def test_uncertainty_is_explicit_and_fail_closed(self) -> None:
        expected = {
            "uncertainty_confidence_overclaim": "UNCERTAINTY_CONFIDENCE_OVERCLAIM",
    ...
    def test_cli_pass_is_value_free_and_all_authority_flags_are_false(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            ...
            path.write_text(
                json.dumps(self.by_name["valid_model_current"][1]),
                encoding="utf-8",
            )
        ...
        self.assertNotIn("synthetic-weather-service", completed.stdout)
        self.assertNotIn("air_temperature_max", completed.stdout)
