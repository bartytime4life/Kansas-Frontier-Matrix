...
from tools.validators.evidence import validate_access_observation as validator
...
class AccessObservationTests(unittest.TestCase):
    ...
    def test_positive_cases_cover_families_states_zero_service_and_correction(self) -> None:
        manifest = validator.load_fixtures()
        passing = [
        ...
        self.assertIn("SOURCE_DEFINED", {item["measure"]["service_domain"] for item in passing})
        self.assertIn("EMERGENCY_SERVICES", {item["measure"]["service_domain"] for item in passing})
    ...
    def test_fixture_has_no_provider_route_geometry_or_eligibility_payload(self) -> None:
        manifest = validator.load_fixtures()
    ...
    def test_bounded_reader_rejects_duplicate_keys_and_non_object_roots(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            ...
            self.assertEqual("ACCESS_JSON_DUPLICATE_KEY", findings[0].code)

            root = Path(directory) / "root.json"
            root.write_text("[]", encoding="utf-8")
            value, findings = validator._read(root)
            self.assertIsNone(value)
            self.assertEqual("ACCESS_JSON_ROOT_INVALID", findings[0].code)
    ...
    def test_serialized_result_disclaims_authority(self) -> None:
        ...
        self.assertIn("no_route_computation", payload["non_effects"])
        self.assertIn("no_eligibility_or_emergency_guidance", payload["non_effects"])
