...
MODULE_PATH = ROOT / "tools/validators/evidence/validate_feature_transform_receipt.py"
SPEC = importlib.util.spec_from_file_location("feature_transform_receipt_validator", MODULE_PATH)
...
class FeatureTransformReceiptTests(unittest.TestCase):
    @classmethod
    ...
    def test_schema_is_valid_closed_payload_free_and_inactive(self) -> None:
        ...
        self.assertFalse(schema["x-kfm"]["feature_values_allowed"])
        self.assertNotIn("feature_values", schema["properties"])
    ...
    def test_pass_cases_remain_held_and_non_authoritative(self) -> None:
        for name in ("pass_scaling_and_selection", "pass_normalization", "pass_encoding"):
    ...
    def test_unresolved_declarations_abstain(self) -> None:
        for name in (
            ...
            "abstain_parameter_unresolved",
            "abstain_split_unresolved",
            "abstain_fit_scope_unresolved",
        ):
            self.assertEqual(MODULE.validate_candidate(self._candidate(name)).outcome, "ABSTAIN")

    def test_chain_count_state_selection_and_leakage_fail_closed(self) -> None:
        expected = {
            "deny_chain_start_mismatch": ["TRANSFORM_CHAIN_MISMATCH"],
            "deny_chain_gap": ["TRANSFORM_CHAIN_MISMATCH"],
            "deny_final_output_mismatch": ["TRANSFORM_CHAIN_MISMATCH"],
            "deny_feature_count_mismatch": ["FEATURE_COUNT_MISMATCH"],
            "deny_selection_not_reductive": ["FEATURE_SELECTION_NOT_REDUCTIVE"],
            "deny_output_state_mismatch": ["FEATURE_STATE_MISMATCH"],
            "deny_evaluation_leakage": ["EVALUATION_LEAKAGE_RISK"],
        }
        for name, codes in expected.items():
            self.assertEqual(MODULE.validate_candidate(self._candidate(name)).codes, codes)

    def test_canonical_review_time_and_hash_guards(self) -> None:
        expected = {
            "deny_noncanonical_evidence_refs": ["REFERENCE_ARRAY_NOT_CANONICAL"],
            "deny_public_review_missing": ["PUBLIC_CANDIDATE_REVIEW_REFERENCE_MISSING"],
    ...
    def test_profile_hash_replays_and_binds_transform_semantics(self) -> None:
        candidate = self._candidate("pass_scaling_and_selection")
        self.assertEqual(candidate["profile_spec_hash"], MODULE.compute_profile_hash(candidate))
        changed = copy.deepcopy(candidate)
        changed["transforms"][0]["fitted_on"] = "EXTERNAL_REFERENCE"
