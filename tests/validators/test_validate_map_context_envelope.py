...
class Tests(unittest.TestCase):
    ...
    def test_schema_closed_and_bound(self):
        s=json.loads(SCHEMA.read_text()); Draft202012Validator.check_schema(s); self.assertFalse(s["additionalProperties"]); self.assertEqual(s["x-kfm"]["contract_doc"],"contracts/ui/map_context_envelope.md"); p=s["$defs"]["permalink_policy"]; self.assertEqual((p["properties"]["mode"]["const"],p["properties"]["outcome"]["const"]),("DISABLED","DENY")); self.assertFalse(p["properties"]["raw_envelope_serialization"]["const"]); self.assertFalse(p["properties"]["exact_location_state_allowed"]["const"]); self.assertFalse(p["properties"]["restricted_context_allowed"]["const"])
    ...
    def test_schema_and_semantic_negative_boundary(self):
        ...
        schema_neg=[c for c in self.cases if c["case_kind"]=="SCHEMA_NEGATIVE"]; semantic=[c for c in self.cases if c["case_kind"]=="SEMANTIC_NEGATIVE"]; self.assertEqual((len(schema_neg),len(semantic)),(7,10))
    ...
    def test_permalink_policy_is_fail_closed_and_identity_bound(self):
        obj=self.record(self.by_id("valid-view")); policy=obj["permalink_policy"]; self.assertEqual(policy,{"exact_location_state_allowed":False,"mode":"DISABLED","outcome":"DENY","raw_envelope_serialization":False,"reason_codes":["PERMALINK_SERIALIZER_NOT_ADMITTED"],"restricted_context_allowed":False}); prior=MODULE._identity_hash(obj); obj.pop("permalink_policy"); self.assertNotEqual(MODULE._identity_hash(obj),prior)
    ...
    def test_no_network_replay_and_cli(self):
        ...
        c=subprocess.run([sys.executable,str(VALIDATOR),"--fixtures"],cwd=ROOT,capture_output=True,text=True,check=False); self.assertEqual(c.returncode,0,c.stdout+c.stderr); self.assertEqual(len(c.stdout.splitlines()),19); self.assertNotIn('"suite_match":false',c.stdout)
