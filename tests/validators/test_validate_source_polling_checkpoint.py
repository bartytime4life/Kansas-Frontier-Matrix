...
ROOT=Path(__file__).resolve().parents[2];TOOL=ROOT/"tools/validators/source/validate_source_polling_checkpoint.py";FIX=ROOT/"fixtures/contracts/v1/source/source_polling_checkpoint/cases.json";SCHEMA=ROOT/"schemas/contracts/v1/source/source_polling_checkpoint.schema.json"
spec=importlib.util.spec_from_file_location("poll",TOOL);M=importlib.util.module_from_spec(spec);sys.modules[spec.name]=M;spec.loader.exec_module(M)
class Tests(unittest.TestCase):
 ...
 def test_polarity(self):
  o=[x["expected_outcome"] for x in self.m["cases"]];self.assertEqual((o.count("PASS"),o.count("ABSTAIN"),o.count("ERROR"),o.count("DENY")),(2,2,1,10))
 def test_cases(self):
 ...
 def test_candidate_is_nonexecuting(self):
  d=M.materialize_case(self.m,self.c["modified-pass"]);self.assertEqual(d["decision"],"FETCH_CANDIDATE");self.assertFalse(d["governance"]["fetch_executed"])
 def test_identity(self):self.assertEqual(M.materialize_case(self.m,self.c["not-modified-pass"]),M.materialize_case(self.m,self.c["not-modified-pass"]))
 def test_cli(self):
  d=M.materialize_case(self.m,self.c["not-modified-pass"])
