...
SOIL_SCHEMA = ROOT / "schemas/contracts/v1/domains/soil/evidence_bundle.schema.json"
...
class SoilEvidenceBundleSchemaConvergenceTests(unittest.TestCase):
    def load(self, path: Path) -> dict:
        return json.loads(path.read_text(encoding="utf-8"))

    def test_soil_projection_delegates_shape_to_shared_schema(self) -> None:
        schema = self.load(SOIL_SCHEMA)
    ...
    def test_soil_projection_accepts_and_rejects_shared_fixtures(self) -> None:
        validator = load_validator(SOIL_SCHEMA)
