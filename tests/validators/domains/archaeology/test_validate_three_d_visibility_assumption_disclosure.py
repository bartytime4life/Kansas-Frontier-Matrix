...
MODULE_PATH = ROOT / "tools/validators/domains/archaeology/validate_three_d_visibility_assumption_disclosure.py"
SPEC = importlib.util.spec_from_file_location("three_d_visibility_assumption_disclosure_validator", MODULE_PATH)
...
class ThreeDVisibilityAssumptionDisclosureTests(unittest.TestCase):
    @classmethod
    ...
    def test_pass_is_non_authoritative_and_coordinate_free(self) -> None:
        for name in ("pass_internal_controlled", "pass_public_generalized_and_withheld"):
    ...
    def test_identity_binds_assumptions(self) -> None:
        candidate = self._candidate("pass_internal_controlled")
        original = MODULE.compute_identity(candidate)
        changed = copy.deepcopy(candidate)
        changed["observers"]["height_m"] = 1.7
        self.assertNotEqual(original, MODULE.compute_identity(changed))

    def test_public_candidate_fails_closed_on_exact_geometry_or_missing_rollback(self) -> None:
        for name in ("deny_public_exact_geometry", "deny_public_release_closure_missing"):
            self.assertEqual(MODULE.validate_candidate(self._candidate(name)).outcome, "DENY")

    def test_unknown_assumptions_abstain(self) -> None:
        for name in (
            "abstain_model_resolution_unresolved",
            "abstain_observer_height_unresolved",
            "abstain_obstacle_assumptions_partial",
            "abstain_propagation_assumption_unresolved",
        ):
