...
MODULE_PATH = ROOT / "tools/validators/domains/archaeology/validate_three_d_documentation.py"
SPEC = importlib.util.spec_from_file_location("three_d_documentation_validator", MODULE_PATH)
...
class ThreeDDocumentationTests(unittest.TestCase):
    @classmethod
    ...
    def test_pass_is_non_authoritative(self) -> None:
        for name in ("pass_internal_reality_based", "pass_public_candidate"):
    ...
    def test_identity_binds_paradata(self) -> None:
        candidate = self._candidate("pass_internal_reality_based")
        original = MODULE.compute_identity(candidate)
        changed = copy.deepcopy(candidate)
        changed["capture"]["method_statement"] = "Synthetic capture declaration changed for a deterministic identity test."
        self.assertNotEqual(original, MODULE.compute_identity(changed))

    def test_interpretation_and_representation_fail_closed(self) -> None:
        for name in ("deny_interpretive_steps_missing", "deny_2_5d_loss_undisclosed"):
            self.assertEqual(MODULE.validate_candidate(self._candidate(name)).outcome, "DENY")

    def test_public_candidate_requires_safe_assets_and_release_closure(self) -> None:
        for name in (
            "deny_public_safe_asset_missing",
            "deny_public_sensitivity_unresolved",
            "deny_public_transform_missing",
