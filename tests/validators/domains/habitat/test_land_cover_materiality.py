...
from tools.validators.domains.habitat.validate_land_cover_materiality import (
    FIXTURE_ROOT,
    PROFILE_PATH,
    PROFILE_SCHEMA_PATH,
    REPO_ROOT,
...
class LandCoverMaterialityTests(unittest.TestCase):
        ...
        for name, expected in sorted(manifest.items()):
            with self.subTest(path=name):
                ...
                with tempfile.TemporaryDirectory() as directory:
                    ...
                    shared_result = validate_assessment(output)
                self.assertTrue(shared_result.ok, shared_result.findings)
    ...
    def test_evaluation_is_deterministic(self) -> None:
        path = FIXTURE_ROOT / "valid/valid_material_reclassification.json"
    ...
    def test_thresholds_are_strictly_greater_than(self) -> None:
        source = json.loads(
            (FIXTURE_ROOT / "valid/valid_semantic_non_material.json").read_text(
                encoding="utf-8"
            )
        )
        source["metrics"]["reclassification_fraction"] = 0.02
        source["metrics"]["max_net_class_delta_ha"] = 250.0
        with tempfile.TemporaryDirectory() as directory:
    ...
    def test_profile_tampering_fails_closed(self) -> None:
        profile = json.loads(PROFILE_PATH.read_text(encoding="utf-8"))
        profile["triggers"]["reclassification_fraction"]["threshold"] = 0.5
        with tempfile.TemporaryDirectory() as directory:
    ...
    def test_fixture_cli_profile_passes(self) -> None:
        result = subprocess.run(
            [
                ...
                (
                    ...
                    "validate_land_cover_materiality.py"
                ),
                "--fixtures",
            ],
            cwd=REPO_ROOT,
        ...
        self.assertIn('"assessment_outcome":"PROMOTION_CANDIDATE"', result.stdout)
        self.assertNotIn(
            "kfm://habitat/land-cover/example-county",
            result.stdout,
        )
