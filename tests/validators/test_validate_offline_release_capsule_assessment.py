...
from tools.validators.runtime import validate_offline_release_capsule_assessment as validator

ROOT = Path(__file__).resolve().parents[2]


class OfflineReleaseCapsuleAssessmentTests(unittest.TestCase):
    ...
    def test_exact_fixture_cases(self) -> None:
        ...
        for case in manifest["cases"]:
            with self.subTest(case=case["case_id"]):
                ...
                self.assertEqual(case["expected_capsule_state"], result.capsule_state)
    ...
    def test_fixture_polarity_and_states_are_non_vacuous(self) -> None:
        ...
        states = {case["expected_capsule_state"] for case in cases if case["expected_status"] == "PASS"}
        self.assertGreaterEqual(statuses["PASS"], 10)
        self.assertGreaterEqual(statuses["DENY"], 6)
        self.assertEqual({"READY_FOR_SEPARATE_INSTALL_REVIEW", "UPDATE_PENDING", "STALE", "INCOMPLETE", "ROLLBACK_REQUIRED", "WITHDRAWN"}, states)

    def test_pass_never_claims_install_render_or_trust(self) -> None:
        manifest = validator.load_fixtures()
        for case in manifest["cases"]:
            if case["expected_status"] != "PASS":
                continue
            document = validator.materialize_case(manifest, case)
            self.assertEqual(validator.expected_capsule_assessment(document), document["capsule_assessment"])
            self.assertFalse(document["capsule_assessment"]["trusted_render_allowed"])
            self.assertFalse(document["capsule_assessment"]["install_allowed"])
            self.assertFalse(document["governance"]["artifact_bytes_read"])
            self.assertFalse(document["governance"]["cache_mutated"])

    def test_identity_tamper_is_denied(self) -> None:
        manifest = validator.load_fixtures()
        document = validator.materialize_case(manifest, manifest["cases"][0])
        document["capsule"]["capsule_ref"] = "kfm:offline-capsule:tampered"
        result = validator.validate_payload(document)
        self.assertEqual("DENY", result.status)
        self.assertEqual("OFFLINE_CAPSULE_SPEC_HASH_MISMATCH", result.findings[0].code)

    def test_duplicate_keys_fail_closed_without_echo(self) -> None:
        marker = "OFFLINE_CAPSULE_ECHO_SENTINEL"
    ...
    def test_validator_has_no_network_crypto_cache_or_render_client(self) -> None:
        source = Path(validator.__file__).read_text(encoding="utf-8")
        for token in ("requests", "urllib.request", "httpx", "aiohttp", "boto3", "subprocess", "cryptography", "CacheStorage", "maplibre"):
            self.assertNotIn(token, source)

    def test_source_map_names_full_atlas_lineage_and_existing_boundaries(self) -> None:
        source = (ROOT / "docs/intake/exploratory/offline-release-capsule-assessment-source-map.md").read_text(encoding="utf-8")
        for value in ("KFM-TRIAD-051", "KFM-CAND-0151", "KFM-CAND-0152", "KFM-CAND-0153", "MapReleaseManifest", "PMTiles"):
