...
VALIDATOR_PATH = REPO_ROOT / "tools/validators/validate_aggregate_statistic.py"
SCHEMA_PATH = REPO_ROOT / "schemas/contracts/v1/common/aggregate_statistic.schema.json"

spec = importlib.util.spec_from_file_location("validate_aggregate_statistic", VALIDATOR_PATH)
...
class AggregateStatisticTests(unittest.TestCase):
    @classmethod
    def setUpClass(cls) -> None:
        cls.cases = validator.load_fixture_cases()
        cls.by_name = {raw["name"]: (raw, candidate) for raw, candidate in cls.cases}
    ...
    def test_exact_fixture_matrix(self) -> None:
        ...
        for raw, candidate in self.cases:
            ...
            self.assertEqual([item.code for item in result.findings], raw["expected_findings"], raw["name"])
        self.assertEqual(observed, {"PASS": 4, "DENY": 18, "ERROR": 1})

    def test_identity_is_stable_across_mapping_order(self) -> None:
        candidate = self.by_name["valid_statewide_percentage"][1]
        ...
        self.assertEqual(validator.expected_statistic_id(reordered), candidate["aggregate_statistic_id"])

    def test_aggregate_role_support_and_local_truth_boundaries_are_fixed(self) -> None:
        expected = {
            ...
            "local_truth_overclaim": "LOCAL_TRUTH_OVERCLAIM",
            "exact_local_geometry_overclaim": "EXACT_LOCAL_GEOMETRY_OVERCLAIM",
        }
    ...
    def test_source_release_numerator_and_denominator_are_explicit(self) -> None:
        expected = {
            "source_release_missing": "SOURCE_RELEASE_REQUIRED",
            "percentage_numerator_missing": "NUMERATOR_REQUIRED",
            "percentage_denominator_missing": "DENOMINATOR_REQUIRED",
        }
        for name, code in expected.items():
            result = validator.validate_payload(self.by_name[name][1])
            self.assertEqual([item.code for item in result.findings], [code])

    def test_weighting_and_missing_data_treatment_are_bound(self) -> None:
        expected = {
            "weighted_method_reference_missing": "WEIGHTING_REFERENCE_REQUIRED",
            "weighting_reference_conflict": "WEIGHTING_REFERENCE_CONFLICT",
            "missing_data_treatment_unbound": "MISSING_DATA_TREATMENT_UNBOUND",
        }
        for name, code in expected.items():
            result = validator.validate_payload(self.by_name[name][1])
            self.assertEqual([item.code for item in result.findings], [code])

    def test_source_time_and_geography_fail_closed(self) -> None:
        expected = {
            "data_cutoff_after_source_release": "DATA_CUTOFF_AFTER_SOURCE_RELEASE",
            "validity_interval_inverted": "VALIDITY_INTERVAL_INVALID",
            "unresolved_geometry_overclaim": "UNRESOLVED_GEOMETRY_OVERCLAIM",
        }
        for name, code in expected.items():
            result = validator.validate_payload(self.by_name[name][1])
            self.assertEqual([item.code for item in result.findings], [code])
    ...
    def test_cli_pass_is_value_free_and_authority_is_false(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            ...
            path.write_text(json.dumps(self.by_name["valid_statewide_percentage"][1]), encoding="utf-8")
        ...
        self.assertNotIn("synthetic-statistical-release", completed.stdout)
        self.assertNotIn("synthetic_condition_share", completed.stdout)
