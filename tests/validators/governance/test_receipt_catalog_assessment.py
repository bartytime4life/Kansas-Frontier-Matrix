...
VALIDATOR_PATH = ROOT / "tools/validators/governance/validate_receipt_catalog_assessment.py"
spec = importlib.util.spec_from_file_location("receipt_catalog_assessment", VALIDATOR_PATH)
...
class ReceiptCatalogAssessmentTests(unittest.TestCase):
    @classmethod
    ...
    def test_exact_fixture_matrix(self) -> None:
        self.assertGreaterEqual(len(self.manifest["cases"]), 13)
    ...
    def test_positive_assessment_preserves_conflict_and_non_authority(self) -> None:
        ...
        self.assertFalse(value["summary"]["canonical_catalog_written"])
        self.assertEqual("CONFLICTED_REVIEW_REQUIRED", value["source_snapshot"]["authority_state"])
    ...
    def test_declared_contract_references_exist(self) -> None:
        ...
        for family in value["families"]:
            if family["semantic_contract_ref"] is None:
                continue
            path = ROOT / family["semantic_contract_ref"]
    ...
    def test_duplicate_key_nonfinite_symlink_and_oversize_are_errors(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            ...
            self.assertEqual("RECEIPT_CATALOG_JSON_DUPLICATE_KEY", validator.validate_file(duplicate).findings[0].code)
            nonfinite = root / "nonfinite.json"
            nonfinite.write_text('{"value":NaN}\n', encoding="utf-8")
            self.assertEqual("RECEIPT_CATALOG_JSON_NONFINITE_NUMBER", validator.validate_file(nonfinite).findings[0].code)
            target = root / "target.json"
            target.write_text("{}\n", encoding="utf-8")
            symlink = root / "link.json"
            symlink.symlink_to(target)
            self.assertEqual("RECEIPT_CATALOG_INPUT_SYMLINK_DENIED", validator.validate_file(symlink).findings[0].code)
            oversized = root / "oversized.json"
            oversized.write_bytes(b"{" + b" " * validator.MAX_BYTES + b"}")
            self.assertEqual("RECEIPT_CATALOG_INPUT_TOO_LARGE", validator.validate_file(oversized).findings[0].code)
    ...
    def test_serialization_does_not_echo_candidate_values(self) -> None:
        case = {"mutations": [{"path": "/families/0/semantic_contract_ref", "value": "contracts/runtime/secret-marker.md"}]}
        ...
        self.assertIn("RECEIPT_CATALOG_CONTRACT_REFERENCE_MISSING", output)
