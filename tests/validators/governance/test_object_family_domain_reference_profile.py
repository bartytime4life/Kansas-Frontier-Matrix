...
VALIDATOR_PATH = (
    ...
    "validate_object_family_domain_reference_profile.py"
)
spec = importlib.util.spec_from_file_location("object_family_domain_profile", VALIDATOR_PATH)
...
class ObjectFamilyDomainReferenceProfileTests(unittest.TestCase):
    @classmethod
    ...
    def test_positive_profile_is_review_required_only(self) -> None:
        ...
        self.assertFalse(value["summary"]["canonical_register_written"])
    ...
    def test_declared_contract_references_exist(self) -> None:
        ...
        for family in value["families"]:
            path = ROOT / family["owner"]["contract_ref"]
    ...
    def test_duplicate_key_nonfinite_symlink_and_oversize_are_errors(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            ...
            self.assertEqual("ERROR", validator.validate_file(duplicate).outcome)
            self.assertEqual(
                "OBJECT_FAMILY_JSON_DUPLICATE_KEY",
            ...
            self.assertEqual(
                "OBJECT_FAMILY_JSON_NONFINITE_NUMBER",
            ...
            self.assertEqual(
                "OBJECT_FAMILY_INPUT_SYMLINK_DENIED",
    ...
    def test_serialization_does_not_echo_candidate_values(self) -> None:
        case = {
            "mutations": [
                {
                    "path": "/families/0/owner/domain_id",
                    "value": "secret-marker-domain",
                }
            ]
        }
        value = validator.materialize_case(self.manifest, case)
        ...
        self.assertNotIn("secret-marker-domain", output)
        self.assertIn("OBJECT_FAMILY_OWNER_DOMAIN_UNKNOWN", output)
