...
class DependencyOriginPolicyTests(unittest.TestCase):
        ...
        with tempfile.TemporaryDirectory() as temporary:
            ...
            (root / "apps/example/package.json").write_text(
                json.dumps(
                    {
                        "name": "example",
                        "dependencies": {"@kfm/ui": "workspace:*"},
                    }
                ),
                encoding="utf-8",
            )
            (root / "packages/ui/package.json").write_text(
                json.dumps({"name": "@kfm/ui", "version": "0.0.0"}),
                encoding="utf-8",
            )
        ...
        with tempfile.TemporaryDirectory() as temporary:
            ...
            (root / "apps/example/package.json").write_text(
                json.dumps(
                    {
                        "name": "example",
                        "dependencies": {"@kfm/private": "^1.0.0"},
                    }
                ),
                encoding="utf-8",
            )
    ...
    def test_validator_has_no_network_or_write_surface(self) -> None:
        ...
        self.assertTrue(
            {"write_bytes", "unlink", "rename", "replace"}.isdisjoint(attributes)
        )
