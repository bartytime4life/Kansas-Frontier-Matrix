...
from tools.validators import validate_identifier_precision_lineage_assessment as validator

ROOT = Path(__file__).resolve().parents[2]


class IdentifierPrecisionLineageAssessmentTests(unittest.TestCase):
    ...
    def test_fixture_polarity_and_outcomes_are_non_vacuous(self) -> None:
        ...
        self.assertEqual({"RESOLVED", "HOLD", "ABSTAIN"}, outcomes)
    ...
    def test_payload_is_digest_only_and_governance_is_inactive(self) -> None:
        manifest = validator.load_fixtures()
        document = validator.materialize_case(manifest, manifest["cases"][0])
        ...
        self.assertFalse(document["summary"]["identity_resolution_authorized"])
        self.assertFalse(document["summary"]["public_use_allowed"])

    def test_duplicate_keys_fail_closed_without_echo(self) -> None:
        marker = "IDENTIFIER_PRECISION_ECHO_SENTINEL"
    ...
    def test_source_map_names_full_atlas_triad_and_candidates(self) -> None:
        source_map = (ROOT / "docs/intake/exploratory/identifier-precision-lineage-assessment-source-map.md").read_text(encoding="utf-8")
        for value in ("KFM-TRIAD-034", "KFM-CAND-0100", "KFM-CAND-0101", "KFM-CAND-0102"):
