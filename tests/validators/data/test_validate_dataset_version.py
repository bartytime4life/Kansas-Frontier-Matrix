...
MODULE_PATH = REPO_ROOT / "tools/validators/data/validate_dataset_version.py"
...
SPEC = importlib.util.spec_from_file_location("validate_dataset_version", MODULE_PATH)
...
class DatasetVersionTests(unittest.TestCase):
    def test_fixture_inventory_is_exact(self) -> None:
        self.assertEqual(
            [
                "valid_processed_derivative.json",
                "valid_published_version.json",
                "valid_retrieval_snapshot.json",
            ],
        ...
        self.assertEqual(
            [
                ...
                "invalid_schema_missing_source_ref.json",
                "semantic_invalid_corrected_lineage.json",
                "semantic_invalid_identity.json",
                "semantic_invalid_placeholder_digest.json",
                "semantic_invalid_processed_lineage.json",
                "semantic_invalid_published_rights.json",
                "semantic_invalid_published_without_release.json",
                "semantic_invalid_refs_not_canonical.json",
                "semantic_invalid_self_reference.json",
    ...
    def test_valid_fixtures_pass(self) -> None:
        for path in sorted(VALID_ROOT.glob("*.json")):
            with self.subTest(path=path.name):
                self.assertTrue(validator.validate_dataset_version(path).ok)
    ...
    def test_invalid_fixtures_match_reviewed_codes(self) -> None:
        ...
        for path in invalid_paths:
            with self.subTest(path=path.name):
                result = validator.validate_dataset_version(path)
    ...
    def test_schema_and_semantic_negative_lanes_are_explicit(self) -> None:
        schema_invalid = INVALID_ROOT / "invalid_schema_missing_source_ref.json"
    ...
    def test_fixture_entrypoint_passes(self) -> None:
        ...
        self.assertIn(
            "CONFIRMED: 3 valid and 10 invalid DatasetVersion fixtures passed exact polarity.",
            stdout.getvalue(),
        )
    ...
    def test_duplicate_keys_fail_closed(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            ...
            path.write_text(
                '{"object_type":"DatasetVersion","object_type":"other"}',
                encoding="utf-8",
            )
            result = validator.validate_dataset_version(path)
            self.assertIn(
    ...
    def test_cli_does_not_echo_candidate_values(self) -> None:
        marker = "UNIQUE_DATASET_VERSION_ECHO_SENTINEL"
        with tempfile.TemporaryDirectory() as directory:
    ...
    def test_identity_is_deterministic(self) -> None:
        for path in sorted(VALID_ROOT.glob("*.json")):
            with self.subTest(path=path.name):
                ...
                expected = validator.canonical_dataset_version_id(candidate)
                self.assertEqual(candidate["id"], expected)
                self.assertEqual(
                    expected,
                    validator.canonical_dataset_version_id(candidate),
                )
