...
MODULE_PATH = REPO_ROOT / "tools/validators/validate_webhook_ingress_decision.py"
...
SPEC = importlib.util.spec_from_file_location(
    "validate_webhook_ingress_decision",
    MODULE_PATH,
)
...
class WebhookIngressDecisionTests(unittest.TestCase):
    def test_fixture_inventory_is_exact(self) -> None:
        self.assertEqual(
            [
                "valid_accept.json",
                "valid_conditional_poll_fallback.json",
                "valid_duplicate_noop.json",
            ],
        ...
        self.assertEqual(
            [
                ...
                "semantic_invalid_duplicate_digest_conflict.json",
                "semantic_invalid_governance_overreach.json",
                "semantic_invalid_nonce_replay_accept.json",
                "semantic_invalid_poll_without_conditionals.json",
                "semantic_invalid_secret_persistence.json",
                "semantic_invalid_spec_hash.json",
                "semantic_invalid_stale_accept.json",
            ],
    ...
    def test_valid_fixtures_pass(self) -> None:
        for path in sorted(VALID_ROOT.glob("*.json")):
            with self.subTest(path=path.name):
                self.assertTrue(validator.validate_decision(path).ok)
    ...
    def test_fixture_entrypoint_passes(self) -> None:
        ...
        self.assertIn(
            "CONFIRMED: 3 valid and 7 invalid webhook ingress fixtures passed exact polarity.",
            stdout.getvalue(),
        )
    ...
    def test_duplicate_keys_fail_closed(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            ...
            path.write_text(
                '{"object_type":"WebhookIngressDecision","object_type":"other"}',
                encoding="utf-8",
            )
            result = validator.validate_decision(path)
            self.assertIn(
    ...
    def test_cli_does_not_echo_candidate_values(self) -> None:
        marker = "WEBHOOK_SECRET_ECHO_SENTINEL"
        with tempfile.TemporaryDirectory() as directory:
            path = Path(directory) / "bad.json"
            path.write_text(json.dumps({"secret": marker}), encoding="utf-8")
            run = subprocess.run(
    ...
    def test_all_vectors_are_schema_valid(self) -> None:
        fixture_paths = list(VALID_ROOT.glob("*.json")) + list(
            INVALID_ROOT.glob("semantic_invalid_*.json")
        )
        for path in sorted(fixture_paths):
            with self.subTest(path=path.name):
                self.assertEqual([], validator._schema_findings(load_json(path)))

    def test_valid_outcomes_cover_accept_duplicate_and_fallback(self) -> None:
        outcomes = {
            load_json(path)["decision"]["outcome"]
            for path in VALID_ROOT.glob("*.json")
        }
        self.assertEqual({"ACCEPT", "DUPLICATE_NOOP", "QUARANTINE"}, outcomes)
        fallback = load_json(VALID_ROOT / "valid_conditional_poll_fallback.json")
        self.assertEqual("CONDITIONAL_POLL", fallback["fallback"]["mode"])
        self.assertTrue(fallback["fallback"]["conditional_request_required"])
