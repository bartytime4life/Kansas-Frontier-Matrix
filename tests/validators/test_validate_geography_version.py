...
from tools.validators import validate_geography_version as validator
...
class GeographyVersionTests(unittest.TestCase):
    ...
    def test_positive_cases_cover_lineage_and_representation_postures(self) -> None:
        manifest = validator.load_fixtures()
        passing = [
        ...
        self.assertEqual(
            {"NOT_APPLICABLE", "REFERENCED_NOT_RESOLVED", "UNRESOLVED"},
            {item["lineage"]["crosswalk_state"] for item in passing},
        )
        self.assertEqual(
            {"GENERALIZED_DERIVATIVE", "SYNTHETIC_FIXTURE"},
            {item["representation"]["geometry_role"] for item in passing},
        )
        outcomes = Counter(case["expected_outcome"] for case in manifest["cases"])
        self.assertEqual({"PASS", "DENY"}, set(outcomes))
        self.assertGreaterEqual(outcomes["DENY"], 15)
    ...
    def test_identity_is_content_addressed_and_replay_stable(self) -> None:
        ...
        changed["version"]["version_key"] = "v1-corrected"
    ...
    def test_serialization_does_not_echo_payload_values(self) -> None:
        manifest = validator.load_fixtures()
        document = validator.materialize_case(manifest, manifest["cases"][0])
        sentinel = "do-not-echo-geography-value"
        document["geography"]["label"] = sentinel
        result = validator.validate_payload(document)
        rendered = validator.serialize(Path("candidate.json"), result)
        self.assertNotIn(sentinel, rendered)
        self.assertNotIn("synthetic_county", rendered)
    ...
    def test_duplicate_nonfinite_symlink_and_oversize_are_errors(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            ...
            for path, code in (
                (duplicate, "GEOGRAPHY_JSON_DUPLICATE_KEY"),
                (nonfinite, "GEOGRAPHY_JSON_NONFINITE_NUMBER"),
                (link, "GEOGRAPHY_INPUT_SYMLINK_DENIED"),
                (oversized, "GEOGRAPHY_INPUT_TOO_LARGE"),
            ):
