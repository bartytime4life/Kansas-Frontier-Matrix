...
from tools.validators.data.validate_validation_report import (
    FIXTURES_ROOT,
    SCHEMA_PATH,
    validate_path,
)
...
class ValidationReportValidatorTests(unittest.TestCase):
    ...
    def test_nonfinite_number_fails_closed(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            ...
            path.write_text('{"id":"one","score":NaN}', encoding="utf-8")
            self.assertEqual(("JSON_INVALID",), validate_path(path))

    def test_fixture_cli_is_no_network_and_passes(self) -> None:
        ...
        self.assertIn("VALIDATION_REPORT_FIXTURES_VALID", result.stdout)
