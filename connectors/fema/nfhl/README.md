...
title: connectors/fema/nfhl/ — FEMA NFHL Product Connector Lane
type: readme
version: v0.1
status: draft
...
truth_posture: CONFIRMED path exists / PROPOSED nested product-lane contract / CANONICALITY NEEDS VERIFICATION
related:
  - ../README.md
  - ../../fema-nfhl/README.md
...
notes:
  - "This README fills a previously blank nested FEMA NFHL product connector lane."
  - "The FEMA family connector is documented at connectors/fema/README.md; the sibling split path connectors/fema-nfhl/README.md also exists and should be reconciled with this nested path by ADR or migration note."
  - "NFHL is regulatory flood hazard context, not observed inundation, forecast, warning, insurance determination, legal advice, or life-safety guidance."
  ...
  - "Specific endpoints, bulk-download behavior, analytic vs visualization surfaces, descriptors, tests, fixtures, CI wiring, terms, and activation state remain NEEDS VERIFICATION."
[/KFM_META_BLOCK_V2] -->

<a id="top"></a>

# FEMA NFHL Product Connector Lane

> Nested FEMA-family product connector lane for National Flood Hazard Layer source admission.
...
`connectors/fema/nfhl/` is the nested FEMA-family product connector lane for NFHL intake and admission helpers.

It may contain connector-local documentation, configuration examples, source-admission code, bounded client helpers, parser helpers, bulk-download helpers, and tests for NFHL source material.
...
The repo now has both a nested FEMA-family NFHL path and a split product-specific path:
...
Treat the relationship as **NEEDS VERIFICATION** until one of these is confirmed:

1. `connectors/fema/nfhl/` is the canonical product lane under the FEMA family connector;
2. `connectors/fema-nfhl/` is the canonical product-specific connector lane;
3. one path is a compatibility mirror or migration target;
4. an ADR or migration note resolves the split.

Do not duplicate implementation code across both paths. Prefer a single source of connector truth and use redirects, compatibility notes, or migration docs if both paths remain visible.
...
connectors/fema/nfhl/                           # this nested NFHL product lane
connectors/fema-nfhl/                           # split NFHL product lane; canonicality NEEDS VERIFICATION
...
docs/sources/catalog/fema/map-service-center.md # companion Map Service Center profile
```
...
- test fixtures that preserve source shape without becoming legal, insurance, or life-safety guidance.
...
| Insurance, legal, emergency, or life-safety determination text | Out of scope; redirect to official channels and reviewed public guidance. |
...
- [ ] The canonical relationship between `connectors/fema/nfhl/` and `connectors/fema-nfhl/` is resolved.
...
|---|---:|---|
| Confirm whether canonical NFHL connector home is `connectors/fema/nfhl/` or `connectors/fema-nfhl/`. | **NEEDS VERIFICATION** | Directory Rules, repo tree, source-catalog metadata, and ADR/migration note. |
...
Treat this nested product lane as regulatory-context intake, not flood-event truth. It can make NFHL material easier to inspect, stage, validate, and cite, but it must not make regulatory layers look like observed flooding, forecasts, warnings, legal advice, insurance determinations, or public-safety guidance.
