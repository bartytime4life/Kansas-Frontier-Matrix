...
[`tools/local_data/file_io.py`](../../tools/local_data/file_io.py) now supplies
bounded regular-file reads, streaming SHA-256 capture, private directory
creation, and atomic no-overwrite writes for
[`tools/local_data/manage.py`](../../tools/local_data/manage.py). `fetch.py` in
this package performs no filesystem writes or source admission; it is an
