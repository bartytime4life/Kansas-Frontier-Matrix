...
title: connectors/kansas_state_archives/src/ — Kansas State Archives Compatibility Source Layout Boundary
type: readme
version: v0.1
status: draft
...
proposed_path: connectors/kansas_state_archives/src/README.md
...
related:
  ...
  - ../../../policy/rights/
  - ../../../release/
tags: [kfm, connectors, kansas-state-archives, src, package-layout, kshs, archives, compatibility, no-network, source-admission, raw, quarantine, governance]
notes:
  - "This README fills a blank `src/` layout-boundary README under the legacy-style Kansas State Archives compatibility connector."
  ...
  - "The child package README at `src/kansas_state_archives/README.md` defines the package implementation boundary; this file defines the broader `src/` layout boundary."
...
# Kansas State Archives Compatibility Source Layout Boundary

> Layout-boundary README for `connectors/kansas_state_archives/src/`. This folder groups implementation package code for the noncanonical Kansas State Archives compatibility connector. It is **not** itself a connector, policy root, schema root, release root, public API, or truth store.
...
> **Path:** `connectors/kansas_state_archives/src/README.md`  
...
`connectors/kansas_state_archives/src/` is the source-layout container for the legacy Kansas State Archives compatibility connector.

It may contain package directories, package-level README files, small helper modules, and implementation-boundary docs for source-admission parsing, normalization, validation, surface-identity preservation, and RAW/QUARANTINE handoff helpers.
...
| Surface | Role | Status |
|---|---|---:|
| `connectors/kansas_state_archives/src/` | Compatibility source-layout container. | **CONFIRMED path / NEEDS VERIFICATION package inventory** |
| `connectors/kansas_state_archives/src/kansas_state_archives/` | Compatibility package namespace. | **CONFIRMED README path / NEEDS VERIFICATION modules** |
...
- package directories such as `kansas_state_archives/`;
...
- public map tiles, public APIs, public summaries, public timelines, or published artifacts;
...
- code that treats `connectors/kansas_state_archives/` as canonical without ADR/migration evidence.
...
```text
connectors/kansas_state_archives/src/
├── README.md                       # CONFIRMED — this layout-boundary README
└── kansas_state_archives/
    └── README.md                   # CONFIRMED — package-boundary README
```

Potential package files are documented in `src/kansas_state_archives/README.md`; do not create them merely because they are listed there.
...
|---|---:|---|---|
| `connectors/kansas_state_archives/src/README.md` | **CONFIRMED** | Target source-layout README exists and was blank before this update. | Does not prove modules, tests, or CI. |
| `connectors/kansas_state_archives/src/kansas_state_archives/README.md` | **CONFIRMED** | Child package README defines package implementation boundary. | Does not prove modules exist. |
...
- deterministic failure when SourceDescriptor, activation, rights, sensitivity, CARE review, surface identity, item identity, source role, metadata shape, or handoff target is unresolved.
...
- no canonicality assumption for `connectors/kansas_state_archives/` without ADR/migration evidence.
...
| Confirm canonical KSHS / Kansas State Archives source-layout path. | **NEEDS VERIFICATION** | Directory Rules, ADR, migration note, or repo convention. |
