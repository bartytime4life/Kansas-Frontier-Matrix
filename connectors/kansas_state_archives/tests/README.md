...
title: connectors/kansas_state_archives/tests/ — Kansas State Archives Compatibility Connector Tests
type: readme
version: v0.1
status: draft
...
proposed_path: connectors/kansas_state_archives/tests/README.md
...
related:
  ...
  - ../../../policy/rights/
  - ../../../release/
tags: [kfm, connectors, kansas-state-archives, tests, kshs, archives, compatibility, no-network, fixtures, rights, sensitivity, care, source-admission, raw, quarantine, governance]
notes:
  - "This README replaces a thin greenfield tests stub under a legacy-style top-level Kansas State Archives connector path."
  - "The parent `connectors/kansas_state_archives/README.md` documents this path as noncanonical compatibility only; canonical connector work should converge under `connectors/kansas/` unless an ADR says otherwise."
  - "Tests must not require live source access by default and must not create public claims or release artifacts."
  - "Kansas State Archives / KSHS rights, sensitivity, cultural-care, surface-specific access mechanics, per-surface descriptors, activation, fixture inventory, tests, CI wiring, and public-release classes remain NEEDS VERIFICATION."
...
# Kansas State Archives Compatibility Connector Tests

> Test contract for the legacy-style `connectors/kansas_state_archives/` compatibility path. These tests should protect source-admission boundaries, fixture safety, per-surface identity, rights/sensitivity/CARE gates, and migration toward the canonical Kansas connector lane.
...
> **Path:** `connectors/kansas_state_archives/tests/README.md`  
...
`connectors/kansas_state_archives/tests/` is a compatibility-path test lane for Kansas State Archives / KSHS source-admission behavior.

It may contain or document tests for fixture-only parsing, SourceDescriptor gating, per-surface identity preservation, metadata preservation, rights/sensitivity/CARE fail-closed behavior, RAW/QUARANTINE handoff, migration safety, and prevention of direct release behavior.
...
| Surface | Role | Status |
|---|---|---:|
| `connectors/kansas_state_archives/tests/` | Tests for noncanonical compatibility path. | **CONFIRMED path / NEEDS VERIFICATION test files** |
| `connectors/kansas_state_archives/README.md` | Parent compatibility README. | **CONFIRMED** |
...
- no committed fixture containing unreviewed rights, sensitivity, cultural-care, sovereignty, archaeology-sensitive, infrastructure-sensitive, or living-person material;
- fixture-driven parser and boundary tests only;
- fail closed for unresolved rights, source role, surface identity, item identity, access method, metadata shape, sensitivity, cultural-care, or schema drift.
...
| Surface-identity tests | Preserve KSHS umbrella, Kansas Memory, KHRI, and State Archives proper as distinct surfaces. | **PROPOSED** |
...
| Identity preservation tests | Preserve surface ID, collection ID, item ID, source URI, title/creator/date, access method, and retrieval time. | **PROPOSED** |
...
|---|---:|---|---|
| `connectors/kansas_state_archives/tests/README.md` | **CONFIRMED** | Target tests README exists and previously contained only a greenfield stub. | Does not prove tests exist or pass. |
...
| `docs/sources/catalog/kansas/kansas-state-archives.md` | **CONFIRMED** | KSHS/Kansas State Archives source brief identifies a KSHS umbrella layer, per-surface adapters under `connectors/kansas/`, and rights/sensitivity verification needs. | Does not prove endpoint availability or fixture safety. |
...
| Missing surface identity | Quarantine/abstain. |
...
| Treating `connectors/kansas_state_archives/` as canonical | Fail unless ADR/migration evidence is provided. |

---

## Rollback

Rollback is required if this README is used to justify live harvesting, source activation, public release, fixture dumping, rights/sensitivity/CARE bypass, surface-role collapse, or treating this top-level snake_case path as canonical.
...
| Confirm canonical KSHS / Kansas State Archives connector/test path. | **NEEDS VERIFICATION** | Directory Rules, ADR, migration note, or repo tree. |
...
Tests here should make the compatibility path safer, not more authoritative. When KSHS/Kansas State Archives work moves under the canonical `connectors/kansas/` lane, migrate or retire these tests with a documented rollback path.
