...
title: connectors/idigbio/specimens/ — iDigBio Specimens Connector Lane
type: readme
version: v0.1
status: draft
...
proposed_path: connectors/idigbio/specimens/README.md
...
related:
  ...
  - ../../../policy/rights/
  - ../../../release/
tags: [kfm, connectors, idigbio, specimens, biodiversity, darwin-core, flora, fauna, occurrence, source-admission, geoprivacy, rights, raw, quarantine, governance]
notes:
  - "This README fills a previously blank nested connector README for iDigBio specimen-record admission."
  - "The iDigBio source dossier positions iDigBio as a specimen-backed corroborative biodiversity source, not the canonical aggregator."
  - "Record-level source role, rights, license, sensitivity, geoprivacy, and dedupe posture must be preserved; iDigBio material is sensitivity-unevaluated at admission."
...
# iDigBio Specimens Connector Lane

> Source-admission surface for iDigBio specimen-backed biodiversity records. It is **not** the canonical biodiversity aggregator, taxonomic authority, public occurrence layer, or publication path.

<p>
  <img alt="Status: experimental" src="https://img.shields.io/badge/status-experimental-yellow">
  <img alt="Owner: OWNER_TBD" src="https://img.shields.io/badge/owner-OWNER__TBD-lightgrey">
  <img alt="Source role: specimen-backed corroborative" src="https://img.shields.io/badge/source__role-specimen__backed__corroborative-blue">
...
> **Path:** `connectors/idigbio/specimens/README.md`  
> **Truth posture:** `CONFIRMED` file exists · `PROPOSED` connector-lane contract · `NEEDS VERIFICATION` canonical implementation home  
> **Boundary:** source-admission only; no public occurrence claims, no taxonomic authority, no direct publication.
...
`connectors/idigbio/specimens/` is a proposed nested connector lane for iDigBio specimen-record source admission.

It may contain connector-local documentation, compatibility notes, safe fixture rules, parser expectations, source-admission envelopes, and validation expectations for iDigBio specimen-record-shaped material.
...
| Surface | Role | Status |
|---|---|---:|
| `connectors/idigbio/specimens/` | Product-specific connector lane for iDigBio specimen record admission. | **PROPOSED / NEEDS VERIFICATION** |
| `connectors/idigbio/` | Parent connector family lane named by the iDigBio source dossier. | **PROPOSED / NEEDS VERIFICATION** |
...
> [!NOTE]
> The iDigBio dossier flags path-convention questions between flat dossiers and family-folder layouts. This nested connector lane should therefore remain **CANONICALITY NEEDS VERIFICATION** until Directory Rules, an ADR, migration note, or repo convention confirms the implementation home.
...
Material belongs here only when it supports governed iDigBio specimen-record admission.

Accepted content:

- connector README and navigation notes;
- specimen-record fixture rules;
- parser expectations for Darwin Core-shaped record fields;
- SourceDescriptor-gate notes;
- rights, license, attribution, and recordset-provider preservation expectations;
- validation notes for specimen-backed source-role handling;
...
- published species occurrence or range claims;
...
|---|---:|---|---|
| `connectors/idigbio/specimens/README.md` | **CONFIRMED** | Target file exists and was blank before this update. | Does not prove implementation files, tests, or CI. |
| `docs/sources/catalog/idigbio/README.md` | **CONFIRMED** | iDigBio is a U.S. specimen-record aggregator, positioned in KFM as specimen-backed corroborative evidence rather than canonical aggregator; connector home is named as `connectors/idigbio/`. | Does not prove this nested `specimens/` path is canonical. |
| `docs/sources/catalog/idigbio/README.md` source-role section | **CONFIRMED** | Specimen records with specimen basis are proposed as observed/specimen-backed; low-quality or incomplete records route to candidate/quarantine posture; media and recordset metadata have separate roles. | Does not prove parser implementation or active SourceDescriptor. |
| `docs/sources/catalog/idigbio/README.md` sensitivity section | **CONFIRMED** | Sensitivity is record-level; sensitive taxa/location classes require deny-by-default handling, redaction/generalization, review, and policy decisions before publication. | Does not prove current policy-code enforcement. |
| `connectors/idigbio/specimens/` child tree | **NEEDS VERIFICATION** | Target path exists. | Child files, tests, package layout, fixtures, and workflows remain unverified. |
...
```mermaid
flowchart LR
  A[iDigBio specimen source material] --> B[SourceDescriptor gate]
  ...
  D --> E[Darwin Core normalization check]
  Q --> E
...
Expected behavior for iDigBio specimen connector work:
...
- no conversion of specimen rows into confirmed species-presence, range, habitat, or conservation-status claims;
...
1. iDigBio is a specimen-backed corroborative source, not the canonical biodiversity aggregator.
...
- iDigBio specimen records remain source-admission candidates until downstream validation;
...
- [ ] Canonicality of the nested `connectors/idigbio/specimens/` path is confirmed or tracked.
- [ ] SourceDescriptor home and iDigBio source ID are verified.
...
|---|---:|---|
| Confirm actual iDigBio specimen connector inventory below this path. | **NEEDS VERIFICATION** | Repo tree or mounted workspace. |
| Confirm canonicality of `connectors/idigbio/specimens/`. | **NEEDS VERIFICATION** | Directory Rules, ADR, migration note, or repo convention. |
| Confirm iDigBio SourceDescriptor home and source ID. | **NEEDS VERIFICATION** | Source registry entry and accepted schema. |
...
Keep this connector narrow. iDigBio specimen material can corroborate biodiversity occurrence claims after review, but this folder must not become a taxonomic authority, species-presence authority, sensitive-location exposure path, release path, or public truth surface.
