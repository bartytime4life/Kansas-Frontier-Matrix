...
> [!IMPORTANT]
> This README is grounded in two evidence layers at once: March–April 2026 KFM doctrine and the current public `main` tree. The root `configs/` lane and its child lanes are no longer hypothetical; they are visible in the public repo. What still needs verification is deeper than lane presence: exact runtime consumers, validator wiring, branch-local additions, and how much of the lane is actively consumed beyond the visible README surfaces.
...
| --- | --- |
| **CONFIRMED** | The current public `main` tree contains `configs/README.md`, `configs/env.schema.json`, and the child lane READMEs under `deployment/`, `env/`, `observability/`, `security/`, and `ui/`. |
...
| Current public snapshot | `README.md`, `env.schema.json`, and five child lanes: `deployment/`, `env/`, `observability/`, `security/`, `ui/`; each child lane is README-led in current public view |
| Primary companion surfaces | `../apps/`, `../packages/README.md`, `../pipelines/README.md`, `../infra/README.md`, `../tests/README.md`, `../tools/README.md`, `../scripts/README.md` |
| Evidence and doctrine neighbors | `../data/README.md`, `../docs/README.md` |
...
| Evidence and doctrine neighbors | [`../data/README.md`](../data/README.md) · [`../docs/README.md`](../docs/README.md) | **CONFIRMED** |
| Runtime and execution companions | [`../apps/`](../apps/) · [`../packages/README.md`](../packages/README.md) · [`../pipelines/README.md`](../pipelines/README.md) · [`../infra/README.md`](../infra/README.md) | **CONFIRMED** path / **INFERRED** active consumption |
...
| Pipeline orchestration logic, watcher code, or scheduler behavior | [`../pipelines/README.md`](../pipelines/README.md) or deployable runtime surfaces | Config may parameterize execution, but should not become execution logic |
...
| Release-backed evidence artifacts, run manifests, attestations, or canonical truth objects | governed data / catalog / release-proof surfaces | Config may point at them, but does not replace them |
...
├── deployment/
│   └── README.md                    # CONFIRMED README-led child lane
├── env/
│   └── README.md                    # CONFIRMED README-led child lane
├── observability/
│   └── README.md                    # CONFIRMED README-led child lane
...
3. Decide whether the change is truly configuration, or whether it belongs in `contracts/`, `schemas/`, `policy/`, `pipelines/`, `migrations/`, or host-local secret surfaces.
...
Config should reinforce the separation of shell, renderer, style, server, workers, pipelines, contracts, policy, and evidence resolution — not blur them.
...
| `configs/deployment/README.md` | **CONFIRMED** | deployment-lane guidance | current public lane is README-led |
| `configs/env/README.md` | **CONFIRMED** | environment-lane guidance | current public lane is README-led |
| `configs/observability/README.md` | **CONFIRMED** | observability-lane guidance | current public lane is README-led |
| `configs/security/README.md` | **CONFIRMED** | security-lane guidance | current public lane is README-led |
| `configs/ui/README.md` | **CONFIRMED** | UI-configuration guidance | current public lane is README-led |
...
Yes. The current public `main` tree confirms the root lane, `env.schema.json`, and the child lane READMEs.

### Are the child lanes active implementation surfaces?

They are **real lanes**, but the current public tree shows them as README-led directories. Presence is confirmed; deep usage still depends on the target checkout.
...
Because config often parameterizes runtime and publication behavior across those neighboring lanes, even though it does not own their truth objects or orchestration logic.
...
Because the current config guidance separates repo-visible configuration from secret-bearing local environment files.
...
| `configs/deployment/README.md` | present |
| `configs/env/README.md` | present |
| `configs/observability/README.md` | present |
| `configs/security/README.md` | present |
| `configs/ui/README.md` | present |
...
| `KFM_ARTIFACT_ROOT`, `KFM_POLICY_DIR`, `KFM_DB_DSN`, `KFM_MODEL_ADAPTER_URL`, `KFM_PUBLISHED_ONLY`, `KFM_CITATIONS_REQUIRED`, `KFM_BIND` | illustrative environment-key family | documented as illustrative only |
