...
remains `OWNER_TBD`. The scope ID is `people-dna-land/land-ownership`.
Land-record, title, survey, privacy, rights, sensitivity, and release reviewers
must be named before a future declaration seeks activation.
...
The retained `.gitkeep` is a compatibility marker only. It does not establish a
source, declaration, parcel record, write path, or publication authority.
