...
- The official context registry exposes fifteen source-specific controls, including NASA GIBS daily VIIRS NOAA-20 thermal-anomaly image context. The fire image uses the provider-default daily frame; the Site does not resolve its exact UTC date. The main map can request a bounded rolling 24-hour NOAA HMS smoke footprint feed, a Kansas-bounded Raspberry Shake AM FDSN station inventory, and USGS 3DEP LiDAR-derived hillshade and slope raster functions. Each source has a visible endpoint, freshness axis, limitation, fallback, opacity control, and historical-frame hold.
...
| Official Kansas context adapters | Census TIGERweb 2020 decennial population/housing baseline, USGS Water Data, USGS Earthquake Hazards, NASA GIBS daily VIIRS NOAA-20 thermal-anomaly image, NOAA HMS smoke, Raspberry Shake FDSN stations, USGS 3DEP LiDAR-derived terrain, NOAA/NWS alerts, NOAA NWM, and NOAA nowCOAST/NWS/OAR MRMS radar | `GAP-P1-012`, `GAP-P1-013`, `GAP-P2-014` | Fifteen fixed connections; bounded current/rolling context, provider-default daily fire imagery, dated smoke intervals, station metadata, and optional LiDAR-derived slope/hillshade | `EXTERNAL_CONTEXT_ONLY`; excluded from KFM reports, exports, admission, release, and EvidenceBundles | `app/live-context.ts`, `app/api/live-context/route.ts`, `app/event-atlas.ts`, `app/api/event-atlas/upstream.ts`, `app/terrain-sources.ts`, `app/source-intelligence.ts`, `app/noaa-radar.ts`, `app/api/noaa-radar/frames/route.ts`, `app/page.tsx` | Fixed allowlists, bounded response budgets, exact smoke interval parsing, FDSN header/coordinate validation, dynamic raster registry, temporal holds, build/type/rendered-shell checks | Provider cadence, exact image date, revisions, FDSN latency, station quality, raw-count response, work-unit identity, datum/accuracy, and emergency/life-safety boundaries remain explicit; no smoke, station, raster, model, or alert layer is KFM evidence | Remove the new registry/adapter blocks and retain the prior site-local layers |
