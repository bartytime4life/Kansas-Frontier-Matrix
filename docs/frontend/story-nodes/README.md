...
- **Entry points into Focus Mode**
  - From a Story Node page (`/story-node/:id`), users can open Focus Mode scoped to that node.
  - From map or 3D, selecting a feature that is backed by a Story Node offers a “Open in Focus Mode” option.

- **Shared contracts**
  - Focus Mode uses the same Story Node payloads as the Story Node page, plus a expanded graph neighborhood and overlay configuration.
...
- **Contract tests**
  - Type-check API responses against frontend types (e.g., `StoryNodeDto`, `GeoethicsPanelProps`).
...
- **STAC**
  - When Story Nodes reference STAC Items (e.g., imagery layers, elevation models), UI controls should:
...
|--------:|------------|-------------------|----------------------------------------------------------------------------|
| v11.2.4 | 2025-12-05 | Active / Enforced | Initial LTS Story Node frontend guide aligned with KFM v11.2.4 and Geoethical Reflection Layer. |
