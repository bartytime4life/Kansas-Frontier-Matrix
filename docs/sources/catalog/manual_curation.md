# Manual Curation (steward-authored evidence)

> Umbrella entry for the `manual_curation` source family.

Status: **stub** (placeholder generated from `SKELETON_MAP.md`).

## What this page is

This file is the top-level catalog umbrella for the `manual_curation` source family.
It links to the source descriptor(s), connector, and per-product catalog entries.

## Where the canonical material lives

- Source descriptors: `data/registry/sources/manual_curation.yaml` (per-product descriptors under each domain register)
- Connector: `connectors/manual_curation/`
- Per-product catalog entries: [`manual_curation/`](./manual_curation/)
