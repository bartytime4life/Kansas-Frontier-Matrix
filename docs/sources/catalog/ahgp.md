# American Hereditary Genealogy Project (AHGP)

> Umbrella entry for the `ahgp` source family.

Status: **stub** (placeholder generated from `SKELETON_MAP.md`).

## What this page is

This file is the top-level catalog umbrella for the `ahgp` source family.
It links to the source descriptor(s), connector, and per-product catalog entries.

## Where the canonical material lives

- Source descriptors: `data/registry/sources/ahgp.yaml` (per-product descriptors under each domain register)
- Connector: `connectors/ahgp/`
- Per-product catalog entries: [`ahgp/`](./ahgp/)
