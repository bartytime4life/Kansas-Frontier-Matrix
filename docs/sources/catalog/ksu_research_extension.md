...
> Umbrella entry for the `ksu_research_extension` source family.

Status: **stub** (placeholder generated from `SKELETON_MAP.md`).

## What this page is

This file is the top-level catalog umbrella for the `ksu_research_extension` source family.
It links to the source descriptor(s), connector, and per-product catalog entries.

## Where the canonical material lives

- Source descriptors: `data/registry/sources/ksu_research_extension.yaml` (per-product descriptors under each domain register)
- Connector: `connectors/ksu_research_extension/`
- Per-product catalog entries: *(not yet created)*
