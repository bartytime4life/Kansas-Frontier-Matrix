...
- 2026-05-16 — OPEN: Finalize canonical path + provenance mapping for doctrine inputs: (a) admit a canonical PDF source for `KFM_Domains_Culmination_Atlas_v1_1` (currently Markdown-only at `docs/KFM_Domains_Culmination_Atlas_v1_1.md`), and (b) approve alias/canonical naming decisions for `docs/New Ideas 5-8-26.pdf` ↔ `New_Ideas_5-8-26.pdf` and `docs/DomainDriven Design Reference.pdf` ↔ `DomainDriven_Design_Reference.pdf` so full evidence-index corroboration beyond `directory-rules.md` can be completed.
