...
- 2026-05-16 — OPEN: Finalize canonical path + provenance mapping for doctrine inputs by (a) admitting a canonical PDF source for `KFM_Domains_Culmination_Atlas_v1_1` (currently Markdown-only), and (b) approving alias/canonical naming decisions for `docs/New Ideas 5-8-26.pdf` ↔ `New_Ideas_5-8-26.pdf` and `docs/DomainDriven Design Reference.pdf` ↔ `DomainDriven_Design_Reference.pdf`.
