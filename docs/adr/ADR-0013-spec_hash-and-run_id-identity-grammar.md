...
![Last reviewed: 2026-05-11](https://img.shields.io/badge/last_reviewed-2026--05--11-lightgrey?style=flat-square)
...
1. **Algorithm.** `spec_hash` MUST be computed by:
   1. Serializing the target object as JSON.
   2. **Canonicalizing** that JSON per **RFC 8785 / JSON Canonicalization Scheme (JCS)**.
   3. Taking **SHA-256** over the canonical bytes.
...
4. **Canonicalization rules.** The JCS canonicalization MUST enforce:
   - Sorted object keys (lexicographic by code point).
   - NFC Unicode normalization on all strings.
   - Finite floats only; `NaN` / `±Inf` MUST cause emit-time failure.
   - Explicit handling of zero (`-0` is canonicalized to `0`).
   - No trailing whitespace; no insignificant whitespace anywhere.
   - UTF-8 encoding of the final canonical bytes before hashing.
5. **Transient-field exclusion (`TRANSIENT_FIELDS`).** The hash MUST be computed **after** removing volatile fields. The minimum exclusion set is:
```text
...
6. **Forbidden fields.** The `--forbid-field` set MUST reject any object that contains a field intended to be volatile but found leaking into the hashed payload (e.g., misplaced `generated_at` inside a `record.payload`). This is a hard validator gate, not a warning.
7. **Graph-shaped content.** For JSON-LD graphs that may have been merged from multiple contexts, `spec_hash` MAY alternatively be computed via **URDNA2015 RDF dataset canonicalization** followed by SHA-256. When this path is used the recorded value MUST be prefixed `urdna2015:sha256:<hex>` so the canonicalization route is auditable from the string alone. This is the **only** other permitted route; mixing canonicalizers for the *same* object family within a release is forbidden.

### 2.2 `run_id` — the activity-identity term

1. **Definition.** `run_id` identifies **one** execution of a governed step (a watcher pull, a pipeline run, a tile build, a catalog emission, a release decision, an AI inference). It is the activity identity that PROV-O and OpenLineage refer to as the **run** or **activity**.
...
```
   ...
   - `<ulid>` is a [ULID](https://github.com/ulid/spec) — 26-character Crockford base32, time-ordered, monotonic within a millisecond.
3. **Stability.** `run_id` MUST be generated **once**, at the start of the run, and propagated through every artifact, receipt, log, and downstream side-effect of that run. A run that emits multiple artifacts MUST share a single `run_id` across all of them.
4. **PROV / OpenLineage binding.** The same `run_id` MUST appear:
   - In the run receipt's `run_id` field.
   - In the PROV-O activity's `prov:id` (or equivalent local identifier).
   - In the OpenLineage `run.runId` if OpenLineage events are emitted.
   - In STAC item `properties.kfm:run_id` (per `kfm:` namespace discipline).
5. **Independence from `spec_hash`.** `run_id` MUST NOT participate in `spec_hash` computation. `spec_hash` is content-identity; `run_id` is activity-identity. Conflating them is the primary anti-pattern this ADR exists to prevent (see §13.1).
...
| **Catalog references** | STAC items carry `properties.kfm:spec_hash` and `properties.kfm:run_receipt_url`; DCAT distributions carry `dct:identifier` set to `jcs:sha256:<hex>`. |
| **Tiles** | PMTiles archives carry `spec_hash` in the metadata block (`kfm:spec_hash`) and `run_receipt_url`. |
| **AI receipts** | Each `AIReceipt` carries `run_id`, `inputs_spec_hash` (array), and the AI envelope's own `spec_hash`. |
...
| `docs/directory-rules.md` §6.1, §6.4 | `docs/adr/` is the canonical ADR home; `schemas/contracts/v1/...` is the canonical machine-schema home (per ADR-0001). | CONFIRMED in doc |
| `docs/directory-rules.md` §13.3, §18 | Schema-home authority is contested in some legacy paths; `policies/` vs `policy/` and `contracts/` vs `schemas/` remain NEEDS VERIFICATION. | CONFIRMED note |
...
The diagram below shows where each identifier is **minted** and where it **flows**. Boxes labeled with `NEEDS VERIFICATION` indicate planned locations not yet checked against a mounted repo.
...
> [!TIP]
> The dashed groups (`CATALOG`, `RELEASE`) reflect *proposed* terminal homes per `docs/directory-rules.md` §6 and §9. Their **paths** are CONFIRMED doctrine; their **presence** in the current mounted repo is NEEDS VERIFICATION.
...
- **Float precision corner cases.** RFC 8785 prescribes number serialization, but very large integers (> 2⁵³) need a documented convention. Left as Open Question §12.
...
### 8.4 Use OpenLineage `run_id` directly as KFM `run_id`

- **Pro:** Avoids a second identifier.
- **Con:** OpenLineage `runId` is a UUID; ULID gives time-ordering and a smaller string. Forcing UUIDs would lose ordering in logs and receipts.
- **Verdict:** Rejected. KFM `run_id` is the canonical term; OpenLineage `runId` is carried *as a parallel field* in the receipt when emitted, and the KFM `run_id` is mirrored into OpenLineage's `run.runId`.
...
Per `docs/directory-rules.md` §14.2, structural identity changes require an ADR plus a migration manifest. Authority status of each PROPOSED path below is NEEDS VERIFICATION against a mounted repo.
...
| **CI workflow** | `.github/workflows/identity-validate.yml` | Runs all of the above on every PR that touches `schemas/contracts/v1/common/`, `runtime/`, or `tools/validators/spec_hash/`. |

> [!NOTE]
> Test homes follow `docs/directory-rules.md` §6.6; CI workflow placement follows `.github/` per §5 canonical roots. Presence of these files in the current repo is **NEEDS VERIFICATION**.
...
- **Doctrine rollback.** Mark the ADR `superseded` and forward-link to the replacement. Per Directory Rules §14.2, superseded ADRs MUST be retained.
...
- **Caveat.** A rollback that re-permits `sha256:` (bare) as canonical MUST add a drift entry to `docs/registers/DRIFT_REGISTER.md` per Directory Rules §2.5.
...
- **Large-integer JSON encoding.** RFC 8785 does not impose limits, but JSON integers above 2⁵³ are unsafe in many parsers. **PROPOSED:** require string-encoded large integers and forbid raw integers > 2⁵³ at canonicalization time. NEEDS VERIFICATION.
...
**Fix.** Rollback is a **governed state transition**, not a file move (Directory Rules §9.1). The `rollback_card` MUST cite the `spec_hash` it is rolling back to.
...
| **`run_id`** | Deterministic activity identity, `run:<orchestrator>:<ULID>`. |
...
> Links are repo-relative and reflect canonical homes per `docs/directory-rules.md`. Presence in the current mounted repo is NEEDS VERIFICATION; mark broken links in `docs/registers/DRIFT_REGISTER.md`.
...
**Last reviewed:** 2026-05-11  ·  **Doc version:** v1  ·  **Status:** `proposed`
