...
related: [./README.md, ./ADR-0303-hydrology-source-descriptor-activation-gates.md, ./ADR-0304-hydrology-first-proof-lane.md, ./ADR-0310-hydrology-wbd-terms-rights-review.md, ../domains/hydrology/README.md]
...
notes: [
  ...
  Documentation verification prepares source-readiness review only. It does not activate connectors, publish artifacts, or support hydrologic claims.
  Exact owners, created date, policy label, source packet paths, validator behavior, CI enforcement, and live connector state remain NEEDS VERIFICATION.
]
[/KFM_META_BLOCK_V2] -->
...
<p align="center">
  <strong>Source documentation may prepare a source for review. It must never become hydrologic claim evidence.</strong>
</p>
...
<p align="center">
  ...
  <img alt="claim evidence: denied" src="https://img.shields.io/badge/claim%20evidence-denied-orange">
...
> [!IMPORTANT]
> **Preserved decision:** add hydrology source documentation verification artifacts and a manual review packet while keeping all hydrology sources blocked.
>
> **Preserved consequence:** source documentation checks are review support only. They are not claim evidence, not connector activation, not public release, and not a shortcut around EvidenceBundle closure.

> [!NOTE]
> **ADR numbering watch:** `docs/adr/README.md` lists this ADR and flags duplicate-number risk as an inventory item. Current revision work should not silently renumber this file. If maintainers later renumber or supersede it, update the ADR index and preserve this file as lineage.
...
| Connector posture | **Blocked by default.** No live connector is activated by this ADR. |
| Evidence posture | Source documentation supports source-readiness review only; it is not hydrologic claim evidence. |
| Protected boundary | Source documentation → inactive SourceDescriptor candidate → fixture review → EvidenceBundle → ReleaseManifest → governed public surface. |
...
| Current implementation state | `NEEDS VERIFICATION` for source packet paths, owners, CI enforcement, validator execution, source descriptor status, and live connector state. |

This ADR decides that **source documentation verification is a prerequisite to source activation**, not a substitute for evidence, validation, catalog closure, proof, review, release, or rollback.
...
Hydrology is KFM’s preferred first proof lane because it is public-safe enough to test early, but demanding enough to exercise source identity, source roles, time-series evidence, map layers, EvidenceBundle closure, finite negative outcomes, catalog/provenance closure, and rollback.

That strength creates a specific failure mode:

> “We reviewed the source documentation” can be mistaken for “the source now supports public claims.”

This ADR blocks that drift.

| Drift risk | Failure mode | ADR response |
|---|---|---|
| Documentation laundering | A source documentation page is cited as proof of a hydrologic fact. | Documentation checks are review artifacts only. |
| Premature connector activation | A live connector is enabled after an API page or URL is found. | Connectors remain blocked until source, fixture, policy, review, release, and rollback gates pass. |
| Source-role collapse | NFHL, USGS observations, WBD boundaries, NHDPlus HR identity, and terrain derivatives are treated as interchangeable. | Source-role review is mandatory before descriptor activation. |
...
| False certainty | Source docs say a field exists, so generated language treats the claim as settled. | Runtime answers still require EvidenceRef → EvidenceBundle resolution or must abstain. |
...
This ADR is a decision record for **source documentation verification**. It is not a source registry, connector, fixture set, policy file, schema, release manifest, proof pack, or runtime enforcement claim.

| Evidence class | Status | Supports | Does not prove |
|---|---|---|---|
| Existing ADR-0305 file | `CONFIRMED` | Target path and prior decision/consequence existed before this revision. | CI enforcement, owners, validator execution, live connector state, or public-release readiness. |
| ADR index | `CONFIRMED repo doc / NEEDS VERIFICATION coverage` | ADR index expects ADR navigation, truth labels, rollback/supersession discipline, and duplicate-number review. | Exhaustive ADR coverage or absence of all numbering collisions. |
| Hydrology domain README | `CONFIRMED repo doc / draft` | Hydrology is framed as first proof lane; source roles and fixture-first posture are explicit. | That all adjacent hydrology paths, commands, owners, or public surfaces are enforced. |
| ADR-0303 activation gates | `CONFIRMED repo doc` | Descriptor-first hydrology source registration is allowed only while blocked, non-fetching, non-public, and manually reviewed before activation. | Source activation, data fetch, or public publication. |
| ADR-0310 WBD terms/rights review | `CONFIRMED repo doc` | Public-domain source status does not equal KFM public-release eligibility. | Public WBD release or live WBD ingestion. |
| Hydrology architecture corpus | `CONFIRMED doctrine / PROPOSED implementation` | WBD/HUC12, NHDPlus HR, USGS Water Data, NFHL, terrain derivatives, EvidenceBundle, ReleaseManifest, and source-role separation belong in the proof-lane design. | Current runtime behavior or emitted proof objects. |
...
| Label | Meaning |
|---|---|
| `CONFIRMED` | Verified from current repository evidence or supplied KFM doctrine. |
...
| `UNKNOWN` | Not verified strongly enough in this revision context. |
| `CONFLICTED` | Multiple repository facts, naming patterns, or authority claims appear to collide and require maintainer decision. |
...
| `ERROR` | Finite outcome when required artifacts, references, validation state, or schema shape is missing or malformed. |
...
The packet verifies the source’s documentation and review readiness. It does **not** verify hydrologic truth.
...
4. **Source descriptors start inactive.**  
   Source descriptor records must default to a blocked or inactive state until source role, rights, cadence, scope, caveats, validation, and review are complete.
...
> “Do we understand this source well enough to draft an inactive source descriptor and no-network fixture review?”
...
> [!CAUTION]
> The `active` state in this diagram requires a later source activation decision. ADR-0305 by itself never moves a source to active.
...
| Source family | Source role | Documentation review must establish | Must not be used to claim |
|---|---|---|---|
| WBD / HUC12 | Hydrologic-unit boundary and watershed packaging context. | Official docs, service/download path, field list, hierarchy, geometry metadata, update fields, review cadence, geometry/content fingerprint plan. | That a public hydrologic claim is true without fixture validation and EvidenceBundle closure. |
...
| USGS Water Data | Observation and time-series evidence. | API family, parameter-code semantics, unit handling, timestamp behavior, qualifiers, provisional/approved state, no-data behavior, retrieval/cadence limits. | Claim support if qualifiers, units, time, approval state, or no-data reason are missing. |
...
This ADR expects three categories of artifacts. Candidate paths are **PROPOSED / NEEDS VERIFICATION** until active repository conventions are confirmed.
...
```text
data/registry/sources/hydrology/*.source_descriptor.json
```

If the repository standardizes a different hydrology source registry home, use that home and update this ADR or a successor ADR. Every descriptor must default to a blocked or inactive state until review gates pass.
...
If the active repository uses a different lifecycle or receipt convention, adapt through an ADR or migration note instead of creating parallel authority.
...
## Path and responsibility-root basis

`docs/adr/` is the correct home for this file because it is a human-facing architecture decision record. It governs source documentation verification posture; it does not store source data, machine schemas, policy code, fixtures, receipts, proofs, or release artifacts.

| Concern | Correct responsibility root | This ADR’s posture |
|---|---|---|
| Human decision record | `docs/adr/` | `CONFIRMED` target path. |
| Hydrology domain guidance | `docs/domains/hydrology/` | Link and align; do not duplicate all lane docs. |
| Source descriptors | `data/registry/...` or accepted source registry home | `PROPOSED / NEEDS VERIFICATION`. |
| Machine schemas | `schemas/` or accepted schema home | Out of scope; reference only. |
| Semantic contracts | `contracts/` | Out of scope; reference only. |
| Policy rules | `policy/` or accepted policy home | Out of scope; reference only. |
| Validators | `tools/validators/...` or accepted validator home | Out of scope; reference only. |
| Tests and fixtures | `tests/`, `fixtures/`, or accepted homes | Out of scope; reference only. |
| Receipts/proofs/release | `data/receipts/`, `data/proofs/`, `release/`, or accepted homes | Out of scope; reference only. |

> [!IMPORTANT]
> Do not create a root-level `hydrology/` or `source_documentation/` folder for this ADR. Domain materials belong under responsibility roots.
...
| Policy review | Rights, sensitivity, public-surface, and activation checks pass. | `DENY` on failure. |
...
| “NFHL is a regulatory flood context source family.” | Yes, if supported by reviewed source role and later fixture/proof path. | Proceed only after EvidenceBundle closure for public use. |
...
|---|---|---|---|
| 0 | Repo and ADR inventory | Confirm target file, ADR index, numbering status, owners, hydrology docs, registry paths. | No implementation claim without evidence. |
...
- [ ] ADR index lists this ADR with status and decision summary.
- [ ] ADR numbering inventory has been run and any collision is resolved, accepted, or explicitly tracked.
...
- [ ] ADR index is updated or a numbering/inventory issue is tracked.
...
|---|---|---|
| ADR index entry and numbering inventory | `NEEDS VERIFICATION` | ADR history and references should not collide silently. |
| Created date | `NEEDS VERIFICATION` | Existing metadata did not confirm original creation date. |
...
| Exact source descriptor path | `NEEDS VERIFICATION` | Align with the accepted source registry convention. |
...
<details>
<summary><strong>Appendix A — Maintainer commands before accepting this ADR</strong></summary>

Run from the repository root on the branch where this ADR will be committed.
...
# Confirm target ADR and nearby hydrology ADRs.
find docs/adr -maxdepth 1 -type f -name 'ADR-03*.md' | sort
...
# Find references to ADR-0305.
grep -RInE 'ADR-0305|hydrology-source-documentation-verification' docs data contracts schemas policy tests tools release 2>/dev/null || true

# Inspect hydrology source descriptor, validator, and fixture surfaces.
find data/registry/sources/hydrology tools/validators tests/domains/hydrology docs/domains/hydrology \
...
<details>
<summary><strong>Appendix B — Working glossary</strong></summary>

| Term | Meaning in this ADR |
|---|---|
| `SourceDescriptor` | Governed source-admission record declaring source identity, role, rights, sensitivity, support, cadence, review state, validation plan, and publication intent. |
| `source documentation verification` | Review step that checks source documentation, terms, fields, cadence, caveats, access, and source role before descriptor or fixture review. |
| `claim evidence` | EvidenceBundle-resolved support for a public or semi-public claim. Source documentation review is not claim evidence. |
| `EvidenceBundle` | Resolved support package linking claims to evidence refs, artifacts, catalogs, provenance, limitations, integrity, policy, and release state. |
| `ReleaseManifest` | Release object tying artifacts, catalogs, proofs, policy decisions, rollback target, and public aliases together. |
| `LayerManifest` | Downstream layer delivery object. It can point to released artifacts but cannot become canonical truth. |
| `documentation_reviewed` | Review milestone showing source docs were inspected. It is not activation. |
| `VERIFIED_INACTIVE` | Source activation posture used by adjacent ADRs for reviewed-but-not-public source candidates. |
| `DENY` | Policy or release state blocks an action. |
| `ABSTAIN` | Evidence, identity, source support, or citation support is insufficient. |
| `ERROR` | Required artifact, schema, reference, or validation state is missing or malformed. |
