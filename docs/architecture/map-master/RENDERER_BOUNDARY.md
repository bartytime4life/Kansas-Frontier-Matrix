...
The acquisition inventory scans renderer dependencies and runtime-acquisition paths. It caps the number of candidate files, each candidate input at 1 MiB, and the complete scan at 16 MiB. Symlink entries under scanned roots are denied without reading their targets, and every regular input must resolve within the repository root. Reads then traverse from a pinned repository directory descriptor with no-follow protection on every component and require the opened descriptor to retain the pre-open device, inode, and file type. Missing descriptor safeguards or an input replacement returns `ERROR` without classifying the diagnostic as renderer acquisition. It is **not** a source registry, a package allowlist, or source admission.
