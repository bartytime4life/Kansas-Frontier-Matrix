...
| Corrections filename | **CONFLICTED references** | Current file is `corrections-first-class.md`; repository code search finds 25 references to absent `corrections-are-first-class.md` |
...
- This v0.8 change updates the evidence checkpoint, ADR inventory, decision status, and related conflict disclosure. It does **not** amend doctrine meaning or accept any draft doctrine document.
...
| [`ADR-0006`](../adr/ADR-0006-maplibre-boundary--only-maplibreadapter-imports-maplibre.md) | Only `MapLibreAdapter` imports MapLibre | `accepted` |
...
| [`ai-as-assistant.md`](./ai-as-assistant.md) | `cacdd29…` | AI as assistant, never authority | v1.1 `draft`; metadata proposes a different filename |
| [`encyclopedia.md`](./encyclopedia.md) | `4392fb9…` | Doctrine vocabulary and concept index | v0.1 `draft` |
...
| `DOC-DOC-003` | Corrections doctrine filename | Actual file is `corrections-first-class.md`; code search finds 25 references to absent `corrections-are-first-class.md` | Use the actual current path for new links; do not mass-rewrite without inbound-reference and identity review | Naming migration or reviewed alias |
...
| `DOC-DOC-005` | `docs/architecture/maplibre-3d.md` references | Repeatedly referenced but absent at the snapshot; ADR-0007 is accepted for renderer architecture only | Do not claim the page exists or infer runtime readiness from ADR status | Verification, creation request, or migration decision |
...
### No-loss ledger for v0.7 → v0.8

| v0.7 concern | v0.8 treatment |
|---|---|
...
9. **NEEDS VERIFICATION — renderer documentation.** Is a `maplibre-3d` architecture page still intended or superseded by another current page now that ADR-0007 is accepted for architecture only?
