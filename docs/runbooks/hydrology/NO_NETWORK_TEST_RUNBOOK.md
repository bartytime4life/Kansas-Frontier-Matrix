...
type: runbook
version: v1.2.2
prior_version: v1.2.1
prior_state: repository-grounded shared startup guard whose public SocketType alias still exposed the unpatched extension socket type
...
|---|---|---|---|
| `test_no_network_proof.py` and `tools/ci/kfm_no_network/sitecustomize.py` | Fresh Python interpreters require explicit activation, route the public `socket.SocketType` constructor alias through the guarded subclass, and exercise IPv4/IPv6 connection, connected and destination-bearing socket-send, direct resolver, and `urllib` URL-open paths | Shared startup enforcement for the 15 named Python APIs and their guarded public socket constructor path in the bounded validation step | Runner-wide, non-Python, dependency-install, operating-system, container, namespace, private `_socket.socket`, or non-named API isolation |
