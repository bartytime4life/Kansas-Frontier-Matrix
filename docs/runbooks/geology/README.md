...
notes:
  ...
  - "The Geology workflow executes four bounded no-network profiles although its opening comment still says three; this README records the conflict without changing workflow bytes."
...
| [`NO_NETWORK_TEST_RUNBOOK.md`](./NO_NETWORK_TEST_RUNBOOK.md) | Repository-grounded v0.2 draft; four bounded synthetic profiles | Useful for exact offline profiles; broader Geology validation and operational maturity remain `HOLD` |
...
| [`domain-geology`](../../../.github/workflows/domain-geology.yml) | Read-only workflow executes four bounded profiles; opening comment still says three; broader proof and release producers remain held | `CONFLICTED` workflow prose versus body; current executable boundary is four named profiles |
...
The workflow's opening comment says three profiles while the body executes four. The body and current no-network runbook establish the bounded executable inventory; the stale comment is a documentation conflict to repair separately. This README does not edit workflow bytes or rewrite any workflow-bound receipt.

### No-network limit

`KFM_NO_NETWORK=1` and in-process socket/HTTP guards are bounded controls. They are not an operating-system firewall, namespace, proxy, egress policy, or proof that every possible network path is blocked. Use the exact child-runbook qualification.
