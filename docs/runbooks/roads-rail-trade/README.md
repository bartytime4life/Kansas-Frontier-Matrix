...
status: draft; repository-grounded; bounded synthetic CorridorRoute validation executable; promotion and rollback readiness reconciled; production source operation, policy, proof, rollback execution, release, deployment, and publication held
owners:
...
evidence_snapshot:
  ...
  rollback_runbook_blob: 9b9acd430faa916b8645d398edbf6a326b1ff466
...
notes:
  - "v1.2 reconciles the no-network and rollback children with current repository evidence; production rollback execution remains held."
  ...
  - "The source-refresh child remains a stale proposal-era procedure with no-mounted-repository claims and is not current operational authority."
...
one implemented synthetic validation profile and repository-grounded promotion
and rollback readiness procedures separate from stale source-refresh instructions. It is an index and boundary contract, not
...
|---|---|---|
| [`NO_NETWORK_TEST_RUNBOOK.md`](./NO_NETWORK_TEST_RUNBOOK.md) | Repository-grounded bounded procedure for the implemented synthetic `CorridorRoute` profile | Use its exact no-network commands and finite outcomes; it does not establish broader domain, policy, proof, release, or publication readiness |
...
| [`ROLLBACK_RUNBOOK.md`](./ROLLBACK_RUNBOOK.md) | Repository-grounded rollback-readiness validation and accountable-review handoff; production execution held | Use it to classify incidents, validate a proposed generic RollbackCard candidate, run synthetic rehearsals, and prepare a handoff; it does not mutate public state |
...
3. Keep the proposal-era source-refresh runbook labeled until its complete
   procedure contract is reconciled.
