...
telemetry_ref: "../../releases/v11.2.6/docs-modelcard-template-telemetry.json"
telemetry_schema: "../../schemas/telemetry/docs-modelcard-template-v11.2.6.json"
...
provenance_chain:
  - "docs/templates/model_card.md@v11.2.6"
  - "docs/templates/model_card.md@v11.2.4"
...
json_schema_ref: "../../schemas/json/kfm-model-card-template-v11.2.6.schema.json"
shape_schema_ref: "../../schemas/shacl/kfm-model-card-template-v11.2.6-shape.ttl"
story_node_refs: []

immutability_status: "version-pinned"
doc_uuid: "urn:kfm:doc:templates:model-card:v11.2.6"
semantic_document_id: "kfm-model-card-template-v11.2.6"
event_source_id: "ledger:kfm:doc:templates:model-card:v11.2.6"
...
The template is compatible with **STAC 1.x**, **DCAT 3.0**, and **PROV-O**, enabling catalog and knowledge graph ingestion.
...
All KFM models (e.g., Focus Mode inference, entity extraction, summarization, ranking) **must** include a `model_card.md` derived from this template and:

- Pass **`docs-lint.yml`** (front-matter, headings, links, Mermaid rules).  
...
- Align with **KFM-MDP v11.2.6** and the KFM-STAC / KFM-DCAT / KFM-PROV profiles for catalog and graph ingestion.
...
📁 KansasFrontierMatrix/
  📁 docs/                                # All documentation for KFM
    📁 templates/                         # Reusable documentation templates
      📄 README.md                        # Templates index & usage rules
    ...
    📁 ai/
      📁 models/                          # 🤖 Per-model directories (one per governed model)
        📁 <model_name>/                  # e.g., focus_transformer_v2_1
          ...
          📁 config/                      # ⚙️ Config files (YAML/JSON)
...
- Ensure `path` in the YAML front-matter matches the actual model card location.  
- Keep emojis and comments in directory trees consistent with KFM-MDP’s layout profile.
...
- Train, fine-tune, or adopt a **governed AI/ML model** that will run in any KFM environment (ETL, Focus Mode, web, batch, etc.).  
...
- **NLP models** — summarization, NER, relation extraction, embeddings.  
...
- **Vision/models-on-maps** — imagery annotation, symbol detection for historical maps.

### 2. Section Map for Instantiated Cards

Concrete model cards created from this template should map content as:

- **Model Overview** — identity, version, authors, license, repo path.  
...
- **Evaluation Metrics** — test results, robustness, OOD behavior.  
...
In instantiated model cards, include:
...
|-------------------------|-------------|
| **Base Architecture**   | Encoder-only, encoder–decoder, LLM, CNN, etc. |
| **Backbone / Checkpoint** | e.g., `bert-base-uncased`, `llama-3-8b`. |
| **Inputs**              | Text, imagery, raster tiles, metadata (time/location/CARE tags). |
| **Outputs**             | Entities, summaries, scores, embeddings, masks. |
| **Parameter Count**     | Approximate parameter count. |
| **Frameworks**          | PyTorch / TensorFlow versions. |
| **Dependencies**        | Major libs, external services (e.g., vector DB, Redis). |
~~~
...
| **Hyperparameters**    | Summary; configs live under `config/`. |
| **Hardware/Env**       | GPU model, CPU, RAM, region, Docker image digest, random seeds. |
...
semantic_document_id: "<kfm-model-<model_name>-vX.Y.Z>"
doc_uuid: "<urn:kfm:doc:model:<model_name>:vX.Y.Z>"
event_source_id: "ledger:src/ai/models/[model_name]/model_card.md"
immutability_status: "version-pinned"
...
Missing or malformed required fields will cause `docs-lint.yml` and schema checks to fail.
...
| **License**        | Code & weights license(s).                       |
...
- **STAC**  
  - If model checkpoints or inference products are exposed as STAC Items, note:
    - Collection ID, Item IDs, and `assets` pointing to weights/configs.  

- **DCAT**  
  - If the model is cataloged as a `dcat:Dataset` or `dcat:DataService`, specify:
    - Service endpoints (for hosted APIs).  
    - Distribution URLs for model artifacts (weights, configs, docs).  

- **PROV-O**  
  - Model training and evaluation should be captured as:
    - `prov:Activity` — train/eval runs.  
    - `prov:Entity` — model weights, metrics, this model card.  
    - `prov:Agent` — authors, CI workflows, or external providers.  

Example alignment note in a concrete card:
...
- Story Nodes may reference a model via IDs like `urn:kfm:model:<model_name>@vX.Y.Z`.  
- Focus Mode surfaces model card content when explaining why a model behaved a certain way.  
- Cards should explicitly include stable identifiers (model name, version, `semantic_document_id`).

When writing a concrete model card:

- Use **clear, segmented subsections** so Focus Mode can extract coherent answers.  
- Avoid ambiguous pronouns; refer to the model by name (e.g. `Focus Transformer v2.1`).  
- Include **short recap lists** at the end of complex sections (e.g., biases, mitigations, known failure modes).

Focus Mode:

- ✅ MAY summarize sections, highlight metrics, and link to STAC/DCAT/graph records.  
- ❌ MUST NOT fabricate model capabilities, data sources, or governance approvals not present in the card.  
- ❌ MUST NOT override or weaken any safety restrictions documented here.

---

## 🗺️ Diagrams

Model cards may include **one optional Mermaid diagram per section** for architecture or flow (per KFM-MDP v11.2.6).
...
Rules for concrete cards:

- At most **one Mermaid** diagram per section.  
- Always explain the diagram in the surrounding text so it is understandable without rendering.
...
- **`docs-lint.yml`** validates:
  ...
  - Approved H2 headings and order (emoji + label).  
  - Mermaid diagrams and links.

- **`faircare-validate.yml`** checks:
  - Presence of a **FAIR+CARE summary**.  
  - Bias & risk documentation.  
  - CARE tags and references to relevant governance docs.

- **`ai-train.yml` & `ai-explainability.yml`**:
  - Training and eval runs must emit metrics JSON + explainability reports.  
  - Concrete cards should link to those outputs under `reports/ai/<model_name>/`.

- **`telemetry-export.yml`**:
  - Aggregates runtime + metrics into `focus-telemetry.json`, enabling trend visualization per model.

Concrete cards should link to representative artifacts:
...
Treat failing CI as a **spec or governance issue** to fix in the model card and associated workflows.
...
- **Accessible:** License, access controls, API endpoints or deployment locations.
- **Interoperable:** Alignment with KFM schemas, STAC/DCAT/PROV.
- **Reusable:** SBOM + manifest availability; documented limitations and data licenses.
...
- **CARE – Responsibility & Ethics:** Known risks, mitigations, monitoring commitments, and shutdown criteria.
~~~

Also include:

- Governance decision summary: `Approved / Pending / Changes Required / Deprecated`.  
- Links to FAIR+CARE and audit reports, such as:
...
For models touching Indigenous data, sensitive locations, or community-held knowledge:

- Document how sensitive content was filtered, masked, or omitted.  
- Confirm adherence to sovereignty policies and any additional review processes.

---

## 🕰️ Version History

Update this table as the template evolves. Concrete model cards should maintain their own independent history.

| Version   | Date       | Author        | Summary                                                                                                               |
|----------:|-----------:|--------------|-----------------------------------------------------------------------------------------------------------------------|
| v11.2.6   | 2025-12-11 | `@kfm-docs`  | Updated to KFM‑MDP v11.2.6; aligned paths to v11.2.6 releases/schemas; synchronized with Templates Index and Core Markdown Template; clarified STAC/DCAT/PROV, Story Node, and CI integration. |
...
| v10.2.2   | 2025-11-12 | `@kfm-docs`  | Tied to telemetry v2; clarified governance and reproducibility gates.                                                 |
...
🤖 **Kansas Frontier Matrix — AI/ML Model Card Template (v11.2.6)**  
