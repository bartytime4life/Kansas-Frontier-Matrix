...
**CONFIRMED:** KFM model cards are stored in `mcp/model_cards/` and carry a standardized YAML front matter including `intent`, `doc_uuid`, `release_stage`, `lifecycle`, `fair_category`, and `care_label`.
...
- **CONFIRMED:** Markdown model cards in `mcp/model_cards/` with required YAML front matter.
- **PROPOSED:** A generated `docs/ai/model_cards.index.json` (or similar) produced in CI from model-card front matter.
...
**Legend:**
- **Evidence:** `CONFIRMED` = explicitly present in model-card front matter (or model-card content) as indexed from project docs.
