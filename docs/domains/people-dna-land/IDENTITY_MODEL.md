<!-- [KFM_META_BLOCK_V2]
doc_id: kfm://doc/people-dna-land/identity-model/v1
title: People / DNA / Land — Identity Model
type: standard
version: v1
status: draft
owners: <TODO: Docs steward + People/DNA/Land domain steward>
created: 2026-05-18
updated: 2026-05-18
policy_label: public
related:
...
notes:
  ...
  - Doctrine claims grounded in Atlas v1.1 Ch. 16 / §7.14, Pass-10 Idea Index, Directory Rules v1.1, and the Domain-Driven Design Reference.
[/KFM_META_BLOCK_V2] -->
...
[![Doc Lint: TODO](https://img.shields.io/badge/doc%20lint-TODO-lightgrey)](#)
[![License: TODO](https://img.shields.io/badge/license-NEEDS%20VERIFICATION-lightgrey)](#)
...
| **Authority of these rules** | **CONFIRMED doctrine** where derived from Atlas v1.1 Ch. 16 / §7.14, the Pass-10 Idea Index, Directory Rules v1.1, and the Domain-Driven Design Reference |
...
| **Responsibility roots** | `schemas/contracts/v1/people/`, `contracts/people/`, `policy/sensitivity/people/`, `policy/consent/people/` (PROPOSED per Atlas v1.1 Ch. 24.13) |
...
| **Last reviewed** | 2026-05-18 (initial draft) |
...
- **Directory Rules v1.1 §6.1** — `docs/domains/<domain>/` is the canonical location for human-facing domain references, with `people-dna-land/` named explicitly.
- **Directory Rules v1.1 §4** Step 1 — "Explains something to humans" → `docs/`.
- **Directory Rules v1.1 §4** Step 3 — domain appears as a **segment** inside the responsibility root, never as a root itself.
...
└── domains/
    ...
    └── people-dna-land/
        ...
        ├── CONSENT_MODEL.md       (PROPOSED — not yet authored)
        ├── LAND_OWNERSHIP.md      (PROPOSED — not yet authored)
        └── DNA_HANDLING.md        (PROPOSED — not yet authored)
```
...
- `docs/standards/ISO-19115.md` — metadata profile (relevant for source-descriptor crosswalk)
...
> [!NOTE]
> **Domain-Driven Design framing.** The Domain-Driven Design Reference distinguishes objects defined by **identity** (Entities — life-cycle continuity) from objects defined by **attributes** (Value Objects — immutable, side-effect-free). KFM's People/DNA/Land model places the identity-bearing objects above as Entities and treats things like `NameVariant`, normalized `LegalDescription` strings, and DNA segment endpoints as Value Objects that **describe** an entity without **being** one.
...
> [!NOTE]
> **Term stability is non-negotiable.** Preserve KFM compound terms exactly: `PersonCanonical` (not "canonical person"), `LifeEvent` (not "life-event"), `DNAKitToken` (not "DNA token"), `Ownership Interval` (not "ownership period"). Renaming silently to industry-generic vocabulary is a doctrine violation per Directory Rules §13.
...
**PROPOSED** (Atlas v1.1 §16.E, repeated across every object family in this domain):
...
| **normalized digest** | A canonicalized hash over the assertion's identity-relevant payload, excluding volatile fields (timestamps, ordering, UUIDs that are themselves derived). | **CONFIRMED** (Pass-10 C1-02): canonicalize via **RFC 8785 JCS** then **SHA-256**, recorded as `jcs:sha256:<hex>`. |

### 5.3 Canonicalization — what gets normalized

**CONFIRMED** for the canonicalization step (Pass-10 C1-02, Atlas KFM-P32-PROG-0003):

- **JSON layer:** RFC 8785 JCS (JSON Canonicalization Scheme). Deterministic key ordering, whitespace stripping, number normalization. The KFM `spec_hash` is `jcs:sha256:<hex>` over the JCS output.
...
> [!IMPORTANT]
> **The choice of canonicalization is reviewable and must be recorded.** Mixing JCS and URDNA2015 hashes for the same logical object produces silent verification failures downstream (Pass-10 C8-05, identified contradiction). This document defaults to **JCS+SHA-256** for all People/DNA/Land identity digests. Any departure requires an ADR and a recorded reason in the EvidenceBundle.
...
flowchart LR
    subgraph SOURCES["📚 Source records"]
        VR["Vital / Census /<br/>Probate / Military"]
        GED["GEDCOM /<br/>tree overlay"]
        DTC["DNA vendor<br/>match data"]
        LR["Land instrument /<br/>assessor record"]
    end

    subgraph LEVEL1["① Person Assertion (source-anchored)"]
        PA["Person Assertion<br/>identity = src + role + scope + digest"]
    end

    subgraph LEVEL2["② Person Identity Candidate (proposed match)"]
        PIC["Person Identity Candidate<br/>scoring band + match evidence<br/>+ steward state"]
    end

    subgraph LEVEL3["③ PersonCanonical (steward-approved)"]
        PC["PersonCanonical<br/>authority IRIs (LCNAF / VIAF / ISNI / Wikidata)<br/>+ EvidenceBundle + sensitivity tier"]
    end

    subgraph PUBLIC["🌐 Public surface (governed API only)"]
        PUB["Public-safe<br/>identity projection<br/>(filtered by tier + consent)"]
    end
    ...
    PIC -- "steward review<br/>(ANSWER / ABSTAIN / DENY)" --> PC
...
|---|---|---|
| **① Person Assertion** | A source said this. The assertion is reproducible from the source. Identity is digest-stable. | KFM has not yet claimed two assertions refer to the same person. |
| **② Person Identity Candidate** | KFM has scored a possible match between two or more assertions, with explicit match evidence and a confidence band. The candidate is reviewable. | KFM has not yet declared the match canonical. The candidate can be rejected, narrowed, or split. |
| **③ PersonCanonical** | A steward (with separation-of-duties at release maturity) has approved the consolidated identity. Authority IRIs anchor it. An EvidenceBundle resolves it. | The canonical identity is not a public release. Release requires a separate `ReleaseManifest`, tier compatibility, consent (for living persons), and rollback target. |
...
**CONFIRMED / PROPOSED** (Atlas v1.1 Ch. 24.5 — Master Sensitivity / Rights Tier Reference):
...
|---|---|---|---|
| T4 → T3 | PolicyDecision + ReviewRecord + agreement | Steward + rights-holder where applicable | Reversible |
...
| Any tier → T4 (downgrade) | CorrectionNotice + ReviewRecord | Steward + rights-holder where applicable | Always permitted |
...
> [!NOTE]
> **`k` is policy, not model.** The model permits k-anonymity as a tier-transition transform; the specific *value* of `k` and the named redaction profile (`profile:sinc-obscure-10km`, etc.) live in `policy/sensitivity/people/` (PROPOSED) — not in this document.
...
DNA evidence enters KFM as **tokens and aggregates**, never as raw genotype. The tokenization model is **PROPOSED** (Atlas KFM-P18-PROG-0026 — Client-side DNA salt and token model):
...
> [!CAUTION]
> **Revocation is part of the identity contract.** When a DTC consent is revoked, the revocation **must** propagate through the canonical store and every derivative: tombstones for the canonical record, invalidation of any DNAKitToken-bearing match evidence, and `CorrectionNotice` for any release that referenced it. The boundary between tombstone (audit trace retained) and full erasure (right-to-be-forgotten) is **OPEN** — see §17 and Pass-10 C5-09.
...
| **Deed Instrument / Title Instrument / Assessor Record / TaxRecord** | Specializations of LandInstrument by recording-authority class. | Each has its own identity within its recording class. |
...
```mermaid
flowchart TD
    LI1["LandInstrument<br/>(Deed, 1872)"] --> LOA1["Land Ownership Assertion<br/>(1872–1889)"]
    LI2["LandInstrument<br/>(Probate, 1889)"] --> LOA2["Land Ownership Assertion<br/>(1889–1903)"]
    LI3["LandInstrument<br/>(Deed, 1903)"] --> LOA3["Land Ownership Assertion<br/>(1903–1947)"]
    LI4["LandInstrument<br/>(Tax sale, 1947)"] --> LOA4["Land Ownership Assertion<br/>(1947–…)"]

    LOA1 --> OI["Ownership Interval<br/>(composed)"]
    LOA2 --> OI
    LOA3 --> OI
    LOA4 --> OI

    OI -. "gaps + conflicts<br/>surfaced, never smoothed" .-> GAP["chain-of-title gap test<br/>(PROPOSED, §16.K)"]

    OI --> PV["Parcel Version graph<br/>(time-aware)"]
...
> [!WARNING]
> - **Assessor records are not title evidence.** They are value-and-roll information; treating them as title fails the `assessor-as-title denial` test (PROPOSED, Atlas §16.K).
...
> - **Private person-parcel joins are denied by default.** Joining a living person to a private parcel is a T4 join. Public surfaces never emit it without consent, redaction, and review (Atlas v1.1 §24.5.2; deny-default Register §20.5).
...
```mermaid
flowchart LR
    RAW["📥 RAW<br/><sub>SourceDescriptor exists<br/>(role, rights, sensitivity, hash)</sub>"]
    WORK["🛠️ WORK<br/>QUARANTINE<br/><sub>normalize schema/geometry/time/<br/>identity/evidence/rights/policy</sub>"]
    ...
    PUB["🌐 PUBLISHED<br/><sub>ReleaseManifest + correction path<br/>+ rollback target</sub>"]

    RAW -- "Admission gate<br/>(SourceDescriptor)" --> WORK
    WORK -- "Normalization gate<br/>(TransformReceipt +<br/>ValidationReport +<br/>PolicyDecision)" --> PROC
    PROC -- "Catalog-closure gate<br/>(CatalogMatrix +<br/>EvidenceBundle)" --> CAT
    CAT -- "Release gate<br/>(ReleaseManifest +<br/>ReviewRecord +<br/>rollback target)" --> PUB

    WORK -. "fail-closed<br/>QUARANTINE" .-> Q[("🔒 quarantine<br/>with reason")]
    PROC -. "structured FAIL" .-> HOLD1[("⏸ HOLD in WORK")]
    CAT -. "no public edge" .-> HOLD2[("⏸ HOLD in PROCESSED")]
    PUB -. "CorrectionNotice<br/>→ T4 demotion" .-> CORR[("🛑 demote or<br/>rollback")]
...
| Read RAW or WORK content | ❌ Never | (this is `T4` for AI per the deny register) |
...
| **Revocation cleanup tests** | DTC consent revocation propagates to tombstones, derivative invalidation, and CorrectionNotice. | PROPOSED |
...
| OQ-IDM-02 | Verify DNA consent/revocation enforcement (tokenization, no-log, tombstone, derivative invalidation). | `policy/consent/people/`, ingestion tests, revocation runbook. |
...
| OQ-IDM-06 | Pin canonicalization default (JCS vs URDNA2015) and document the receipt-recorded choice. | ADR + test-vector suite (Pass-10 C8-05). |
| OQ-IDM-07 | Define VIAF cluster-split rebinding policy for PersonCanonical. | VIAF stability policy doc (Pass-10 C7-03). |
| OQ-IDM-08 | Define tombstone-vs-erasure boundary for living-person and DTC data. | Revocation runbook (`docs/runbooks/people-dna-land/revocation.md` PROPOSED, NEEDS VERIFICATION). |
...
- **Doctrine**
  - `docs/doctrine/directory-rules.md` — placement law that puts this file here
...
- **Standards (KFM crosswalks to external)**
  ...
  - `docs/standards/ISO-19115.md` — metadata profile
...
- **Within this domain (PROPOSED, not yet authored)**
  - `docs/domains/people-dna-land/README.md`
...
- **Atlas / encyclopedia**
  ...
  - Atlas v1.1 Ch. 24.5 — Master Sensitivity / Rights Tier Reference
  - Atlas v1.1 Ch. 24.13 — Atlas ↔ Dossier ↔ Responsibility-Root Crosswalk
  - `KFM-encyclopedia.md` §7.14 — People/DNA/Land encyclopedia entry
...
- **Co-mentions** — same county, both in Douglas County, KS. Same birth place (Indiana). Age progression consistent (34 → 54 over 20 years).
- **Negative evidence** — no death certificate in Douglas County 1880–1900 indexed for "Mary E. Johnson"; supports continuity.
- **Confidence band** — `0.78` (PROPOSED scoring scale; actual scoring rubric lives in `policy/match-scoring/people/` — PROPOSED, NEEDS VERIFICATION).
...
Because the person is historical (not living), the default tier is **T1** rather than T4. A `ReleaseManifest` and `ReviewRecord` permit emission via the governed API. Public clients query the `PeopleDNALandDecisionEnvelope` resolver and receive an `ANSWER` with the EvidenceBundle reference. Living-person fields, DNA fields, and private person-parcel joins would emit **DENY** instead.
...
> **Related docs:** [Directory Rules](../../doctrine/directory-rules.md) · [Truth Posture](../../doctrine/truth-posture.md) · [Trust Membrane](../../doctrine/trust-membrane.md) · [Lifecycle Law](../../doctrine/lifecycle-law.md) · [PROV](../../standards/PROV.md) · [Atlas v1.1 Ch. 16](../../atlases/) · [People/DNA/Land README](./README.md)
>
> **Last updated:** 2026-05-18 · **Status:** draft · **Owner:** *TODO* — Docs steward + People/DNA/Land domain steward
>
> [↑ Back to top](#mini-toc)
