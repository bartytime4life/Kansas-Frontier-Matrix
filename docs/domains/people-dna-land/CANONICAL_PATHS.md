...
version: v1
status: draft
owners: <people-dna-land stewards; see CODEOWNERS — TODO confirm>
created: 2026-05-18
updated: 2026-05-18
policy_label: public
related:
...
notes:
  - "Doctrine grounded in directory-rules.md §§4, 6.3–6.5, 7, 12; KFM Encyclopedia §7.14; Domains Culmination Atlas Ch. 16 and §24.13."
  - "Specific repo presence of any path below is PROPOSED until verified against a mounted repository."
  - "Surface conflict between Directory Rules and the Atlas Crosswalk (§24.13) on schema-home segment shape."
[/KFM_META_BLOCK_V2] -->
...
[![last reviewed: 2026--05--18](https://img.shields.io/badge/last%20reviewed-2026--05--18-blue)](#)

> **Status:** draft · **Owners:** <people-dna-land stewards — TODO confirm via `CODEOWNERS`> · **Last updated:** 2026-05-18
...
2. **Tags** each lane with its lifecycle phase, authority class, and sensitivity posture so reviewers can place new files without re-deriving Directory Rules from first principles.
3. **Surfaces** the small, real conflicts between `directory-rules.md` and the Domains Culmination Atlas's responsibility-root crosswalk so future ADRs can resolve them, rather than letting drift accumulate.

**Out of scope.** This document does not define schemas, contracts, policy bundles, or test fixtures themselves; it only states **where** they live. It is not the source-of-truth for living-person, DNA, consent, or title doctrine — those live in the People / DNA / Land dossier (`[DOM-PEOPLE]`), the KFM Encyclopedia §7.14, the deny-by-default sensitivity register (Encyclopedia §13), and forthcoming policy bundles.

> [!NOTE]
> A README-equivalent **orientation** document for this domain — explaining purpose, object families, source families, and contributor expectations — should live at `docs/domains/people-dna-land/README.md`. This file is the **path register**; the README is the **doorway**.
...
|---|---|---|---|
| 1 | `directory-rules.md` — placement law and lifecycle invariant | **CONFIRMED** | `directory-rules.md` §§2.5, 3, 4, 6.3–6.5, 7, 12 |
...
| 3 | KFM Encyclopedia §7.14 — People / Genealogy / DNA / Land mission, boundary, objects | **CONFIRMED doctrine / PROPOSED implementation** | `kfm_encyclopedia.pdf` p.54 |
| 4 | Domains Culmination Atlas Ch. 16 and §24.13 — domain dossier and crosswalk | **CONFIRMED doctrine / PROPOSED placement** | `KFM_Domains_Culmination_Atlas_v1_1.pdf` Ch. 16, §24.13 |
...
|---|---|---|---|
| **Domain segment name** | `people-dna-land` | **CONFIRMED** | `directory-rules.md` §12 (named instance), §6.5 (literal listing under `policy/domains/`) |
| **Display name** | People / Genealogy / DNA / Land Ownership | **CONFIRMED doctrine** | Encyclopedia §7.14; Atlas Ch. 16 |
| **Citation short name** | `[DOM-PEOPLE]` | **CONFIRMED** | Atlas §2 Master Source Ledger; Appendix B |
| **Authority root for human doctrine** | `docs/domains/people-dna-land/` | **CONFIRMED rule / PROPOSED presence** | `directory-rules.md` §4 Step 3, §12, §13.4 |
| **Sensitivity default class (cross-lane)** | **T4 deny-default** for living-person, DNA, exact land/identity | **CONFIRMED doctrine** | Encyclopedia §13; Atlas §24.5 |
| **Lifecycle invariant applies** | RAW → WORK / QUARANTINE → PROCESSED → CATALOG / TRIPLET → PUBLISHED | **CONFIRMED** | `directory-rules.md` §3, §7.6; Atlas Ch. 16 §H |
| **Trust membrane** | Public reads route through `apps/governed-api/`, never directly to canonical stores | **CONFIRMED doctrine** | `directory-rules.md` §7.1; Anti-pattern §13.5 |

> [!NOTE]
> Two alternate segment forms appear in the corpus: `people` (Atlas §24.13) and `people-dna-land` (Directory Rules §6.5, §12). This document follows **Directory Rules** and uses `people-dna-land` everywhere; see [§9.2](#9-conflicts-surfaced).
...
|---|---|---|---|---|
| `docs/domains/people-dna-land/` | Human-facing domain dossier, runbooks, this file | Canonical | `directory-rules.md` §4 Step 3, §13.4 fix | PROPOSED |
| `contracts/domains/people-dna-land/` | Object-meaning Markdown for `PersonAssertion`, `LandOwnershipAssertion`, `DNAMatchEvidence`, etc. (semantic, not machine schema) | Canonical | `directory-rules.md` §6.3 | PROPOSED |

> [!NOTE]
> Per `directory-rules.md` §6.3–6.4 and Anti-pattern §13.1: **`contracts/`** holds *meaning* (Markdown), **`schemas/`** holds *machine shape* (JSON Schema). Maintaining both `contracts/.../*.schema.json` and `schemas/.../*.schema.json` is drift — `schemas/contracts/v1/...` wins per **ADR-0001**.

### 5.2 Validation surfaces

| Path | Owns | Authority class | Rule citation | Repo presence |
|---|---|---|---|---|
| `schemas/contracts/v1/domains/people-dna-land/` | JSON Schema (machine shape) for People / DNA / Land object families | Canonical (ADR-0001) | `directory-rules.md` §6.4, §17 (ADR table) | PROPOSED |
| `policy/domains/people-dna-land/` | Per-domain allow / deny / restrict / abstain rules (Rego or equivalent) | Canonical | `directory-rules.md` §6.5 (literal listing) | PROPOSED |
| `tests/domains/people-dna-land/` | Proof that schemas + policy + contracts are enforceable end-to-end | Canonical | `directory-rules.md` §4 Step 3, §12 | PROPOSED |
| `fixtures/domains/people-dna-land/` | Golden, valid, and invalid test inputs for the domain | Canonical | `directory-rules.md` §4 Step 3, §12 | PROPOSED |
...
| `pipeline_specs/people-dna-land/` | Declarative pipeline configuration — note the **absence** of the `domains/` segment under this root | Canonical | `directory-rules.md` §4 Step 3, §7.4, §12 | PROPOSED |

> [!CAUTION]
> **Connectors do not get a `connectors/domains/<domain>/` lane.** Per `directory-rules.md` §7.3, connectors are organized by *source family* (`connectors/<source>/`, e.g., `connectors/census/`, `connectors/kansas/`), not by KFM domain. A People / DNA / Land ingest pipeline composes connectors that may emit into `data/raw/people-dna-land/`, but the connectors themselves live elsewhere. Connectors **MUST NOT** write to `data/processed/`, `data/catalog/`, or `data/published/`.
...
|---|---|---|---|---|
| `data/raw/people-dna-land/` | RAW | Immutable source payloads or references with source role, rights, sensitivity, citation, time, and hash | `directory-rules.md` §4 Step 2, §12; Atlas Ch. 16 §H | PROPOSED |
| `data/work/people-dna-land/` | WORK | In-progress normalization (schema, geometry, time, identity, evidence, rights, policy) | `directory-rules.md` §4 Step 2, §12 | PROPOSED |
| `data/quarantine/people-dna-land/` | QUARANTINE | Held failures with recorded quarantine reason | `directory-rules.md` §4 Step 2, §12 | PROPOSED |
| `data/processed/people-dna-land/` | PROCESSED | Validated normalized objects, public-safe candidates with `EvidenceRef`, `ValidationReport`, digest closure | `directory-rules.md` §4 Step 2, §12; Atlas Ch. 16 §H | PROPOSED |
| `data/catalog/domain/people-dna-land/` | CATALOG | Catalog records, `EvidenceBundle` projections, graph / triplet projections, release candidates — note **singular `domain/`** segment under `data/catalog/` | `directory-rules.md` §4 Step 3, §12 | PROPOSED |
| `data/published/layers/people-dna-land/` | PUBLISHED | Released public-safe layer artifacts served via the governed API | `directory-rules.md` §4 Step 3, §12 | PROPOSED |
| `data/registry/sources/people-dna-land/` | REGISTRY (alongside) | Source descriptors and source-activation state for People / DNA / Land sources | `directory-rules.md` §4 Step 3, §12 | PROPOSED |

> [!IMPORTANT]
> **Lifecycle skip is an anti-pattern** (`directory-rules.md` §13.5). A pipeline writing directly from `data/raw/people-dna-land/` to `data/published/layers/people-dna-land/` violates the invariant. Promotion is a **governed state transition**, not a file move. For this domain, gates must additionally screen living-person status, DNA restriction, source terms, evidence closure, temporal validity, title / assessor / geometry source-role distinction, sensitivity policy, review state, release manifest, correction path, and rollback target. Source: Atlas Ch. 16 §I; *Unified Build Manual* §10.

### 5.5 Release lanes

| Path | Owns | Authority class | Rule citation | Repo presence |
|---|---|---|---|---|
| `release/candidates/people-dna-land/` | Per-domain release **candidates** awaiting promotion review | Canonical | `directory-rules.md` §4 Step 3, §12 | PROPOSED |
...
|---|---|---|---|
| `data/receipts/` | Ingest receipts, transform receipts (including **redaction receipts** for living-person / DNA / land-geometry transforms), run receipts | `directory-rules.md` §13.2, §13.5; Atlas Appendix A | PROPOSED |
| `data/proofs/` | Resolved `EvidenceBundle` payloads referenced by `EvidenceRef` | `directory-rules.md` §13.2; Glossary §19 | PROPOSED |
| `release/manifests/` | `ReleaseManifest` artifacts (the *release decision*, distinct from the released artifact in `data/published/`) | `directory-rules.md` §13.2, §19 | PROPOSED |
...
| `policy/sensitivity/` | Cross-cutting **sensitivity class** definitions (living-person, DNA / genomics, etc.) — *not* domain-segmented in §6.5 | `directory-rules.md` §6.5 | PROPOSED |
...
| `apps/governed-api/` | Public trust path; all public reads of People / DNA / Land artifacts MUST route here | `directory-rules.md` §7.1, §13.5 | PROPOSED |

> [!CAUTION]
> **Trust content does not live in `artifacts/`** (`directory-rules.md` §8.2, §13.5). Release manifests, evidence bundles, signed receipts, correction notices, and rollback cards belonging to this domain live in `data/receipts/`, `data/proofs/`, `release/`, or `data/published/` — **never** under `artifacts/`.

---

## 7. Sensitivity, rights, and consent surfaces

People / DNA / Land carries multiple deny-default classes from the **KFM Sensitive / Deny-by-Default Register** (Encyclopedia §13). The table below maps each class to the policy / data paths most directly responsible. Class assignment itself is CONFIRMED doctrine; specific transform profiles and gates are PROPOSED.
...
- **Assessor / tax records are NOT title truth.** A test under `tests/domains/people-dna-land/` must explicitly DENY any public claim that equates assessor records with title.
...
> [!WARNING]
> **No `policy/consent/` lane is established by Directory Rules §6.5.** The Atlas crosswalk (§24.13) proposes `policy/consent/people/` as a separate lane. Until an ADR creates `policy/consent/` as a sibling of `policy/sensitivity/` and `policy/rights/`, consent rules SHOULD land under `policy/domains/people-dna-land/consent/` or `policy/sensitivity/consent/`, and the ambiguity should be tracked in `VERIFICATION_BACKLOG.md`. See [§9.3](#9-conflicts-surfaced).
...
Use this list when a PR adds, moves, or renames a path that involves People / DNA / Land. It is the §16 reviewer checklist from `directory-rules.md`, specialized for this domain.
...
- [ ] **Lifecycle phase correct** (data only). The file is in the right phase and does **not** skip phases (anti-pattern §13.5 "Lifecycle skip").
- [ ] **Domain segment correct.** The domain appears as `people-dna-land` segment — *not* as a root, *not* under another domain, *not* under `people/` (Atlas form), and *not* split into `people/`, `dna/`, `land/` siblings.
- [ ] **No new root without ADR.** Any new canonical or compatibility root (e.g., a proposed `policy/consent/`) carries an ADR reference.
- [ ] **No parallel authority.** No new home for schemas / contracts / policy / sources / registries / releases / proofs / receipts without ADR.
...
- [ ] **Source-role distinction preserved.** Patent vs. deed vs. assessor vs. parcel-geometry sources are not collapsed.
...
Three real conflicts exist between authoritative project documents. Per `directory-rules.md` §2.5, Directory Rules wins on path questions, but these inconsistencies should be resolved by ADR rather than smoothed over.

### 9.1 Schema-home segment shape

| Source | Stated path for this domain | Status |
|---|---|---|
| `directory-rules.md` §4 Step 3, §6.4, §12 | `schemas/contracts/v1/domains/people-dna-land/` | **Wins** (canonical) |
| Atlas Culmination v1.1 §24.13 row 16 | `schemas/contracts/v1/people/` | Lineage / PROPOSED |

The Atlas crosswalk omits the `/domains/` segment under `schemas/contracts/v1/`. Directory Rules consistently include it. **This document uses the Directory Rules form.** Tracking item: open ADR-class question (Atlas §24.12 ADR-S-01 covers schema-home confirmation broadly).

### 9.2 Domain segment name

| Source | Segment used | Status |
|---|---|---|
| `directory-rules.md` §6.5 (literal `policy/domains/` listing) and §12 (Domain Placement Law instance list) | `people-dna-land` | **Wins** (canonical) |
| Atlas Culmination v1.1 §24.13 row 16 | `people` | Lineage / PROPOSED |

The Atlas uses the short form `people` only in the responsibility-root crosswalk; the dossier text consistently uses "People / DNA / Land". **This document uses `people-dna-land`** consistently with the explicit Directory Rules listings.

### 9.3 Consent-policy lane

| Source | Proposed home | Status |
|---|---|---|
| Atlas Culmination v1.1 §24.13 row 16 | `policy/consent/people/` | PROPOSED — *new lane* under `policy/` |
| `directory-rules.md` §6.5 | No `policy/consent/` lane established; siblings shown are `bundles/`, `fixtures/`, `tests/`, `runtime/`, `promotion/`, `sensitivity/`, `rights/`, `domains/`, `release/` | **Wins** — silent on consent lane |

Adding `policy/consent/` would be a new top-level lane under `policy/`. Per `directory-rules.md` §17, that is **ADR-class** and requires an ADR before treatment as canonical. Until then, consent rules SHOULD live under `policy/domains/people-dna-land/consent/` or under `policy/sensitivity/`.
...
| VB-PDL-02 | Schema-home shape: confirm `schemas/contracts/v1/domains/people-dna-land/` per Directory Rules vs. `schemas/contracts/v1/people/` per Atlas | ADR-0001 text + mounted repo | NEEDS VERIFICATION |
...
| VB-PDL-04 | Consent-policy home: ADR creating `policy/consent/` lane, or formal placement under `policy/domains/people-dna-land/consent/` or `policy/sensitivity/consent/` | ADR | OPEN / NEEDS VERIFICATION |
...
| VB-PDL-06 | Steward authority and confidentiality regime for People / DNA / Land (CODEOWNERS, review records, access tiers) | CODEOWNERS + review-record evidence | NEEDS VERIFICATION |
...
- `docs/registers/VERIFICATION_BACKLOG.md` — Outstanding verification items, including VB-PDL-01 through VB-PDL-10 above.
- `contracts/domains/people-dna-land/` — Object-meaning Markdown for People / DNA / Land families (PROPOSED).
- `schemas/contracts/v1/domains/people-dna-land/` — Machine schemas for People / DNA / Land (PROPOSED).
- `policy/domains/people-dna-land/` — Domain policy bundles (CONFIRMED rule, PROPOSED presence).
- `kfm_encyclopedia.pdf` §7.14 — People, Genealogy, DNA, and Land Ownership domain definition.
- `KFM_Domains_Culmination_Atlas_v1_1.pdf` Ch. 16, §24.13 — Domain dossier and responsibility-root crosswalk.
- `KFM_Unified_Implementation_Architecture_Build_Manual.pdf` §10 — Schema and contract plan; ADR-0001 reconciliation.
...
<details>
<summary><strong>A.1 — A new schema for <code>LandOwnershipAssertion</code></strong> (click to expand)</summary>

Applying `directory-rules.md` §4 Placement Protocol to a new JSON Schema for `LandOwnershipAssertion`:

1. **Identify the responsibility.** Defines an object's machine shape → `schemas/`.
2. **Identify the lifecycle phase.** Not applicable (this is schema, not lifecycle data).
3. **Identify the domain.** People / DNA / Land → segment `people-dna-land`. Under `schemas/`, the domain lives at `schemas/contracts/v1/domains/people-dna-land/` (Directory Rules §6.4, §12).
4. **Confirm authority.** `schemas/contracts/v1/domains/people-dna-land/` must already exist or be created with a per-root README (§9 / §15). No ADR needed unless this introduces a new sibling under `data/` or a new top-level root.
5. **Cite the rule.** PR description cites `directory-rules.md` §6.4 (schema home) + §12 (Domain Placement Law).
...
2. **Identify the lifecycle phase.** Receipts are emitted alongside lifecycle phases (Directory Rules §4 Step 2). Not RAW; not PROCESSED — it's a `receipts/` sibling.
3. **Identify the domain.** Receipts are typically **not** domain-segmented at the top level. The receipt itself carries `EvidenceRef` and source identity; the home is `data/receipts/`, with sub-organization driven by receipt type or run, not by domain segment unless the per-root README specifies otherwise.
4. **Confirm authority.** `data/receipts/` is a canonical lane (Directory Rules §13.2). No ADR needed.
5. **Cite the rule.** PR cites §13.2 (anti-pattern fix) and §13.5 ("Trust content in `artifacts/`" anti-pattern).
...
2. **Identify the lifecycle phase.** Not data.
3. **Identify the domain.** Per-domain policy → `policy/domains/people-dna-land/` (Directory Rules §6.5 literal listing). A consent-specific sub-lane could be `policy/domains/people-dna-land/consent/`.
4. **Confirm authority.** Sub-lane creation under `policy/domains/people-dna-land/` does **not** require an ADR; creating a new top-level `policy/consent/` lane **does** (see [§9.3](#9-conflicts-surfaced)).
5. **Cite the rule.** PR cites §6.5 (`policy/` shape) and §17 (no parallel authority without ADR).
...
- ❌ `people-dna-land/` at repo root with subfolders `data/`, `schemas/`, `policy/`, `docs/` — violates Domain Placement Law (§12, anti-pattern §13.4).
- ❌ `schemas/people-dna-land/...` — wrong root prefix; schema home is `schemas/contracts/v1/domains/...` per ADR-0001.
- ❌ `contracts/people-dna-land/land_ownership_assertion.schema.json` — schema in `contracts/`; per §6.4 schemas live under `schemas/`, contracts hold Markdown only (anti-pattern §13.1).
- ❌ `artifacts/release/people-dna-land/release_manifest.json` — release manifests live in `release/manifests/`, never `artifacts/` (anti-pattern §13.2, §13.5).
- ❌ `connectors/domains/people-dna-land/census/` — connectors are organized by source family, not by KFM domain (§7.3).
- ❌ Pipeline writing `data/raw/people-dna-land/` → `data/published/layers/people-dna-land/` directly — lifecycle skip (anti-pattern §13.5).
- ❌ Public client (`apps/explorer-web/`) reading `data/processed/people-dna-land/` directly — trust-membrane violation; routes go through `apps/governed-api/` (§7.1, anti-pattern §13.5).
...
> [!NOTE]
> **Document discipline.** This file is governance, not commentary. Material changes to canonical paths require an ADR per `directory-rules.md` §17. Typo and clarification fixes are routine PRs. Every change MUST update the `updated:` field in the KFM Meta Block.
...
**Last reviewed:** 2026-05-18 · **Doc version:** v1 (draft) · **Authority class:** Standard / Canonical-path register · **Domain:** `people-dna-land`
