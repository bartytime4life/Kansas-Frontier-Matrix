<!-- [KFM_META_BLOCK_V2]
doc_id: kfm://doc/domains/people-dna-land/file-system-plan
title: People / Genealogy / DNA / Land — File System Plan
type: standard
version: v1
status: draft
owners: TBD — People/DNA/Land domain steward (NEEDS VERIFICATION)
created: 2026-05-18
updated: 2026-05-18
policy_label: public
related:
...
notes:
  ...
  - Domain slug `people-dna-land` follows Directory Rules §12; a shorter `people/` variant
    appears in the Atlas v1.0 §24.13 crosswalk — surfaced in §13 Open Questions.
[/KFM_META_BLOCK_V2] -->
...
![status](https://img.shields.io/badge/status-draft-orange) ![authority](https://img.shields.io/badge/authority-doctrine--grounded-blue) ![sensitivity](https://img.shields.io/badge/sensitivity-deny--by--default-critical) ![lifecycle](https://img.shields.io/badge/lifecycle-RAW%20%E2%86%92%20PUBLISHED-informational) ![placement](https://img.shields.io/badge/placement-Directory%20Rules%20%C2%A712-success)
<!-- All five badges use shields.io static endpoints; targets are informational only. -->
...
| **Authority** | Directory Rules §12 (Domain Placement Law) is the placement authority. This plan is the domain's instantiation of that law. |
...
2. [Domain identity, boundary, and non-ownership](#2-domain-identity-boundary-and-non-ownership)
3. [The §12 lane pattern, instantiated](#3-the-12-lane-pattern-instantiated)
...
6. [Sensitivity, consent, and policy lanes](#6-sensitivity-consent-and-policy-lanes)
7. [Cross-domain and multi-domain rules](#7-cross-domain-and-multi-domain-rules)
8. [Sources and connectors lane](#8-sources-and-connectors-lane)
9. [Domain-specific anti-patterns](#9-domain-specific-anti-patterns)
10. [Placement protocol walk-through](#10-placement-protocol-walk-through)
11. [Verification checklist for reviewers](#11-verification-checklist-for-reviewers)
...
This document maps the **People / Genealogy / DNA / Land** domain onto the KFM monorepo lane pattern defined in **Directory Rules §12 (Domain Placement Law)**. It answers, for every governance and lifecycle responsibility this domain bears, the question:
...
**Authority of these rules:** CONFIRMED — Directory Rules §12 is canonical placement law.
...
| **Parcel Version** | Versioned parcel geometry; explicitly **not** title-boundary proof without source role. |
...
Directory Rules §12 names `people-dna-land` as the canonical domain slug for this lane. Below is the §12 pattern with the slug substituted in. **Each path is PROPOSED until verified in the mounted repo.**
...
> [!NOTE]
> **Slug consistency caveat.** Directory Rules §12 enumerates the slug as `people-dna-land`. The Atlas v1.0 §24.13 crosswalk uses a shorter `people/` segment under `schemas/contracts/v1/`, `contracts/`, `policy/sensitivity/`, and `policy/consent/`. Per Authority Order §2.1, Directory Rules wins. This plan uses `people-dna-land` throughout; reconciliation is open question **OQ-01** in §12.
...
flowchart LR
  subgraph Doctrine[Doctrine & meaning]
    DOCS["docs/domains/<br/>people-dna-land/"]
    CON["contracts/domains/<br/>people-dna-land/"]
    SCH["schemas/contracts/v1/<br/>domains/people-dna-land/"]
    POL["policy/domains/<br/>people-dna-land/"]
    SENS["policy/sensitivity/<br/>people-dna-land/"]
    CONS["policy/consent/<br/>people-dna-land/"]
  end

  subgraph Proof[Proof & enforceability]
    TST["tests/domains/<br/>people-dna-land/"]
    FIX["fixtures/domains/<br/>people-dna-land/"]
  end

  subgraph Build[Build & execution]
    PKG["packages/domains/<br/>people-dna-land/"]
    PSP["pipeline_specs/<br/>people-dna-land/"]
    PIP["pipelines/domains/<br/>people-dna-land/"]
    CONN["connectors/<br/>&lt;source-id&gt;/"]
  end

  subgraph Lifecycle[Lifecycle data]
    REG["data/registry/sources/<br/>people-dna-land/"]
    RAW["data/raw/<br/>people-dna-land/"]
    WRK["data/work/<br/>people-dna-land/"]
    QRN["data/quarantine/<br/>people-dna-land/"]
    PRC["data/processed/<br/>people-dna-land/"]
    CAT["data/catalog/domain/<br/>people-dna-land/"]
    PUB["data/published/layers/<br/>people-dna-land/"]
  end

  subgraph Release[Release & rollback]
    RC["release/candidates/<br/>people-dna-land/"]
  end

  subgraph Restricted[Restricted store - NOT under data/published/]
    DNA["DNA evidence -<br/>restricted access store<br/>(PROPOSED home: TBD)"]
  end
...
- **`docs/domains/people-dna-land/`** — Domain README, this `FILE_SYSTEM_PLAN.md`, the domain model, the source ledger, the sensitivity profile, the consent register, design notes. Explains; does not decide. PROPOSED.
- **`contracts/domains/people-dna-land/`** — Object-family **meaning** in Markdown form: Person Assertion, PersonCanonical, LifeEvent, RelationshipAssertion, DNA Match Evidence, LandInstrument, Ownership Interval, etc. Pairs with `schemas/`. Never the only place validation lives. PROPOSED.
...
- **`policy/consent/people-dna-land/`** — DNA-kit consent records, consent-state lifecycle (granted, restricted, revoked), revocation cleanup rules, AI-inference consent classes. PROPOSED.
...
- **`tests/domains/people-dna-land/`** — Tests that prove policy and schema rules **actually deny / abstain / answer / error** as written. Includes the seven validator categories from [DOM-PEOPLE]:
  ...
  2. GEDCOM import rights / living-flag tests
  3. DNA consent and raw-ID no-log tests
  4. Revocation cleanup tests
  5. Legal-description and chain-of-title gap tests
  6. Assessor-as-title denial
  7. Graph projection safety tests
...
- **`release/manifests/`**, **`release/rollback_cards/`**, **`release/correction_notices/`** — Cross-domain, **not** domain-segmented. The People/DNA/Land manifest, rollback card, or correction notice for a given release lives alongside every other domain's, indexed by release ID.
...
This domain's sensitivity posture is the **strictest** in KFM. The lanes below carry the rules; the tests in §5.3 prove they enforce.
...
| 2 | DNA segments, raw kit IDs, vendor IDs | DENY public | Restricted steward/research access only with policy approval. |
| 3 | Person-to-parcel join exposing private ownership | DENY public | Public surfaces show parcel context with **warnings**, not title truth. |
...
- DNA segment access classes; raw kit ID no-log rule; vendor ID hashing/tokenization.
- Residence redaction profiles (cell-level generalization, point-to-tract suppression).
...
- DNA-kit consent records and consent-state lifecycle (granted → restricted → revoked).
- Consent classes for AI inference (e.g., "evidence summary allowed," "relationship hypothesis allowed," "public surface forbidden").
- Revocation cleanup rules: what data, derivatives, receipts, and graph edges must be purged or marked stale on revocation.
- Living-person consent for personal data publication (where applicable).

### 6.4 Revocation cleanup contract (PROPOSED)

When a `Consent` record transitions to **revoked**, the pipelines lane MUST:

1. Emit a **CorrectionNotice** in `release/correction_notices/` for any prior public derivative.
2. Mark affected `EvidenceBundle` references **stale** and detach them from public claim surfaces.
3. Purge or tombstone the revoked kit's segments and tokens in the restricted store; emit a redaction receipt under `data/receipts/`.
4. Schedule a **RollbackCard** in `release/rollback_cards/` if any public release depended on the revoked data.
5. Update the graph projection: remove or shade Relationship Hypotheses derived from the revoked DNA evidence.

This contract is **PROPOSED**; concrete tooling and SLAs are NEEDS VERIFICATION.
...
This domain crosses lanes more than most. Apply Directory Rules §12's *multi-domain and cross-cutting files* rule when in doubt.

### 7.1 Cross-lane relations (CONFIRMED / PROPOSED)

| Related lane | Relation type | Constraint |
|---|---|---|
| Settlements / Infrastructure | residence, cemetery, school, court, county, township, place relation | Relation MUST preserve ownership, source role, sensitivity, and `EvidenceBundle` support. |
...
| Agriculture | farm, land use, producer-adjacent context with privacy | Agriculture's farm-private rules and People's living-person rules both apply. |
...
- **Person × parcel join validator** → `tools/validators/joins/person-parcel/` — not `tools/validators/domains/people-dna-land/`.
- **Cemetery person-place ontology** → `contracts/joins/people-settlements/cemetery/` or `docs/architecture/people-place-joins.md` — not under one of the two domains. **PROPOSED home.**
- **Migration × roads route schema** → `schemas/contracts/v1/joins/migration-routes/` — not under one of the two domains. **PROPOSED home.**
- **Cross-domain sensitivity policy** (e.g., archaeology × people for historic-burial joins) → `policy/sensitivity/joins/archaeology-people/` — **PROPOSED home; OQ-03.**
...
| GEDCOM / GEDZip / tree overlays | observation / context | Living-flag honored; rights NEEDS VERIFICATION per upload | `data/raw/people-dna-land/<gedcom-source-id>/` |
| DNA vendor match / segment / triangulation CSVs | observation (restricted) | **DENY public**; restricted store only | `data/raw/people-dna-land/<dna-source-id>/` (restricted) |
...
| Plat / survey / metes-bounds / PLSS / subdivision / derived geometry | observation / model | Geometry-role distinction enforced | `data/raw/people-dna-land/<geometry-source-id>/` |
...
- Emit a `RunReceipt` per fetch including: source role, rights state, sensitivity state, citation, observed time, retrieval time, content hash.
...
| **A new `people/` or `genealogy/` root folder** | Domain becomes root; lifecycle fragments. | Domain lives only as a segment inside responsibility roots. |
...
**Scenario.** A reviewer adds a Rego policy fragment that denies any public response that joins a `PersonCanonical` to a current-vintage `Parcel Version` unless an explicit `ConsentRecord` and a `ReleaseManifest` reference exist.
...
**Proposed path.** `policy/sensitivity/people-dna-land/person_parcel_join.deny.rego` — **PROPOSED**, pending repo verification of the `.deny.rego` naming convention.
...
- [ ] Path uses the §12 lane pattern with slug `people-dna-land` (not `people`, not `genealogy`, not `dna`, not `land`).
- [ ] No new root-level `people/`, `genealogy/`, `dna/`, or `land/` folder is introduced.
- [ ] Schema home is `schemas/contracts/v1/domains/people-dna-land/` (per ADR-0001).
...
|---|---|---|
| **OQ-01** | Reconcile slug `people-dna-land` (Directory Rules §12) vs `people` (Atlas v1.0 §24.13 crosswalk). | ADR amending Directory Rules §12, or Atlas supplement adopting `people-dna-land`. |
| **OQ-02** | Canonical home for the **restricted DNA store**. Atlas implies a "separate restricted store" but does not name the path. | ADR + per-root README declaring the store's class and access controls. |
| **OQ-03** | Whether cross-domain sensitivity policy (archaeology × people, agriculture × people) belongs under `policy/sensitivity/joins/<a>-<b>/` or under `policy/sensitivity/<class>/<a>-<b>/`. | ADR or `policy/sensitivity/README.md` per-root rule. |
...
| **OQ-07** | Whether runbooks for this domain live under `docs/runbooks/people-dna-land/` (subfolder convention) or `docs/runbooks/people_dna_land_<topic>.md` (flat-prefix). Fauna and Habitat sessions have raised this question already without resolution. | ADR; see Directory Rules §18 open question on runbook subfolder convention. |
| **OQ-08** | Exact route names for the four governed-API People/DNA/Land resolvers (`PeopleDNALandDecisionEnvelope`, `LayerManifest`, Evidence Drawer payload, Focus Mode answer). | `apps/governed-api/` routing manifest. |
...
| **OQ-10** | DNA consent/revocation enforcement: SLA from revocation to graph/receipt/manifest cleanup. | `policy/consent/people-dna-land/` + revocation tests + a runbook. |
...
| **OQ-14** | Whether MapLibre / Evidence Drawer / Focus Mode have any People/DNA/Land-specific implementation today. | Repo inspection of `packages/maplibre/`, `apps/explorer-web/`, `apps/governed-api/`. |
...
- [`docs/domains/people-dna-land/README.md`](./README.md) — People/DNA/Land domain landing page. **PROPOSED.**
- [`docs/domains/people-dna-land/PEOPLE_DOMAIN_MODEL.md`](./PEOPLE_DOMAIN_MODEL.md) — object families, ubiquitous language, identity rules. **PROPOSED.**
- [`docs/domains/people-dna-land/SOURCE_LEDGER.md`](./SOURCE_LEDGER.md) — accepted source families, rights, citations. **PROPOSED.**
- [`docs/domains/people-dna-land/SENSITIVITY_PROFILE.md`](./SENSITIVITY_PROFILE.md) — living-person, DNA, person-parcel deny defaults. **PROPOSED.**
- [`docs/domains/people-dna-land/CONSENT_REGISTER.md`](./CONSENT_REGISTER.md) — DNA-kit consent lifecycle and revocation contract. **PROPOSED.**
- [`docs/domains/people-dna-land/CHAIN_OF_TITLE_NOTES.md`](./CHAIN_OF_TITLE_NOTES.md) — chain-of-title reasoning, gap policy, assessor-vs-title distinction. **PROPOSED.**
- [`docs/runbooks/people-dna-land/SOURCE_REFRESH_RUNBOOK.md`](../../runbooks/people-dna-land/SOURCE_REFRESH_RUNBOOK.md) — lifecycle operations for refreshing People/DNA/Land sources. **PROPOSED / TODO**; subfolder convention unresolved (OQ-07).
...
- [`docs/domains/frontier-matrix/FILE_SYSTEM_PLAN.md`](../frontier-matrix/FILE_SYSTEM_PLAN.md) — Frontier Matrix matrix-cell release rules; Public Land Records and Land Office Records aggregation. **TODO** if absent.
...
- [`docs/architecture/people-place-joins.md`](../../architecture/people-place-joins.md) — cross-domain person-place doctrine. **PROPOSED home.**
...
**Last reviewed:** 2026-05-18 · **Edition:** v1 (draft) · **Authority:** Directory Rules §12 (Domain Placement Law) · [↑ Back to top](#contents)
