<!-- [KFM_META_BLOCK_V2]
doc_id: kfm://doc/domains/hazards/verification-backlog
...
# Hazards Domain — Verification Backlog
...
**Status:** draft · **Owners:** `<hazards-domain-steward>`, `<governance-steward>` · **Last updated:** 2026-05-17

> [!IMPORTANT]
> **KFM is not an emergency alert system.** Every item in this backlog inherits that boundary [DOM-HAZ §§1–2; KFM-IDX-POL-007]. No verification path in this file may be used to justify life-safety instructions, real-time alerting behavior, or regulatory determinations. When a hazard surface drifts toward alerting, the resolution is **refer to NWS, FEMA, or state emergency channels**, not "improve KFM coverage."
...
```mermaid
flowchart LR
    A["docs/domains/hazards/<br/>VERIFICATION_BACKLOG.md<br/>(this file)"] -->|domain-scoped items| B["docs/registers/<br/>VERIFICATION_BACKLOG.md<br/>(central register)"]
    A -->|ADR-class items| C["docs/adr/<br/>(open ADR backlog)"]
    A -->|doc/repo conflicts| D["docs/registers/<br/>DRIFT_REGISTER.md"]
    A -->|source/rights items| E["data/registry/sources/<br/>hazards/  (PROPOSED)"]
    A -->|test/fixture items| F["tests/domains/hazards/<br/>fixtures/domains/hazards/  (PROPOSED)"]
    A -->|policy items| G["policy/domains/hazards/<br/>(PROPOSED)"]
    B -.->|control plane index| H["control_plane/<br/>verification_backlog.yaml"]
```
...
| `HAZ-VB-002` | Confirm NWS alerts feed is treated as **context only**, never authority, and is wired to a freshness gate. | "Operational warning products are contextual only and not for life safety" [DOM-HAZ §I]. | A `SourceDescriptor` for NWS where `source_role ∈ {context, observation}` only, plus a passing freshness gate fixture. | `<hazards-source-steward>` | `BLOCK_RELEASE` | NEEDS VERIFICATION | [DOM-HAZ §§B, I] [POL-007] |
| `HAZ-VB-003` | Verify FEMA NFHL / MSC flood layers are labeled as **regulatory context**, not observed flood. | Misreading regulatory polygons as observed flood is a known false-precision risk [Atlas §12 Risks]. | `SourceDescriptor` with `source_role = regulatory_context` and a validator denying "observed flood" labels on NFHL layers. | `<hazards-source-steward>` | `BLOCK_PUBLIC` | NEEDS VERIFICATION | [DOM-HAZ §B] [ENCY §7.10] |
| `HAZ-VB-004` | Confirm NASA FIRMS / NOAA HMS feeds carry knowledge-character labels (`detection`, `model`, `context`) and never appear as observed ground fire. | "Model-as-observed" denial is required [DOM-AIR validators, applied here]. | A fixture where a FIRMS detection without knowledge-character label fails closed in CI. | `<hazards-source-steward>` | `BLOCK_PUBLIC` | NEEDS VERIFICATION | [DOM-HAZ §K] [APP-005] |
...
> Hazards strictly separates **observation**, **regulatory archive**, **public report**, **model field**, **remote-sensing detection**, **operational context**, and **resilience summary** [APP-005]. Source-role anti-collapse is the load-bearing test for this lane [DOM-HAZ §K].

| ID | Item | Why it matters | Evidence that would settle it | Owner | Severity | Status | Refs |
|---|---|---|---|---|---|---|---|
| `HAZ-VB-010` | Implement the Hazards source-role taxonomy and pin it as the source-role enum for this lane. | Source-role confusion is the most common failure mode in this domain [Atlas §12 Risks]. | A pinned `source_role` enum (e.g., in `schemas/contracts/v1/domains/hazards/source_descriptor.schema.json`) plus a source-role anti-collapse test that fails closed when two roles are merged. | `<hazards-domain-steward>` | `BLOCK_PROMOTION` | NEEDS VERIFICATION | [DOM-HAZ §K] [ADR-S-04 PROPOSED] |
...
> [!WARNING]
> This section verifies that the **"not an emergency alert system"** boundary is enforced in code, schemas, policy, and UI — not merely declared in docs.

| ID | Item | Why it matters | Evidence that would settle it | Owner | Severity | Status | Refs |
|---|---|---|---|---|---|---|---|
| `HAZ-VB-020` | Verify emergency-alert denial is enforced at the policy layer (not only in docs). | Policy enforcement, not prose, is the durable defense [DIRRULES §13]. | A policy fixture in `policy/domains/hazards/` that returns `DENY` with reason `not_emergency_alert_system` for life-safety instruction payloads. | `<governance-steward>` | `BLOCK_PUBLIC` | NEEDS VERIFICATION | [DOM-HAZ §K] [POL-007] [Atlas §20.5] |
...
| ID | Item | Why it matters | Evidence that would settle it | Owner | Severity | Status | Refs |
|---|---|---|---|---|---|---|---|
| `HAZ-VB-040` | Implement the **source-role anti-collapse** test in `tests/domains/hazards/`. | First-line defense against source-role confusion [Atlas §12 Risks]. | Test that fails closed when two distinct source-roles are merged on the same object. | `<hazards-domain-steward>` | `BLOCK_PROMOTION` | NEEDS VERIFICATION | [DOM-HAZ §K] |
...
| ID | Item | Why it matters | Evidence that would settle it | Owner | Severity | Status | Refs |
|---|---|---|---|---|---|---|---|
| `HAZ-VB-050` | Confirm the canonical schema home for Hazards DTOs (`schemas/contracts/v1/domains/hazards/...`) against the mounted repo. | Schema-home rule is ADR-required [DIRRULES §2.4(3); ADR-S-01]. | A passing ADR-0001 reference plus presence of the schema files at the canonical path. | `<governance-steward>` | `BLOCK_PROMOTION` | NEEDS VERIFICATION | [DIRRULES §2.4; ADR-S-01 PROPOSED] |
| `HAZ-VB-051` | Pin the Hazards `source_role` vocabulary v1 (Atlas open-ADR ADR-S-04). | Source-role anti-collapse depends on stable vocabulary. | Accepted ADR documenting the enum, plus a schema referencing the ADR. | `<governance-steward>` | `BLOCK_PROMOTION` | NEEDS VERIFICATION | [Atlas §24.12 ADR-S-04] |
...
| `HAZ-VB-061` | Verify Hazards Focus Mode citation validation fails closed on uncited authoritative claims. | "Citation validation, finite outcomes and no direct model-to-public path" [Atlas §12 Risks]. | A negative fixture where an uncited authoritative Hazards claim returns `ABSTAIN` or `DENY` with reason. | `<governed-ai-steward>` | `BLOCK_PUBLIC` | NEEDS VERIFICATION | [DOM-HAZ §L] [Atlas §12] |
| `HAZ-VB-062` | Verify Hazards Focus Mode `DENY` is returned for life-safety prompt patterns. | The boundary must hold at the AI surface, not just at the data surface. | Negative fixtures with prompts requesting life-safety instructions, all returning `DENY` with reason `not_emergency_alert_system`. | `<governed-ai-steward>` | `BLOCK_PUBLIC` | NEEDS VERIFICATION | [POL-007] [DOM-HAZ §L] |
...
| ID | Item | Why it matters | Evidence that would settle it | Owner | Severity | Status | Refs |
|---|---|---|---|---|---|---|---|
| `HAZ-VB-070` | Verify MapLibre / Evidence Drawer / Focus Mode integration for Hazards layers. | This is a NEEDS VERIFICATION item lifted directly from the v1.0 §12.N table. | An end-to-end fixture rendering one Hazards layer through MapLibre with Evidence Drawer + Focus Mode wired and a passing e2e smoke test. | `<hazards-ui-steward>` | `BLOCK_PUBLIC` | NEEDS VERIFICATION | [DOM-HAZ §12.N analog; Atmosphere §11.N] |
...
> These items are not pure verification — they are **decisions** that, per Directory Rules §2.4 and Atlas §24.12, warrant an ADR before they can close. They are listed here because they block multiple verification items above.

| ADR-ref (PROPOSED) | Question | Blocks items |
|---|---|---|
...
| `ADR-S-04` | Source-role enum — canonical vocabulary and evolution rule for Hazards. | `HAZ-VB-010`, `HAZ-VB-040`, `HAZ-VB-051`. |
...
> [!NOTE]
> The two `ADR-HAZ-NN` entries are PROPOSED **new** ADRs; they are not numbered in any existing ADR register in this session. Final numbering follows the ADR home convention once verified.
...
sequenceDiagram
    ...
    note over Backlog,Central: If repo and docs conflict,<br/>file a DRIFT_REGISTER entry first.
```
...
> *(empty at v1 — no items have moved to RESOLVED or SUPERSEDED yet)*
...
- `docs/registers/DRIFT_REGISTER.md` — doc/repo conflicts <!-- TODO: link -->
- `docs/registers/OBJECT_FAMILY_MAP.md` — object family ownership <!-- TODO: link -->
- `docs/domains/hazards/README.md` — Hazards lane README <!-- TODO: link -->
...
- `docs/runbooks/hazards/SOURCE_REFRESH_RUNBOOK.md` — Hazards source refresh runbook *(PROPOSED, not yet authored — parallel to `docs/runbooks/fauna/SOURCE_REFRESH_RUNBOOK.md`)*
- `docs/adr/` — accepted and PROPOSED ADRs referenced above
- Atlas v1.1 §12 — Hazards (chapter)
- Atlas v1.1 §24.8, §24.11, §24.12 — stale-state markers, governance health, open-ADR backlog
...
**Related docs:** [central VERIFICATION_BACKLOG](../../registers/VERIFICATION_BACKLOG.md) · [Hazards README](README.md) · [Directory Rules](../../doctrine/directory-rules.md)
