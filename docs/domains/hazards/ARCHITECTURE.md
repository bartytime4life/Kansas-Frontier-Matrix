...
| **Schema home (default)** | `schemas/contracts/v1/domains/hazards/` per ADR-0001 (schema home) |
...
It is a **proof-bearing domain lane**, not a root folder. Under the Domain Placement Law (Directory Rules §12), Hazards files live as segments inside the relevant responsibility roots (`docs/domains/hazards/`, `schemas/contracts/v1/domains/hazards/`, `policy/domains/hazards/`, `data/<phase>/hazards/`, etc.) and never as a `hazards/` root folder.
...
Authority order for this document follows Directory Rules §2.1: KFM core invariants → accepted ADRs amending them → this domain architecture → per-root READMEs → domain dossiers → current mounted-repo convention. Lineage and supporting doctrine:

| Source | Role | Status |
|---|---|---|
| KFM Encyclopedia §7.10 (Hazards) | CONFIRMED doctrine — mission, boundary, source families, object families, knowledge systems | Doctrinal anchor |
...
| KFM Unified Implementation Architecture Build Manual §30.6 (Hazards) | CONFIRMED scope, sources of authority, sensitivity posture; PROPOSED object families, pipelines, publication gates | Doctrinal anchor |
| Pass 20 Idea Index — `KFM-IDX-POL-007`, `KFM-IDX-APP-005`, `KFM-IDX-PLN-002` | CONFIRMED — life-safety boundary; atmosphere/hazards knowledge-character separation; hazards as planning context, not alerting | Doctrinal anchor |
| Directory Rules §12 (Domain Placement Law) | CONFIRMED — domain placement and lane pattern | Governance anchor |
| Whole-UI + Governed AI Expansion Report | CONFIRMED — Evidence Drawer, Focus Mode, finite-outcome envelopes | Cross-cutting anchor |
| Master MapLibre Atlas (NFHL handling) | CONFIRMED — NFHL is regulatory baseline, not observed inundation; vector vs. WMS analytic distinction | Cross-cutting anchor |
...
`PROPOSED` enforcement: a `not_emergency_alert_system` flag on every Hazards `RuntimeResponseEnvelope` whose object class is `operational_warning`, `operational_advisory`, or `operational_watch`, plus a fail-closed validator that rejects emission without it (see §12).
...
> Collapsing any of these is a fail-closed validator violation.
...
CONFIRMED canonical object families (KFM Encyclopedia §7.10.C, Domains Atlas §12.E). Identity rules are **PROPOSED** (`source id + object role + temporal scope + normalized digest`); temporal-field separation (source, observed, valid, retrieval, release, correction) is **CONFIRMED doctrine**.
...
CONFIRMED source families (KFM Encyclopedia §7.10.B, Domains Atlas §12.D). Source roles must be **explicit and non-collapsible**: a regulatory layer is not an observation; an observation is not a model; an operational feed is not an alert authority inside KFM.
...
> [!NOTE]
> CONFIRMED cross-domain rule (Build Manual §11): *source role cannot be inferred from convenience*. Regulatory flood layers are not observed inundation; operational warnings are not life-safety authority inside KFM; remote-sensing anomalies are candidates until reviewed.
...
CONFIRMED doctrine (Domains Atlas §12.F, §24.4.10): every cross-lane relation must preserve **ownership, source role, sensitivity, and EvidenceBundle support**. Hazards cites other lanes for context; it never overrides their canonical claims, and it never becomes the alert authority for any of them.

```mermaid
flowchart LR
  HAZ[Hazards] -->|flood/drought/water context| HYD[Hydrology]
  HAZ -->|smoke · heat/cold · AQI · fire-weather| AIR[Atmosphere / Air]
  HAZ -->|exposure · lifelines · dependencies<br/>default-deny on critical detail| SET[Settlements / Infrastructure]
  ...
  HAZ -.->|hazard event context cited;<br/>KFM is never an alert authority| ALL[All other lanes]
```
...
> [!IMPORTANT]
> CONFIRMED rule: AI must **DENY** any request that would produce or imply an emergency alert, life-safety instruction, or regulatory determination — regardless of how the request is framed. Refusal surfaces the official-source referral instead.
...
PROPOSED governed API surfaces. Exact routes, DTO field shapes, and schema homes are **PROPOSED** pending mounted-repo verification and ADR confirmation. The schema responsibility root is `schemas/contracts/v1/domains/hazards/` per ADR-0001 (Directory Rules §7.4).

| Surface | DTO / artifact | Finite outcomes | Status |
|---|---|---|---|
| Hazards feature/detail resolver (route TBD) | `HazardsDecisionEnvelope` | ANSWER · ABSTAIN · DENY · ERROR | PROPOSED governed-API surface; route UNKNOWN |
...
| Schema responsibility root | `schemas/contracts/v1/domains/hazards/` | finite validator outcomes | PROPOSED; verify per ADR-0001 |
| Contract semantic root | `contracts/domains/hazards/` (Markdown) | n/a | PROPOSED; semantic Markdown only — no parallel schema authority |
| Policy domain bundle | `policy/domains/hazards/` | ALLOW · DENY · RESTRICT · ABSTAIN | PROPOSED; includes life-safety boundary gate |
...
CONFIRMED fixture posture (`KFM-IDX-VAL-001`): **no-network fixture-first**. The first Hazards slice runs on synthetic public-safe fixtures covering positive, negative, denied, abstained, ambiguous, stale-source, and rollback cases — never live source activation.
...
PROPOSED viewing products (KFM Encyclopedia §7.10.E, Domains Atlas §12.G). All map products consume released artifacts via the trust membrane; the renderer is downstream of trust.
...
> [!WARNING]
> NFHL ArcGIS REST/FeatureServer endpoints are for **analytic vector access**; WMS endpoints are **visualization-only** and not suitable for analytic joins. Mixing these collapses the regulatory-baseline / analytics distinction.
...
CONFIRMED doctrine. Hazards inherits KFM's default-deny posture and adds lane-specific constraints.
...
contracts/domains/hazards/    # semantic Markdown only (no parallel schema authority)
schemas/contracts/v1/domains/hazards/   # canonical machine schemas (per ADR-0001)
...
- **Schema home.** `schemas/contracts/v1/domains/hazards/` is canonical (ADR-0001). `contracts/domains/hazards/` retains semantic Markdown only.
- **No root-level `hazards/` folder.** A root `hazards/` directory is a §13.4 anti-pattern.
...
CONFIRMED doctrine (`KFM-IDX-PLN-003`): domain expansion is **proof-bearing, not coverage-bearing**. The first Hazards slice should demonstrate descriptor → evidence → policy → validation → release closure on a tightly scoped fixture, not broad horizontal coverage.

PROPOSED first credible thin slice (KFM Encyclopedia §7.10.N):
...
4. **Operational warning feeds disabled or contextual-only** for this slice; if included at all, a `not_for_life_safety` `DecisionEnvelope` fixture is mandatory (PROPOSED first artifact per `KFM-IDX-APP-005`).
...
| AI hallucination on Hazards questions | Citation validation; finite outcomes (ANSWER / ABSTAIN / DENY / ERROR); no direct AI-to-public path; freshness gate |
...
These items are **NEEDS VERIFICATION** against mounted-repo evidence, source documentation, or ADR resolution.

| Item | Evidence that would settle it | Status |
|---|---|---|
| Verify official-source endpoints, terms of use, rate limits, and rights for NWS, FEMA OpenFEMA, NFHL, USGS Earthquake, USGS Water, NASA FIRMS, NOAA HMS, drought monitors, Kansas EM | Mounted-repo source descriptors + current source docs | NEEDS VERIFICATION |
...
| ADR or convention note for runbook subfolder pattern — `docs/runbooks/hazards/...` mirroring the Fauna lane | Accepted ADR or directory-rules update | PROPOSED |
...
| Resolve `PROV.md` vs. `PROVENANCE.md` naming for the related provenance standards profile (carried from prior docs) | ADR | PROPOSED |
| Hazards Decision Envelope finite-outcome route name | Mounted governed-API code | UNKNOWN |
| Source-health watcher placement (likely `apps/workers/` per Directory Rules §7.1) and receipt schema | Mounted repo evidence + ADR | NEEDS VERIFICATION |
...
**Last updated:** 2026-05-17  ·  **Owners:** TODO — Hazards lane steward + Docs steward  ·  **Status:** `draft`
