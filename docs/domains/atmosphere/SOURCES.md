...
A `policy/sensitivity/` entry SHOULD govern any join that could surface a sensitive species location through smoke/fire/drought context. **NEEDS VERIFICATION:** presence of such an entry in the current repository.
...
| OQ-AIR-SRC-02 | What is the canonical `SourceDescriptor` schema home and actual field set in the current repository? | Schema owner | Repo inspection vs. Atlas §24.1.3 / ADR-0001 |
