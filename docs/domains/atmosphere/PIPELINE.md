...
notes:
  ...
  - Grounded in current repository evidence; every path, route, and runtime claim remains PROPOSED or NEEDS VERIFICATION where direct repo confirmation is still pending.
...
> [!NOTE]
> The **lifecycle law** (`RAW → WORK / QUARANTINE → PROCESSED → CATALOG / TRIPLET → PUBLISHED`) and the **A–G gate sequence** are CONFIRMED doctrine. Their **application to the Atmosphere/Air lane** is PROPOSED. This document is grounded in current repository evidence; every path, route name, runtime behavior, and enforcement claim remains PROPOSED or NEEDS VERIFICATION where direct repo confirmation is still pending.
