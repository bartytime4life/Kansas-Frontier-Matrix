...
tags: [kfm, atmosphere, air, planning, register, directory-rules, propsed-paths]
notes:
...
[![Truth posture: PROPOSED](https://img.shields.io/badge/truth_posture-PROPOSED-yellow)](#verification-posture)
[![Lifecycle: RAW%20%E2%86%92%20PUBLISHED](https://img.shields.io/badge/lifecycle-RAW%20%E2%86%92%20PUBLISHED-555)](#pipeline-shape-and-promotion-gates)
[![Authority: Doctrine](https://img.shields.io/badge/authority-doctrine_grounded-blueviolet)](#evidence-basis)
[![Last Reviewed: 2026-05-16](https://img.shields.io/badge/last_reviewed-2026--05--16-informational)](#footer)

> **Status:** draft · **Owners:** TODO-atmosphere-domain-steward · TODO-docs-steward · **Updated:** 2026-05-16
...
- Hazards-owned emergency / advisory truth, which is the **Hazards** lane's responsibility (Atmosphere/Air carries `ALERT_AND_ADVISORY_CONTEXT` and `AdvisoryContext` only as context, not as life-safety claims).
...
Atmosphere/Air is a **domain**, not a root. Under KFM Directory Rules, a domain appears as a **segment** inside a responsibility root, never as a root of its own:
...
> [!WARNING]
> **Do not** create `atmosphere/` at the repository root, and **do not** create a parallel `schemas/atmosphere/` outside `schemas/contracts/v1/...`. Either is a Directory Rules §2.4 violation and requires an ADR before merging. If the mounted repo already contains such a structure, raise it as a **DRIFT_REGISTER** entry rather than treating it as canon.
...
> [!NOTE]
> **NEEDS VERIFICATION (repository state).** The repository was not mounted during the authoring of this register. Every "exists / does not exist" claim below is therefore an inference from doctrine and PDF planning artifacts, not a repo fact. Once a mounted-repo inspection is performed, this register **MUST** be regenerated and rows reclassified:
>
...
Doctrine sources used to derive this register, all CONFIRMED in attached project documents:

- **Directory Rules** — domain placement law, responsibility-root mapping, ADR-class changes, required README contract.
- **KFM Domain and Capability Encyclopedia §7.9** — Atmosphere/Air mission, sources, object families, knowledge systems.
- **KFM Domains Culmination Atlas §11** — Atmosphere/Air ubiquitous language, source families, objects, cross-lane relations, validators, governed-AI behavior, publication gates, verification backlog.
- **Unified Implementation Architecture Build Manual §30.10** — Atmosphere/Air lane scope and gate summary.
- **Whole-UI + Governed AI Expansion Report** — pattern for "files that should exist next" registers and per-subsystem path-by-path tables.

External research was **not** consulted for this register; KFM-specific paths and doctrine are governed by project knowledge alone.
...
> [!NOTE]
> **Diagram status:** PROPOSED — the diagram reflects the responsibility-root + domain-segment pattern required by Directory Rules. Actual repo topology **NEEDS VERIFICATION**.
...
|---|---|---|---|---|---|
| `docs/domains/atmosphere/README.md` | Atmosphere/Air domain landing page; declares scope, ownership, non-ownership, ubiquitous language, source families, governed-AI posture. | PROPOSED | Directory Rules §15 README contract | All other atmosphere docs | Required by `docs/domains/README.md` listing; declares authority class and review burden. |
| `docs/domains/atmosphere/UBIQUITOUS_LANGUAGE.md` | Stable definitions for `OBSERVED_SENSOR`, `PUBLIC_AQI_REPORT`, `REGULATORY_ARCHIVE`, `LOW_COST_SENSOR`, `ATMOSPHERIC_MODEL_FIELD`, `REMOTE_SENSING_MASK`, `CLIMATE_ANOMALY_CONTEXT`, `DERIVED_FUSION`, `METEOROLOGICAL_CONTEXT`, `ALERT_AND_ADVISORY_CONTEXT`, `NETWORK_AND_SITE_CONTEXT`. | PROPOSED | README | Contracts, schemas, validators | Knowledge-character vocabulary per Atlas §11.C. |
| `docs/domains/atmosphere/OBJECT_FAMILY_MAP.md` | Per-object table: AirStation, AirObservation, PM2.5 Observation, Ozone Observation, SmokeContext, AODRaster, Weather Station, Weather Observation, WindField, Precipitation Observation, Temperature Observation, Climate Normal, Climate Anomaly, Forecast Context, Advisory Context. | PROPOSED | Ubiquitous language | Schemas, fixtures | Crosswalks to `contracts/OBJECT_MAP.md`. |
| `docs/domains/atmosphere/SOURCE_FAMILIES.md` | Source-family roster with role / rights / freshness columns: OpenAQ-like, EPA AQS-like, AirNow / agency, CAMS / ECMWF-family, HRRR-Smoke, HMS smoke, GOES/ABI AOD, VIIRS fire/hotspot, NWS, Kansas Mesonet, climate normals. | PROPOSED | Encyclopedia §7.9.B; Atlas §11.D | Source descriptors; connector READMEs | All entries `NEEDS VERIFICATION` for rights / cadence / endpoint. |
...
| `docs/domains/atmosphere/CROSS_LANE_RELATIONS.md` | How Atmosphere/Air relates to Hazards, Agriculture, Hydrology, Biodiversity, Settlements — with the boundary that life-safety belongs to Hazards. | PROPOSED | README | Policy bindings | Atlas §11.F. |
...
> [!NOTE]
> Per Directory Rules and ADR-0001 (verify), `contracts/` owns object **meaning** in semantic Markdown; `schemas/contracts/v1/...` owns machine **shape**. Both layers are required; neither replaces the other.
...
| `contracts/domains/atmosphere/SmokeContext.md` | Semantic contract for SmokeContext (HMS / HRRR-Smoke source role). | PROPOSED | Ubiquitous language | Schemas, hazards crosswalk |
...
| `contracts/domains/atmosphere/AtmosphereAirDecisionEnvelope.md` | Decision envelope for Atmosphere/Air feature/detail resolver: ANSWER / ABSTAIN / DENY / ERROR. | PROPOSED | Runtime envelope contract | Governed-API route |

### 6.3 `schemas/contracts/v1/domains/atmosphere/` — machine shape

> [!NOTE]
> Default schema home is `schemas/contracts/v1/...` per ADR-0001. **NEEDS VERIFICATION** that this is the live authority in the mounted repo. If `contracts/<...>.schema.json` is used instead, raise a drift entry; do not duplicate.

| Path | Schema | Truth label | Notes |
|---|---|---|---|
| `schemas/contracts/v1/domains/atmosphere/air_station.schema.json` | AirStation | PROPOSED | Identity rule: `source_id + object_role + temporal_scope + normalized_digest`. |
| `schemas/contracts/v1/domains/atmosphere/air_observation.schema.json` | AirObservation | PROPOSED | Time fields: source, observed, valid, retrieval, release, correction (distinct). |
...
| `schemas/contracts/v1/domains/atmosphere/parameter_registry.schema.json` | Atmospheric parameter / unit registry | PROPOSED | Drives unit-normalization tests. |
...
> [!CAUTION]
> Canonical singular is `policy/` per Directory Rules. If `policies/` exists, treat as compatibility/mirror and do not evolve independently.

| Path | Policy intent | Truth label | Notes |
|---|---|---|---|
| `policy/domains/atmosphere/README.md` | Folder contract per §15. | PROPOSED | Declares authority class. |
...
| `policy/domains/atmosphere/advisory_no_life_safety.rego` | Deny life-safety instructional output from AdvisoryContext; redirect to authoritative source. | PROPOSED | Hazards lane boundary. |
| `policy/domains/atmosphere/freshness_gate.rego` | Stale-state policy keyed to source cadence. | PROPOSED | Pairs with stale-state UI badge. |
...
| `tests/domains/atmosphere/test_temporal_fields_distinct.py` | source, observed, valid, retrieval, release, correction must not collapse. | PROPOSED | — |
...
> [!NOTE]
> Validators belong under `tools/` (repo-wide validator/generator/checker). One-off helpers under `scripts/` graduate to `tools/` once trust-bearing.
...
> [!IMPORTANT]
> **Connectors do not publish.** Output goes to `data/raw/atmosphere/<source_id>/<run_id>/` or `data/quarantine/atmosphere/<reason>/<run_id>/`. Watchers do not publish either. First Atmosphere/Air PR is **docs/registry/schema/fixture/validator/policy/dryrun only** — no live fetch, no public promotion, no UI/API binding beyond typed contract notes.

| Path | Purpose | Truth label | Notes |
|---|---|---|---|
| `connectors/epa_aqs/` | EPA AQS-like archive connector. | PROPOSED | Rights NEEDS VERIFICATION. |
...
> [!WARNING]
> A path-level move that bypasses validators, policy gates, EvidenceBundle creation, catalog closure, and release-decision recording is a Directory Rules violation **regardless** of which directory the bytes end up in. Promotion is a governed state transition, not a `mv`.
...
| `data/receipts/atmosphere/` (if domain-scoped) | Domain-scoped receipts; default global home is `data/receipts/` | PROPOSED | **NEEDS VERIFICATION:** confirm whether receipts are global-only or also per-domain. |
...
> [!NOTE]
> **Deferred.** Per Directory Rules §7.2, code only graduates to `packages/` once it is reusable across deployables. The first Atmosphere/Air PR is doctrine-only; package code is **deferred** until the no-network proof slice clears.
...
| `docs/adr/ADR-XXXX-atmosphere-advisory-non-life-safety.md` | Record that AdvisoryContext is referral-only; no life-safety output from Atmosphere/Air. | PROPOSED — boundary with Hazards. |
...
Per Atlas §11.H and §11.M, each transition is a **governed state change**, never a file move. Gates required at PUBLISHED: ReleaseManifest, EvidenceBundle, validation/policy support, review state where required, correction path, stale-state rule, rollback target.
...
Recommended PR order, modeled on the cross-domain Encyclopedia roadmap and constrained to Atmosphere/Air. Each PR is small, reversible, and tied to validators or contracts.
...
|---|---|---|---|
| 1 | Add `docs/domains/atmosphere/README.md`, `MISSING_OR_PLANNED_FILES.md` (this), `UBIQUITOUS_LANGUAGE.md`, `KNOWLEDGE_CHARACTERS.md` | Stabilizes vocabulary before code. | Doc link check; KFM Meta Block v2 valid. |
...
| 6 | Add `tools/validators/domains/atmosphere/validate_air_observation.py` and friends | Validator entry points. | Round-trip tests pass. |
...
| 10 | Open ADRs: schema home, knowledge-character vocabulary, advisory non-life-safety, runbook subfolder convention | Lock the doctrine the above codifies. | ADRs accepted; supersession recorded. |
...
- **Root-level `atmosphere/`** — domain at root violates Rules §3 and §12 (Domain Placement Law).
- **Parallel schema home** — `schemas/atmosphere/` outside `schemas/contracts/v1/...` requires an ADR.
...
- **`AdvisoryContext` rendered as life-safety instruction** — Atmosphere/Air is **not** an emergency alert system; redirect to the authoritative source.
...
- **Browser direct access to RAW / WORK / QUARANTINE / canonical stores or model runtimes** — trust membrane breach.
...
- **OPEN:** Should `AODRaster` schema include a mandatory `not_pm25: true` marker, or is source-role enum enforcement sufficient?
- **OPEN:** Cardinality of `data/registry/sources/atmosphere/` — one descriptor per source family, or per endpoint within a family?
- **OPEN:** Whether `parameter_registry.schema.json` belongs under `schemas/contracts/v1/domains/atmosphere/` or under a cross-domain `schemas/contracts/v1/registries/`. Lean toward cross-domain if Soil / Hydrology / Hazards reuse it.
- **OPEN:** Whether `ClimateNormal` / `ClimateAnomaly` should be modeled inside Atmosphere/Air or split into a `climate/` sub-segment within the domain. Atlas §11.B places them inside Atmosphere/Air.
- **OPEN:** How `SmokeContext` deconflicts ownership with the Hazards domain's `SmokeContext` — same name appears in both per Atlas §12.B. Suggested resolution: ADR clarifying that Atmosphere/Air owns the atmospheric reading; Hazards owns the event/impact projection of the same phenomenon.
- **NEEDS VERIFICATION:** Whether any of the paths above already exist in the mounted repo. If yes, classify CONFIRMED or DRIFT; if no, keep PROPOSED.
...
**Related:** [README](./README.md) · [Domains Index](../README.md) · [Directory Rules](../../doctrine/directory-rules.md) · [Verification Backlog](../../registers/VERIFICATION_BACKLOG.md) · [Drift Register](../../registers/DRIFT_REGISTER.md)

**Last updated:** 2026-05-16 · **Version:** v1 · **Status:** draft

[⤴ Back to top](#atmosphereair--missing-or-planned-files-register)
