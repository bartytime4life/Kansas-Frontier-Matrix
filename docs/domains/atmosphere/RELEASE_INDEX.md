...
title: Atmosphere — Release Index
type: standard
version: v1
status: draft
owners: <atmosphere-domain-steward> · <release-steward>
created: 2026-05-16
updated: 2026-05-16
policy_label: public
related:
...
<p align="left">
  ...
  <a href="#"><img alt="Domain" src="https://img.shields.io/badge/domain-atmosphere--air-7aa7ff"></a>
  <a href="#"><img alt="Policy label" src="https://img.shields.io/badge/policy_label-public-2ea44f"></a>
  <a href="#"><img alt="Lifecycle" src="https://img.shields.io/badge/lifecycle-RAW%E2%86%92PUBLISHED-6f42c1"></a>
  <a href="#"><img alt="Trust posture" src="https://img.shields.io/badge/posture-cite--or--abstain-blue"></a>
  <a href="#"><img alt="Authority" src="https://img.shields.io/badge/authority-doctrine--anchored-informational"></a>
  <!-- TODO: replace placeholder badges below with live CI/release endpoints once available -->
  <a href="#"><img alt="Release gate (placeholder)" src="https://img.shields.io/badge/release_gate-TODO-lightgrey"></a>
  <a href="#"><img alt="Last reviewed" src="https://img.shields.io/badge/last_reviewed-2026--05--16-informational"></a>
</p>

| Field | Value |
|---|---|
| **Owners** | `<atmosphere-domain-steward>` · `<release-steward>` *(placeholder, NEEDS VERIFICATION)* |
| **Authority** | Human-facing navigation index. Not a release decision authority. |
| **Canonical release home** | `release/manifests/`, `release/candidates/atmosphere/`, `release/promotion_decisions/`, `release/rollback_cards/`, `release/correction_notices/` |
...
- Object families, ubiquitous language, and pipeline shape → see [`./README.md`](./README.md) and the domain dossier.
- Source descriptors, rights, cadence → see [`./SOURCE_INDEX.md`](./SOURCE_INDEX.md) *(PROPOSED neighbor doc)*.
- Catalog records and STAC closure → see [`./CATALOG_INDEX.md`](./CATALOG_INDEX.md) *(PROPOSED neighbor doc)*.
...
- `artifacts` (pmtiles | stac | geojson | parquet | model | manifest | receipt) — CONFIRMED enum
...
> [!IMPORTANT]
> **Watcher-as-non-publisher invariant** (CONFIRMED). Workers, watchers, and pipelines write candidates and receipts; they do not publish. Publication is a governed state transition recorded in `release/promotion_decisions/` and reflected here only after the fact. This index never *causes* a release; it *reflects* one.
...
The release surface for Atmosphere/Air is distributed across the canonical responsibility roots described in [Directory Rules](../../doctrine/directory-rules.md) §9.
...
> [!CAUTION]
> **Do not** place release manifests in `data/published/layers/atmosphere/`, and do not place published PMTiles bundles in `release/`. The decision and the artifact are different objects and live in different homes ([Directory Rules](../../doctrine/directory-rules.md) §13.2).
...
flowchart LR
  ...
  classDef proposed fill:#fff8e1,stroke:#b08900,color:#5a4400;
  classDef index fill:#eef2ff,stroke:#3b5bdb,color:#1c2d70;
  class RAW,WQ,PROC,CAT,PUB,CAND,PROM,MAN,SIG,ROLL,CORR confirmed;
  class IDX index;
```
...
| **REVOKED** | Release pulled; rollback target restored. | No. | Terminal; lineage retained. |
...
> [!WARNING]
> Atmosphere has four standing **knowledge-character** rules (CONFIRMED in the domain dossier):
...
> [!NOTE]
> **Role-collapse is a release-blocking error.** A `PromotionDecision` for an atmosphere release must reject (`ROLE_COLLAPSE`, `ROLE_DOWNCAST_FORBIDDEN`) any candidate that upcasts an observation to authority, a model to observation, or a context to authority. This is a CONFIRMED gate-failure reason code applicable across domains.
...
Atmosphere is overwhelmingly a **T0 (public)** domain. Sensitivity escalation is rare but not impossible — examples include sensitive joins (e.g., air observation × residential location for a small monitoring cohort), embargoed advisory previews, and low-cost networks whose terms forbid bulk redistribution.
...
| Low-cost sensor public release | T0 conditional on **CorrectionReceipt** | `CorrectionReceipt` + caveat string in `EvidenceDrawerPayload` |
...
| Model-as-observed framing | Blocked (T-not-applicable; gate failure) | not releasable; correct framing first |
...
> [!IMPORTANT]
> `release_id` is **not** a content hash. The content-addressed identity is `spec_hash` (CONFIRMED required field on `ReleaseManifest`). Catalog and proof indexing should key on `spec_hash` first and `release_id` second; never on mutable paths.
...
This example is **illustrative only**. The field set is CONFIRMED from the `ReleaseManifest` schema; the values, paths, and digests are placeholders. It is not a real release and must not be treated as one.
...
Atmosphere releases must carry a visible correction path and a resolvable rollback target before they may be treated as safely publishable (CONFIRMED doctrine in the publication, correction, and rollback model).
...
| **Rollback drill** | `docs/runbooks/atmosphere/RELEASE_ROLLBACK_RUNBOOK.md` | Periodic drill restoring a prior release; receipt emitted to `data/receipts/atmosphere/`. *PROPOSED path; runbook may not yet exist.* |
...
Every atmosphere release must close the following gates before transitioning to `PUBLISHED`. The list reflects CONFIRMED doctrine from the universal closure rules and atmosphere-specific validator list; the implementation maturity of each test is UNKNOWN without mounted-repo evidence.
...
| **Rights & sensitivity** | resolved `policy_label`, `rights_status`, `sensitivity` | source-rights-unknown denial; low-cost-sensor terms check |
...
> [!IMPORTANT]
> **Public clients reach `PUBLISHED` only through governed surfaces.** Atmosphere releases must not be addressable from `data/raw/`, `data/work/`, `data/quarantine/`, canonical/internal stores, or model runtimes. The trust membrane is the only public route (CONFIRMED doctrine).
...
| Publishing low-cost sensor data without `CorrectionReceipt` and caveat string | Required correction-and-caveat artifact missing. |
...
|---|---|---|
| Confirm canonical schema path for `ReleaseManifest` (vs `schemas/contracts/v1/release/...`) | mounted repo + ADR-0001 reference | NEEDS VERIFICATION |
| Confirm Atmosphere source rights and endpoint terms for AQS, AirNow, Mesonet, OpenAQ-like, HRRR-Smoke, HMS, GOES/ABI AOD, VIIRS | mounted repo + source registry entries | NEEDS VERIFICATION |
| Confirm presence of atmosphere-specific knowledge-character registry tests | mounted `tests/domains/atmosphere/` | NEEDS VERIFICATION |
| Verify path segments `data/proofs/atmosphere/` and `data/receipts/atmosphere/` (Directory Rules §18 open question on data/manifests) | mounted repo | PROPOSED / NEEDS VERIFICATION |
| Confirm `RELEASE_ROLLBACK_RUNBOOK.md` for atmosphere exists or is planned under `docs/runbooks/atmosphere/` | mounted repo | NEEDS VERIFICATION |
| Confirm `release_id` naming ADR exists or is needed | repo `docs/adr/` review | PROPOSED |
| Verify Evidence Drawer / Focus Mode integration for atmosphere releases | mounted repo + UI tests | NEEDS VERIFICATION |
| Confirm `release_state_register.yaml` includes atmosphere entries | mounted `control_plane/release_state_register.yaml` | NEEDS VERIFICATION |
| Resolve `PROV.md` ↔ `PROVENANCE.md` naming discrepancy (cross-doc) | ADR | PROPOSED |

> [!TIP]
> Items above are intentionally bounded. None of them block this index from existing as a navigation doc; they block specific *rows* in the register from being treated as authoritative. Add resolved items to `docs/registers/VERIFICATION_BACKLOG.md` as they close.
...
- [`./SOURCE_INDEX.md`](./SOURCE_INDEX.md) — Atmosphere source descriptors index *(PROPOSED neighbor)*
- [`./CATALOG_INDEX.md`](./CATALOG_INDEX.md) — Atmosphere catalog/STAC index *(PROPOSED neighbor)*
- [`../README.md`](../README.md) — `docs/domains/` landing
- [`../../doctrine/directory-rules.md`](../../doctrine/directory-rules.md) — Directory Rules (§9.2 release, §12 Domain Placement, §13 anti-patterns)
- [`../../doctrine/lifecycle-law.md`](../../doctrine/lifecycle-law.md) — Lifecycle invariant *(PROPOSED path)*
- [`../../doctrine/trust-membrane.md`](../../doctrine/trust-membrane.md) — Trust membrane doctrine *(PROPOSED path)*
...
<sub>**Last reviewed:** 2026-05-16 · **Doc type:** Release index (human-facing navigation; not a release decision authority) · **Authority anchor:** Directory Rules §9.2, §12 · **Truth posture:** cite-or-abstain · [⬆ Back to top](#table-of-contents)</sub>
