...
notes:
  - PROPOSED placement per Directory Rules §3 Step 3 and §12 Domain Placement Law.
  - Doctrinal artifact only; canonical machine-readable registry lives under data/registry/sources/atmosphere/.
[/KFM_META_BLOCK_V2] -->

[![Status: draft](https://img.shields.io/badge/status-draft-orange)](#status)
[![Domain: atmosphere](https://img.shields.io/badge/domain-atmosphere-0aa)](#scope)
...
[![Schema home: schemas%2Fcontracts%2Fv1%2F](https://img.shields.io/badge/schema--home-schemas%2Fcontracts%2Fv1%2F-success)](#-schema-and-contract-anchors)
[![CI: TODO](https://img.shields.io/badge/CI-TODO-lightgrey)](#-validators-tests-and-fixtures)
...
5. [Source families (Atmosphere)](#-source-families-atmosphere)
6. [Source-role anti-collapse register](#-source-role-anti-collapse-register)
7. [Knowledge-character vocabulary](#-knowledge-character-vocabulary)
8. [Source activation flow](#-source-activation-flow)
9. [Pipeline shape (RAW → PUBLISHED)](#-pipeline-shape-raw--published)
10. [Sensitivity, rights, and publication posture](#-sensitivity-rights-and-publication-posture)
11. [Schema and contract anchors](#-schema-and-contract-anchors)
12. [Validators, tests, and fixtures](#-validators-tests-and-fixtures)
13. [Governed AI behavior](#-governed-ai-behavior)
14. [Companion machine-readable locations](#-companion-machine-readable-locations)
15. [Open questions and verification backlog](#-open-questions-and-verification-backlog)
...
> and advisory context. It is **not** a bibliography. [DOM-AIR] [ENCY] [BLD-GREEN §9]
...
make its source-role enforceable. [BLD-COMP §8] [IMPL-PIPE §13]

[Back to top](#-atmosphere--source-registry)
...
- Reference to the `SourceActivationDecision` and its current state (`allowed`,
  `restricted`, `denied`, `needs-review`). [BLD-COMP §§8.1-8.2]
...
| API keys or secrets | Never in repo — environment / secret store only [ML-061-049] |
| A bibliography or citations index | Not here — this is an admission surface, not a reading list. [BLD-GREEN §9] |
...
convenience**. [DOM-AIR] [ENCY] [BLD-COMP §8] [IMPL-PIPE §13]
...
| **EPA AirNow** | Public AQI report | `regulatory` (AQI authority), `observed` | API-key gated; preliminary; not regulatory until QA/QC | Near real-time; lag ~hour | PROPOSED [DOM-AIR] [C10-02] |
...
| **PurpleAir (low-cost network)** | Low-cost sensor | `observed` (community), never `regulatory` | API ToS NEEDS VERIFICATION; keys never in catalog metadata | Sub-hourly | PROPOSED [DOM-AIR] [C10-02] [ML-061-049] |
| **OpenAQ-like aggregators** | Aggregated observation | `aggregate`, `observed` | Terms NEEDS VERIFICATION | Source-vintage / cadence specific | PROPOSED [DOM-AIR] [ENCY] |
...
| **Kansas Mesonet** | Network and site context | `observed` | **Written consent required** (`kansas-wdl@k-state.edu`); fail-closed if absent | Sub-hourly | PROPOSED [DOM-AIR] [New-Ideas-5-8-26] |
| **HRRR-Smoke / NOAA smoke forecast** | Atmospheric model field | `modeled` | Public; redistribution per NOAA terms | Hourly model run | PROPOSED [DOM-AIR] [ENCY] |
...
| **MAIAC AOD (MCD19 / VNP19)** | Remote-sensing mask | `observed` (sensor), `modeled` (retrieval) | NASA Earthdata terms | Daily | PROPOSED [DOM-AIR] [New-Ideas-5-8-26] |
...
> Low-cost sensor public release requires correction (e.g., a versioned regression such
> as the EPA Barkjohn correction for PurpleAir), caveats, confidence, and limitations.
...
publishable observation. The Atlas §24.1 maps each source role to mandatory
SourceDescriptor fields. [ENCY §24.1]
...
| `LOW_COST_SENSOR` | Community/low-cost sensor stream. | Correction + caveat + confidence required. |
...
the fixtures, the validators, and the policy gates exist. [BLD-COMP §§8.1-8.2]
[IMPL-PIPE §13]
...
> [DIRRULES §13.4-§13.5] [New-Ideas-5-8-26]
...
| Public AQI report (AirNow) | T0 | None for AQI surface; concentration claim requires source-role separation. | `PolicyDecision` (AQI-vs-concentration). |
| Model field (HRRR-Smoke, CAMS) | T0 (manifest) | None on model surface; **DENY** as observation. | `ModelRunReceipt` resolves. |
| Low-cost sensor (PurpleAir) | T1 (corrected) | Apply versioned correction; preserve uncorrected pair; surface confidence band. | Correction-version pinned in receipt. |
...
| Low-cost sensor caveat | Public release requires correction-version pinned receipt; uncorrected pair preserved. | PROPOSED [C10-02] |
...
| Rights / terms posture | `SourceActivationDecision` exists before any connector activation; ToS-changed sources fail closed. | PROPOSED [BLD-COMP §§8.1-8.2] |
| Stale-state | Sources past cadence display stale badge; AI ABSTAINs without fresh evidence. | PROPOSED [DOM-AIR] |
| Dry-run / no-live-fetch | First-PR fixtures and validators run with **no network**; receipts emitted deterministically. | PROPOSED [DOM-AIR] [ML-063-057] |
...
| Policy deny / abstain | OPA / Conftest equivalents return finite outcomes for unknown rights and uncertified providers. | PROPOSED [ML-063-055] |
...
> binding beyond typed contract notes. [BLD-COMP §§8.1-8.2]
...
|---|---|---|
| Current rights and terms of every Atmosphere source family | Steward-recorded rights register + `SourceActivationDecision` per family | NEEDS VERIFICATION |
| PurpleAir API ToS posture and Barkjohn correction version pin | Current ToS read + correction-version policy entry | NEEDS VERIFICATION [C10-02] |
| Kansas Mesonet written-consent record on file | Steward record with contact `kansas-wdl@k-state.edu` | NEEDS VERIFICATION [New-Ideas-5-8-26] |
| Knowledge-character registry exists in repo with tests | Validator + fixtures + CI green | NEEDS VERIFICATION [DOM-AIR] |
| `SourceDescriptor` schema home (`schemas/contracts/v1/source/source-descriptor.json`) | Mounted-file inspection | NEEDS VERIFICATION [DIRRULES §7.4] |
| Source-role enum stability and evolution rule | ADR-S-04 accepted | OPEN [ENCY §24.12] |
| Receipt schema split (cross-cutting vs per-domain) | ADR-S-03 accepted | OPEN [ENCY §24.12] |
| Sensitivity tier scheme (T0–T4) adopted as canonical | ADR-S-05 accepted | OPEN [ENCY §24.12] |
| Atmosphere governed-API route surface and route names | Mounted `apps/governed-api/` inspection | UNKNOWN [DOM-AIR] |
| Dry-run / no-live-fetch CI gate in place for Atmosphere | Workflow + fixture + receipt presence | NEEDS VERIFICATION [ML-063-057] |
| Sensitive-geometry deny tests for Atmosphere joins (residences, critical assets) | Negative fixture + policy DENY | NEEDS VERIFICATION [DOM-SETTLE] |
...
- Project knowledge: **KFM Domains Culmination Atlas v1.1** §§11 (Atmosphere/Air), §24.1
  ...
  Backlog); **KFM Domain and Capability Encyclopedia** §7.9; **KFM Unified Implementation
  Architecture Build Manual** §§3.6, 6.10.

---

<sub>**Last updated:** 2026-05-16 · **Doc id:** `kfm://doc/atmosphere-source-registry` ·
**Version:** v1 (draft) · [Back to top](#-atmosphere--source-registry)</sub>
