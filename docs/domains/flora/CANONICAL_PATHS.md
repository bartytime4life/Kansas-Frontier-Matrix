...
version: v1
status: draft
owners: [Flora domain steward, Docs steward]   # TODO: confirm against CODEOWNERS
created: 2026-05-16
updated: 2026-05-16
policy_label: public
related:
  - docs/doctrine/directory-rules.md
  - docs/domains/flora/README.md                # TODO: verify presence
  - docs/domains/fauna/CANONICAL_PATHS.md       # parallel domain register
  - docs/adr/ADR-0001-schema-home.md            # NEEDS VERIFICATION
...
notes:
  ...
  - Lane pattern CONFIRMED by Directory Rules §12.
...
[![Status: Draft](https://img.shields.io/badge/status-draft-orange)](#status-and-authority)
...
[![Lifecycle: RAW→WORK→PROCESSED→CATALOG→PUBLISHED](https://img.shields.io/badge/lifecycle-RAW%E2%86%92...%E2%86%92PUBLISHED-informational)](#7-lifecycle-data-lanes)
![Updated: 2026-05-16](https://img.shields.io/badge/updated-2026--05--16-lightgrey)

**Status:** Draft · **Owners:** Flora domain steward + Docs steward (placeholders — confirm against `CODEOWNERS`) · **Last updated:** 2026-05-16
...
| **Schema-home convention** | `schemas/contracts/v1/...` default per **ADR-0001**. *(NEEDS VERIFICATION: ADR-0001 file presence.)* |
| **Owners** | Flora domain steward + Docs steward. *(TODO: confirm against `CODEOWNERS`.)* |
...
- **CONFIRMED:** *"This pattern applies uniformly to: hydrology, soil, fauna, **flora**, habitat, geology, atmosphere, roads-rail-trade, settlements-infrastructure, archaeology, hazards, agriculture, people-dna-land, and any new domain."* — Directory Rules §12.
- **CONFIRMED:** Flora ownership and object families — `PlantTaxon`, `SpecimenRecord`, `FloraOccurrence`, `RarePlantRecord`, `VegetationCommunity`, `InvasivePlantRecord`, `PhenologyObservation`, `RangePolygon`, `HabitatAssociation`, `BotanicalSurvey`, `RestorationPlanting`, `RedactionReceipt`, plus `FloraTaxon Crosswalk` — per KFM Encyclopedia §7.6 and KFM Domains Culmination Atlas §C/E.
- **CONFIRMED:** Sensitivity posture — *"Rare plants, steward-controlled records, exact sensitive occurrence geometry, and culturally sensitive plant knowledge require fail-closed treatment, redaction, generalization, review, or denial."* — DOM-FLORA §§11–12 / Unified Implementation Manual §30.5.
- **CONFIRMED:** Lifecycle invariant — `RAW → WORK / QUARANTINE → PROCESSED → CATALOG / TRIPLET → PUBLISHED`, with promotion as a **governed state transition**, not a file move. *(Directory Rules §9.1, Lifecycle phase rules.)*
- **CONFIRMED:** Watcher-as-non-publisher invariant — workers emit receipts and candidate decisions only; they MUST NOT publish, mutate canonical records, or bypass review. *(Directory Rules §7.1.)*
...
flowchart TD
    ...
    ROOT --> POL[policy/domains/flora/<br/>policy/sensitivity/flora/]:::gov

    ROOT --> TESTS[tests/domains/flora/]:::test
    ROOT --> FIX[fixtures/domains/flora/]:::test

    ROOT --> PKG[packages/domains/flora/]:::impl
    ROOT --> CONN[connectors/&lt;source&gt;/<br/>data flows to data/raw/flora/]:::impl
    ROOT --> PIPE[pipelines/domains/flora/<br/>pipeline_specs/flora/]:::impl

    ROOT --> DATA[data/raw/flora/<br/>data/work/flora/<br/>data/quarantine/flora/<br/>data/processed/flora/<br/>data/catalog/domain/flora/<br/>data/published/layers/flora/<br/>data/registry/sources/flora/<br/>data/rollback/flora/]:::data
...
| `docs/runbooks/flora/SOURCE_REFRESH_RUNBOOK.md` | Source refresh runbook (mirrors the existing `docs/runbooks/fauna/` convention; flagged as a subfolder pattern that may diverge from a flat-prefix repo convention). | PROPOSED / NEEDS VERIFICATION |
...
| `schemas/contracts/v1/domains/flora/redaction_receipt.schema.json` | PROPOSED |

> [!WARNING]
> **Schema-home rule (ADR-0001 default).** `schemas/contracts/v1/...` is canonical. A schema at `contracts/domains/flora/<x>.schema.json` is **lineage / CONFLICTED** under Directory Rules §13.1 and must be migrated. *(NEEDS VERIFICATION: ADR-0001 file presence and exact filename.)*
...
> [!CAUTION]
> **Default-deny for public exact geometry.** Rare plants, steward-controlled records, exact sensitive occurrence geometry, and culturally sensitive plant knowledge **MUST fail closed** unless a documented geoprivacy transform and review state allow release. *(DOM-FLORA §§11–12; KFM Encyclopedia §7.6.)*
...
`RedactionReceipt` records live under `data/receipts/flora/redaction/` *(PROPOSED — exact path subject to receipts-class layout ADR-S-03 per Master Open-ADR Backlog).*
...
| `apps/` | *(none — flora is data, not an app)* | The public trust path is `apps/governed-api/`. Flora-specific endpoints surface there, **not** as a `apps/flora/`. |
...
> [!CAUTION]
> The flora first-slice rule is **CONFIRMED**: *"PROPOSED first slice is schema/fixture/validator-first and uses public-safe flora fixtures before any live source activation."* No live connector to GBIF, iNaturalist, NatureServe, USFWS ECOS, or KNHI is activated before the synthetic-fixture proof lands.
...
Connectors fetch from external sources and emit **only** to `data/raw/flora/<source_id>/<run_id>/` or `data/quarantine/flora/...`. They MUST NOT publish, mutate canonical truth, or write under `data/processed/`, `data/catalog/`, or `data/published/`. *(Directory Rules §7.3.)*
...
| Source family | CONFIRMED source basis | Connector path (PROPOSED) | Sensitivity notes |
|---|---|---|---|
| USDA PLANTS (taxonomic backbone) | KFM Encyclopedia §7.6; New Ideas 5-10-26 | `connectors/usda_plants/` | Taxonomy authority; benign in isolation, **sensitive when joined** (PLANTS join rule). |
| GBIF (occurrence aggregator) | KFM Encyclopedia §7.6; EXT-GBIF | `connectors/gbif/` | Licenses, sensitive records, and uncertainty vary by record. |
| iNaturalist | KFM Encyclopedia §7.6; EXT-INAT | `connectors/inaturalist/` | Usage limits and geoprivacy apply. Not authoritative for legal status. |
| NatureServe Explorer / Explorer Pro | KFM Encyclopedia §7.6; EXT-NATSERVE | `connectors/natureserve/` | Explorer Pro is access-controlled; rare-species precise data requires account. |
| USFWS ECOS | KFM Encyclopedia §7.6; EXT-FWS | `connectors/usfws_ecos/` | Threatened/endangered status; public sensitivity policy still required. |
| iDigBio | KFM Domains Culmination Atlas | `connectors/idigbio/` | Specimen records; license per record. |
| Kansas Biological Survey / KU Herbarium / KNHI | KFM Domains Culmination Atlas; New Ideas 5-8-26 | `connectors/kbs/` *(or `connectors/ku_herbarium/`)* | **KNHI rare-plant locations are restricted**; must be redacted/generalized before any publication. |
| KDWP (state listed species) | KFM Domains Culmination Atlas | `connectors/kdwp/` | State sensitivity rules; coordination with fauna connector likely needed. |
| KDWP Ecological Review Tool | KFM Domains Culmination Atlas | `connectors/kdwp_ert/` | Stewardship outputs; rights NEEDS VERIFICATION. |
| EDDMapS (invasives) | Adjacent to fauna; flora invasives | `connectors/eddmaps/` | Cross-cuts with fauna; route product through both domains as appropriate. |
| Remote-sensing vegetation indices (NDVI, EVI, MODIS-derived) | KFM Encyclopedia §7.6 | `connectors/<rs_index>/` | Modeled / derived; source-role label `model`. |
| Restoration project records | KFM Encyclopedia §7.6 | `connectors/restoration_<program>/` | Rights per program. |
...
| **Taxonomy drift mistaken for presence drift** | A PLANTS county package "addition" reflects a name change, not a real new occurrence | Track taxonomy version drift separately from package drift; require stable taxonomy version before computing drift candidates. *(Pass 20 idea index.)* |
...
| 10 | Whether flora-scoped ADRs live flat under `docs/adr/` or nested under `docs/domains/flora/adr/` | OPEN | ADR or Docs-steward decision |
| 11 | The `restricted/` subfolder convention within `data/processed/flora/<dataset>/<version>/` | OPEN | ADR or per-root README |
| 12 | Whether `RedactionReceipt` belongs under `data/receipts/redaction/flora/` or under a shared kernel home | OPEN | ADR-S-03 |
| 13 | Cross-domain placement of invasive-plant content (flora-owned vs habitat / fauna shared) | OPEN | Cross-domain doctrine doc |
...
- [`docs/domains/flora/README.md`](./README.md) — Flora domain dossier landing. **TODO target.**
- [`docs/domains/fauna/CANONICAL_PATHS.md`](../fauna/CANONICAL_PATHS.md) — Parallel canonical-paths register for the fauna domain.
- [`docs/runbooks/flora/SOURCE_REFRESH_RUNBOOK.md`](../../runbooks/flora/SOURCE_REFRESH_RUNBOOK.md) — Flora source refresh runbook. **TODO target.**
- [`docs/runbooks/flora/ROLLBACK_RUNBOOK.md`](../../runbooks/flora/ROLLBACK_RUNBOOK.md) — Flora rollback runbook. **TODO target.**
...
- [`docs/registers/DRIFT_REGISTER.md`](../../registers/DRIFT_REGISTER.md) — Cross-repo drift register.
- [`docs/registers/CANONICAL_LINEAGE_EXPLORATORY.md`](../../registers/CANONICAL_LINEAGE_EXPLORATORY.md) — Path migration / lineage notes.
- [`contracts/domains/flora/`](../../../contracts/domains/flora/) — Flora object-family meaning. **TODO target.**
- [`schemas/contracts/v1/domains/flora/`](../../../schemas/contracts/v1/domains/flora/) — Flora machine shape. **TODO target.**
- [`policy/sensitivity/flora/`](../../../policy/sensitivity/flora/) — Flora sensitivity classes and redaction rules. **TODO target.**
...
The full canonical path inventory for the Flora lane, expanded. Use this as a copy-paste reference when scaffolding the lane. **All paths are PROPOSED until verified against the live repo.**
...
docs/runbooks/flora/                           ← runbook subfolder (parallel to fauna)
...
> [!NOTE]
> **A note on placeholders.** Every `TODO`, `NEEDS VERIFICATION`, and `PROPOSED` marker in this document is intentional. Per Directory Rules §0, the lane *pattern* is CONFIRMED while any *specific path* remains PROPOSED until verified against current mounted-repo evidence. Resolve placeholders by inspection or ADR — do not silently promote them to fact.

---

**Related docs:** [Directory Rules](../../doctrine/directory-rules.md) · [Flora README](./README.md) (TODO) · [Fauna CANONICAL_PATHS](../fauna/CANONICAL_PATHS.md) · [Flora SOURCE_REFRESH_RUNBOOK](../../runbooks/flora/SOURCE_REFRESH_RUNBOOK.md) (TODO) · [VERIFICATION_BACKLOG](../../registers/VERIFICATION_BACKLOG.md) · [DRIFT_REGISTER](../../registers/DRIFT_REGISTER.md)

**Last updated:** 2026-05-16 · **Version:** v1 (draft) · [Back to top](#table-of-contents)
