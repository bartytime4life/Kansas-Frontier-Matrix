...
notes:
  ...
  - Primary rows mirror Atlas v1.0 §10.N verbatim; supplementary rows are derived from Atlas §10.D/K/I, Encyclopedia §7.8.L, and Unified Build Manual §6.11.
  - Cross-references Atlas v1.1 §24.12 (Master Open-ADR Backlog) where rows are ADR-class.
[/KFM_META_BLOCK_V2] -->
...
| **Doctrinal basis** | Atlas v1.0 §10 (Geology and Natural Resources) · Atlas v1.1 §24.12 (Master Open-ADR Backlog) · Encyclopedia §7.8 · Unified Build Manual §6.11 (30.11) · Directory Rules §12 (Domain Placement Law) and §18 (Open Questions). |
...
- **What carries open rights or sensitivity uncertainty** that blocks public promotion
  ...
  resource, well-log, and private well locations (Atlas v1.0 §10.I).
...
- **Geology object families** *(CONFIRMED doctrine, PROPOSED field realization — Atlas
  v1.0 §10.B, §10.E)*: `GeologicUnit`, `SurficialUnit`, `Lithology`,
...
- **Geology source families** *(Atlas v1.0 §10.D — all listed `NEEDS VERIFICATION`)*:
...
```mermaid
flowchart LR
  A[Doctrine sources<br/>Atlas v1.0/v1.1<br/>Encyclopedia<br/>Unified Build Manual] --> B[Geology backlog<br/>this file]
...
> **Source:** Atlas v1.0 §10.N (verbatim). All four rows are doctrinally listed as
...
> (Atlas v1.0 §10.I; Encyclopedia §7.8 risk register). No public release may bypass
...
Atlas v1.0 §10.D lists **eight** geology source families, all with rights and current
...
> [!WARNING]
> Per Atlas v1.0 §10.I and Directory Rules trust-membrane doctrine, **unclear rights**
...
> **Source:** Atlas v1.0 §10.K (all `PROPOSED`) and Encyclopedia §7.8.K. Cross-domain
...
|---|---|---|---|
| **GEO-VT-01** | Source-role validators | Prevent collapse of `authority`, `observation`, `context`, `model`, `regulatory`, and `legal` roles; require declared role to match source kind. | PROPOSED |
| **GEO-VT-02** | Resource-class anti-collapse tests | Keep `MineralOccurrence`, `ResourceDeposit`, `ResourceEstimate`, permit, production, and reserve claims distinct; deny merges that erase the distinction (Atlas v1.0 §10.I). | PROPOSED |
...
the repository-wide validator families documented in Encyclopedia §7.8.K and the
...
> **Source:** Encyclopedia §7.8.L (geology feature backlog) and Atlas v1.0 §10.G
...
| **GEO-FB-06** | Thin-slice (Atlas §10.N + Encyclopedia §7.8.N) | One-county geologic-unit fixture with borehole / cross-section evidence and public-safe generalized resource context | steward | `EvidenceBundle`-backed unit inspector | catalog closure + Drawer + citation tests | PROPOSED |
...
| # | Question | Why ADR-class | Linked Master Open-ADR (Atlas v1.1 §24.12) | Status |
|---|---|---|---|---|
| **GEO-AQ-01** | Confirm schema home for geology contracts: `schemas/contracts/v1/domains/geology/...` per ADR-0001 default, or alternative. | Schema-home rule is explicitly ADR-required (Directory Rules §2.4(3)). | **ADR-S-01** (Schema home) | OPEN |
| **GEO-AQ-02** | Source-role vocabulary and evolution rule for geology: `authority` / `observation` / `context` / `model` / `regulatory` / `legal`. | Source-role anti-collapse is doctrine-significant for resource claims. | **ADR-S-04** (Source-role vocabulary v1) | OPEN |
| **GEO-AQ-03** | Sensitivity tier scheme (T0–T4) and placement of exact boreholes, samples, sensitive resources, well-logs, and private wells into the deny-by-default tier. | Tier scheme directly governs public release; adoption is doctrine-class. | **ADR-S-05** (Sensitivity tier scheme) | OPEN |
...
| **GEO-AQ-06** | Cross-lane join policy: which `Geology × Soil`, `Geology × Hydrology`, `Geology × Hazards`, `Geology × People/Land` joins require steward review, are denied, or are open. | Cross-lane joins are inference-risk multipliers; geology sits on the boundary of soil parent-material, hydrostratigraphy, fault/landslide context, and lease/parcel/operator claims (Atlas v1.0 §10.F). | **ADR-S-14** (Cross-lane join policy) | OPEN |
| **GEO-AQ-07** | Stale-state propagation: how `ResourceEstimate`, `Borehole`, and source-vintage cadence interact with downstream Hydrology / Hazards consumers. | Cross-lane staleness is a correction-path question. | **ADR-S-10** (Stale-state propagation) | OPEN |
...
> preserves the doctrinal lineage Directory Rules §14.2 expects for structural moves.
...
> **Source:** Unified Build Manual §6.11 (30.11) `Open verification items`. Repeated
> here for visibility; closure depends on a mounted repository scan and so is bounded
> per the repository preflight rule. Each row is `UNKNOWN` until inspected on a
> mounted repo.
...
> [!NOTE]
> The triage cadence below is **PROPOSED** and mirrors the pattern in Atlas v1.1
> §24.11 (governance health indicators). It becomes operational when the geology
...
- **OPEN — per-domain register convention.** Is `docs/domains/<domain>/VERIFICATION_BACKLOG.md`
  ...
  a `docs/registers/DRIFT_REGISTER.md` entry per Directory Rules §2.5.)*
...
- Atlas v1.0 §10 (Geology and Natural Resources) and v1.1 §24.12 (Master Open-ADR Backlog) — doctrinal source for this register.
- Encyclopedia §7.8 (Geology and Natural Resources) — feature backlog and risk register.
- Unified Implementation Architecture Build Manual §6.11 / 30.11 (Geology / Natural Resources) — open verification items.

---

**Last updated:** 2026-05-17 · **Lane:** Geology and Natural Resources · **Authority class:** domain feeder register · [⬆ back to top](#contents)
