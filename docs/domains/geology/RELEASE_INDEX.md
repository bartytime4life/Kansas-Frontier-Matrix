...
**Status:** `draft` · **Owners:** `<geology-domain-steward>`, `<release-steward>`, `<docs-steward>` · **Last updated:** `2026-05-17`
...
| Geology sensitivity tier rules in depth | `docs/domains/geology/SENSITIVITY.md` (see related docs) |
...
[Back to top](#geology-and-natural-resources--release-index)
...
> [!NOTE]
> The doctrinal slug for the Geology domain in `docs/domains/` is **`geology/`** per Directory Rules §6.1 [DIRRULES]. A prior Encyclopedia draft section (`7.8` of `kfm_encyclopedia.pdf`) also lists a long-form `docs/domains/geology-and-natural-resources/`. The directory-rules form is canonical; the longer slug is treated as a **drift candidate** and flagged in [§12 Open verification items](#12-open-verification-items).
...
| **`ValidationReport`** | Records validator outcomes for Geology objects (schema, geometry validity, source-role, resource-class, public-safe geometry, borehole/well-log rights, catalog closure, AI evidence-before-model). | `data/proofs/validation_report/` | CONFIRMED doctrine / PROPOSED implementation; validators are PROPOSED per §K of Atlas Ch. 10 |
| **`PolicyDecision`** | `ALLOW / RESTRICT / DENY / ABSTAIN / ERROR` verdict at promotion or runtime for a Geology candidate or query. | `policy/domains/geology/` (eval) → emitted under `data/receipts/` | CONFIRMED doctrine / PROPOSED implementation |
...
Geology inherits the **cross-domain release gates** (validation, policy, evidence closure, catalog closure, manifest, signing, rollback target) and adds **domain-specific** ones drawn from the Atlas Ch. 10 §I–§K [DOM-GEOL] [ENCY]. All listed validators/tests are CONFIRMED doctrine / PROPOSED implementation.
...
| `ReleaseManifest` missing `rollback_target` | Manifest closure | `DENY`; release fails closed; ML-058-045 [DIRRULES][MAP-MASTER]. |
...
CONFIRMED doctrine: Geology's sensitivity posture defaults to **restricted or generalized** for exact subsurface and resource detail; aggregate and unit-level public geometry are routine [DOM-GEOL] [ENCY]. Tier values below are PROPOSED (per Atlas §24.5, T0–T4 scheme) and subject to ADR-S-05.

| Object family | Public-default sensitivity (PROPOSED) | Release-time rule |
|---|---|---|
| `GeologicUnit`, `Lithology`, `StratigraphicInterval`, `GeologicAge` | T0 (aggregate / unit polygon) | Public-safe by default; cite source and version. |
| `FaultStructure`, `CrossSection` (interpretation-bounded) | T0 / T1 depending on uncertainty surface | Public; `RealityBoundaryNote` for reconstruction-heavy cross-sections. |
| `Borehole`, `WellLog`, `CoreSample`, `GeochemistrySample` | **T2 / T4 default** for exact location | DENY exact coordinates by default; allow only generalized geometry with steward review. |
| `GeophysicalObservation` (raster / volumes) | T0 / T1 | Public for unit-scale; finer detail per source rights. |
| `MineralOccurrence` | T0 aggregate / T2 detail in sensitive contexts | Keep `Occurrence` distinct from `Deposit` / `Estimate` / `Reserve`. |
| `ResourceDeposit`, `ResourceEstimate` | T2 default for detail | DENY field-level economic-significance claims without explicit rights. |
| `ExtractionSite`, `ReclamationRecord` | T0 / T1 | Public-safe for status; private operator/permit detail held back. |
| `HydrostratigraphicUnit` | T0 | Public; cross-links to Hydrology domain per Atlas §2.2 [ENCY]. |

> [!IMPORTANT]
> **Borehole / well-log / sensitive resource public policy is NEEDS VERIFICATION** [DOM-GEOL] Atlas §10.N. This index uses the doctrinal default (deny-by-default for exact location) until the policy is resolved and recorded in `policy/domains/geology/` plus an ADR.
...
> [!CAUTION]
> **Rollback is required to be drilled** — an untested rollback is not a rollback. The Master MapLibre Atlas (ML-058-043) and Directory Rules both require correction/rollback replay verification as part of any map-bearing release. This applies to Geology cross-section, bedrock, and resource layers as much as to any other geo-bearing carrier [MAP-MASTER] [DIRRULES].
...
| Confirm borehole / well-log public release policy. | **NEEDS VERIFICATION** | `policy/domains/geology/` + ADR | Atlas §10.I, §10.N |
...
| Resolve docs slug: `docs/domains/geology/` (Directory Rules canonical) vs `docs/domains/geology-and-natural-resources/` (Encyclopedia §7.8 PROPOSED). | **OPEN** | Drift register entry + ADR | DIRRULES §6.1; ENCY §7.8 |
...
| Confirm Geology rollback drill exists and replays prior `release_id`. | **NEEDS VERIFICATION** | `tests/domains/geology/rollback/` + receipt | DIRRULES §9.2; MAP-MASTER ML-058-043 |
...
- `docs/domains/geology/SENSITIVITY.md` — Per-object sensitivity defaults and decision lattice (TODO).
- `docs/domains/geology/SOURCES.md` — KGS / KCC / USGS source families and roles (TODO).
...
Drawn from Atlas Ch. 10 §B (object scope) and §I (sensitivity, rights, publication posture) [DOM-GEOL] [ENCY]. Sensitivity tier values are **PROPOSED** pending ADR-S-05.

| Object family | Geometry typically released publicly | Default sensitivity | Anti-collapse partner(s) | Notes |
|---|---|---|---|---|
| `GeologicUnit` | unit polygon | T0 | `Lithology`, `StratigraphicInterval`, `GeologicAge` | unit ≠ lithology; preserve distinct fields. |
| `Lithology` | inherits unit | T0 | `GeologicUnit` | — |
| `StratigraphicInterval` | column / cross-section span | T0 | `GeologicAge` | — |
| `GeologicAge` | annotation | T0 | `StratigraphicInterval` | — |
| `FaultStructure` | line | T0 / T1 | — | uncertainty surface where applicable. |
| `Borehole` | **generalized** point (default) | **T2 / T4** | `WellLog`, `CoreSample` | exact location DENY by default. |
| `WellLog` | sidecar to borehole | **T2 / T4** | `Borehole`, `CoreSample` | rights resolution prerequisite. |
| `CoreSample` | sidecar to borehole | T2 / T4 | `Borehole`, `WellLog` | — |
| `GeophysicalObservation` | raster / volume | T0 / T1 | — | scale / support must be validated. |
| `GeochemistrySample` | generalized point | T2 / T4 | — | rights and laboratory chain. |
| `MineralOccurrence` | point / polygon (aggregate) | T0 aggregate; T2 detail | `ResourceDeposit`, `ResourceEstimate` | distinct from `Deposit` / `Estimate`. |
| `ResourceDeposit` | polygon / aggregate | T2 detail | `MineralOccurrence`, `ResourceEstimate` | — |
| `ExtractionSite` | site polygon | T0 / T1 | `ReclamationRecord` | private operator detail held back. |
| `ReclamationRecord` | site polygon | T0 / T1 | `ExtractionSite` | — |
| `CrossSection` | line + interpretation sidecar | T0 / T1 | `GeologicUnit`, `StratigraphicInterval` | `RealityBoundaryNote` for reconstruction-heavy. |
| `HydrostratigraphicUnit` | polygon / column | T0 | `GeologicUnit` (geol) ↔ aquifer/aquitard (hydro) | cross-links to Hydrology per Atlas §2.2. |
...
Drawn from Atlas §24.2 (Receipt Catalog), Directory Rules §19 (Glossary), and the `release_manifest.schema.json` skeleton appearing in the `New_Ideas_5-10-26.pdf` source packet [ENCY] [DIRRULES]. **All field-level shape is normative in `schemas/contracts/v1/release/release_manifest.schema.json` (PROPOSED path) — this is a documentation expectation only.**
...
| `artifacts[]` | Includes layer artifacts (PMTiles, GeoJSON, GeoParquet, COG sidecar), `LayerManifest`, `TileArtifactManifest`, and any cross-section / 3D sidecar; each with `digest`. | CONFIRMED doctrine / PROPOSED enum coverage (cf. ML-058-044) |
| `evidence_refs[]` | Resolves to one or more `EvidenceBundle`s under `data/proofs/evidence_bundle/`. | CONFIRMED doctrine |
| `spec_hash` | Carried through; must exclude itself from canonicalization (cf. ML-M-046). | CONFIRMED doctrine |
| `published_at` | UTC timestamp. | CONFIRMED doctrine |
| `rollback_target` | Prior `release_id` for this domain/layer set; required by ML-058-045 policy. | CONFIRMED doctrine |
| `policy_decision_ref` | Reference to the `PolicyDecision` that approved this release. | PROPOSED |
...
| `cache_keys[]` | For tile / CDN invalidation (cf. ML-M-105). | PROPOSED |
...
- **[DOM-GEOL]** — `KFM_Domains_Culmination_Atlas_v1_1.pdf`, Ch. 10 (Geology and Natural Resources).
- **[ENCY]** — `kfm_encyclopedia.pdf`, §7.8 (Geology and Natural Resources) and §11 (Master Viewing Mode Atlas).
- **[DIRRULES]** — `directory-rules.md`, §6.1 (`docs/`), §9.1 (`data/`), §9.2 (`release/`), §12 (Domain Placement Law), §13 (anti-patterns), §15 (Required README contract).
- **[MAP-MASTER]** — `Master MapLibre Components-Functions-Features.pdf`, Ch. M (LayerManifest, StyleManifest, TileArtifactManifest, MapReleaseManifest) and Ch. U (Testing, CI, Validation, Rollback, and Proof Objects).
- **[UIAI]** — `KFM_Whole_UI_Governed_AI_Expansion_Report.pdf`, §29 Appendix A (PROPOSED tree).

---

<div align="center">

**Related docs:** [README](./README.md) · [Lifecycle law](../../doctrine/lifecycle-law.md) · [Directory Rules](../../doctrine/directory-rules.md) · [release/](../../../release/README.md)

**Last updated:** 2026-05-17 · **Doc version:** v1 (draft)
