...
notes:
  - "Companion to (not replacement for) the fauna README."
  - "Naming follows user task; the Whole-UI report uses CONTINUITY_NOTES.md — see §17."
[/KFM_META_BLOCK_V2] -->
...
| **Companion docs (PROPOSED)** | `docs/domains/fauna/README.md`, `docs/runbooks/fauna/SOURCE_REFRESH_RUNBOOK.md` |
...
| **[DOM-FAUNA]** | `KFM_Fauna_Architecture_PDF_Only_Report.pdf` | Fauna domain dossier (sensitivity posture, source-role patterns, proof-slice candidates). PDF-only lineage. |
| **[DOM-HF]** | `KFM_Habitat_Fauna_Thin_Slice_Extended_Pro_Blueprint.pdf` | Habitat-fauna thin-slice blueprint; first public-safe occurrence-to-habitat assignment proof. PDF-only lineage. |
| **[DOM-HAB]** | `kfm_habitat_architecture_pdf_only_blueprint_2026-04-21.pdf` | Habitat lane dossier, cited for cross-lane relations and sensitivity inheritance. PDF-only lineage. |
| **[DOM-FLORA]** | `KFM_Flora_Architecture_PDF_Only_Implementation_Blueprint.pdf` | Flora lane dossier, cited for cross-lane relation (food-web, pollinator, invasive). PDF-only lineage. |
| **[ENCY]** | `kfm_encyclopedia.pdf` §7.5 Fauna | Domain/capability encyclopedia chapter for Fauna. CONFIRMED doctrine. |
| **[ATLAS]** | `KFM_Domains_Culmination_Atlas_v1_1.pdf` §7 Fauna | Cumulative domain atlas chapter for Fauna. CONFIRMED doctrine. |
| **[UNIFIED]** | `KFM_Unified_Implementation_Architecture_Build_Manual.pdf` §6.4 Fauna | Unified build-manual reading of fauna scope, sensitivity, pipelines, and open items. CONFIRMED doctrine. |
| **[GAI]** | `KFM_Governed_AI_Extended_Pro_Source_Ledger_PDF_Only_Architecture_Report_2026-04-20.pdf` | Governed AI architecture for adapter-first, evidence-bound runtime behavior across all domains. PDF-only lineage. |
| **[MAP-MASTER]** | `Master_MapLibre_Components-Functions-Features.pdf` | Cross-cutting map/timeline/Evidence Drawer/Focus Mode doctrine that the fauna lane inherits. PDF-only lineage. |
| **[P19]** | `KFM_Components_Pass_19_Idea_Index_Category_Atlas_and_Expansion_Dossier.pdf` | Direct prior-pass baseline; carries earlier fauna-relevant ideas forward. PDF-only lineage. |
| **[P20-IDX]** | `KFM_Pass_20_Part_2_Idea_Index_Category_Atlas_and_Expansion_Dossier.md` | Normalized idea index (KFM-IDX-* entries) including the fauna-relevant POL-005, APP-002, VAL-001, VAL-002, PLN-003. |
| **[UI-EXP]** | `KFM_Whole_UI_Governed_AI_Expansion_Report.pdf` | Source of the *Prior gains and continuity inventory* pattern itself; also the cross-cutting UI/Focus/Drawer/Story carry-forward template. |
...
> [!NOTE]
> **Continuity determination, unchanged from doctrine:** prior gains are not discarded. They are carried forward as doctrine, lineage, or proposed design pressure. None are represented as mounted fauna-lane implementation without repo evidence.
...
flowchart LR
    ...
    subgraph NEXT["Preserved next behavior (PROPOSED until verified)"]
        ...
        N3["Policy bundles<br/>(policy/domains/fauna/, policy/sensitivity/)"]
...
The fauna corpus names a stable vocabulary. Casing, compounding, and spelling are preserved exactly when these terms cross into schemas, validators, contracts, or tests.

<details>
<summary><b>Canonical fauna object families (click to expand)</b></summary>

| Object family | Classification | Evidence basis |
|---|---|---|
| **Taxon** | KEEP AS DOCTRINE | [ENCY §7.5.C], [ATLAS §7.E] |
| **Taxon Crosswalk** | KEEP AS DOCTRINE | [ATLAS §7.E] |
| **Conservation Status** | KEEP AS DOCTRINE | [ATLAS §7.E], [ENCY §7.5.C] |
| **Occurrence Evidence** | KEEP AS DOCTRINE | [ATLAS §7.E], [ENCY §7.5.C] |
| **Occurrence Restricted** | KEEP AS DOCTRINE | [ATLAS §7.E] |
| **Occurrence Public** | KEEP AS DOCTRINE | [ATLAS §7.E] |
| **RangePolygon** | KEEP AS DOCTRINE | [ATLAS §7.E], [ENCY §7.5.C] |
| **SeasonalRange** | KEEP AS DOCTRINE | [ATLAS §7.E] |
| **MigrationRoute** | KEEP AS DOCTRINE | [ATLAS §7.E] |
| **SensitiveSite** | KEEP AS DOCTRINE | [ATLAS §7.E] |
| **NestDenRoostSpawningSite** | KEEP AS DOCTRINE | [ENCY §7.5.C] |
| **MortalityObservation** | KEEP AS DOCTRINE | [ENCY §7.5.C], [ATLAS §7.E] |
| **DiseaseObservation** | KEEP AS DOCTRINE | [ENCY §7.5.C] |
| **Invasive Species Record** | KEEP AS DOCTRINE | [ENCY §7.5.C], [ATLAS §7.E] |
| **AbundanceIndicator** | KEEP AS DOCTRINE | [ENCY §7.5.C] |
| **RichnessIndicator** | KEEP AS DOCTRINE | [ENCY §7.5.C] |
| **Redaction Receipt** | KEEP AS DOCTRINE | [ATLAS §7.E], [ENCY §7.5.C] |
| **Geoprivacy transform** (term) | KEEP AS DOCTRINE | [ATLAS §7.C], [DOM-FAUNA], [DOM-HF] |
| **Public-safe derivative** (term) | KEEP AS DOCTRINE | [ATLAS §7.C], [DOM-FAUNA] |
| **MonitoringEvent** | KEEP AS DOCTRINE | [ATLAS §7.C] |
...
| Object families surface a 14-item list ([DOM-FAUNA scope]); the unified manual cites 9 (subset). | **CONFLICTED / OPEN** | [ATLAS §7.B], [UNIFIED §6.4] | Reconcile via an object-family map entry under `contracts/domains/fauna/` (PROPOSED); track in `control_plane/object_family_register.yaml` (PROPOSED). See §19. |

> [!IMPORTANT]
> KFM terminology, casing, and compound terms (e.g., **NestDenRoostSpawningSite**, **SensitiveSite**, **Occurrence Restricted**) are preserved exactly. Generic industry renames (e.g., "biodiversity record", "wildlife sighting", "PII") are not introduced silently — see [DIRRULES] §1 and the project-wide non-negotiable rules.
...
| Live wildlife connectors are **not** activated in the first PR. The first slice is synthetic, fixture-first, with a source-registry skeleton and public-safety validators. | KEEP AS DOCTRINE / KEEP AND EXTEND | [UNIFIED §6.4], [P20-IDX KFM-IDX-VAL-001], [DOM-FAUNA] | First-PR scope: `pipelines/domains/fauna/` and `fixtures/domains/fauna/` (both PROPOSED); no real-source endpoints. The runbook home is `docs/runbooks/fauna/` (PROPOSED) — see related entry in §22. |
...
> [!CAUTION]
> This is the fauna lane's **strongest invariant**. Exact sensitive occurrence, nest, den, roost, hibernacula, spawning, and steward-controlled records **fail closed**. Public exact-occurrence tiles for sensitive taxa are **denied by default**. Any release that bends this invariant requires an explicit, reviewed geoprivacy transform, a recorded **Redaction Receipt**, and steward sign-off — and even then it does not become the default.

| Surface or prior gain | Classification | Evidence basis | Preserved next behavior |
|---|---|---|---|
| **Deny-by-default register entry for rare species:** exact taxa occurrence / nest / den / roost / spawning sites → DENY public exact location; generalized public products only; geoprivacy transform receipt + steward review required. | KEEP AS DOCTRINE | [ENCY §13 Sensitive Register], [ATLAS §20.5], [DOM-FAUNA] | Codified in `policy/sensitivity/` and `policy/domains/fauna/` (both PROPOSED per [DIRRULES] §6.5). Each release-candidate carries an explicit redaction receipt. |
...
| **PUBLISHED** | Serve released public-safe artifacts through governed APIs and manifests. | ReleaseManifest, correction path, rollback target, and review/policy state exist. | KEEP AS DOCTRINE / PROPOSED. [ATLAS §7.H], [ENCY], [DIRRULES §0 Lifecycle invariant] |

> [!NOTE]
> The lifecycle invariant `RAW → WORK / QUARANTINE → PROCESSED → CATALOG / TRIPLET → PUBLISHED` is repo-wide doctrine, not fauna-specific. The fauna lane inherits it from [DIRRULES] and [ENCY]; it does not redefine the invariant. Promotion is a **governed state transition, not a file move**.
...
| MapLibre remains a **disciplined 2D renderer** behind an adapter; it is not a truth source. | WRAP WITH ADAPTER | [UI-EXP §6], [MAP-MASTER] | Fauna layers are sourced from governed APIs only; the renderer never reads canonical stores directly. |
| Cesium / 3D fauna surfaces. | DEFER | [UI-EXP §12] | Trigger to un-defer: StoryManifest, Evidence Drawer, and policy gates pass in 2D for fauna first. |
...
| Surface or prior gain | Classification | Evidence basis | Preserved next behavior |
|---|---|---|---|
| Source-role authority tests | KEEP AND EXTEND | [ATLAS §7.K] | Validator entry under `tools/validators/fauna/` (PROPOSED); negative-fixture coverage required. |
...
| Runtime Response Envelope negative cases (ABSTAIN / DENY / ERROR) for fauna requests | KEEP AND EXTEND | [ATLAS §7.K] | Negative tests live under `tests/runtime_proof/` (PROPOSED) and `tests/domains/fauna/` (PROPOSED). |
...
| AI must **ABSTAIN** when evidence is insufficient and **DENY** where policy, rights, sensitivity, or release state blocks the request. | KEEP AS DOCTRINE | [ATLAS §7.L], [GAI], [DIRRULES §19 Glossary on Trust membrane] | Finite outcomes (ANSWER / ABSTAIN / DENY / ERROR) are typed and rendered visibly. |
...
This document's filename and path were chosen against the canonical placement rules in [DIRRULES] §4 and §6.
...
| Adjacent files (PROPOSED) | `docs/domains/fauna/README.md`, `docs/domains/fauna/SENSITIVITY_POSTURE.md`, `docs/domains/fauna/SOURCE_ROLES.md` | All PROPOSED; verify against repo before authoring. |
...
| Fauna Cesium / 3D surfaces | DEFER | 3D handoff is conditional in cross-cutting doctrine. [UI-EXP §12] | After 2D fauna Evidence Drawer + Focus Mode + policy gates pass. |
| Disease cluster context as life-safety output | DEFER (permanent unless override doctrine) | KFM is not a life-safety authority. [ENCY §13], [P20-IDX KFM-IDX-POL-006] | Not enabled as a life-safety output by KFM under any normal trigger; only context labelling. |
...
|---|---|---|
| **Object-family count** — ATLAS §7.B lists ~14 object families; UNIFIED §6.4 names a subset (9). | The ATLAS list is the longer surface; UNIFIED is a reading of dossier scope. Neither names which is canonical. | Reconcile in `contracts/domains/fauna/` and `control_plane/object_family_register.yaml` (both PROPOSED); resolve by ADR if either source must be amended. |
| **NestDenRoostSpawningSite vs. SensitiveSite** — ENCY uses `NestDenRoostSpawningSite` as a distinct object family; ATLAS uses `SensitiveSite` as a broader category that includes them. | Hierarchy is doctrinally implied but not formalized. | Object-family map should establish whether `NestDenRoostSpawningSite` is a subtype of `SensitiveSite` or a sibling. PROPOSED resolution: subtype. |
| **Filename `_INVENTORY` vs. `_NOTES`** — Whole-UI report (§29 Appendix A) uses `CONTINUITY_NOTES.md` under `docs/architecture/<subsystem>/`. Fauna domain task uses `CONTINUITY_INVENTORY.md` under `docs/domains/<domain>/`. | Different roots (architecture vs. domain) and different content shape (prose continuity notes vs. tabular inventory). | KEEP AS-IS for now. If repo evidence shows other domain lanes are using `CONTINUITY_NOTES.md`, raise a drift entry and propose a single convention by ADR. |
| **Fauna runbook home** — User memory indicates `docs/runbooks/fauna/SOURCE_REFRESH_RUNBOOK.md` was authored; project-wide convention elsewhere uses flat `docs/runbooks/<subsystem>_<topic>.md`. | Memory-derived reference, not repo-verified. [DIRRULES §6.1] lists `docs/runbooks/` with no fixed subfolder rule. | NEEDS VERIFICATION against the mounted repo; if a subfolder convention is emerging, document it in `docs/runbooks/README.md` (PROPOSED) and link from §22 here. |
| **Schema home for fauna** — [DIRRULES §6.4] (ADR-0001) sets `schemas/contracts/v1/domains/fauna/` as default; some dossier text implies `contracts/<domain>/<x>.schema.json`. | [DIRRULES §6.4] flags this as CONFLICTED until repo-verified. | Default to ADR-0001 (`schemas/contracts/v1/domains/fauna/`); raise drift entry if the live repo diverges. |
| **`triplets/` vs. `triplet/`** — [DIRRULES §18] notes plural form is used; doctrine pipeline says CATALOG / **TRIPLET** (singular). | Naming variance is unresolved repo-wide. | Inherit the cross-cutting ADR resolution; not fauna-specific. |
| **Owner names and team handles** | Owners are placeholders pending steward assignment. | Replace `<fauna-domain-steward>` with the real handle in [KFM Meta Block v2] before this doc leaves `draft`. |
...
| Geoprivacy transform catalogue and Redaction Receipt schema | Contract page under `contracts/domains/fauna/`; schema at `schemas/contracts/v1/domains/fauna/`; fixture; tests. [P20-IDX KFM-IDX-POL-005] | NEEDS VERIFICATION |
| Fauna routes through the governed API | Route definition, typed client, validator, mock fixture adapter. [UI-EXP §17] | UNKNOWN |
| Existence of `docs/domains/fauna/README.md`, `SENSITIVITY_POSTURE.md`, `SOURCE_ROLES.md` | Repo `ls`; per-root README contract per [DIRRULES §15] | UNKNOWN |
...
| Promotion-gate policy bundle for fauna | `policy/promotion/`, `policy/domains/fauna/` bundle; tests; negative fixtures. [DIRRULES §6.5] | UNKNOWN |
...
| Sensitivity / geoprivacy posture change | `policy/sensitivity/` (PROPOSED) | `contracts/domains/fauna/redaction_receipt.md` (PROPOSED) | negative + denial fixtures | — | **this file (§10)** | rollback drill | Track receipt schema impact |
| New geoprivacy transform type | `policy/sensitivity/` (PROPOSED) | `contracts/domains/fauna/` | transform fixtures | refresh runbook | **this file (§10)** | rollback path per transform | Verify residual-risk policy |
...
| Cross-lane relation change (habitat/flora/hydrology/hazards) | both lane READMEs | shared `OBJECT_MAP.md` | join fixtures | — | **this file (§13)** | release rollback if published | Verify ownership preserved |
| AI behavior / Focus Mode for fauna | `docs/architecture/governed-ai/README.md` | `contracts/runtime/` | focus fixtures | governed-AI runbooks | **this file (§15)** | governed-AI rollback | Verify citation validation |
...
> [!NOTE]
> All `docs/` paths below other than `directory-rules.md` are **PROPOSED / NEEDS VERIFICATION**. They reflect the placement rules in [DIRRULES] §6.1; their **presence** in the live repo has not been checked in this session.
