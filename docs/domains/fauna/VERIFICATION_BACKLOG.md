...
![Version: v1 (draft)](https://img.shields.io/badge/version-v1%20draft-1976D2)

**Status:** `draft` · **Owners:** `TODO (fauna-domain-steward, sensitivity-reviewer, ecology-lead)` · **Last updated:** `2026-05-16`
...
12. [Appendix A — Item template](#appendix-a--item-template)
13. [Appendix B — Doctrinal anchors quick map](#appendix-b--doctrinal-anchors-quick-map)
...
| **Central register (upstream)** | `docs/registers/VERIFICATION_BACKLOG.md` per **Directory Rules §6.1 and §18**. PROPOSED target. |
...
|---|---|---|
| **Lifecycle invariant** `RAW → WORK / QUARANTINE → PROCESSED → CATALOG / TRIPLET → PUBLISHED`; promotion is a governed state transition, not a file move. | `[DIRRULES] §0, §3`; `[ENCY]`; `[DOM-FAUNA]` | A "verified" item must show evidence at the right lifecycle stage; PUBLISHED claims need PUBLISHED-stage evidence. |
| **Trust membrane:** public clients read only via `apps/governed-api/`; connectors and watchers are not publishers. | `[DIRRULES]`; `[GAI]`; `[ENCY]` | Verification must include "where the public reads it from"; bypassing `apps/governed-api/` is itself a finding. |
...
| **Deny-by-default sensitivity:** exact sensitive occurrence, nest, den, roost, hibernacula, spawning, steward-controlled records fail closed. | `[DOM-FAUNA]`; `[ENCY] §13`; `[ENCY] §20.5` | Items touching sensitive geometry are flagged `Critical` and may not close without a Redaction Receipt + review record. |
| **Source-role discipline:** a source is not authoritative for claims outside its source role. | `[DOM-FAUNA]`; `[ENCY]` | Source-rights and source-role items cannot be closed by partial evidence (e.g., "rights are clear" without "and roles are recorded"). |
...
| **Watcher non-publisher invariant:** watchers observe and record; they do not promote. | `[DIRRULES]`; `[ENCY]`; `[KFM-IDX-SRC-003]` | Items about watchers MUST verify the non-publisher boundary, not just watcher liveness. |
...
> [!NOTE]
> This flowchart is `PROPOSED` shape — it reflects Directory Rules §2.5 (conflict resolution), §6.1 (docs/registers/), and §6.2 (control_plane/). The exact promotion pathway from a domain-scoped register to the central register **NEEDS VERIFICATION** against any existing ADR or workflow.
...
| `VB-FAUNA-SRC-002` | Verify **source role per family** (authority / observation / context / model) is explicit and validator-enforced, not inferred from name. | A community-science feed is not a legal-status authority; an aggregator is not an observation. | Source-role denial tests rejecting a claim whose cited source lacks the role. | **High** | `NEEDS VERIFICATION` | `[DOM-FAUNA] §D`; `[KFM-IDX-SRC-002]` |
...
| `VB-FAUNA-SRC-005` | Verify **watcher implementations** for Fauna sources observe and record only — they MUST NOT promote artifacts to `PUBLISHED` or main. | Silent watcher commit-to-main is a known severe risk. | Code path inspection + tests asserting watchers raise `PromotionDecision` candidates only. | **Critical** | `NEEDS VERIFICATION` | `[DIRRULES]`; `[BLD-GREEN] §26`; `[KFM-IDX-SRC-003]` |
| `VB-FAUNA-SRC-006` | Verify **per-source freshness/cadence** is recorded and surfaced to the Evidence Drawer. | Stale-state misleads researchers; stale-state UI is a `CONFIRMED` cross-cutting product. | `update_cadence` field present per descriptor + drawer payload renders freshness state. | **Medium** | `NEEDS VERIFICATION` | `[DOM-FAUNA] §D`; `[MAP-MASTER]` |
...
| `VB-FAUNA-TAX-004` | Verify **Conservation Status** is sourced only from authority-role sources (USFWS, KDWP, NatureServe, IUCN) and never from aggregator/observation sources. | A legal-status claim from an observation aggregator is a doctrinal collapse. | Source-role denial test rejects a Conservation Status claim cited to GBIF/eBird/iNaturalist. | **Critical** | `NEEDS VERIFICATION` | `[DOM-FAUNA] §C, §D`; `[KFM-IDX-SRC-002]` |
...
> [!WARNING]
> Items in this section are the highest-consequence in the register. Any closure must include a **Redaction Receipt**, a **review record**, and a **policy decision** — not just a code change.
...
| `VB-FAUNA-SEN-002` | Verify **exact sensitive occurrence, nest, den, roost, hibernacula, spawning, steward-controlled records fail closed** at every public surface (tiles, API, drawer, Focus Mode, exports, graph). | This is the explicit deny-by-default register entry for fauna. | Policy deny tests covering each surface with negative-state UI rendering. | **Critical** | `NEEDS VERIFICATION` | `[DOM-FAUNA] §I`; `[ENCY] §13, §20.5` |
| `VB-FAUNA-SEN-003` | Verify **geoprivacy transform receipt schema** is defined (proposed name `RedactionReceipt` or `GeoprivacyTransformReceipt`) and required before any generalized public derivative of a sensitive taxon. | Without a receipt, a generalized public layer cannot be audited. | Schema file + valid/invalid fixtures + validator. ADR may be required to fix the canonical name (`ADR-S-03` per `[ENCY] Ch. 24.12` open-ADR backlog). | **Critical** | `NEEDS VERIFICATION` / `CONFLICTED` (naming) | `[ENCY] §24.5`; `[DOM-FAUNA] §I` |
| `VB-FAUNA-SEN-004` | Verify the **sensitivity tier scheme (T0–T4)** has a per-fauna-object-class mapping: `Fauna — sensitive occurrence` defaults to **T4** with a geoprivacy transform path to **T1**. | Without the mapping, "publish at tier N" is not reviewable. | Per-class tier matrix entry + transform pipeline + gates. | **High** | `NEEDS VERIFICATION` | `[ENCY] §24.5.2`; `[DIRRULES]` |
...
| ID | Item | Why it matters | Evidence that would settle it | Severity | Status | Anchor |
|---|---|---|---|---|---|---|
| `VB-FAUNA-CSP-001` | Verify the **canonical schema home** for fauna object families is `schemas/contracts/v1/domains/fauna/` per `ADR-0001` (schema home). | Parallel schema homes in `contracts/` and `schemas/` are a known drift pattern. | Inspection of `schemas/contracts/v1/domains/fauna/`; ADR-0001 status. | **High** | `NEEDS VERIFICATION` | `[DIRRULES] §7.4, §13.1`; `ADR-0001` |
...
| `VB-FAUNA-CSP-003` | Verify **deterministic identity rule** (`source id + object role + temporal scope + normalized digest`) is implemented for each fauna object family. | Non-deterministic IDs break catalog closure and rollback. | ID-generation utility + tests asserting same inputs → same ID, different inputs → different ID. | **High** | `NEEDS VERIFICATION` | `[DOM-FAUNA] §E`; `[ENCY] §6` |
| `VB-FAUNA-CSP-004` | Verify **policy lane** `policy/domains/fauna/` (or repo equivalent) contains policy rules for: sensitive-location denial, source-role mismatch denial, rights-unknown denial, stale-state handling, AI-no-leak. | Policy that lives only in code is invisible to reviewers. | Policy files + policy-engine-loaded receipts + reason codes. | **High** | `NEEDS VERIFICATION` | `[DOM-FAUNA] §K`; `[DIRRULES] §6.5`; `[ADR-0010]` |
...
| ID | Item | Why it matters | Evidence that would settle it | Severity | Status | Anchor |
|---|---|---|---|---|---|---|
| `VB-FAUNA-LIF-001` | Verify the fauna lane has the **canonical lifecycle data lanes**: `data/raw/fauna/`, `data/work/fauna/`, `data/quarantine/fauna/`, `data/processed/fauna/`, `data/catalog/domain/fauna/`, `data/published/layers/fauna/`. | Lifecycle without lanes flattens governance. | Directory presence + per-lane README. | **High** | `NEEDS VERIFICATION` | `[DIRRULES] §12`; `[DOM-FAUNA] §H` |
| `VB-FAUNA-LIF-002` | Verify **promotion** between lanes is a governed state transition with a receipt, not a file move. | Moving a file is a governance bypass; the invariant says so explicitly. | `PromotionDecision` records + closure tests rejecting `mv`-only transitions. | **Critical** | `NEEDS VERIFICATION` | `[DIRRULES] §0`; `[ENCY]` |
...
| `VB-FAUNA-VAL-003` | Verify **occurrence restricted/public split tests** exist and prevent cross-loading. | The split is the load-bearing sensitivity boundary. | Test + denied envelope on cross-load attempt. | **Critical** | `NEEDS VERIFICATION` | `[DOM-FAUNA] §K`; `[ENCY] §13` |
...
| `VB-FAUNA-VAL-006` | Verify **Runtime Response Envelope negative cases** are tested (`ABSTAIN`, `DENY`, `ERROR`). | Finite outcomes are doctrine; only `ANSWER` paths is over-permissive. | Test cases for each outcome with reason codes. | **High** | `NEEDS VERIFICATION` | `[DOM-FAUNA] §K`; `[GAI]` |
...
| ID | Item | Why it matters | Evidence that would settle it | Severity | Status | Anchor |
|---|---|---|---|---|---|---|
| `VB-FAUNA-PUB-001` | Verify **Fauna feature/detail resolver** route exists in `apps/governed-api/` and returns `FaunaDecisionEnvelope` with finite outcomes (`ANSWER` / `ABSTAIN` / `DENY` / `ERROR`). | Public clients must read through the governed API. | Route + OpenAPI/GraphQL schema + envelope fixtures. | **High** | `UNKNOWN` (exact route) | `[DOM-FAUNA] §J` |
...
| `VB-FAUNA-PUB-006` | Verify **release/rollback dossier** lives at `release/candidates/fauna/<release_id>/` with the documented file set. | The release lane must be machine-readable for closure. | Directory layout + per-candidate dossier. | **Medium** | `NEEDS VERIFICATION` | `[DIRRULES] §6`; `[ENCY] App. E` |
...
| ID | Item | Why it matters | Evidence that would settle it | Severity | Status | Anchor |
|---|---|---|---|---|---|---|
| `VB-FAUNA-AI-001` | Verify **Fauna Focus Mode** answers ride a `RuntimeResponseEnvelope` + `AIReceipt` and never bypass released EvidenceBundles. | AI must be subordinate to evidence; doctrine forbids RAW/WORK access. | Adapter wiring + tests rejecting unreleased context. | **Critical** | `NEEDS VERIFICATION` | `[DOM-FAUNA] §L`; `[GAI]` |
...
| `VB-FAUNA-AI-004` | Verify **`AIReceipt`** persists summaries and hashes only (no chain-of-thought, no secrets). | Cross-cutting risk from `[BLD-GREEN] §26`. | Receipt schema + retention test. | **High** | `NEEDS VERIFICATION` | `[BLD-GREEN] §26` |
...
| `VB-FAUNA-UI-003` | Verify the **sensitivity-redacted view** is the default for sensitive fauna taxa, and the **steward exact-location view** is gated by role + audit. | Deny-by-default for sensitivity is doctrine. | Two view fixtures + access-control test. | **Critical** | `NEEDS VERIFICATION` | `[ENCY] §I (Restricted/Steward Surfaces)` |
...
5. **Surface coverage.** Items touching the trust membrane (sensitivity, API, UI, AI) require coverage at **every** public surface listed in `[ENCY] §13`: ingest, transform, publish, serve, focus, AI, exports, tiles, graph.
...
> [!NOTE]
> The exact promotion workflow (whether by PR, by validator, by scheduled job, or by hand) `NEEDS VERIFICATION` against any existing ADR. Until that ADR is in hand, promote by PR and link the local row in the PR description.
...
1. **`RedactionReceipt` vs `GeoprivacyTransformReceipt` canonical name.** The Atlas uses `Redaction Receipt`; some lineage uses `GeoprivacyTransformReceipt`. Likely belongs to an open `ADR-S` slot (per `[ENCY] Ch. 24.12 Open-ADR Backlog`).
2. **Domain-scoped backlog promotion.** Should domain-scoped items always be mirrored to the central register, or only when severity ≥ High? Pending ADR.
...
5. **Steward-role naming.** The doctrine names "fauna-domain steward," "sensitivity reviewer," and "ecology lead" but does not freeze the org-chart names. Pending governance ADR.
6. **Subfolder convention for runbooks.** The Fauna lane currently uses `docs/runbooks/fauna/...` (see related docs). Directory Rules describes a flat `docs/runbooks/` lane; reconcile by per-root README or ADR.
7. **First closure target.** Which item should be the first closed — most likely a candidate from §6.1 (`VB-FAUNA-SRC-003`, the no-network synthetic fixture) — to seed the trust spine?
...
| `docs/runbooks/fauna/SOURCE_REFRESH_RUNBOOK.md` | Operational counterpart to §6.1 / §6.5. |
...
| `[ENCY]` | KFM Domain and Capability Encyclopedia | Object/source/capability spine, sensitive register, App. E (release/correction/rollback), App. J/K (open questions and verification backlog). |
...
| `[DIRRULES]` | Directory Rules | Placement authority, lifecycle invariant, anti-patterns. |
...
| `[KFM-IDX-*]` | Pass 20 Idea Index (Part II) | Cross-cutting idea references. |
| `[ADR-0001]` | ADR-0001 (schema home) | Canonical schema home. |
| `[ADR-S-*]` | Open-ADR backlog | 15 pending ADR slots in `[ENCY] Ch. 24.12`. |
...
**Related docs:** `docs/domains/fauna/README.md` · `docs/domains/fauna/CANONICAL_PATHS.md` · `docs/domains/fauna/OPEN_QUESTIONS.md` · `docs/runbooks/fauna/SOURCE_REFRESH_RUNBOOK.md` · `docs/registers/VERIFICATION_BACKLOG.md` · `control_plane/verification_backlog.yaml` · `docs/doctrine/directory-rules.md`

**Last updated:** 2026-05-16 · **Version:** v1 (draft) · [⬆ Back to top](#fauna-domain--verification-backlog)
