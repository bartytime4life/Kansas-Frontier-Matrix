<!-- [KFM_META_BLOCK_V2]
doc_id: kfm://doc/domains/fauna/object-families
...
notes:
  ...
  - "Family names, purposes, identity rule, and temporal rule are CONFIRMED from Atlas v1.1 §7.B/§7.C/§7.E; field-level realization is PROPOSED."
  - "Atlas internal inconsistency: §7.B ownership enumerates 14 families and omits MonitoringEvent; §7.C ubiquitous language lists MonitoringEvent as a CONFIRMED term; the cross-domain core-families index lists only 12 (drops Invasive Species Record and Redaction Receipt). Reconciled here as CONFLICTED — see §3 and §6.13."
  - "FILENAME DUPLICATION: OBJECT_FAMILIES.md (this file, the name the planned-files register uses) and OBJECTS.md cover the same roster. Pick one canonical home via DRIFT_REGISTER; the other becomes a stub redirect. See §10 and OQ-FAUNA-OF-08."
  - "All repo paths PROPOSED until verified against a mounted repo."
[/KFM_META_BLOCK_V2] -->

# 🦌 Fauna — Object Families

> **The canonical roster of object families the Fauna domain owns: what each means, what makes two of them "the same thing," how the six time facets stay distinct, and which families fail closed by default.**
...
[![Families: CONFIRMED](https://img.shields.io/badge/families-CONFIRMED-2e7d32)](#3-the-family-roster-at-a-glance)
[![Field shape: PROPOSED](https://img.shields.io/badge/field__shape-PROPOSED-orange)](#2-doctrinal-anchors)
...
> [!IMPORTANT]
> **Filename duplication, flagged up front.** This file and `docs/domains/fauna/OBJECTS.md` describe the **same** Fauna roster. `OBJECT_FAMILIES.md` is the name the planned-files register (`MISSING_OR_PLANNED_FILES.md` §6.1) points at. Two live copies of the same roster is a documentation-as-truth anti-pattern. **Resolve to one canonical file and make the other a one-line redirect stub**, recorded in `docs/registers/DRIFT_REGISTER.md`. See §10 and OQ-FAUNA-OF-08.
...
This document is the **object-family reference** for the Fauna domain (`DOM-FAUNA`). It names every object family the domain owns, gives each a one-line purpose, records the identity rule and temporal rule that apply uniformly across the roster, and flags the families that carry deny-by-default sensitivity.

It is the *roster* companion to its siblings: `IDENTITY_MODEL.md` explains *how* identity is computed (the four-part basis and the `spec_hash`), `MAP_UI_CONTRACTS.md` explains *how* these families render and resolve at the Map UI seam, and `MISSING_OR_PLANNED_FILES.md` tracks *where* their contracts, schemas, and tests are expected to land. This file answers the question those assume: *which families exist, and what does each one mean?*
...
> [!NOTE]
> **Object-family membership is trust-bearing, not a label of convenience.** The family a record belongs to determines its identity digest scope, its sensitivity default, its routable surface, and its release gate. A `SensitiveSite` is not an `Occurrence Public` with a flag flipped; an `Occurrence Restricted` is not an `Occurrence Evidence` with a different audience.
...
Each anchor is **CONFIRMED doctrine**; its concrete realization in the repository is **PROPOSED** until verified against a mounted repo.
...
> [!CAUTION]
> **Atlas internal inconsistency — `MonitoringEvent` and two index drops.** The roster above follows the Atlas **§7.B ownership enumeration**. Two reconciliations are needed and are flagged **CONFLICTED** pending an ADR:
> - **`MonitoringEvent`** is a **CONFIRMED ubiquitous-language term** in Atlas §7.C ("MonitoringEvent is used inside this domain…") and the domain one-liner (§7.A) names *monitoring* explicitly — **but it is absent from the §7.B ownership enumeration**. It is listed in §6 as a CONFLICTED 15th candidate, not in the confirmed roster above.
...
>
> See OQ-FAUNA-OF-01 and OQ-FAUNA-OF-02.
...
The diagram groups the roster by the role each family plays in the evidence-and-release flow. The grouping is a **presentational aid**, not an Atlas structure; arrows show responsibility, not runtime call order.
...
flowchart TB
    ...
    subgraph EVENTS["Event observations"]
        ...
        ME["MonitoringEvent<br/><sub>(CONFLICTED — §6.13)</sub>"]
    end
...
CONFIRMED across Atlas §7.I, §20.5 Deny-by-Default Register, and §24.5.2 tier matrix (the T0–T4 tier scheme itself is PROPOSED under ADR-S-05). The Fauna deny-by-default rule is the strongest constraint on this roster.
...
- **Notes** — **T4 deny-by-default.** Never routable on a public surface. Whether it is a physically separate object or a `(spec_hash, access_class)`-keyed view of Evidence is an open schema question (OQ-FAUNA-OF-04).
...
|---|---|---|---|
| OQ-FAUNA-OF-01 | Is **`MonitoringEvent`** a Fauna-owned family? It is a CONFIRMED §7.C term but absent from the §7.B ownership list. | Fauna steward + Schema steward | Ownership ADR; reconcile §7.B / §7.C / cross-domain index; update `object_family_register.yaml`. |
| OQ-FAUNA-OF-02 | Reconcile the cross-domain core-families index (12 families; drops Invasive Species Record + Redaction Receipt) against §7.B (14). | Docs steward | Errata / atlas-supplement entry; §7.B governs. |
| OQ-FAUNA-OF-03 | Concrete `site_type` enum for `SensitiveSite` (nest / den / roost / hibernaculum / spawning / …). | Schema steward | Schema enum under `schemas/contracts/v1/domains/fauna/`. |
| OQ-FAUNA-OF-04 | Is `Occurrence Restricted` a physically separate object or a `(spec_hash, access_class)`-keyed view of `Occurrence Evidence`? | Schema steward | Schema + restricted/public split tests (`DOM-FAUNA §K`). |
| OQ-FAUNA-OF-05 | Required vs optional anchors for `Conservation Status` inputs (USFWS / NatureServe / IUCN / KDWP). | Fauna steward | `data/registry/sources/fauna/` entries + rights review. |
| OQ-FAUNA-OF-06 | Sensitivity tier scheme (T0–T4) ratification. | Steward + release authority | ADR-S-05. |
| OQ-FAUNA-OF-07 | Whether `AbundanceIndicator` / `RichnessIndicator` (Encyclopedia §7.5.C viewing products) become owned families or stay derivative-only. | Schema steward | Object-family ADR. |
| OQ-FAUNA-OF-08 | **Filename duplication:** `OBJECT_FAMILIES.md` vs `OBJECTS.md` — which is canonical? | Docs steward | DRIFT_REGISTER entry; one becomes a redirect stub. |
...
1. `MonitoringEvent` ownership resolved against a mounted repo's `contracts/domains/fauna/` + `object_family_register.yaml` (OQ-FAUNA-OF-01).
2. Filename duplication resolved: one of `OBJECT_FAMILIES.md` / `OBJECTS.md` is canonical, the other a redirect stub (OQ-FAUNA-OF-08).
3. Per-family contract `.md` and schema `.json` presence under the PROPOSED homes (`contracts/domains/fauna/`, `schemas/contracts/v1/domains/fauna/`).
4. `site_type` enum for `SensitiveSite` confirmed in schema (OQ-FAUNA-OF-03).
5. Restricted-vs-Evidence object/view decision confirmed (OQ-FAUNA-OF-04).
6. Sensitivity tier values per family confirmed against `policy/sensitivity/fauna/` once ADR-S-05 lands.
7. Cross-domain index errata filed for the 12-vs-14 family discrepancy (OQ-FAUNA-OF-02).
...
|---|---|---|
| New document created at `docs/domains/fauna/OBJECT_FAMILIES.md`. | new | Per-family roster reference; the name the planned-files register (§6.1) points at. |
| Flagged the `OBJECT_FAMILIES.md` ↔ `OBJECTS.md` filename duplication at the top of the file and as OQ-FAUNA-OF-08. | clarification | Two live copies of the same roster is a documentation-as-truth anti-pattern; one must become a redirect stub. |
...
> **Backward compatibility.** First version; no prior anchors. **Resolve the duplication before merge:** if `OBJECTS.md` is chosen canonical, this file becomes a one-line redirect; if this file is chosen canonical, the planned-files register row already points here and `OBJECTS.md` becomes the stub. Record the decision in `docs/registers/DRIFT_REGISTER.md`.
...
- the `OBJECT_FAMILIES.md` / `OBJECTS.md` duplication is resolved to a single canonical file (OQ-FAUNA-OF-08), the other reduced to a redirect stub, both recorded in `docs/registers/DRIFT_REGISTER.md`;
- it is placed at `docs/domains/fauna/OBJECT_FAMILIES.md` per Directory Rules §6.1 (CONFIRMED subpath);
...
- the `MonitoringEvent` ownership conflict (OQ-FAUNA-OF-01) is logged in `docs/registers/DRIFT_REGISTER.md`;
...
- [`docs/domains/fauna/OBJECTS.md`](./OBJECTS.md) — **duplicate roster; resolve to one canonical file** (OQ-FAUNA-OF-08).
