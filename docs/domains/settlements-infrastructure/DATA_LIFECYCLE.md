...
notes:
  ...
  - Domain segment form (`settlements-infrastructure` vs `settlement`) is NEEDS VERIFICATION — see §11 Open Questions.
[/KFM_META_BLOCK_V2] -->
...
1. **CONFIRMED doctrine.** `RAW → WORK / QUARANTINE → PROCESSED → CATALOG / TRIPLET → PUBLISHED`, promotion as a **governed state transition** (not a file move), failure-closed gates, the cite-or-abstain truth posture, and the deny-default posture for unresolved rights, sensitivity, or release state. Sources: Directory Rules §9.1; Domains Atlas v1.1 Ch. 14 §H; Encyclopedia §24.6.1. [`[DIRRULES]`](../../doctrine/directory-rules.md) [`[DOM-SETTLE]`](../../atlases/domains-v1.1-ch14.md) [`[ENCY]`](../../encyclopedia/README.md)
2. **CONFIRMED doctrine / PROPOSED implementation.** Per-domain lane application of the invariant to Settlements / Infrastructure, including object families, source families, sensitivity defaults, and cross-lane relations. Sources: Domains Atlas v1.1 Ch. 14 §A–§N.
3. **PROPOSED only.** Concrete repo paths, schema homes, validator IDs, route names, policy bundle locations, and CI workflow filenames. All path-bearing statements below carry an explicit label.
...
> [!NOTE]
> Scope statements below are **CONFIRMED doctrine / PROPOSED field realization** from Domains Atlas v1.1 §14.B. The lane name in this document is hyphenated (`settlements-infrastructure`) to match the `docs/domains/` listing in Directory Rules §6.1; the domain segment under other responsibility roots (`schemas/`, `policy/`, `data/`, `release/`) is **NEEDS VERIFICATION** — see [§11 OPEN-SI-01](#11-open-questions-and-verification-backlog).

### 2.1 Owned objects (CONFIRMED / PROPOSED)

`Settlement`, `Municipality`, `CensusPlace`, `Townsite`, `GhostTown`, `Fort`, `Mission`, `ReservationCommunity`, `InfrastructureAsset`, `NetworkNode`, `NetworkSegment`, `Facility`, `ServiceArea`, `Operator`, `ConditionObservation`, `Dependency`.
...
| Hazard events, warnings, declarations | Hazards | Exposure and resilience overlays cite Hazards; never republish a hazard claim from this lane. |
| Ownership, parcels, living-person privacy | People / Land | Residence / migration context relations carry restrictions that may **block** settlements promotion. |
...
**CONFIRMED doctrine.** Promotion is a **governed state transition**, not a file move. A bytes-only move from one directory to another that bypasses validators, policy gates, EvidenceBundle creation, catalog closure, and the release decision is a **violation of the invariant** regardless of where the bytes ended up. (Directory Rules §9.1.)

```mermaid
flowchart LR
    A[Source edge<br/>discovery] -->|Admission gate<br/>SourceDescriptor| RAW[(RAW<br/>immutable capture)]
    RAW -->|Normalization gate<br/>TransformReceipt + ValidationReport + PolicyDecision| WORK[(WORK)]
    RAW -.->|failure-closed| Q[(QUARANTINE<br/>reason recorded)]
    WORK -->|Validation gate<br/>ValidationReport pass + RedactionReceipt if applies| PROC[(PROCESSED)]
    WORK -.->|stays in WORK<br/>structured FAIL| WORK
    PROC -->|Catalog closure gate<br/>EvidenceBundle + CatalogMatrix + triplet projection| CAT[(CATALOG / TRIPLET)]
    PROC -.->|HOLD<br/>no public edge| PROC
    CAT -->|Release gate<br/>ReleaseManifest + ReviewRecord + rollback target + correction path| PUB[(PUBLISHED<br/>public-safe artifacts)]
    CAT -.->|HOLD<br/>no public surface change| CAT
    PUB -->|Correction<br/>CorrectionNotice + invalidates[]| PUB
    PUB -.->|Rollback<br/>RollbackCard| CAT
...
**Reading note.** Dashed edges are failure-closed outcomes — the artifact does **not** advance. The Atlas v1.1 §14.H table marks every gate as PROPOSED for this lane's implementation maturity; the diagram above renders the gates as doctrine, not as evidence of working code.
...
Per Directory Rules §9.1, lifecycle phases live under `data/` with the **phase named explicitly**. Receipts, proofs, registry, and rollback sit **alongside** the phase directories — they do not replace them.

> [!NOTE]
> All `data/...` paths below are **PROPOSED** and follow Directory Rules §9.1's pattern `data/<phase>/<domain>/`. The literal domain segment (`settlements-infrastructure` vs `settlement` vs another form) is **NEEDS VERIFICATION** — see [§11 OPEN-SI-01](#11-open-questions-and-verification-backlog).
...
> [!IMPORTANT]
> **Two homes, one boundary.** `data/published/` holds released **artifacts** (the public-safe outputs). `release/` holds release **decisions** (manifests, proofs, signatures, rollback/correction). A `ReleaseManifest` does **not** live in `data/published/`; a published PMTiles file does **not** live in `release/`. Directory Rules §9.2.
...
**CONFIRMED doctrine** (Encyclopedia §24.6.1, Directory Rules §9.1). The artifacts named below are the **minimum** required to clear each gate for this lane. Domain-specific additions (e.g., critical-infrastructure review) extend, never relax, these minimums.

| Gate (transition) | Pre-condition | Required artifacts (minimum) | Failure-closed outcome |
|---|---|---|---|
| **Admission** (— → `RAW`) | Source identity and rights minimally established; source-role intent is set (e.g., Census TIGER = `authority`; KDOT bridge inventory = `observation`; historical gazetteer = `context`). | `SourceDescriptor` (role, authority, rights, sensitivity, cadence); payload or reference **hash**. | Source not admitted; logged as candidate awaiting steward. |
...
| **Correction** (`PUBLISHED` → `PUBLISHED'`) | Detected error or new evidence; downstream derivatives identified. | `CorrectionNotice`; `invalidates[]`; review record. | Public surface displays a stale/corrected badge; derivative invalidation cascades. |

> [!WARNING]
> **Critical-infrastructure materiality.** When the release surface touches critical infrastructure, utilities, bridge/condition/inspection details, operator-sensitive information, or exact vulnerable-facility geometry, the **release authority MUST be distinct from the original author** and `ReviewRecord` is **required**, not optional. (Atlas v1.1 §14.I; Unified Manual §6.9 Publication gates.)
...
**CONFIRMED doctrine** (Encyclopedia §24.2.2). A `•` means the receipt is normally **emitted, amended, or referenced** at that phase. Receipts created earlier remain **referenced via `EvidenceRef`** at later phases — they are not duplicated.
...
**CONFIRMED / PROPOSED** (Atlas v1.1 §14.I, Unified Manual §6.9). Within this lane, the following **default to restricted or review** and require an explicit `PolicyDecision = allow` plus `ReviewRecord` before any movement toward `PUBLISHED`:

- Critical infrastructure assets (water, wastewater, electric, gas, communications)
- Utilities operational details
- Condition observations (bridge inspections, asset condition reports)
- Dependency relations between infrastructure assets
- Operator-sensitive details (operator identities, contact, schedules)
- Exact facility geometry for vulnerable or restricted facilities
...
When a sensitive observation is needed in a public surface, the path is **transform-then-release**, not release-then-redact. Transforms include:
...
Settlements / Infrastructure features almost always **cite** evidence owned by another lane. The lifecycle implication: an artifact in this lane **cannot promote ahead of its cited claims**. Stale or quarantined cross-lane evidence holds the citing artifact back.
...
| Residence / ownership / parcel relations to People / Land | Living-person and parcel restrictions can **force** a settlements artifact into the transform-then-release path or into denial. |
...
Quarantine is **never** silent. Each held artifact carries a structured `reason` recorded with its `TransformReceipt`, `ValidationReport`, and `PolicyDecision`. The following are PROPOSED lane-specific reasons; they extend the universal quarantine grammar, they do not replace it.
...
> [!NOTE]
> Quarantined artifacts are **retained**, not deleted. Rejected joins, like rejected habitat joins elsewhere in KFM, are stored with **method, confidence, reason, inputs, run receipt, and review state** — never silently dropped. (Pass 25 doctrine, idea card `KFM-P25-PROG-0020`.)
...
**CONFIRMED doctrine / PROPOSED implementation** (Atlas v1.1 §14.M, Encyclopedia Appendix E).

### 10.1 Publication

A settlements / infrastructure release reaching `PUBLISHED` requires **all** of:

1. `ReleaseManifest` — names every included dataset by stable ID, every bundle by `spec_hash`, every PMTiles archive by `spec_hash`, and every layer manifest by `spec_hash`.
2. Resolvable `EvidenceBundle`(s) — every public claim binds to evidence; cite-or-abstain holds.
3. `PolicyDecision = allow` — including any required sensitivity gates.
4. `ReviewRecord` where materiality applies (critical infrastructure, etc.).
5. **Rollback target** — a prior `release_id` consumers can revert to.
6. **Correction path** — a documented mechanism for issuing `CorrectionNotice`.

> [!TIP]
> **Content-addressed binding.** Consumers (the web client, the catalog harvester, downstream pipelines) bind to the `ReleaseManifest`, not to floating "latest" pointers. A consumer recording the `ReleaseManifest`'s `spec_hash` is recording exactly which evidence it observed. (See `[ReleaseManifest](../../../contracts/release/release_manifest.md)` — **PROPOSED path; NEEDS VERIFICATION**.)

### 10.2 Correction

A correction does **not** edit the prior release. It emits a new `CorrectionNotice` referencing the prior `release_id`, names the changed claim, lists `invalidates[]` (downstream derivatives), and ties to a `ReviewRecord`. The corrected artifact becomes `PUBLISHED'`; the prior `PUBLISHED` is retained with a stale/corrected badge.

### 10.3 Rollback

Rollback re-points the public surface to a prior `release_id` via a `RollbackCard`. Rollback **does not silently delete history**: the rolled-from release remains in `release/manifests/` with its lineage intact. (Atlas v1.1 §20; Encyclopedia §24.)
...
Reading note (CONFIRMED doctrine, Encyclopedia §24.2.x): *upgrade* toward more public always requires both a transform receipt and a review record; *downgrade* toward less public never requires both — `CorrectionNotice` alone is sufficient to remove or restrict.
...
|---|---|---|---|
| `OPEN-SI-01` | Domain segment form under `schemas/`, `policy/`, `data/`, `release/` — `settlements-infrastructure` (matches `docs/domains/`) vs `settlement` (Atlas v1.1 §14 crosswalk) vs another form. | ADR; mounted-repo inspection. | NEEDS VERIFICATION |
| `OPEN-SI-02` | Source rights and **municipal legal-source roles** (Census TIGER, GNIS, state/local GIS, municipal records, KDOT, FEMA, operators, historical gazetteers). | Mounted-repo `SourceDescriptor` entries; source registry. | NEEDS VERIFICATION |
...
- `../../doctrine/directory-rules.md` — placement protocol and per-phase rules (§9.1 data; §9.2 release)
...
- `./ARCHITECTURE.md` — lane architecture and bounded context *(TODO)*
...
<sub>Last updated: 2026-05-19 · status: draft · v1 · `kfm://doc/settlements-infrastructure/data-lifecycle` · [↑ back to top](#settlements--infrastructure--data-lifecycle)</sub>
