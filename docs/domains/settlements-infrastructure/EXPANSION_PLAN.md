...
notes:
  - "Path home (PROPOSED): docs/domains/settlements-infrastructure/EXPANSION_PLAN.md per Directory Rules §12."
...
![sensitivity lane: critical-asset deny](https://img.shields.io/badge/sensitivity-critical--asset%20deny-red)
![pipeline: RAW→PUBLISHED](https://img.shields.io/badge/pipeline-RAW%E2%86%92PUBLISHED-informational)
![last updated: 2026-05-19](https://img.shields.io/badge/updated-2026--05--19-lightgrey)

| Field | Value |
|---|---|
| **Status** | Draft (Phase-1 documentation control plane) |
| **Owners** | `<TBD>` domain steward + docs steward · **NEEDS VERIFICATION** |
| **Doctrinal authority** | KFM Domains Atlas v1.1, Ch. 14 — Settlements / Infrastructure; §24.13 Atlas ↔ Dossier ↔ Responsibility-Root Crosswalk; Directory Rules v1.1 §12 |
...
- Surface every **deny-by-default** posture (critical assets, dependencies, condition detail) before any public-safe layer ships.
...
| Object families | Atlas v1.1 Ch. 14 §E; Atlas Appendix C | **CONFIRMED spine / PROPOSED identity rules** |
...
| Pipeline shape | Atlas v1.1 Ch. 14 §H; Directory Rules §3 lifecycle invariant | **CONFIRMED doctrine / PROPOSED lane application** |
| Sensitivity, rights, publication posture | Atlas v1.1 Ch. 14 §I; Atlas §20.5 Deny-by-Default Register | **CONFIRMED doctrine / PROPOSED enforcement** |
...
| Responsibility-root crosswalk | Atlas v1.1 §24.13 row 14 | **PROPOSED root assignments** |
| Domain-as-lane placement | Directory Rules v1.1 §12 (domain pattern explicitly lists `settlements-infrastructure`) | **CONFIRMED rule / PROPOSED presence** |
...
**CONFIRMED rule (Directory Rules §12) / PROPOSED path presence.** Files for this domain are distributed across responsibility roots as a **lane segment**, never as a root-level domain folder. The mapping below follows §24.13 row 14 (Settlements / Infrastructure) and the §12 pattern.

| Responsibility root | Settlements / Infrastructure lane segment | Purpose |
|---|---|---|
| `docs/` | `docs/domains/settlements-infrastructure/` | Human-facing lane doctrine, runbooks, expansion plan (this file). |
| `contracts/` | `contracts/domains/settlements-infrastructure/` *(or `contracts/settlement/` per §24.13)* | Object **meaning** (semantic contracts). **NEEDS VERIFICATION — ADR required** to resolve segment name. |
| `schemas/` | `schemas/contracts/v1/domains/settlements-infrastructure/` *(or `schemas/contracts/v1/settlement/` per §24.13)* | Machine-checkable **shape**. **NEEDS VERIFICATION — ADR required**. |
| `policy/` | `policy/domains/settlements-infrastructure/` + `policy/sensitivity/infrastructure/` | Allow / deny / restrict / abstain / review decisions; critical-asset deny lane. |
...
> [!WARNING]
> **Segment naming is unresolved.** §24.13 row 14 lists `schemas/contracts/v1/settlement/` and `contracts/settlement/` (singular, short), while §12 demonstrates the pattern with `<domain>` segments matching the doc folder name (`settlements-infrastructure`). This is a **drift candidate** requiring an ADR. See [§16](#16-open-adrs--verification-backlog) — `ADR-S-SETTLE-01: Lane segment naming`.
...
> [!NOTE]
> **PROPOSED tree.** No path below has been verified against a mounted repo. Use as a planning sketch, not a repo description.
...
contracts/domains/settlements-infrastructure/        # NEEDS VERIFICATION (vs. contracts/settlement/)
...
schemas/contracts/v1/domains/settlements-infrastructure/   # NEEDS VERIFICATION (vs. .../settlement/)
...
**CONFIRMED doctrine / PROPOSED lane application** — Atlas v1.1 Ch. 14 §H, §J; Directory Rules §3.
...
> [!IMPORTANT]
> **Promotion is a governed state transition, not a file move** (Directory Rules §3, lifecycle invariant). The lane never advances an object by copy/rename; it advances by `PromotionDecision` against the gates above.
...
flowchart TD
  ...
  subgraph Membrane["Governed APIs · Policy gates · Release manifests"]
    ...
    G4["Focus Mode answer<br/>(AI · runtime envelope)"]
  end
...
| Schema home | `schemas/contracts/v1/…` *(segment NEEDS VERIFICATION)* | finite validator outcomes | **PROPOSED** — pending ADR-0001 application |
...
> [!NOTE]
> Phase ordering follows the operational overlay in the KFM Unified Build Manual (Phase 0–7+, IMPL-PIPE §25). It is **not** a substitute for Greenfield phases 22.0–22.15 + Phase 16 (Signing & Attestation) and Phase 17 (Pre-RAW Event Family) when those are scheduled at repo scope. Lane phases below sequence work **within** the repo-wide schedule.
...
| **7** | Infrastructure Assets — public-safe generalization only | Public-safe asset view backed by generalization receipts; critical-asset attributes and condition detail remain **DENY by default**. | Critical-asset deny fixture passes; vector-tile attribute whitelist enforced; Redaction Receipts emitted. | Pull layer from publication if any leak fixture fails. |
| **8** | Service Area + Operator + Dependency aggregates | Aggregate views (service-area, dependency-summary) with stewardship review; **no exact infrastructure exposure** in public surfaces. | Steward review record + EvidenceBundle + aggregation receipts. | Tombstone aggregate; rollback to prior manifest. |
...
| **Infrastructure Asset** | 7 | State/local GIS · operators · KDOT · FEMA | **DENY exact / publish generalized only** | Critical-asset deny lane. |
| **Network Node** | 7–8 | State/local GIS · operators · KDOT | **DENY exact / aggregate only** | Topology shape may publish; identifiers may not. |
| **Network Segment** | 7–8 | State/local GIS · operators · KDOT | **DENY exact / aggregate only** | Distinct from Roads/Rail network segments — see §11. |
...
| **Service Area** | 8 | Operators · state/local GIS | Aggregate-only | Aggregation receipts required. |
| **Operator** | 8 | Operators · state/local GIS · municipal-legal | Identity may publish; sensitive ops detail **DENY** | Operator-sensitive joins fail closed. |
| **Condition Observation** | 7–8 | Operators · KDOT · FEMA | **DENY by default** (condition detail) | Steward review per release. |
| **Dependency** | 8 | Operators · state/local GIS | **DENY by default** (dependency graph) | Dependency-summary aggregate only. |
...
|---|---|---|---|---|---|
| Census TIGER / census-place geography | authority / context | 6 | **NEEDS VERIFICATION** | source-vintage / decennial | `[DOM-SETTLE] [ENCY]` |
| GNIS and gazetteers | authority / observation | 5–6 | **NEEDS VERIFICATION** | source-vintage | `[DOM-SETTLE] [ENCY]` |
| State / local GIS — Kansas Geoportal-style | authority / observation / context | 5–7 | **NEEDS VERIFICATION** | source-vintage / cadence-specific | `[DOM-SETTLE] [ENCY]` |
...
| Historical gazetteers and maps | context / observation | 5 | **NEEDS VERIFICATION** | source-vintage | `[DOM-SETTLE] [ENCY]` |
| Infrastructure operators and providers | authority / observation | 7–8 | **NEEDS VERIFICATION** — operator agreements likely required | cadence-specific | `[DOM-SETTLE] [ENCY]` · **deny-by-default for sensitive detail** |
| KDOT / bridge / facility sources | authority / observation | 7 | **NEEDS VERIFICATION** | source-vintage / cadence-specific | `[DOM-SETTLE] [ENCY]` |
...
**CONFIRMED doctrine** — Atlas v1.1 §20.5 row "Infrastructure" and Ch. 14 §I.
...
| Class | Default | Allowed only when | Receipt required |
|---|---|---|---|
| Exact critical-asset geometry | **DENY** | steward review + public-safe generalization | Redaction Receipt + EvidenceBundle |
| Critical-asset attributes (capacity, vulnerabilities, condition) | **DENY** | steward review + EvidenceBundle + Redaction Receipt | Redaction Receipt + EvidenceBundle |
| Network topology (exact) | **DENY** | aggregate-only derivative | Aggregation receipt |
| Dependency graphs | **DENY** | dependency-summary aggregate | Aggregation receipt |
| Operator-sensitive ops detail | **DENY** | consent + agreement + restricted authorized surface | PolicyDecision + EvidenceBundle |
| Reservation-community boundaries | **REVIEW** (sovereignty) | sovereignty / steward review | Review record + EvidenceBundle |
| Townsite / GhostTown historic geometry | **ALLOW** when evidence + period known | EvidenceBundle resolves | EvidenceBundle |
| Public-safe Municipality / CensusPlace boundaries | **ALLOW** when source rights resolved | EvidenceBundle + rights resolution | EvidenceBundle |
...
| Hazards | exposure · resilience · warnings · declarations | KFM is **never** an alert authority; Hazards owns events | `[DOM-HAZ]` |
| Hydrology | water · wastewater · stormwater · floodplain · drainage | Water evidence stays in Hydrology | `[DOM-HYD]` |
| People / Land | residence · ownership · parcel · migration context | Person↔parcel joins **deny by default**; living-person privacy lives in People/Land | `[DOM-PEOPLE]` |

> [!NOTE]
> **Adjacent-lane caution:** Forts and Missions overlap with Archaeology / Cultural Heritage (`[DOM-ARCH]`). When an object is also an archaeological site, defer site-coordinate sensitivity to Archaeology's deny defaults; this lane retains the settlement-role projection only.
...
**PROPOSED lane products** (Atlas v1.1 Ch. 14 §G) over the **CONFIRMED cross-cutting doctrine** (Evidence Drawer, time-aware state, trust badges, sensitivity-redacted view, correction/stale-state view, governed Focus Mode — `[MAP-MASTER]` / `[GAI]`).
...
| Public-safe asset view | 7 | MapLibre + generalization | **Deny exact** — generalization receipt required |
| Service-area aggregate view | 8 | MapLibre + aggregation receipt | Aggregate-only |
| Dependency-summary view | 8 | Evidence Drawer + aggregate | Aggregate-only; deny graph |
| Restricted internal review view | — | Authorized surface only | Never reachable on public path |
...
| Generalization-receipt test | Public-safe asset view | 7 | `DENY` if receipt absent or transform unrecorded |
...
|---|---|---|
| Summarize released Settlements / Infrastructure EvidenceBundles | EvidenceBundle resolves and policy allows | `ANSWER` + `AIReceipt` |
...
4. Review-state record where the §10 sensitivity matrix requires one (critical-asset, reservation-community, operator-sensitive)
5. **Correction path** declared in the manifest
6. **Stale-state rule** declared (when does the layer go stale?)
7. **RollbackCard** identifying the rollback target

```mermaid
flowchart LR
  Cand["release candidate"] -->|gates A-G| Decision{"PromotionDecision"}
  Decision -->|all green| Release["ReleaseManifest<br/>+ RollbackCard<br/>+ Correction path"]
  Decision -->|any red| Hold["HOLD<br/>(reason-coded)"]
  Release --> Public["Public-safe layer<br/>via governed API"]
  Public -->|stale rule fires| Stale["Stale state notice"]
  Public -->|correction needed| Correct["Correction notice<br/>+ new candidate"]
...
| ADR (PROPOSED) | Question | Affects |
|---|---|---|
| `ADR-S-SETTLE-01` — Lane segment naming | Resolve `domains/settlements-infrastructure/` (per §12 pattern) vs. `settlement/` (per §24.13 row 14) for `contracts/`, `schemas/contracts/v1/`, `policy/sensitivity/`. | `contracts/` · `schemas/` · `policy/` |
| `ADR-S-SETTLE-02` — Critical-asset deny taxonomy | Define the canonical list of critical-asset classes, denied fields, and allowed public-safe transforms. | `policy/sensitivity/infrastructure/` · `schemas/` |
| `ADR-S-SETTLE-03` — Legal municipality source role | Define what counts as a `legal-source-role` for a municipality; clarify rights handling for municipal records. | `contracts/` · `policy/` · source ledger |
| `ADR-S-SETTLE-04` — Reservation-community publication path | Define sovereignty / steward review flow before any ReservationCommunity layer publishes. | `policy/` · `release/` |
| `ADR-S-SETTLE-05` — Cross-lane Fort/Mission overlap | Decide how Fort/Mission objects in this lane defer to Archaeology when an object is also an archaeological site. | `contracts/` · `policy/` |
...
| Lane segment naming (resolve §12 vs §24.13) | Accepted `ADR-S-SETTLE-01` | **UNKNOWN** until ADR resolves |
...
- [`docs/doctrine/directory-rules.md`](../../doctrine/directory-rules.md) — placement authority (CONFIRMED rule / PROPOSED presence)
...
**Last updated:** 2026-05-19 · **Doctrine:** Atlas v1.1 Ch. 14, §20.5, §24.13; Directory Rules v1.1 §3, §12 · **Implementation:** PROPOSED until mounted-repo verification.
