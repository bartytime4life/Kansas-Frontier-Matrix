...
notes: [
  "PROPOSED placement per Directory Rules v1.1 §6.1 (docs/domains/settlements-infrastructure/) and §4 Step 3 (domain-as-segment).",
  ...
  "Critical-infrastructure sensitivity posture is inherited from [DOM-SETTLE §I] and [ENCY §11] and is non-negotiable across every backlog item below."
]
[/KFM_META_BLOCK_V2] -->
...
| Field | Value |
|---|---|
| **Authority of this doc as a backlog** | **CONFIRMED** — Directory Rules §6.1 places domain-scoped backlogs under `docs/domains/<domain>/`. |
| **Authority of specific items** | **PROPOSED** — see truth label per row. No implementation maturity is asserted. |
| **Domain identity citation** | `[DOM-SETTLE]` / `[ENCY §11]` — Settlements and Infrastructure. |
| **Path basis** | Directory Rules v1.1 §6.1 (`docs/domains/settlements-infrastructure/`) and §4 Step 3 (domain-as-segment). |
| **Critical-infrastructure posture** | `[DOM-SETTLE §I]`, `[ENCY §11]`, Atlas 24.13 — critical-asset deny lane. |
| **Updated** | 2026-05-19 |
...
> [!IMPORTANT]
> Every item in this backlog inherits the Settlements/Infrastructure sensitivity posture: **critical infrastructure, utilities, condition observations, dependencies, operator-sensitive details, and exact vulnerable facility geometry default to restricted or review**, and **legal place / census place / historic place / townsite / ghost-town identity must remain distinct**. No item below relaxes that posture. CONFIRMED doctrine (`[DOM-SETTLE §I]`, `[ENCY §11]`, Atlas Ch. 24.13, `[UNIFIED §6.9]`).
...
|---|---|---|
| Directory Rules v1.1 | Decides where this document and any item it spawns lives. | `directory-rules.md §§3–6, §12` |
...
| Atlas Ch. 24.13 (Atlas ↔ Dossier ↔ Responsibility-Root Crosswalk) | PROPOSED responsibility-root targets: `schemas/contracts/v1/settlement/`, `contracts/settlement/`, `policy/sensitivity/infrastructure/`. | Atlas v1.1 Ch. 24.13 |
| KFM Encyclopedia §11 (Sensitive / Deny-by-Default Posture) | Critical-infrastructure deny lane. | `KFM-encyclopedia .md §11` |
| Whole-System Build Reference §10.12 | Domain risk and public-posture summary. | `kfm_whole_system_build_reference.md §10.12` |
| Unified Implementation Architecture §6.9 | Settlements/Infrastructure scope, sources of authority, sensitivity, object families, pipelines, gates, open verification items. | `KFM_Unified_Implementation_Architecture_Build_Manual.pdf §6.9` |
...
|---|---|
| **CONFIRMED** | Grounded in attached KFM doctrine (Atlas Ch. 14, Atlas 24.13, Encyclopedia, Unified Build Manual, Directory Rules) visible to this authoring session. |
...
> [!NOTE]
> No mounted repo, CI, workflow, dashboard, or runtime log was inspected in this authoring session. Every path, schema name, policy file, test name, route, and registry entry referenced below is **PROPOSED** until repo inspection. This matches the truth posture of Directory Rules v1.1 §0 and Atlas v1.1 front matter.

### 2.3 In-scope (this lane owns these expansions)

CONFIRMED scope per `[DOM-SETTLE §B]`:

- **Settlements** — generic settled places not yet typed.
- **Municipalities** — legal city, town, village entities with charter / incorporation evidence.
- **CensusPlaces** — Census Bureau place geographies (TIGER), CDPs and incorporated places treated as observations.
- **Townsites** — platted/historical townsite footprints.
- **GhostTowns** — abandoned or depopulated places carrying former-settlement evidence.
- **Forts** — frontier-era military posts (historical and active).
- **Missions** — religious mission sites, historical and active.
- **ReservationCommunities** — communities on or adjacent to recognized tribal lands; sovereignty-sensitive.
- **Infrastructure Asset** — bridges, utility nodes, treatment plants, depots, generators, towers, terminals, depots; rendered public-safe per policy.
...
- **Condition Observation** — bridge inspections, asset condition reports; restricted by default.
- **Dependency** — A-depends-on-B asset/operator relations; restricted by default.
...
| **Hazards** | exposure, resilience, warnings, declarations | KFM is **never** an alert authority; relations are descriptive, not advisory. |
...
flowchart LR
    ...
    subgraph PIPE["RAW → PUBLISHED pipeline (CONFIRMED doctrine)"]
        RAW[(RAW)]
    ...
    subgraph BACK["THIS BACKLOG"]
        ...
        M["§6 Medium-priority"]
        L["§7 Low-priority"]
        PILOT["§8 Thin-slice pilots"]
        VBL["§9 Verification carryover"]
    end
...
> [!NOTE]
> **PROPOSED diagram.** Every "policy/", "SourceDescriptor", "EvidenceBundle", "ReleaseManifest", "RollbackTarget", and "CorrectionNotice" target above resolves to PROPOSED paths per Atlas Ch. 24.13 and Directory Rules §§6–7. The diagram shows responsibility flow, not implementation presence.
...
| Policy steward (sensitivity) | §5 (rows P*) → §10 → `[ENCY §11]` |
...
|---|---|---|---|---|---|---|---|
| **SET-BL-0001** | Legal-place vs CensusPlace vs Townsite vs GhostTown identity separation | Doctrine + contract | T2 (low, but identity-collapse risk) | `contracts/domains/settlements-infrastructure/place-identity.md` | One contract file enumerating each `Place`-class object, its identity rule, and a `KIND_OF_PLACE` enum with mutual-exclusion test. Mirrors Atlas Ch. 14 §C ubiquitous-language table. | `[DOM-SETTLE §B–C]`, `[ENCY §11]` | PROPOSED |
...
| **SET-BL-0003** | Critical-infrastructure deny-default policy bundle | Policy + validator | **T4 (high)** — critical-infrastructure deny lane | `policy/sensitivity/infrastructure/` | Policy bundle that DENIES, by default: exact geometry of utility-operator assets, condition observations, dependencies, operator-sensitive details, and "facility detail" attributes; plus a validator that asserts no PUBLISHED layer carries those fields. | `[DOM-SETTLE §I]`, `[ENCY §11]`, Atlas 24.13 ("Critical-asset deny lane") | PROPOSED |
| **SET-BL-0004** | Restricted-geometry no-leak test | Test + fixture | T4 | `tests/domains/settlements-infrastructure/test_restricted_geometry_no_leak.py` + `fixtures/domains/settlements-infrastructure/restricted/` | Test loads a fixture of restricted-class assets, runs the public-render path, and asserts no exact geometry appears in PUBLISHED output. Mirrors Atlas Ch. 14 §K list. | `[DOM-SETTLE §K]` | PROPOSED |
| **SET-BL-0005** | Census-vs-municipality distinction test | Test + fixture | T2 | `tests/domains/settlements-infrastructure/test_census_vs_municipality.py` | Fixture provides one place that is BOTH a CensusPlace and a Municipality and one that is ONLY a CensusPlace; test asserts the system never collapses the two and never labels a CDP "incorporated city." | `[DOM-SETTLE §K]` | PROPOSED |
| **SET-BL-0006** | Schema home migration to `schemas/contracts/v1/settlement/` | Schema | T2 | `schemas/contracts/v1/settlement/` per Atlas 24.13 | Eight base schemas (Settlement, Municipality, CensusPlace, Townsite, GhostTown, Fort, Mission, ReservationCommunity) plus seven asset/network schemas (Infrastructure Asset, Network Node, Network Segment, Facility, Service Area, Operator, Condition Observation, Dependency), all validated via `schemas/tests/valid/` and `schemas/tests/invalid/`. ADR-0001 schema-home rule applies. | Atlas 24.13, ADR-0001 | PROPOSED |
| **SET-BL-0007** | Infrastructure topology test | Test + fixture | T3 | `tests/domains/settlements-infrastructure/test_topology.py` | Network Node ↔ Network Segment connectivity is closed (no dangling segments, no orphan nodes), and Dependency edges form an acyclic-by-rule graph where the rule applies. | `[DOM-SETTLE §K]` | PROPOSED |
| **SET-BL-0008** | Condition observation temporal test | Test + fixture | T4 (when condition is restricted) | `tests/domains/settlements-infrastructure/test_condition_observed_at.py` | Condition Observation rows carry distinct `source_time`, `observed_at`, `valid_at`, `retrieved_at`, `released_at`, and `corrected_at` where material; collapsing into "observation date alone" is rejected. | `[DOM-SETTLE §E]` ("temporal handling") | PROPOSED |
| **SET-BL-0009** | Catalog / proof / release closure test | Test + fixture | T2 | `tests/domains/settlements-infrastructure/test_catalog_release_closure.py` | A PUBLISHED Settlements/Infrastructure artifact must resolve to a `CATALOG` record, an `EvidenceBundle`, a `ReleaseManifest`, a rollback target, and a correction path; any missing leg fails the test. | `[DOM-SETTLE §M]`, `[ENCY Appendix E]` | PROPOSED |
| **SET-BL-0010** | ReservationCommunity sovereignty review gate | Policy + review | **T4** | `policy/sensitivity/infrastructure/reservation-community.rego` (PROPOSED engine) | Any object with `kind_of_place = ReservationCommunity` or with a relation to a recognized tribal land triggers a steward-review obligation before promotion. Default: ABSTAIN until ReviewRecord is filed. Mirrors archaeology sovereignty pattern (`[DOM-ARCH]`) without owning archaeology semantics. | `[DOM-SETTLE §B]` (ReservationCommunity), `[DOM-ARCH §I]` (sovereignty pattern), Atlas 24.13 | PROPOSED |

### 5.1 Notes on the High band

- **SET-BL-0003** is the rate-limiting row. Without a critical-infrastructure deny-default policy bundle, every other "public-safe" claim in this lane is unsupported. It must precede any pilot in §8.
...
| **SET-BL-0105** | Municipality charter / incorporation evidence rule | Contract + policy | T2 | `policy/domains/settlements-infrastructure/municipality_evidence.rego` (PROPOSED engine) | Any `Municipality` claim asserting "incorporated" requires a charter/ordinance/incorporation-event EvidenceRef; the policy ABSTAINs otherwise. | `[DOM-SETTLE §K]` ("Legal municipality evidence tests") | PROPOSED |
| **SET-BL-0106** | KDOT bridge / facility crosswalk to Roads/Rail | Crosswalk + contract | T3 | `contracts/domains/settlements-infrastructure/roads-rail-crosswalk.md` | Bridges, depots, and grade-crossings: the **asset** is owned by Settlements/Infrastructure; the **route** is owned by `[DOM-ROADS]`. Crosswalk document spells out joining rule and forbids assertion of route designation from the asset side. | `[DOM-SETTLE §F]`, `[DOM-ROADS]`, Atlas 24.13 | PROPOSED |
| **SET-BL-0107** | Hydrology service-area crosswalk | Crosswalk + contract | T2 | `contracts/domains/settlements-infrastructure/hydrology-crosswalk.md` | Wastewater / stormwater / drinking-water service areas: this lane owns the **service-area boundary**; `[DOM-HYD]` owns the **water evidence**. Crosswalk spells out join rules; floodplain joins go to `NFHL zone` (Hydrology-owned). | `[DOM-SETTLE §F]`, `[DOM-HYD]` | PROPOSED |
| **SET-BL-0108** | Hazards exposure crosswalk | Crosswalk + contract | T3 | `contracts/domains/settlements-infrastructure/hazards-crosswalk.md` | "This settlement was affected by hazard X" relations: this lane supplies the **exposure subject**; `[DOM-HAZ]` supplies the **hazard event**; KFM is **never** an alert authority. Atlas 20.5 emergency-alert-boundary DENY applies. | `[DOM-SETTLE §F]`, `[DOM-HAZ]`, Atlas 20.5 | PROPOSED |
| **SET-BL-0109** | People/Land residence crosswalk | Crosswalk + contract | **T4** (living-person) | `contracts/domains/settlements-infrastructure/people-land-crosswalk.md` | Residence and migration-context relations: this lane never owns living-person fields; `[DOM-PEOPLE]` policy stack applies; living-person joins are DENY by default. | `[DOM-SETTLE §F]`, `[DOM-PEOPLE]` | PROPOSED |
| **SET-BL-0110** | Dependency graph public-safe projection | Schema + validator | T4 | `schemas/contracts/v1/settlement/dependency.schema.json` | Dependency edges carry an `exposure_class` (public-safe / restricted / quarantine) and a `generalization_method`; PUBLISHED dependency layers are aggregated to a public-safe density / count, not asset-to-asset edges. | `[DOM-SETTLE §I]`, `[ENCY §11]` | PROPOSED |
| **SET-BL-0111** | LayerManifest entries for the seven viewing products | LayerManifest | Varies | `data/published/layers/settlements-infrastructure/` | Seven `LayerManifest` entries: current settlement view, historic townsite view, legal-status-event view, census-place comparison, public-safe asset view, service-area aggregate view, dependency-summary view; **plus one restricted internal review view** with explicit non-public release class. | Atlas Ch. 14 §G | PROPOSED |
| **SET-BL-0112** | Focus Mode answer envelope for Settlements/Infrastructure | Runtime + AI | T2 | `apps/governed-api/.../settlements_infrastructure_focus_mode.py` (PROPOSED) | Settlements/Infrastructure Focus Mode answer envelope (Atlas Ch. 14 §J row 4) returns ANSWER / ABSTAIN / DENY / ERROR; AI never substitutes for EvidenceBundle. | Atlas Ch. 14 §J, `[GAI]` | PROPOSED |
| **SET-BL-0113** | Source-refresh runbook (Pattern A subfolder, per Directory Rules §6.1.b) | Runbook | Varies | `docs/runbooks/settlements-infrastructure/SOURCE_REFRESH_RUNBOOK.md` | Runbook covering Census TIGER decade refresh, GNIS sync, KDOT inspection-cycle refresh, FEMA NFHL refresh (consumed from Hydrology), and Kansas Geoportal sync; mirrors authored `docs/runbooks/fauna/SOURCE_REFRESH_RUNBOOK.md` structure. | Directory Rules §6.1.b, fauna runbook precedent | PROPOSED |
| **SET-BL-0114** | Evidence Drawer payload spec for this lane | Contract | T3 | `contracts/domains/settlements-infrastructure/evidence-drawer-payload.md` | EvidenceDrawerPayload shape for a Settlements/Infrastructure claim includes: source family, source_role, sensitivity_class, place-kind, legal-status-event provenance (if any), and policy decision trace. | Atlas Ch. 14 §J row 3, `[ENCY §11]` | PROPOSED |
| **SET-BL-0115** | Operator-sensitive condition redaction receipt | Receipt + validator | T4 | `data/receipts/settlements-infrastructure/condition_redaction/` | Every published Condition Observation that began life as a restricted record emits a `RedactionReceipt` recording: original sensitivity class, transformation method, retained signal, and steward sign-off. | `[ENCY §11]`, `[DOM-SETTLE §I]` | PROPOSED |
...
- **SET-BL-0111** depends on **SET-BL-0003** (deny-default policy) and **SET-BL-0004** (no-leak test) before any of the seven public LayerManifest entries can move to PUBLISHED.
...
| **SET-BL-0204** | Census-place-comparison viewing product spec | Viewing-product contract | T2 | `contracts/domains/settlements-infrastructure/viewing-product-census-comparison.md` | Specifies UI/data shape for comparing census-place over decades (population, area, incorporation status); flags `geography_version` per decade. | Atlas Ch. 14 §G | PROPOSED |
| **SET-BL-0205** | Ghost-town authority discipline | Contract + validator | T3 | `contracts/domains/settlements-infrastructure/ghost-town-authority.md` | Defines a `ghost_town_status_evidence_class` (population <X for Y years, post-office closure, etc.); KFM never asserts ghost-town status from a single secondary source. | `[DOM-SETTLE §B]` | PROPOSED |
| **SET-BL-0206** | TOD-style settlement-accessibility indicator (with assumptions visible) | Indicator + contract | T2 | `contracts/domains/settlements-infrastructure/tod-style-indicator.md` | Indicator schema with **algorithm-limitation labels** (per atlas card `KFM-P9-IDEA-0026`, `KFM-P9-IDEA-0027`); cannot be used for investment or planning claims; flagged with model-card. | `KFM-P9-IDEA-0026`, `KFM-P9-IDEA-0027` (PROPOSED idea-index cards) | PROPOSED |
| **SET-BL-0207** | Census-place comparison stale-state rule | Validator | T2 | `tests/domains/settlements-infrastructure/test_stale_census_place.py` | When a CensusPlace's vintage is older than configured threshold, the public layer is rendered with a `stale-state` flag in the Evidence Drawer. | `[DOM-SETTLE §M]` | PROPOSED |
...
| **SET-BL-0209** | Reservation-community boundary deference rule | Policy | **T4** | `policy/sensitivity/infrastructure/reservation-community-boundary.md` | KFM does not render or assert a ReservationCommunity boundary except as supplied by a recognized tribal-government source or federal-recognized dataset; community-asserted boundaries follow steward-review with sovereignty review attached. | `[DOM-SETTLE §B]`, `[DOM-ARCH §I]` (sovereignty pattern) | PROPOSED |
| **SET-BL-0210** | Long-form encyclopedia chapter for Settlements/Infrastructure | Encyclopedia | n/a | `docs/encyclopedia/chapters/11-settlements-infrastructure.md` (PROPOSED; pending OPEN-ENC-02 resolution) | Encyclopedia chapter mirroring Atlas Ch. 14 with stable anchors; resolution depends on encyclopedia chapter-split ADR. | `KFM-encyclopedia .md §15.1 OPEN-ENC-02` | PROPOSED |
...
| **Prerequisites (CONFIRMED required)** | SET-BL-0010 policy file, SET-BL-0209 boundary-deference rule, ReviewRecord schema (cross-cutting). |
...
Drawn from `kfm_whole_system_build_reference.md §10.12`, `[DOM-SETTLE §I]`, `[ENCY §11]`, `[UNIFIED §6.9]`:

| Risk | Posture |
|---|---|
| **Critical-infrastructure exposure** — utility nodes, treatment plants, generators, towers becoming targets. | Restricted/denied by default; public render only after policy + generalization + steward review. |
...
| **Condition/security misuse** — bridge inspection / asset condition becoming attacker telemetry. | Condition Observation restricted by default; aggregate / generalized public projection only. |
...
| **Sovereignty exposure** — ReservationCommunity boundary or facility detail published without tribal-government source or steward review. | SET-BL-0010 + SET-BL-0209 + cross-reference to `[DOM-ARCH §I]` sovereignty pattern. |
...
| **Dependency graph as attack-graph** — publishing asset-to-asset edges that map cascading failure. | SET-BL-0110 — only aggregate / public-safe density projections; never raw edges. |
| **KFM-as-alert-authority drift** — "is this settlement currently in danger?" framed as a Settlements/Infrastructure answer. | Atlas 20.5 DENY (emergency-alert boundary); SET-BL-0108 crosswalk reinforces. |

### 10.2 Sensitivity posture (inherited)

CONFIRMED, from `[ENCY §11]` and `[DOM-SETTLE §I]`:

| Asset class | Default posture | Allowed only when |
|---|---|---|
| Critical infrastructure exact geometry | **DENY** | Steward review + public-safe generalization + EvidenceBundle |
| Condition Observation (bridge inspection, asset condition, etc.) | **DENY** | Steward review + aggregate / generalized projection |
| Dependency edges (asset-to-asset) | **DENY** | Aggregate / public-safe density only |
| Operator-sensitive details (capacity, vulnerabilities, schedules) | **DENY** | Operator agreement + steward review |
| ReservationCommunity boundary | **ABSTAIN / DENY** | Tribal-government source OR federal-recognized dataset + sovereignty review |
| Historical settlement / townsite footprint (non-sovereignty-adjacent) | **ALLOW** with vintage flags | EvidenceBundle + `source_role = historical` |
| CensusPlace (current decade) | **ALLOW** | EvidenceBundle anchored to vintage |
| Municipality (incorporated) | **ALLOW** | Charter / ordinance EvidenceRef (per SET-BL-0105) |

### 10.3 Rollback notes

CONFIRMED doctrine per `[DOM-SETTLE §M]` and `[ENCY Appendix E]`:
...
- A **correction notice** path exists for individual claim retractions (e.g., a townsite footprint challenged, a Municipality charter date revised).
- **Stale-state rule** (SET-BL-0207): when a CensusPlace's vintage falls below the configured freshness threshold, the layer renders with a stale flag, not silent reuse.
...
- **Q-SET-01.** Should `ReservationCommunity` boundary geometry default to ABSTAIN (no rendering until tribal-government source confirms) or DENY (require explicit policy decision to render)? — Resolution path: ADR + tribal-government consultation outside this backlog. Cross-references: `[DOM-ARCH §I]` sovereignty pattern.
- **Q-SET-02.** Is "Service Area boundary" owned by Settlements/Infrastructure when the operator is a hydrologic utility (water/wastewater)? Atlas Ch. 14 §F lists "water, wastewater, stormwater, floodplain, drainage" as a Hydrology cross-lane relation; SET-BL-0107 must spell out the boundary. — Resolution path: ADR.
- **Q-SET-03.** Where does a **depot** asset live: under Settlements/Infrastructure (the building) or Roads/Rail (the transport node)? Atlas Ch. 14 §F lists "depot, bridge, crossing, transport facility" as a Roads/Rail relation; this backlog scopes the building to Settlements/Infrastructure and the transport function to Roads/Rail. — Resolution path: ADR cross-stewarded with Roads/Rail; SET-BL-0106 carries the crosswalk.
- **Q-SET-04.** Does the Settlements/Infrastructure lane own a *legal-status-event* object family (charter / incorporation / disincorporation / annexation / dissolution), or does it carry these as evented attributes of Municipality? — Resolution path: contract decision in SET-BL-0001 and SET-BL-0105.
- **Q-SET-05.** Is `Facility` (school, hospital, fire station, etc.) a Settlements/Infrastructure object or a thin wrapper over operator-typed asset? — Resolution path: contract decision in SET-BL-0001.
- **Q-SET-06.** Should the dependency-summary viewing product (Atlas Ch. 14 §G) be available outside an authenticated, steward-gated surface at all? — Resolution path: policy decision in SET-BL-0110 + SET-BL-0111.

### 11.2 Cross-lane open questions (referred out, tracked here)

- **Q-SET-X1.** Hazards × Settlements: where does the "settlement-as-exposure-subject" record live — in Hazards or in this lane? Atlas suggests the exposure relation; this backlog assumes the *settlement* identity lives here and the *event* lives in Hazards. — Refer to `[DOM-HAZ]` backlog.
- **Q-SET-X2.** People/Land × Settlements: the "migration context with restrictions" relation in Atlas Ch. 14 §F is ambiguous. — Refer to `[DOM-PEOPLE]` backlog.
- **Q-SET-X3.** Archaeology × Settlements: when a Fort, Mission, Townsite, or GhostTown carries an archaeological component, which lane's sensitivity policy dominates? — Default: archaeology dominates where overlap exists (sovereignty / cultural sensitivity outranks settlement-asset disclosure). Refer to `[DOM-ARCH]` backlog.
...
| ADR-Q-SET-B | Should `LayerManifest` carry a `sensitivity_class` field at top level, or only at per-feature level? Atlas Ch. 14 §G implies layer-level (the restricted internal review view is a separate layer). | ADR (cross-lane: affects every domain). |
| ADR-Q-SET-C | Should this lane's sovereignty review for ReservationCommunity reuse the `[DOM-ARCH]` review machinery, or stand up a parallel review queue? Reuse is preferred for governance economy. | ADR. |
...
1. **Responsibility root populated.** The artifact named in "Proposed root" exists in the repo and the path conforms to Directory Rules §§3–6.
...
3. **Policy and sensitivity support attached.** Any T3 or T4 row has a corresponding `policy/sensitivity/infrastructure/` rule and a `tests/` validator.
4. **Atlas / Encyclopedia anchor still holds.** The doctrinal citation in the row remains accurate against the current edition of Atlas Ch. 14 / Encyclopedia §11.
...
6. **No drift entry blocking.** No `docs/registers/DRIFT_REGISTER.md` row contradicts the closure.
...
> [!NOTE]
> Targets below are **PROPOSED paths** per Directory Rules v1.1 §§6–7 and Atlas Ch. 24.13. NEEDS VERIFICATION against a mounted repo.
...
- `contracts/domains/settlements-infrastructure/` — object meaning for this lane.
...
- `docs/doctrine/directory-rules.md` — placement authority (CONFIRMED v1.1).
...
- `docs/atlases/KFM_Domains_Culmination_Atlas_v1_1.pdf` — Ch. 14 + Ch. 24.13.
- `docs/encyclopedia/README.md` — §11 sensitive / deny-by-default register.
...
- `docs/registers/DRIFT_REGISTER.md` — drift entries that may block closure (§12.6).
...
|---|---|---|---|
| Census TIGER / census place geography | authority / observation / context (per descriptor) | rights NEEDS VERIFICATION; CensusPlace ≠ Municipality | `[DOM-SETTLE §D]` |
| GNIS and gazetteers | authority / observation (place-name evidence) | rights NEEDS VERIFICATION; sensitive joins fail closed | `[DOM-SETTLE §D]` |
| State / local GIS / Kansas Geoportal-style sources | authority / observation / context | rights NEEDS VERIFICATION | `[DOM-SETTLE §D]` |
...
| Historical gazetteers and maps | historical / observation / context | source_role = historical only; cannot supply LegalPlace claim | `[DOM-SETTLE §D]`, SET-BL-0103 |
| Infrastructure operators and providers | observation / operator-sensitive | rights NEEDS VERIFICATION; sensitive joins fail closed; condition / dependency restricted | `[DOM-SETTLE §D]`, `[ENCY §11]` |
| KDOT / bridge / facility sources | authority / observation (asset attributes) | condition observations restricted by default | `[DOM-SETTLE §D]`, SET-BL-0106 |
...
sensitivity_flag: T0 | T1 | T2 | T3 | T4
proposed_root: <responsibility-root path per Directory Rules §§3–6>
closure_criterion: |
...
### B.1 Sensitivity-flag legend (lane-local, PROPOSED)

| Flag | Meaning |
|---|---|
| **T0** | No sensitivity beyond default attribution. |
| **T1** | Public-safe but vintage- or precision-sensitive (e.g., historical map). |
| **T2** | Public-safe with identity-collapse risk (legal vs census vs historic). |
| **T3** | Operator-, sovereignty-, or boundary-sensitive; requires steward review on edge cases. |
| **T4** | Critical-infrastructure / sovereignty / living-person adjacency — deny-default; explicit policy required. |

> [!CAUTION]
> The lane-local T0–T4 flag is a **convenience label**, not the canonical sensitivity tier. Canonical sensitivity classes live in `policy/sensitivity/infrastructure/` and `[ENCY §11]`; this label exists to keep backlog rows scannable. Where the two disagree, `policy/` wins.

---

**Related docs:** [Atlas Ch. 14 (PROPOSED link)](../../atlases/KFM_Domains_Culmination_Atlas_v1_1.pdf) · [Encyclopedia §11 (PROPOSED link)](../../encyclopedia/README.md) · [Directory Rules](../../doctrine/directory-rules.md) · [Verification Backlog](../../registers/VERIFICATION_BACKLOG.md) · [Drift Register](../../registers/DRIFT_REGISTER.md)

**Last updated:** 2026-05-19  ·  [⬆ back to top](#contents)
