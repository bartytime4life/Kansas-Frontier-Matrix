...
[![Last updated](https://img.shields.io/badge/updated-2026--05--19-lightgrey)](#)

**Status:** draft · **Owners:** `<TODO: Settlements/Infrastructure steward — verify CODEOWNERS>` · **Last updated:** 2026-05-19
...
CONFIRMED scope (from the Settlements/Infrastructure dossier): `Settlement`, `Municipality`, `CensusPlace`, `Townsite`, `GhostTown`, `Fort`, `Mission`, `ReservationCommunity`, `Infrastructure Asset`, `Network Node`, `Network Segment`, `Facility`, `Service Area`, `Operator`, `Condition Observation`, `Dependency`. [DOM-SETTLE] [ENCY]

### 1.2 What it does **not** cover

CONFIRMED non-ownership boundary: this inventory does **not** track identity threads owned by other lanes — transport routes (Roads/Rail), water evidence (Hydrology), hazard events and warnings (Hazards), ownership and living-person privacy (People/Land). Where a Settlements/Infrastructure thread cites or composes those threads, the citation must preserve owning-lane authority. [DOM-SETTLE] [ENCY]

---

## 2. Continuity, defined

CONFIRMED doctrine (Domain-Driven Design reference, adopted): an entity is "an object distinguished by a thread of identity that runs through time and often across distinct representations." Keeping the class focused on **life-cycle continuity and identity** — and defining "what it means to be the same thing" — is the governing test. [DDD]
...
| **Evidence continuity** | Does the evidence trail survive correction, supersession, and rollback? | An EvidenceBundle replaced by a corrected bundle that retains the prior bundle for audit. [ENCY] |
...
> [!CAUTION]
> Do **not** treat the **Identity rule** column as a contract definition. Treat `contracts/domains/settlements/` (PROPOSED home) as the authority once authored, and `schemas/contracts/v1/settlement/` (PROPOSED, per ADR-0001) as the machine-checkable shape. [DIRRULES] [ENCY]
...
CONFIRMED doctrine: supersession is a **lineage transition**, not a deletion. A superseding object retains a `superseded_by` (or equivalent lineage link) to its predecessor; the predecessor remains queryable for time-bound claims and audit. [ENCY Appendix E] [DIRRULES §14]

### 5.1 Rename and identity-change matrix

A rename that does **not** change identity is an attribute change recorded in the entity's history. A rename that **does** change identity is a content change, not a placement change, and follows the Directory Rules contract: ADR, schema-version bump if shape is affected, compatibility map for fixtures, parity tests, and correction notices for any released artifact that referenced the old identity. [DIRRULES §14.3]
...
CONFIRMED doctrine, drawn from the v1.1 Atlas master supersession reference. [ENCY] [DIRRULES]

- Supersession entry in the source register (where the source itself is replaced).
- `EvidenceBundle` + `CorrectionNotice` + supersession link (where evidence is corrected).
- `ReleaseManifest` history with rollback chain intact (where release state changes).
- ADR (where the supersession changes meaning, contract shape, or authority).

---

## 6. Continuity events & state transitions

CONFIRMED doctrine: state changes of identity-bearing entities SHOULD be modeled as **domain events** — immutable records of "something happened the steward cares about" — with explicit timestamps and the identity of involved entities. [DDD] [INDEX-18]
...
stateDiagram-v2
    ...
    PUBLISHED --> PUBLISHED : attribute change\n(same identity)
    PUBLISHED --> CATALOG : supersession candidate
    CATALOG --> PUBLISHED : superseding release\n(predecessor retained)
...
CONFIRMED doctrine: cross-lane relations must preserve ownership, source role, sensitivity, and EvidenceBundle support; this lane never overrides another lane's identity authority. [DOM-SETTLE] [ENCY]
...
| Hazards exposure, resilience, warnings, and declarations. | Hazards. | Hazard-event identity stays with Hazards; the Critical Infrastructure exposure record uses both identities. KFM is **never** an alert authority. [DOM-HAZ] |
| People/Land residence, ownership, parcel, and migration context. | People/Land. | Living-person and parcel identity stay with People/Land; settlement identity bounds residence context with restrictions. [DOM-PEOPLE] |
| Archaeology cultural temporal periods and survey context. | Archaeology. | Cultural-period identity stays with Archaeology; precise site coordinates remain denied by default. [DOM-ARCH] |

---

## 8. Sensitivity & publication posture

CONFIRMED / PROPOSED: identity threads in this lane carry a default sensitivity tier; continuity treatment does **not** alter the tier without a steward review and a `PolicyDecision`. [DOM-SETTLE] [ENCY] [DIRRULES]

| Thread | Default tier (PROPOSED) | Why |
|---|---|---|
| Settlement, Municipality, CensusPlace, Townsite, GhostTown, Fort, Mission. | T0–T1 | Generally public; some historic precise coordinates may be restricted. |
| ReservationCommunity. | T2 with sovereignty review path. | Sovereignty deference; review state required. |
| Infrastructure Asset (critical). | T4 default for critical detail; T1 for generalized footprint. | Security-sensitive. [DOM-SETTLE] |
| Network Node, Network Segment, Facility (critical). | T2–T4 by content. | Operator-sensitive; condition detail typically restricted. |
| Service Area (aggregate). | T0–T1. | Aggregate boundary; underlying customer detail belongs to People/Land. |
| Operator (identity). | T0–T1. | Operator name is public; internal contact and credentials are not. |
| Condition Observation. | T2–T4 by content. | Inspection detail typically restricted. |
| Dependency. | T2–T4 by content. | Aggregate dependency graphs may leak attack surface; defaults restricted. |

> [!WARNING]
> Continuity bookkeeping does **not** weaken sensitivity. A redacted/generalized PUBLISHED footprint and a restricted internal-review footprint are two different release artifacts sharing one identity thread. Conflating them would collapse the trust membrane. [GAI] [ENCY]

---

## 9. Stale-state markers & required action

CONFIRMED doctrine (Atlas v1.1 §24.8 stale-state register), applied to this lane. KFM separates **stale** from **wrong**; both states have visible markers and traceable lifecycles. [ENCY] [DIRRULES]

| Marker | Triggered by | Action for this lane |
|---|---|---|
| Source freshness expired | `SourceDescriptor` cadence passed without re-admission. | Re-admit (e.g., TIGER vintage, GNIS edition, operator feed); otherwise mark dependent Settlement/Municipality/Network claims stale. |
| Schema version drift | Object schema upgraded past the published claim. | Migrate, re-validate, re-release; or mark stale. PROPOSED schema home: `schemas/contracts/v1/settlement/`. [ADR-0001] |
| Geography version drift | `GeographyVersion` replaced. | Rebind boundaries (annexation, CensusPlace vintage) and re-release; or mark stale. |
| Review aged out | `ReviewRecord` older than tolerance for sensitive lane (e.g., critical infrastructure). | Trigger steward review; potentially downgrade tier. |
| Rights status changed | Rights change in `SourceDescriptor` (operator feed, KDOT, FEMA). | Re-evaluate tier; potentially downgrade; emit `CorrectionNotice` if necessary. |
| Policy version changed | Policy referenced by `PolicyDecision` superseded. | Re-run gate; potentially supersede release. |
...
> [!IMPORTANT]
> This lane does **not** author its own cross-cutting affordances. Evidence Drawer, time-aware state, trust badges, sensitivity-redacted view, correction/stale-state view, and governed Focus Mode belong to the MapLibre / Governed AI doctrine and are reused, not redefined. [MAP-MASTER] [GAI]

---

## 11. Governed-AI behavior

CONFIRMED doctrine / PROPOSED implementation: an AI surface MAY summarize released Settlements/Infrastructure `EvidenceBundles`, compare evidence across editions or supersessions, explain limitations, and draft steward-review notes. It MUST `ABSTAIN` when evidence is insufficient and `DENY` where policy, rights, sensitivity, or release state blocks the request. AI is never root truth. [GAI] [DOM-SETTLE] [ENCY]
...
- A question that depends on an aged source SHOULD return a stale-state-aware answer, not a confident assertion. [ENCY]
...
- **Restricted-geometry no-leak tests** — generalized footprints never carry precise geometry through the public pipeline. [DOM-SETTLE]
...
- **Supersession-link parity tests** — every superseding object resolves its predecessor. [ENCY Appendix E]
- **Rename-without-identity-change parity tests** — a Settlement rename retains identity, aliases, and prior-name searchability. [DDD] [DIRRULES §14.3]

PROPOSED fixture homes (paths NEEDS VERIFICATION against Directory Rules and a mounted repo):

```text
schemas/contracts/v1/settlement/        # PROPOSED schema home (per ADR-0001)
contracts/domains/settlements/          # PROPOSED contract home
policy/sensitivity/infrastructure/      # PROPOSED policy home
tests/fixtures/settlements/             # PROPOSED fixture home
```
...
|---|---|---|---|
| Q1 | Concrete identity-hash canonicalization for each object family (JCS+SHA-256 corpus convention vs. lane-specific extension). | Schema + contract authoring; ADR if cross-lane impact. | NEEDS VERIFICATION |
...
| Q3 | Whether `ReservationCommunity` continuity events require steward sign-off before public surfacing. | Policy + `ReviewRecord` schema. | NEEDS VERIFICATION |
...
| Q5 | Schema home (`schemas/contracts/v1/settlement/` vs. nested domain folders). | ADR-0001 compliance check. | PROPOSED |
...
| Q8 | Whether this file's location (`docs/domains/settlements-infrastructure/CONTINUITY_INVENTORY.md`) conforms to the per-domain dossier convention finalized by Directory Rules §18.a OPEN-ENC-04. | Directory Rules ADR. | NEEDS VERIFICATION |
...
- `docs/atlases/KFM_Domains_Culmination_Atlas_v1_1.pdf` — v1.1 Atlas, Chapter 14 (Settlements / Infrastructure) is the doctrinal anchor for §3 of this file (PROPOSED placement).
...
- `contracts/domains/settlements/` — semantic contracts (PROPOSED home).
- `schemas/contracts/v1/settlement/` — JSON Schema (PROPOSED home, per ADR-0001).
- `policy/sensitivity/infrastructure/` — sensitivity policy (PROPOSED home).
- `control_plane/object_family_register.yaml` — machine-readable object-family registry.

---

<sub>**Citation short-names used above:** `[DOM-SETTLE]` Settlements / Infrastructure dossier · `[ENCY]` KFM Encyclopedia · `[DIRRULES]` Directory Rules · `[MAP-MASTER]` MapLibre Master · `[GAI]` Governed AI dossier · `[DDD]` Domain-Driven Design Reference · `[INDEX-18]` Pass 18 Idea Index · `[DOM-ROADS]` Roads / Rail dossier · `[DOM-HAZ]` Hazards dossier · `[DOM-PEOPLE]` People / DNA / Land dossier · `[DOM-ARCH]` Archaeology dossier. Full ledger in the v1.1 Atlas §2.</sub>

---

**Last updated:** 2026-05-19 · **Owners:** `<TODO: Settlements/Infrastructure domain steward>` · [↑ back to top](#continuity-inventory--settlements--infrastructure)
