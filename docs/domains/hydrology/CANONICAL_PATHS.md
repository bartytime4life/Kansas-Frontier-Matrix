<!-- [KFM_META_BLOCK_V2]
doc_id: kfm://doc/docs/domains/hydrology/canonical-paths
...
[![Doc Type](https://img.shields.io/badge/doc-standard-blue)](../../doctrine/directory-rules.md)
[![Status](https://img.shields.io/badge/status-draft-yellow)](#status)
[![Authority](https://img.shields.io/badge/authority-doctrine--bound-informational)](../../doctrine/directory-rules.md)
[![Lane Pattern](https://img.shields.io/badge/lane-domain--under--roots-success)](../../doctrine/directory-rules.md#12-domain-placement-law)
[![Lifecycle](https://img.shields.io/badge/lifecycle-RAW%E2%86%92PUBLISHED-orange)](../../doctrine/lifecycle-law.md)
[![Build](https://img.shields.io/badge/CI-TODO-lightgrey)](#) <!-- TODO: replace with real CI badge -->
[![Last Updated](https://img.shields.io/badge/updated-2026--05--17-lightgrey)](#)

**Status:** Draft · **Owners:** Hydrology domain steward + Docs steward *(placeholders — confirm)* · **Updated:** 2026-05-17
...
It is **not** a substitute for `docs/doctrine/directory-rules.md` (rule source), the hydrology dossier (domain meaning), or `control_plane/domain_lane_register.yaml` (machine-readable lane registry). When this document disagrees with Directory Rules, **Directory Rules win** and this document is the drift entry.

> [!IMPORTANT]
> **Rule status is CONFIRMED. Path-as-applied is PROPOSED.** The lane pattern from Directory Rules §12 is doctrine. Whether any specific path is materialized in the current repository is **PROPOSED** until verified against mounted-repo evidence. Treat every concrete path below as a *target* the lane may legitimately occupy, not as evidence that the file or directory currently exists.
...
| `docs/domains/hydrology/CANONICAL_PATHS.md` | **This file.** Path-level index. | PROPOSED |
...
| `docs/domains/hydrology/INDEX.md` | (Optional) flat index of files in this folder. | PROPOSED |
...
> [!WARNING]
> **No parallel schema homes.** Per Directory Rules §13.1 (drift §13.1), schemas MUST NOT exist simultaneously under `contracts/domains/hydrology/` **and** `schemas/contracts/v1/domains/hydrology/`. If both are found in the mounted repo, open a drift entry per §2.5 and migrate under ADR-0001.
...
| `fixtures/domains/hydrology/invalid/` | Negative fixtures (rights gap, missing source role, ambiguous reach identity). | PROPOSED |
...
> [!NOTE]
> **Cross-domain validators do not get a domain segment.** Per Directory Rules §12 ("Multi-domain and cross-cutting files"), a hydrology × hazards flood-context validator lives under `tools/validators/<topic>/...`, not `tools/validators/domains/hydrology/...`.
...
```mermaid
flowchart LR
  A["RAW\ndata/raw/hydrology/"] -->|"SourceDescriptor + ingest receipt"| B
  B["WORK\ndata/work/hydrology/"]
  B -->|"validation + policy gate"| C["PROCESSED\ndata/processed/hydrology/"]
  B -.failure.-> Q["QUARANTINE\ndata/quarantine/hydrology/"]
  C -->|"EvidenceBundle + catalog closure"| D["CATALOG / TRIPLET\ndata/catalog/domain/hydrology/\ndata/triplets/..."]
  D -->|"ReleaseManifest +\nrollback + review"| E["PUBLISHED\ndata/published/layers/hydrology/"]
...
> Object-family list is **CONFIRMED** by the hydrology dossier (`kfm_encyclopedia.pdf` §7.2) and Atlas v1.1 Appendix C. Specific filenames are PROPOSED conventions; verify against any existing schema or contract in the repo before creating new ones.
...
| Hydrology MapLibre layer wiring | `packages/maplibre/` or `apps/explorer-web/`. | Renderer/shell paths are doctrine, not domain. |
...
| **Stale gauge data published without freshness markers** | Time-series layer served past its freshness window with no stale badge. | Freshness policy enforced via `policy/domains/hydrology/freshness.rego` + freshness badge in the Evidence Drawer. |
...
These items remain **NEEDS VERIFICATION** until mounted-repo evidence resolves them. They should be carried in `docs/registers/VERIFICATION_BACKLOG.md`.

| # | Question | Status | Resolution path |
|---|---|---|---|
| 1 | Does the mounted repo materialize the lane segments listed in §4 verbatim, or use a variant naming? | NEEDS VERIFICATION | Inspect repo; open drift entries for divergences. |
| 2 | Is `fixtures/domains/hydrology/` or `tests/fixtures/domains/hydrology/` the authority? | NEEDS VERIFICATION | Settle the rule in `tests/README.md` and `fixtures/README.md`. |
| 3 | Are any hydrology schemas already authored under `contracts/domains/hydrology/<x>.schema.json` (drift §13.1)? | NEEDS VERIFICATION | Inspect; if found, open ADR-0001 migration entry. |
| 4 | Which hydrology connectors are activated (USGS, FEMA, NOAA, NRCS, state)? | NEEDS VERIFICATION | Check `data/registry/sources/hydrology/` and connector READMEs. |
| 5 | Does `apps/governed-api/` already expose hydrology routes, and under what shape? | NEEDS VERIFICATION | Inspect routes; record under `docs/architecture/governed-api.md`. |
| 6 | What is the actual hydrology MapLibre layer binding (PMTiles / GeoParquet / API)? | NEEDS VERIFICATION | Inspect `packages/maplibre/` and `apps/explorer-web/`. |
| 7 | Are hydrology release candidates currently produced under `release/candidates/hydrology/`? | NEEDS VERIFICATION | Inspect `release/`. |
| 8 | Is there an ADR governing the hydrology-specific subset of NFHL vs observed vs forecast separation? | NEEDS VERIFICATION | Search `docs/adr/`. |
| 9 | Resolve the `docs/standards/PROV.md` vs `PROVENANCE.md` naming discrepancy, since hydrology relies on PROV-O for time-series lineage. | OPEN ADR | Pending ADR per project memory. |
...
- [`docs/doctrine/directory-rules.md`](../../doctrine/directory-rules.md) — placement doctrine (authoritative).
...
- [`docs/standards/PROV.md`](../../standards/PROV.md) — provenance standard (subject to PROVENANCE.md naming ADR).
...
Collapsing roles (e.g., styling NFHL as observed inundation) is a public-facing trust failure and is denied by lane policy.
...
Per Atlas v1.1 §F (Cross-lane relations), hydrology participates in these cross-lane relations. Files implementing them live under the **lowest common responsibility root** with no domain segment.

| Related lane | Relation type | Likely shared path |
|---|---|---|
| Hazards | Flood / drought / warning / declaration / resilience context | `schemas/contracts/v1/joins/hydrology_hazards/` |
| Soil | Soil moisture / hydrologic group / infiltration / runoff | `schemas/contracts/v1/joins/hydrology_soil/` |
| Agriculture | Irrigation / drought stress / crop-water context | `schemas/contracts/v1/joins/hydrology_agriculture/` |
| Settlements / Infrastructure | Floodplain / bridges / dams / utilities / exposure | `schemas/contracts/v1/joins/hydrology_settlements/` |

All cross-lane relations MUST preserve ownership, source role, sensitivity, and EvidenceBundle support (Atlas v1.1 §F).

</details>

---

**Last updated:** 2026-05-17 · **Version:** v1 (draft) · **Authority:** Directory Rules §12 + §6–9 (CONFIRMED) · Path-as-applied (PROPOSED)
