...
**Status:** draft · **Owners:** `<hydrology-stewards>` *(placeholder — NEEDS VERIFICATION)* · **Last updated:** 2026-05-17
...
| `directory-rules.md` | Domain Placement Law (§12) — hydrology is a **segment**, not a root. Drives every path proposal below. | CONFIRMED placement authority |
...
| Hydrology Focus Mode (bounded synthesis with `AIReceipt`) | `KEEP AND EXTEND` | `[GAI]` / `[DOM-HYD]` / `[UIAI]` | `RuntimeResponseEnvelope` returns `ANSWER / ABSTAIN / DENY / ERROR` over released hydrology `EvidenceBundle`s only. No browser-to-model calls. |
| Hydrology layer manifests + MapLibre disciplined renderer boundary | `KEEP AND EXTEND` | `[MAP-MASTER]` / `[DOM-HYD]` | `LayerManifest` + `StyleManifest` + `TileArtifactManifest` + `MapReleaseManifest` for each hydrology layer; MapLibre as 2D adapter, not truth path. |
...
| Prior gain | Disposition | Lineage basis | Preserved next behavior |
|---|---|---|---|
| HUC12 fingerprint validator | `KEEP AS LINEAGE` → target `KEEP AND EXTEND` | `[DOM-HYD] N` / `[ENCY]` | `PROPOSED` validator under `tools/validators/hydro/` per Directory Rules §7.5; deterministic, offline, no-network. |
...
| Stale source / freshness badges | `KEEP AND EXTEND` | `[ENCY] M` / `[MAP-MASTER]` | Freshness state surfaces on map + drawer; `ABSTAIN`/`DENY` on stale evidence per policy. |
...
|---|---|---|---|
| 3D / Cesium hydrology scene | `DEFER` | `[Whole-UI Expansion] §6` / `[ENCY]` | 2D evidence continuity must be proven first; 3D enters only with `SceneManifest`, `RealityBoundaryNote`, and `3D admission decision`. |
...
Hydrology relations to other lanes are preserved at the **relation type** level only. Ownership stays with the originating domain.
...
| Agriculture | irrigation, drought stress, crop-water context | `KEEP AND EXTEND` | Agriculture owns crop claims; Hydrology supplies water context |
...
| Spatial Foundation | CRS, geometry validation, base layers | `KEEP AND EXTEND` | Hydrology consumes; never overrides CRS rules |
...
> **Kansas HUC12 + one USGS gauge fixture + one NHDPlus identity crosswalk + NFHL contextual overlay + hydrograph panel + EvidenceBundle closure + ABSTAIN on ambiguous reach identity** — `[DOM-HYD]` / `[ENCY]` / `[BLD-GREEN] §21` / `[UNIFIED] §6.1`.
...
Items the continuity inventory cannot settle on its own. Each is `NEEDS VERIFICATION` and SHOULD be resolved by a mounted-repo PR or an accepted ADR. This register pairs with `docs/registers/VERIFICATION_BACKLOG.md` and `control_plane/verification_backlog.yaml`.
...
| 4 | Hydrology governed-API routes + MapLibre layer adapter wired | route inventory, `apps/governed-api/` code, `LayerManifest` entries | `NEEDS VERIFICATION` |
...
| 10 | Resolution of corpus `PROV.md` / `PROVENANCE.md` naming question as it intersects hydrology evidence references | ADR resolving the naming + link audit | `NEEDS VERIFICATION` |
...
- [`docs/doctrine/directory-rules.md`](../../doctrine/directory-rules.md) — Domain Placement Law (§12), Placement Protocol (§4)
- [`docs/standards/PROV.md`](../../standards/PROV.md) — W3C PROV-O / PAV provenance profile *(naming question open with `PROVENANCE.md`)*
...
- **`DEFER`** — Surface acknowledged but explicitly postponed. Reactivation requires a stated gating prerequisite to clear. Example: 3D hydrology scene, cross-domain graph queries, MLT pilot.
- **`EXCLUDE`** — Surface listed only to mark that it is **not** carried forward. Used to fence off anti-patterns and invariant violations. Example: hydrology-as-root-folder, NFHL-as-observed-flood.
...
- **`NEEDS VERIFICATION`** — Checkable, but not yet checked strongly enough to act as fact. Default label for items in the [Verification backlog](#12-verification-backlog).
...
**Last updated:** 2026-05-17 · **Doc ID:** `kfm://doc/docs-domains-hydrology-continuity-inventory` · **Status:** draft · **Version:** v1
