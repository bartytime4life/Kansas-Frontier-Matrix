...
version: v1
status: draft
owners: <hydrology domain steward> + <docs steward>
created: 2026-05-17
updated: 2026-05-17
policy_label: public
related:
...
notes:
  - "PROPOSED file home per Directory Rules §12 (Domain Placement Law)."
  - "All implementation-maturity claims are PROPOSED or NEEDS VERIFICATION pending mounted-repo evidence."
...
![lifecycle: RAW→…→PUBLISHED](https://img.shields.io/badge/lifecycle-RAW%E2%86%92WORK%E2%86%92PROCESSED%E2%86%92CATALOG%E2%86%92PUBLISHED-lightgrey)
...
| **Authority class** | Register / lineage. `docs/` **explains**; this file does not decide alone. Promotions and ADRs decide. |
| **Supersedes** | none (initial draft) |
...
- It is **not** an ADR; it does not amend Directory Rules, schema homes, or lifecycle phases. Where a backlog item requires an ADR (per Directory Rules §2.4), the item links to a `PROPOSED ADR` row, not to a decision.
...
|---|---|
| **CONFIRMED** | Verified in attached KFM doctrine (Encyclopedia, Atlas, Directory Rules) or in completed standards profiles. |
...
Every item in this backlog **MUST** respect the KFM lifecycle and publication invariants:
...
- Watchers observe and record; **watchers do not publish**.
...
> [!CAUTION]
> Hydrology data can be **operationally current and safety-relevant** (gauges, streamflow anomalies, flood context). KFM publishes hydrology as **observational signals and regulatory context**, never as emergency alerts or live regulatory authority. Source-role separation is a hard, non-negotiable invariant.
...
flowchart LR
    subgraph DOC["docs/ (this lane)"]
        D1["docs/domains/hydrology/<br/>README.md · EXPANSION_BACKLOG.md"]
        D2["docs/standards/<br/>(PROV, PMTILES, OGC-API-TILES, ...)"]
        D3["docs/runbooks/<br/>hydrology validation, rollback"]
        D4["docs/adr/<br/>PROPOSED ADRs"]
    end
    ...
    subgraph CODE["tools / pipelines / connectors"]
        T1["tools/validators/hydro/<br/>(crosswalk, fingerprint)"]
        T2["tools/ingest/hydrology/<br/>(USGS streamflow gate, NHD delta watch)"]
        T3["pipeline_specs/hydrology/<br/>+ pipelines/domains/hydrology/"]
        T4["connectors/<br/>(USGS NWIS, WBD, NHDPlus HR, NFHL, 3DEP)"]
    end
...
> [!NOTE]
> **NEEDS VERIFICATION.** The diagram reflects Directory Rules §12 (Domain Placement Law) applied to the hydrology lane. Specific subfolder names and the existence of e.g. `tools/validators/hydro/` vs. `tools/validators/hydrology/` remain **PROPOSED** until mounted-repo evidence confirms the in-use convention.
...
| **T5 — Validators** | `tools/validators/hydro/` — crosswalk, fingerprint, identity, role-separation checks. | Pipelines/tools owner |
| **T6 — Pipelines & Watchers** | `pipelines/domains/hydrology/`, `tools/ingest/hydrology/` watchers, sidecars, receipts. | Pipelines owner + source steward |
...
| **HYD-H05** | COMID ↔ HUC12 crosswalk validator (offline) | T5 (T4) | PROCESSED → CATALOG | `tools/validators/hydro/check_crosswalk.py` + schema + invalid/valid fixtures | PROPOSED |
...
| **HYD-H12** | Hydrology runbooks — validation & rollback | T2 (T8) | n/a | `docs/runbooks/hydrology/VALIDATION.md`, `…/ROLLBACK.md` | PROPOSED |
...
| ID | Title | Track(s) | Lifecycle | First artifact | Status |
|---|---|---|---|---|---|
| **HYD-M01** | USGS streamflow anomaly watcher thin slice | T6 (T5, T8) | RAW → WORK | `tools/ingest/hydrology/usgs_streamflow_gate.py` + sidecar + `RunReceipt` | PROPOSED |
| **HYD-M02** | NHD hydrology delta watcher | T6 (T1, T5) | RAW → WORK | `SourceIntakeRecord` watcher + materiality thresholds; emits candidates only | PROPOSED |
| **HYD-M03** | FEMA NFHL revision watcher | T6 (T1) | RAW → WORK | NFHL revision intake records; role-locked as **context**, never observed | PROPOSED |
...
| **HYD-M07** | Hydrology STAC / GeoParquet profile | T4 (T1) | CATALOG | One CDL-equivalent STAC item for a HUC12 + one for a tile artifact | PROPOSED |
...
| **HYD-L06** | Async / multi-gauge ingest mode | T6 | RAW → WORK | County-scale sweep orchestrator | PROPOSED |
...
These questions are carried forward from the **Domain Encyclopedia §7.2.N** and **Atlas v1.1 [DOM-HYD] N. Verification backlog**, then extended with questions that surfaced while drafting this backlog. Each question is `NEEDS VERIFICATION` until evidence is mounted.

| OQ-ID | Question | Evidence that would settle it | Status |
|---|---|---|---|
| **OQ-HYD-01** | Is `schemas/contracts/v1/domains/hydrology/` the canonical schema home, or does an existing ADR amend that? | ADR inspection + repo `schemas/` walk | NEEDS VERIFICATION |
| **OQ-HYD-02** | Does `tools/validators/hydro/` or `tools/validators/hydrology/` exist; which is canonical? | Mounted repo + per-root README of `tools/validators/` | NEEDS VERIFICATION |
| **OQ-HYD-03** | Are runbook subfolders convention (`docs/runbooks/hydrology/…`) or flat (`docs/runbooks/hydrology_VALIDATION.md`)? | Mounted `docs/runbooks/` inspection; cross-reference to fauna runbook precedent (`docs/runbooks/fauna/SOURCE_REFRESH_RUNBOOK.md`) | NEEDS VERIFICATION |
...
| **OQ-HYD-08** | USGS NWIS endpoint transition: are legacy `waterservices.usgs.gov` endpoints still in use, or has `api.waterdata.usgs.gov` superseded them in our connectors? Project corpus references a 2026/2027 phase-out window, but the current production state is not verified here. | Current USGS docs at endpoint canonical URLs + connector source | NEEDS VERIFICATION |
| **OQ-HYD-09** | NHDPlus version policy: are v2.1 and HR both supported simultaneously, or is HR the lane default? Mixing must be denied per source-role doctrine. | ADR + connector + `nhdplus_version` carry-through | NEEDS VERIFICATION |
...
| **OQ-HYD-12** | Alignment-score threshold: is `0.75` doctrine, or per-source-family configurable? Should it move to a threshold policy registry? | ADR + policy registry | PROPOSED |
...
| **OQ-HYD-14** | Public-safe display of exact gauge coordinates: default permitted, default redacted, or per-site review? | Sensitivity policy + steward review | UNKNOWN |
| **OQ-HYD-15** | Watcher cadence policy: per-source HEAD checks vs. full refresh — what's the default cadence per hydrology source family? | Source-watch registry + ADR | NEEDS VERIFICATION |

> [!NOTE]
> Items marked **PROPOSED ADR** in §§5–6 are mirrored as the relevant open questions above. When an ADR lands, its row in this register moves to **CONFIRMED** with an ADR reference and a forward link in `docs/registers/DRIFT_REGISTER.md` (if applicable).
...
| `SourceDescriptor` schema | Hydrology **depends on** | HYD-H02, HYD-H03, HYD-M01–M04, HYD-M11 | Source admission flows through the descriptor. |
...
| ADR-0001 (schema home) | Hydrology **conforms to** | HYD-H11 and all schema items | Drift entry required if conflict surfaces. |
| `docs/standards/PROV.md` | Hydrology **conforms to** | HYD-H10, HYD-M01–M03, HYD-M11 | Provenance closure on receipts and bundles. |
...
| Soil & Agriculture lanes | Hydrology **shares context with** | HYD-L02, HYD-L03 | Soil-hydrology joins, irrigation links — owned **outside** the hydrology canonical claims. |
...
- [ ] **Lifecycle-respected.** The item touches only the lifecycle phases declared in its row; no phase is skipped.
- [ ] **Watcher invariant honored.** If the item is a watcher, it emits receipts and candidates only — it does not publish.
...
- [ ] **Finite outcome.** Validators and runtime surfaces return `ANSWER` / `ABSTAIN` / `DENY` / `ERROR`, not free text.
...
| AI-generated hydrology forecasts | AI is interpretive, not the root truth source; forecasts require a model lane with its own source role. | Governed AI runtime + model lane; never as canonical hydrology claim |
...
Source: KFM Domain Encyclopedia §7.2; Domains Culmination Atlas v1.1 §[DOM-HYD] D.
...
4. Non-CONUS coverage requires explicit policy; default is `coverage_scope: "CONUS"` for the official crosswalk.
...
- **Inputs.** Mounted repo; existing `tools/validators/`, `pipelines/`, `connectors/`, `schemas/contracts/v1/`, `docs/runbooks/`, `release/` directories.
- **Outputs.** `RepoEvidenceReport (hydrology slice)`; updated truth labels in this file; new rows in `docs/registers/DRIFT_REGISTER.md` if conflicts surface.
- **Closure.** Every `OQ-HYD-*` row labeled NEEDS VERIFICATION has either a CONFIRMED resolution or a documented carry-forward reason.
...
- **Outputs.** `SourceCurrentnessReport` table; per-source-family verification cadence in `control_plane/source_authority_register.yaml`.
...
- **Risk.** Collapsing NFHL into observation. Mitigation: explicit role-separation tests in HYD-H09.
...
- **Goal.** Deterministic, offline-capable, side-effect-free validator for COMID → HUC12 lineage.
- **Decision ladder (PROPOSED, drawn from project corpus):**
  1. Official USGS crosswalk (primary).
  2. Area-weighted polygon overlay (`alignment_score` retained).
  3. Centroid-in-polygon heuristic (recorded as heuristic).
  4. Snap-to-outlet / pour-point with PRNG seed for ties.
- **Required manifest fields (PROPOSED):** `spec_hash`, `source_head { url, etag, last_modified }`, `algorithm_version`, `comid`, `huc12`, `catchment_poly_hash`, `geometry_sanity_flags`, `alignment_score`, `decision_reason`, `provenance { tool, tool_version }`.
- **Exit codes (PROPOSED, aligned with KFM runtime envelope):** `0` PASS / `1` FAIL / `2` ERROR/runtime / `3` ABSTAIN/unresolved.
...
- **Outputs.** Identity fixtures + ambiguity tests; carry-through of `reachcode`, `permanent_identifier`, `vpuid`.
...
- **Outputs.** Negative fixtures + denial tests + Drawer prose for `DENY` state.
...
- **Outputs.** Hydrology bundle emitter; closure tests; Drawer projection.

### HYD-H11 — Schema-home reconciliation for hydrology contracts

- **Goal.** Confirm `schemas/contracts/v1/domains/hydrology/` per ADR-0001, or open ADR amendment.
...
- **Outputs.** `docs/runbooks/hydrology/VALIDATION.md`, `docs/runbooks/hydrology/ROLLBACK.md` (PROPOSED subfolder convention pending OQ-HYD-03).
...
- **Related docs:** see [§12](#12-related-docs)
- **Last updated:** `2026-05-17`
- **Authority class:** register / lineage. `docs/` explains; it does not decide alone.
- **Conformance:** Directory Rules §12 (Domain Placement Law), Lifecycle Law (RAW → … → PUBLISHED), Cite-or-Abstain, Watcher-as-Non-Publisher.
