...
notes:
  ...
  - v1.1 is a MINOR bump per contract §37 (clarifications, reconciliations, gap closures; no breaking anchor changes — but two new sections added at §§7–10 with anchor adjustments documented in the changelog).
...
<p align="left">
  ...
  <img alt="Version" src="https://img.shields.io/badge/version-v1.1-lightgrey">
  ...
  <img alt="Directory Rules" src="https://img.shields.io/badge/Directory%20Rules-%C2%A76.1%20%C2%A712%20%C2%A716-purple">
  <img alt="Last reviewed" src="https://img.shields.io/badge/last_reviewed-2026--05--27-lightgrey">
  <img alt="Mounted repo" src="https://img.shields.io/badge/repo-NOT_MOUNTED-lightgrey">
</p>

**Status:** `draft` · **Owners:** _TODO/REVIEW_ — Archaeology domain stewards · Docs steward · Directory Rules reviewers _(placeholder pending CODEOWNERS)_ · **Last updated:** 2026-05-27 · **Operating contract:** `ai-build-operating-contract.md` v3.0.0
...
9. [Changelog v1 → v1.1](#9-changelog-v1--v11)
...
|---|---|---|
| `docs/doctrine/ai-build-operating-contract.md` v3.0.0 | Operating law, truth labels, sensitive-domain matrix (§23.2), finite outcomes (§8), separation of duties | **CONFIRMED** doctrine |
| `docs/doctrine/directory-rules.md` v1.3 §§4, 6.1, 6.5, 6.6, 7.2.a, 9, 12, 13.3, 15, 16, 18.e | Placement law, README contract, path-validation checklist, MapLibre sole-renderer (v1.3, OPEN-DR-10) | **CONFIRMED** doctrine |
...
| KFM Domains Atlas v1.1 §24.5 — Sensitivity / Rights Tier Reference (T0–T4) | Canonical tier scheme; archaeology object class → tier mapping | **CONFIRMED** doctrine |
...
| `docs/domains/archaeology/MAP_UI_CONTRACTS.md` v0.2 | Map / UI / governed-AI contract surface — sibling planning artifact | **CONFIRMED** authored (this session) |
...
> [!CAUTION]
> **Known doctrine drift — `domains/` segment.** Atlas v1.1 §24.13 lists schema and contract homes **without** the `domains/` segment (e.g., `schemas/contracts/v1/archaeology/`). Directory Rules §12 (v1.2) explicitly requires the segment (`schemas/contracts/v1/domains/archaeology/`). Per Directory Rules §2.5, Directory Rules governs placement; the conflict is a drift candidate for `docs/registers/DRIFT_REGISTER.md`. This ledger adopts the §12 form throughout and flags the conflict at [OQ-AR-MO-02](#7-open-questions-register).
...
Human-facing doctrine, planning, and runbooks for the archaeology lane. Folder placement is **CONFIRMED** per Directory Rules §6.1 (the doc tree explicitly lists `docs/domains/archaeology/`).
...
| `docs/domains/archaeology/MAP_UI_CONTRACTS.md` | Governed surfaces and payload contracts binding the archaeology lane to the Map shell, Evidence Drawer, time-aware UI, and Focus Mode (per `ai-build-operating-contract.md` §9 / §22 / §23). | 🟢 PRESENT _(authored this session, v0.2)_ | ❓ | PROPOSED |
...
| `docs/domains/archaeology/governed-ai-behavior.md` | `ANSWER` / `ABSTAIN` / `DENY` / `ERROR` / `HOLD` posture for archaeology Focus Mode and Evidence Drawer; AI exact-location denial. | 🟡 PLANNED | ❓ | PROPOSED |
...
| `contracts/domains/archaeology/three_d_documentation.md` | Meaning of `ThreeDDocumentation`; access-control + transform-receipt requirements; hosted only via `packages/maplibre-runtime/` (v1.3). | 🟡 PLANNED | ❓ | PROPOSED |
...
| `contracts/domains/archaeology/runtime_envelope_archaeology_projection.md` | How the cross-cutting `RuntimeResponseEnvelope` (operating contract §9.2) is projected for archaeology surfaces — finite outcomes `ANSWER` / `ABSTAIN` / `DENY` / `ERROR` / `HOLD`, archaeology-specific `deny_reason` codes (`SENSITIVITY_EXACT_GEOMETRY`, `SENSITIVITY_BURIAL_OR_HUMAN_REMAINS`, `SENSITIVITY_SACRED_PLACE`, etc.). **Renames the v1 `archaeology_decision_envelope.md` — see [§9 Changelog](#9-changelog-v1--v11)**. | 🟡 PLANNED | ❓ | PROPOSED |
| `contracts/domains/archaeology/decision_envelope_archaeology_projection.md` | How the cross-cutting `DecisionEnvelope` (Atlas §24.3 / KFM-P5-PROG-0001 — `{decision_id, outcome, policy_family, reasons[], obligations[], evaluated_at}`) is used in archaeology policy modules. | 🟡 PLANNED | ❓ | PROPOSED |
...
Machine **shape** for each domain-specific object family. Canonical schema home per **ADR-0001** (CONFIRMED doctrine; mounted-repo presence NEEDS VERIFICATION). Cross-cutting envelope shapes are tracked in [§5](#5-cross-cutting-and-shared-files).
...
| `schemas/contracts/v1/domains/archaeology/payload_archaeology.schema.json` | Archaeology-discriminated payload variant carried by the cross-cutting `RuntimeResponseEnvelope` and `EvidenceDrawerPayload`. _(Replaces the v1 `archaeology_decision_envelope.schema.json` row — see [§9 Changelog](#9-changelog-v1--v11).)_ | 🟡 PLANNED | ❓ | PROPOSED |

> [!NOTE]
> **Removed from v1:** `archaeology_decision_envelope.schema.json`, `evidence_drawer_payload.schema.json`, `layer_manifest.schema.json`. These are **cross-cutting shapes**, not archaeology-specific. They live at `schemas/contracts/v1/runtime/`, `.../ui/`, and `.../layers/`. See [§5](#5-cross-cutting-and-shared-files).
...
> [!CAUTION]
> **Two policy homes in tension.** Atlas §24.13 lists `policy/sensitivity/archaeology/` only; Directory Rules §12 implies `policy/domains/archaeology/` for general rights / access policy with `policy/sensitivity/<domain>/` reserved for sensitivity transforms. v1.1 of this ledger assumes a split layout (rights & cultural review under `policy/domains/`; sensitivity transforms under `policy/sensitivity/`) and flags the conflict at [OQ-AR-MO-03](#7-open-questions-register).
...
| `policy/sensitivity/archaeology/exact_location_deny.rego` | Deny exact site geometry on public paths (T4); require generalization to T1 with `RedactionReceipt` + `PublicationTransformReceipt`. | 🟡 PLANNED | ❓ | PROPOSED |
| `policy/sensitivity/archaeology/burial_and_human_remains_deny.rego` | Deny burial, human-remains, and sacred-site exposure regardless of source role (T4 forever for public tiers; T3 only under named authorization). | 🟡 PLANNED | ❓ | PROPOSED |
...
| `policy/sensitivity/archaeology/generalization_thresholds.rego` | Enforce `generalization_floor` (H3 r7 **PROPOSED**) and `min_buffer_distance` (5 km **PROPOSED**) for any 3D-hosted archaeology terrain. | 🟡 PLANNED | ❓ | PROPOSED |
| `policy/sensitivity/archaeology/tier_transitions.rego` | Enforce Atlas §24.5.3 tier-transition rules (T4 → T1 requires `RedactionReceipt` + `ReviewRecord` + `PolicyDecision`; T4 → T3 requires named-party agreement). | 🟡 PLANNED | ❓ | PROPOSED |
...
| `tests/domains/archaeology/test_source_descriptor.py` | Source descriptor + source-role validation for archaeology sources; source-role anti-collapse (Atlas §24.9.2). | 🟡 PLANNED | ❓ | PROPOSED |
...
| `tests/domains/archaeology/test_public_no_leak.py` | Public layers, manifests, **tile binaries**, popups, and exports contain no exact-site geometry, sensitive attributes, or PII — not just the styled view (operating contract §22.3 — "no style-only hiding of exact sensitive geometry"). | 🟡 PLANNED | ❓ | PROPOSED |
| `tests/domains/archaeology/test_rights_and_cultural_review.py` | Releases without resolved rights or completed cultural review are denied (`DENY`) or held (`HOLD`). | 🟡 PLANNED | ❓ | PROPOSED |
...
| `tests/domains/archaeology/test_tier_transitions.py` | Tier transitions (T4 → T1, T4 → T2, T4 → T3, T1 → T0) emit required artifacts (`RedactionReceipt` / `ReviewRecord` / `PolicyDecision`). | 🟡 PLANNED | ❓ | PROPOSED |
...
| `tests/domains/archaeology/test_3d_admission.py` | _(If any 3D archaeology surface)_ Every 3D-enabled layer passes the 3D Admission Decision before terrain / globe / plugin construction; `RepresentationReceipt` emitted. Gated by OPEN-DR-10. | 🟡 PLANNED | ❓ | PROPOSED |
...
> [!NOTE]
> Whether negative-case archaeology fixtures live at `fixtures/domains/archaeology/` or `tests/fixtures/domains/archaeology/` is **NEEDS VERIFICATION** ([OQ-AR-MO-11](#7-open-questions-register)). Directory Rules §6.6 names both `tests/` and `fixtures/` as canonical roots; the negative-case fixture home requires clarification.
...
|---|---|---|---|
| `schemas/contracts/v1/runtime/runtime_response_envelope.schema.json` | Cross-cutting finite-outcome wrapper consumed by archaeology surfaces. | 🟡 PLANNED | PROPOSED |
| `schemas/contracts/v1/runtime/decision_envelope.schema.json` | Normalized policy-output envelope (Atlas §24.3 / KFM-P5-PROG-0001) consumed by archaeology policy modules. | 🟡 PLANNED | PROPOSED |
...
| `schemas/contracts/v1/receipts/representation_receipt.schema.json` | Cross-cutting 3D representation receipt (gated by OPEN-DR-10). | 🟡 PLANNED | PROPOSED |
...
| `tools/validate_all.py` _(v1.2 CONFIRMED live-repo path; v1.1 doctrine path was `tools/validators/validate_all.py`; see Directory Rules OPEN-DR-07)_ | Validator orchestrator. | 🟡 PLANNED | NEEDS VERIFICATION |
...
| `policy/maplibre/3d-admission.rego` · `plugin-admission.rego` _(v1.3, OPEN-DR-10)_ | 3D admission policy for archaeology 3D surfaces hosted in `packages/maplibre-runtime/`. | 🟡 PLANNED | PROPOSED |
...
| `apps/explorer-web/` | Public shell (Directory Rules §13.3). | 🟡 PLANNED | PROPOSED |
| `packages/ui/` | Shared UI components consuming archaeology layer / drawer / focus contracts. | 🟡 PLANNED | PROPOSED |
| **`packages/maplibre-runtime/`** _(v1.3, sole renderer; supersedes the v1.2 `packages/maplibre/` + retired `packages/cesium/`)_ | Sole governed browser-side renderer adapter; hosts archaeology 3D surfaces. Gated by Directory Rules §18.e **OPEN-DR-10** ADR acceptance. | 🟡 PLANNED | PROPOSED |
...
> [!CAUTION]
> **v1.3 retirement of Cesium.** The v1 of this ledger listed `packages/cesium/` and `packages/maplibre/` as cross-cutting roots. Directory Rules v1.3 names **MapLibre as the sole browser-side renderer** and retires `packages/cesium*`, `policy/cesium*`, `schemas/contracts/v1/cesium*`, and `contracts/cesium*` pending OPEN-DR-10. v1.1 of this ledger replaces both with `packages/maplibre-runtime/`. Any pre-v1.3 reference is superseded; reintroducing a parallel renderer requires a superseding ADR.
...
|---|---|---|---|
| **ADR-0001** | Canonical schema home (`schemas/contracts/v1/...`) | Every row in [§4.3](#43-schemascontractsv1domainsarchaeology) depends on this. | **CONFIRMED** doctrine / NEEDS VERIFICATION in repo |
| **ADR — Schema/contract `domains/` segment** _(needed)_ | Resolve Directory Rules §12 (`schemas/contracts/v1/domains/archaeology/`) vs. Atlas §24.13 (`schemas/contracts/v1/archaeology/`). | Affects every row in [§4.2](#42-contractsdomainsarchaeology) and [§4.3](#43-schemascontractsv1domainsarchaeology). | **CONFLICTED** — needs ADR ([OQ-AR-MO-02](#7-open-questions-register)) |
...
| **ADR — MapLibre as sole browser-side renderer; retire Cesium** | Per `docs/architecture/maplibre-3d.md` Appendix B; Directory Rules §18.e OPEN-DR-10. | Gates [§4.10](#410-data-lifecycle-phases) 3D archaeology surfaces and all `packages/maplibre-runtime/` references. | **PROPOSED** — pending acceptance |
...
| **ADR — Public geometry thresholds & transform profiles** _(needed)_ | Generalization radii (H3 r7 PROPOSED), suppression thresholds, township-cell vs. county vs. region generalization. | Gates `policy/sensitivity/archaeology/generalization_thresholds.rego` and `packages/domains/archaeology/generalization/`. | **NEEDS VERIFICATION** |
...
| **ADR — Runtime / Decision envelope split** _(needed)_ | Formalize the distinction between `RuntimeResponseEnvelope` (op-contract §9.2 runtime wrapper) and `DecisionEnvelope` (Atlas §24.3 / KFM-P5-PROG-0001 policy-output envelope). | Gates the two new contracts/ docs in [§4.2](#42-contractsdomainsarchaeology) and [OQ-AR-MO-12](#7-open-questions-register). | **NEEDS VERIFICATION** |
...
| OQ-AR-MO-02 | Schema / contract home — `schemas/contracts/v1/domains/archaeology/` (Directory Rules §12) vs. `schemas/contracts/v1/archaeology/` (Atlas §24.13) | Docs steward + domain steward | ADR resolving Directory Rules §12 vs. Atlas §24.13 conflict; drift entry in `docs/registers/DRIFT_REGISTER.md` per §2.5. | **CONFLICTED** |
| OQ-AR-MO-03 | Policy bundle home — `policy/domains/archaeology/` (Directory Rules §12) vs. `policy/sensitivity/archaeology/` (Atlas §24.13) | Docs steward + sensitivity reviewer | Both may co-exist (general rights vs. sensitivity transforms); inspect mounted repo and align with §6.5 + §12. | **NEEDS VERIFICATION** |
| OQ-AR-MO-04 | `RuntimeResponseEnvelope` (op-contract §9.2) vs `DecisionEnvelope` (Atlas §24.3 / KFM-P5-PROG-0001) — formalize both. | Governed API owner + docs steward | ADR documenting the distinction; ensure archaeology contracts/ docs name both. | **PROPOSED** |
| OQ-AR-MO-05 | Generalization floor (H3 r7 PROPOSED) and minimum buffer (5 km PROPOSED) | Sensitivity reviewer + Archaeology domain steward | Confirm in `policy/sensitivity/archaeology/`; ADR if tightened or loosened. | **PROPOSED** |
...
| OQ-AR-MO-08 | Map shell migration state (legacy `ui/`, `web/`, `packages/maplibre/`, `packages/cesium/` vs. `apps/explorer-web/` + `packages/maplibre-runtime/`) | Map shell steward | Per Directory Rules §13.3 v1.3; verify migration progress and any retained `cesium*` segments (§18.e OPEN-DR-11). | **NEEDS VERIFICATION** |
...
| OQ-AR-MO-11 | Fixtures home — `tests/fixtures/domains/archaeology/` vs. `fixtures/domains/archaeology/` | Docs steward | Directory Rules §6.6 names both `tests/` and `fixtures/`; clarify owner for negative-case fixtures. | **NEEDS VERIFICATION** |
...
| OQ-AR-MO-15 | Filename casing within `docs/domains/archaeology/` — UPPERCASE_WITH_UNDERSCORES (topical) vs. lowercase-with-hyphens (planning sub-pages) | Docs steward | Directory Rules §6.1.a addresses `docs/standards/` only; per-folder convention may differ. | **NEEDS VERIFICATION** |
...
| 2 | Define public geometry thresholds and transform profiles. | `policy/sensitivity/archaeology/generalization_thresholds.rego`, `packages/domains/archaeology/generalization/`, `pipeline_specs/archaeology/publish.spec.yaml` | Same. |
...
| 10 | Verify validator orchestrator path — `tools/validate_all.py` (live-repo CONFIRMED at commit `b6a27916...` per Directory Rules §0) vs. doctrine `tools/validators/validate_all.py` (§18 OPEN-DR-07). | All test-orchestrator references. | Repo inspection. |
...
| Reframed `docs/domains/archaeology/` placement as **CONFIRMED** (Directory Rules §6.1 lists the path) | clarification | v1 marked the path PROPOSED only; §6.1's docs tree explicitly lists `docs/domains/archaeology/`. |
| Added [`MAP_UI_CONTRACTS.md`](./MAP_UI_CONTRACTS.md) to [§4.1](#41-docsdomainsarchaeology) | gap closure | Sibling planning artifact authored in this session. |
| Renamed `archaeology_decision_envelope.md` → `runtime_envelope_archaeology_projection.md` + added `decision_envelope_archaeology_projection.md` | reconciliation | Operating contract §9.2 names `RuntimeResponseEnvelope`; Atlas §24.3 / KFM-P5-PROG-0001 names `DecisionEnvelope`. Both are needed; one row was conflating them. |
| Removed cross-cutting schemas from [§4.3](#43-schemascontractsv1domainsarchaeology) (`archaeology_decision_envelope.schema.json`, `evidence_drawer_payload.schema.json`, `layer_manifest.schema.json`) | reconciliation | These shapes are cross-cutting, not archaeology-specific. They live under `schemas/contracts/v1/runtime/`, `.../ui/`, `.../layers/`. Moved to [§5](#5-cross-cutting-and-shared-files). Added `payload_archaeology.schema.json` for the archaeology-discriminated payload variant. |
| Split [§4.4](#44-policydomainsarchaeology--policysensitivityarchaeology) into `policy/domains/archaeology/` (rights & cultural review) and `policy/sensitivity/archaeology/` (sensitivity transforms) | gap closure | Atlas §24.13 names `policy/sensitivity/archaeology/`; Directory Rules §12 implies the split. Flagged at [OQ-AR-MO-03](#7-open-questions-register). |
| Added `tier_transitions.rego` and `generalization_thresholds.rego` to [§4.4](#44-policydomainsarchaeology--policysensitivityarchaeology) | gap closure | Atlas §24.5.3 tier-transition rules require enforceable policy. |
| Added `test_map_release_manifest_closure.py`, `test_tier_transitions.py`, `test_generalization_log_presence.py`, `test_3d_admission.py` to [§4.5](#45-testsdomainsarchaeology) | gap closure | Required test coverage per operating contract §22–§23 and Directory Rules §7.2.a v1.3. |
| Added `synthetic_redaction_receipt/`, `synthetic_map_release_manifest/`, `synthetic_correction_notice/` fixtures to [§4.6](#46-fixturesdomainsarchaeology) | gap closure | Cross-cutting receipts require synthetic exemplars. |
| Added `map_release_manifest.json`, `policy_decision.json`, `review_record.json` to [§4.12](#412-releasecandidatesarchaeology--release-artifacts) | gap closure | Operating contract §23.2 row for archaeology requires `MapReleaseManifest`, `PolicyDecision`, and `ReviewRecord`. |
| Replaced `packages/cesium/` + `packages/maplibre/` with `packages/maplibre-runtime/` in [§5](#5-cross-cutting-and-shared-files) | reconciliation | Directory Rules v1.3 names MapLibre as sole renderer and retires Cesium pending OPEN-DR-10. v1 still listed both. |
| Added cross-cutting receipt / governance / release schemas to [§5](#5-cross-cutting-and-shared-files) | gap closure | `RedactionReceipt`, `RepresentationReceipt`, `ReviewRecord`, `RollbackCard`, `CorrectionNotice`, `MapReleaseManifest`, `kfm_geo_manifest`. |
| Added `tools/validate_all.py` v1.2 reconciliation note | clarification | Directory Rules §0 (v1.2) confirms the live-repo path is one level higher than v1.1 doctrine. |
| Reformatted v1 §6 ADR table to include the **MapLibre sole-renderer** ADR, the **Runtime / Decision envelope** split ADR, and a **`domains/` segment** ADR | clarification | Captures the gating ADRs surfaced during reconciliation. |
| Added §7 Open questions register (`OQ-AR-MO-NN`); split v1 §7 into §7 + §8 (verification backlog at file granularity) | clarification | Doctrine doc companion sections per authoring prompt `<doctrine_doc_companion_sections>`. |
| Added §9 Changelog and §10 Definition of done | housekeeping | Required for doctrine docs by the authoring prompt. |
| Updated truth-label legend to include CONFLICTED | clarification | Aligns with `ai-build-operating-contract.md` §8 extended labels. |

> **Backward compatibility.** Section heading anchors `#1` through `#6` are preserved. The v1 §7 ("Verification backlog (file-level)"), §8 ("Update protocol"), §9 ("Related docs"), §10 ("Appendix") anchors are **renumbered** to §8, §11, §12, §13 respectively because §7 (Open questions register), §9 (Changelog), and §10 (Definition of done) are new. Any external link to the old `#7-verification-backlog-file-level`, `#8-update-protocol`, `#9-related-docs`, or `#10-appendix` will need updating. Flagged for the docs steward.
>
> **Version classification.** **MINOR** per contract §37 — clarifications, reconciliations, and gap closures; no breaking schema or doctrine-invariant changes. Anchor renumbering is a docs-side break, not a doctrine break.
...
- the schema-home conflict ([OQ-AR-MO-02](#7-open-questions-register)) and policy-bundle-home conflict ([OQ-AR-MO-03](#7-open-questions-register)) are either resolved by ADR or logged in `docs/registers/DRIFT_REGISTER.md`;
...
- the `GENERATED_RECEIPT.json` planned in the authoring-run Section 2 is wired into CI;
...
2. For each row:
   ...
   d. If doctrine has moved the file out of scope → mark DEFERRED with a reason and a
      forward link.
3. Update the KFM Meta Block v2 `updated:` date and bump `version:` per contract §37.
...
6. Emit a GENERATED_RECEIPT.json for the doc change per contract §34.
```
...
- [`docs/doctrine/directory-rules.md`](../../doctrine/directory-rules.md) — Directory Rules v1.3 §§4, 6, 9, 12, 15, 16, 18.e
...
- [`docs/architecture/maplibre-3d.md`](../../architecture/maplibre-3d.md) — MapLibre sole-renderer doctrine (PROPOSED via ADR)
...
<details>
<summary><strong>D. Sensitivity tier mapping for archaeology (Atlas v1.1 §24.5.2)</strong></summary>

| Archaeology object class | Default tier | Allowed transforms (PROPOSED) | Required gates |
|---|---|---|---|
| Site location (`ArchaeologicalSite`) | **T4** | Steward review + cultural review + generalized geometry + `RedactionReceipt` → T2 or T1 | `RedactionReceipt` + `ReviewRecord` + `PolicyDecision` |
...
**Tier transitions are reversible** per Atlas §24.5.3.
...
| HOLD | Finite outcome — promotion paused pending review |
...
**Related:** [Directory Rules](../../doctrine/directory-rules.md) · [Operating contract](../../doctrine/ai-build-operating-contract.md) · [Verification Backlog](../../registers/VERIFICATION_BACKLOG.md) · [Drift Register](../../registers/DRIFT_REGISTER.md) · [Archaeology README](./README.md) _(PLANNED)_ · [MAP_UI_CONTRACTS.md](./MAP_UI_CONTRACTS.md)

**Last updated:** 2026-05-27 · **Status:** `draft` · **Version:** v1.1 · **CONTRACT_VERSION:** 3.0.0 · **Next review:** when repo is mounted, or within 6 months — whichever comes first.
