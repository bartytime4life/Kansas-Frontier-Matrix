<!-- [KFM_META_BLOCK_V2]
doc_id: kfm://doc/docs/domains/habitat/SOURCE_REGISTRY
title: Habitat Domain — Source Registry
type: standard
version: v1
status: draft
owners: TODO — habitat domain steward; source-registry steward
created: 2026-05-17
updated: 2026-05-17
policy_label: public
related:
...
**Status:** draft &nbsp;·&nbsp; **Owners:** TODO — habitat domain steward; source-registry steward &nbsp;·&nbsp; **Last updated:** 2026-05-17
...
4. [What belongs here — and what does not](#4-what-belongs-here--and-what-does-not)
5. [Source-role vocabulary](#5-source-role-vocabulary)
6. [Habitat source families](#6-habitat-source-families)
7. [Sensitivity posture](#7-sensitivity-posture)
8. [Admission lifecycle](#8-admission-lifecycle)
9. [Watcher discipline](#9-watcher-discipline)
10. [Schema, policy, and validator surfaces](#10-schema-policy-and-validator-surfaces)
11. [Cross-domain joins](#11-cross-domain-joins)
12. [Verification backlog and open questions](#12-verification-backlog-and-open-questions)
13. [Related docs](#13-related-docs)
14. [Appendix A — Source-descriptor field reference](#appendix-a--source-descriptor-field-reference-proposed)
15. [Appendix B — Registry entry template](#appendix-b--registry-entry-template-proposed)
...
- the **declared role** each source family is allowed to play (authority, observation, context, model),
...
> [!IMPORTANT]
> **Source role cannot be inferred from convenience.** A source useful for one role is not automatically admissible for another. Role is set at admission, recorded in the `SourceDescriptor`, and never edited in place — corrections must produce a new descriptor and a `CorrectionNotice`. *(CONFIRMED doctrine — Unified Manual §3.6; Atlas §24.1.3.)*
...
| Machine-readable registry home | `data/registry/sources/habitat/` | **PROPOSED** — Directory Rules §12. |
...
## 4. What belongs here — and what does not
...
- **Source-role declarations** per family (authority / observation / context / model).
...
KFM declares a fixed source-role enumeration that constrains what a source is **allowed to support**. The Habitat lane uses the project-wide enum without extension.
...
### 6.1 Land cover, vegetation, and ecological systems
...
| **NatureServe ecological systems** | `administrative` / `modeled` | Ecological system classifications and controlled biodiversity context. | CONFIRMED listing / **PROPOSED** activation; rights **NEEDS VERIFICATION**. |
...
### 6.2 Regulatory and stewardship overlays
...
| **USFWS NWI** (National Wetlands Inventory) | `regulatory` / `observed` | Wetland inventory and riparian habitat context. | CONFIRMED listing / **PROPOSED** activation. |
...
### 6.3 Biodiversity and occurrence inputs (joined, not owned)
...
| **NatureServe / heritage-style biodiversity sources** | `administrative` / `regulatory` | Conservation status and rare-element context. | CONFIRMED listing / **PROPOSED** activation; **sensitive — see §7**. |

### 6.4 Conservation, restoration, and project inventories
...
Habitat layers can reveal sensitive species context when joined to occurrence records, and several Habitat source families themselves carry sensitivity controls. The registry encodes the posture below; enforcement happens in policy and validators.

| Class | Default outcome | Required controls | Source basis |
|---|---|---|---|
| Exact occurrence-linked habitat outputs | **DENY** public exact precision | Generalization, redaction, steward review, geoprivacy transform receipt | *Unified Manual §6.3; Atlas §I (Habitat).* |
| Rare-species exact location, nest / den / roost / hibernacula / spawning sites (via Fauna join) | **DENY** public exact location | Geoprivacy transform receipt; steward review | *Encyclopedia §13; Atlas §I (Fauna).* |
...
| Habitat sensitivity policy (occurrence joins) | `policy/sensitivity/fauna/` and `policy/domains/habitat/` | **PROPOSED**. |
...
> [!NOTE]
> Cross-domain validators that span habitat × fauna × hydrology live under `tools/validators/<topic>/...` (Directory Rules §12 "Multi-domain and cross-cutting files"), **not** under a single domain segment.
...
|---|---|---|---|
| 1 | Verify official **critical habitat source descriptors** and current USFWS ECOS access terms. | mounted repo files; live descriptor under `data/registry/sources/habitat/`; rights record | **NEEDS VERIFICATION** *(Atlas §N — Habitat)* |
| 2 | Verify **NLCD / LANDFIRE / GAP** current versions, classmaps, redistribution terms, and watcher fixtures. | mounted descriptors; classmap registry entry; watcher fixture pair | **NEEDS VERIFICATION** |
| 3 | Verify **NWI** retrieval method, license, and version pinning for Kansas extent. | live descriptor; rights record | **NEEDS VERIFICATION** |
| 4 | Verify **PAD-US** stewardship version and attribution requirements. | live descriptor; attribution string in `LayerManifest` | **NEEDS VERIFICATION** |
| 5 | Verify **NatureServe** controlled-biodiversity rights state and admissible roles for public surfaces. | rights record; activation decision | **NEEDS VERIFICATION** |
| 6 | Verify **sensitive-occurrence policy** and **geoprivacy transforms** that apply when Habitat joins to Fauna. | `policy/domains/habitat/` and `policy/sensitivity/fauna/`; redaction-receipt fixtures | **NEEDS VERIFICATION** *(Atlas §N — Habitat)* |
| 7 | Verify **model-card requirements** for suitability and connectivity products. | `ModelRunReceipt` schema; model-card template; fixtures | **NEEDS VERIFICATION** *(Atlas §N — Habitat)* |
| 8 | Verify **Habitat MapLibre overlay registry** and Focus Mode behavior for habitat layers. | `LayerManifest` set; Focus Mode fixtures; renderer boundary tests | **NEEDS VERIFICATION** *(Atlas §N — Habitat)* |
| 9 | Confirm `SourceDescriptor` / `SourceIntakeRecord` / `DriftSummary` / `SourceActivationDecision` schema paths and field names. | mounted schema files | **NEEDS VERIFICATION** |
| 10 | Decide whether watcher subfolder convention follows `tools/ingest/<watcher>/` or `pipelines/ingest/` for Habitat-specific watchers. | accepted ADR or local README | **OPEN** |
| 11 | Confirm Habitat-specific runbook naming aligns with the established `docs/runbooks/<domain>/` pattern (cf. `docs/runbooks/fauna/SOURCE_REFRESH_RUNBOOK.md`). | mounted runbook directory; runbook ADR | **OPEN** |
| 12 | Confirm whether this document should be `SOURCE_REGISTRY.md` or aligned to a different filename convention used by Hydrology / Soil. | repo inspection; ADR on registry document naming | **OPEN** |
...
- [`docs/domains/habitat/SOURCE_REFRESH_RUNBOOK.md`](./SOURCE_REFRESH_RUNBOOK.md) — Habitat source refresh runbook *(TODO if not yet present; cf. Fauna companion)*
...
| `sensitivity` | structured sensitivity class | always | Deny-by-default classes from Encyclopedia §13. |
...
| `admissibility_limits` | structured constraints | always | `not_authoritative_for` declarations, scope caps. |
...
| Sensitivity | <class from Encyclopedia §13>
...
<a href="./SOURCE_REFRESH_RUNBOOK.md">Habitat refresh runbook</a> ·
<a href="../../sources/SOURCE_DESCRIPTOR_STANDARD.md">Descriptor standard</a> ·
<a href="../../standards/PROV.md">PROV profile</a> ·
<a href="../../../directory-rules.md">Directory Rules</a>
</sub>

<sub><strong>Last updated:</strong> 2026-05-17 &nbsp;·&nbsp; <strong>Owners:</strong> TODO &nbsp;·&nbsp; <a href="#contents">⬆ back to top</a></sub>
