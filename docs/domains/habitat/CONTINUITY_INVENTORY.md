...
related:
  ...
  - docs/domains/habitat-fauna/THIN_SLICE.md         # PROPOSED placement — verify
...
![evidence posture: doctrine%20%2F%20lineage](https://img.shields.io/badge/evidence-doctrine%20%2F%20lineage-7a3fc4)
![truth labels: CONFIRMED %2F PROPOSED %2F NEEDS%20VERIFICATION](https://img.shields.io/badge/truth%20labels-CONFIRMED%20%2F%20PROPOSED%20%2F%20NV-555)
![lifecycle: RAW%E2%86%92PUBLISHED](https://img.shields.io/badge/lifecycle-RAW%20%E2%86%92%20PUBLISHED-2c8a3d)
![directory rules: §12 Domain Placement Law](https://img.shields.io/badge/dir--rules-%C2%A712%20domain%20lane-1f6feb)
![ci: TODO](https://img.shields.io/badge/ci-TODO-lightgrey)

**Status:** draft &nbsp;·&nbsp; **Owners:** `<habitat-domain-steward>` + `<docs-steward>` *(placeholders — verify)* &nbsp;·&nbsp; **Last updated:** 2026-05-17
...
> [!IMPORTANT]
> This document is a **lineage register**, not a status report. Every implementation-shaped statement here is PROPOSED or NEEDS VERIFICATION until a mounted repo, schema, test, workflow, manifest, or release artifact confirms otherwise. See [`docs/doctrine/directory-rules.md`](../../doctrine/directory-rules.md) §0 and §2.1 for authority order.
...
> [!NOTE]
> The doctrinal anchors above are **named in attached project documents**. Their *file presence in the repository* is PROPOSED until verified.
...
> [!TIP]
> **No prior Habitat gain is currently classified `RETIRE`.** All known prior planning is either being carried forward or held pending verification. If a future ADR retires a surface, list it here with a forward link.
...
> [!NOTE]
> Diagram reflects the doctrinal flow of this inventory, not a live workflow. The repo nodes (`Repo placement`) are PROPOSED targets governed by [Directory Rules §12 Domain Placement Law](../../doctrine/directory-rules.md#12-domain-placement-law).
...
| Critical-habitat view | PROPOSED | **KEEP AS LINEAGE** | Pending USFWS source-descriptor verification. |
| Modeled-habitat view | PROPOSED | **KEEP AS LINEAGE** | Requires model card + `UncertaintySurface`. |
...
> [!NOTE]
> MapLibre is the disciplined 2D renderer behind an adapter. It is **never** truth, policy, publication, or citation authority for Habitat. Cesium / 3D, if added later, consumes the same `EvidenceBundle` and `DecisionEnvelope` as the 2D path. `[MAP-MASTER]`, `[DIRRULES §11]`
...
Habitat follows the canonical lifecycle: **RAW → WORK / QUARANTINE → PROCESSED → CATALOG / TRIPLET → PUBLISHED**. Promotion is a governed state transition, not a file move. `[DIRRULES §0]`, `[ATLAS §6.H]`
...
Cross-domain shared artifacts (e.g., a habitat × fauna × hydrology validator) live at the **lowest common responsibility root without a domain segment**, per `[DIRRULES §12]` — not under `tools/validators/domains/habitat/`.
...
| Public-safe Habitat products require a `Redaction Receipt` (generalization or redaction transform) when sensitive joins are involved. | **CONFIRMED doctrine** | **KEEP AND EXTEND** |
...
> [!WARNING]
> The **deny-by-default** posture is non-negotiable in Habitat for any output that touches sensitive Fauna context. An adapter, viewer, or AI surface that widens visibility without a transform receipt and review record is a publication-gate violation.
...
> [!TIP]
> The thin slice is the right vehicle for converting this Continuity Inventory's `KEEP AND EXTEND` rows into the first Habitat PR. Drive selection of AOI, source descriptor, and fixture set off of `[DOM-HF]` plus `KFM-IDX-PLN-003` (domain lanes as proof-bearing slices) before authoring contracts and schemas.
...
These items are explicitly **NEEDS VERIFICATION**. Each must be settled by mounted-repo evidence, schema files, registry entries, tests, logs, emitted artifacts, review records, or release manifests before being promoted from PROPOSED to CONFIRMED.

| # | Item | Evidence that would settle it | Mirrors |
|---:|---|---|---|
| 1 | Official **critical-habitat** source descriptors (USFWS ECOS). | `data/registry/sources/habitat/` entries + rights record. | `[ATLAS §6.N]` |
| 2 | Sensitive-occurrence policy and **geoprivacy transforms**. | `policy/domains/habitat/` files + tests under `tests/domains/habitat/policy/`. | `[ATLAS §6.N]`, `[DOM-HF]` |
| 3 | **Model-card** requirements for suitability products. | `contracts/domains/habitat/` model-card contract + fixture. | `[ATLAS §6.N]` |
| 4 | Habitat **MapLibre overlay registry** and Focus behavior. | `LayerManifest` schema + viewer registry binding. | `[ATLAS §6.N]`, `[MAP-MASTER]` |
| 5 | **Schema-home convention** for Habitat objects (canonical `schemas/contracts/v1/domains/habitat/` per ADR-0001). | Mounted repo `schemas/` inspection. | `[DIRRULES §7.4]`, ADR-0001 |
| 6 | **Source rights** and snapshot cadence for NLCD, NWI, GAP/LANDFIRE, NatureServe, GBIF, iNaturalist, iDigBio, PAD-US, KDWP. | Source-descriptor records + license review. | `[DOM-HAB §3]`, `[UNIFIED §30.3]` |
| 7 | **Identity-rule** for each Habitat object (`source id + object role + temporal scope + normalized digest`). | Deterministic-identity test pack. | `[ATLAS §6.E]` |
| 8 | **`Redaction Receipt`** schema and storage path for public-safe Habitat products. | Schema + fixture + release dry-run. | `[ATLAS §6.K]`, `[DOM-HF]` |
| 9 | **Evidence Drawer Habitat-panel** payload schema. | `schemas/contracts/v1/ui/evidence_drawer_payload.schema.json` Habitat sub-section. | `[MAP-MASTER]`, `[UIAI Appendix]` |
| 10 | **Cross-lane validator** placement (Habitat × Fauna). | Mounted-repo `tools/validators/<topic>/` inspection. | `[DIRRULES §12]` |

All ten items SHOULD be mirrored as entries in `docs/registers/VERIFICATION_BACKLOG.md` (path PROPOSED — verify on the mounted repo).
...
1. **Habitat schema home.** Default per ADR-0001 is `schemas/contracts/v1/domains/habitat/`. Confirm or amend. `[DIRRULES §7.4]`
2. **Habitat × Fauna join location.** Cross-domain validator and join contracts — under a non-domain responsibility root (`tools/validators/<topic>/`, `schemas/contracts/v1/<topic>/`), not under either domain's segment. `[DIRRULES §12]`
3. **`Redaction Receipt` canonical home.** `data/receipts/` vs. `release/` — and required fields for Habitat-side receipts.
...
6. **Sensitive-occurrence redaction thresholds.** Geographic generalization radii, temporal generalization windows, and review-state requirements per taxon class.
...
- `docs/domains/habitat-fauna/THIN_SLICE.md` — Habitat × Fauna thin-slice plan *(PROPOSED placement)*
...
- `docs/standards/PROV.md` — W3C PROV-O profile, used by Habitat receipts *(PROPOSED placement)*
...
- `docs/registers/DRIFT_REGISTER.md` — placement drift register *(PROPOSED placement)*
- `docs/adr/ADR-0001-schema-home.md` — schema-home decision *(referenced by Directory Rules)*
...
1. **Continuity register placement.** Is `docs/domains/<domain>/CONTINUITY_INVENTORY.md` the canonical home for per-domain continuity inventories, or should a single cross-domain `docs/registers/CONTINUITY_INVENTORY.md` index them with per-domain sub-files? *Tracked here pending ADR.* `[DIRRULES §6.1, §12]`
...
4. **Habitat AOI for first thin slice.** Which Kansas AOI offers the right combination of NLCD/NWI coverage, low sensitivity, and review feasibility? `[DOM-HF]`, Pass-19 `KFM-IDX-PLN-003`.
5. **`Redaction Receipt` field surface.** Are generalization radii, transform algorithm, and reviewer identity all required fields, or only some? `[ATLAS §6.K]`
6. **Cross-lane "habitat assignment" object home.** Habitat × Fauna joins emit a habitat-assignment record. Does this belong to Habitat, Fauna, or a cross-lane responsibility root? Captured under §14 item 2.
7. **Suitability-model retraining cadence.** When NLCD or LANDFIRE versions roll forward, what is the stale-state policy for Habitat suitability outputs?
...
|---|---|---|
| `[DIRRULES]` | §§1, 2, 5, 8, 9, 10, 14, 15 | Placement law, domain-segment-under-responsibility-root pattern, lifecycle law, watcher-as-non-publisher. |
...
| ADR-0001 (schema home) | §§13, 14, 15 | `schemas/contracts/v1/` canonical schema home. |
...
**Related docs (short list):** [`docs/domains/habitat/README.md`](./README.md) *(PROPOSED)* · [`docs/domains/habitat-fauna/THIN_SLICE.md`](../habitat-fauna/THIN_SLICE.md) *(PROPOSED)* · [`docs/doctrine/directory-rules.md`](../../doctrine/directory-rules.md) *(CONFIRMED doctrine; path PROPOSED)* · [`docs/registers/VERIFICATION_BACKLOG.md`](../../registers/VERIFICATION_BACKLOG.md) *(PROPOSED)*

**Last updated:** 2026-05-17 &nbsp;·&nbsp; **Doc class:** standard &nbsp;·&nbsp; **Status:** draft &nbsp;·&nbsp; **Version:** v1
