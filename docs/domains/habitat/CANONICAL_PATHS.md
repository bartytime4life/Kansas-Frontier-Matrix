...
[![Paths: PROPOSED](https://img.shields.io/badge/paths-PROPOSED-orange.svg)](#11-open-questions--needs-verification)
[![Schema home: schemas/contracts/v1](https://img.shields.io/badge/schema--home-schemas%2Fcontracts%2Fv1-blue.svg)](../../doctrine/directory-rules.md#74-pipelines-and-pipeline_specs)
...
[![Last updated](https://img.shields.io/badge/updated-2026--05--17-informational.svg)](#footer)

> **Status:** draft &nbsp;·&nbsp; **Owners:** Habitat domain steward + Docs steward &nbsp;·&nbsp; **Updated:** 2026-05-17
...
| **Schema-home convention** | `schemas/contracts/v1/domains/habitat/` per ADR-0001. |
...
|---|---|---|
| Directory Rules | §§ 4–5 (Placement Protocol, Canonical Root Tree) | The 24 responsibility roots and their authority class. |
| Directory Rules | § 6.4 (`schemas/`) | Schema home: `schemas/contracts/v1/…` per ADR-0001. |
| Directory Rules | § 9 (`data/`) | Lifecycle phases and what each phase MUST NOT do. |
| Directory Rules | § 12 (Domain Placement Law) | Habitat goes **inside** responsibility roots, never as a root. |
| Directory Rules | § 13 (Anti-Patterns) | Drift patterns this file is meant to prevent for Habitat. |
| ADR-0001 | Schema home | `schemas/contracts/v1/...` is canonical; `contracts/<domain>/<x>.schema.json` is lineage / CONFLICTED. |
...
flowchart LR
  subgraph GOV["Governance · CONFIRMED roots"]
    ...
    SCH["schemas/contracts/v1/<br/>domains/habitat/"]
...
| 4.3 | `schemas/contracts/v1/domains/habitat/` | `schemas/` | Machine-checkable shape (JSON Schema). **Canonical schema home per ADR-0001.** | PROPOSED |
...
| 4.15 | `data/triplets/` (shared) | `data/triplets/` | Habitat relationship projections (patch-edge, habitat-assignment) feed shared graph deltas; **no canonical replacement semantics**. | PROPOSED |
...
Habitat-specific reading of the lifecycle invariant. Each phase's rules come from Directory Rules §9.1; the `<segment>` column shows the Habitat-bearing subpath. All paths are **PROPOSED** as repo presence; the **rules are CONFIRMED**.
...
- **`data/proofs/evidence_bundle/…habitat…`** must include a `Redaction Receipt` for any release of a Habitat layer derived from sensitive sources.
...
| Habitat owns → Planetary/3D consumes | `docs/architecture/map-shell.md` | Habitat patches admitted to 3D scenes **only** via generalized geometry; sensitive habitat denied. | `packages/cesium/` consumes `EvidenceBundle` and `DecisionEnvelope`, not direct raster handles. |
...
| A2 | **Parallel schema home** (§13.1) | `contracts/domains/habitat/habitat_patch.schema.json` exists alongside `schemas/contracts/v1/domains/habitat/habitat_patch.schema.json`. | Per ADR-0001, `schemas/contracts/v1/...` is canonical. Freeze the duplicate as `mirror` or migrate. `contracts/domains/habitat/` retains semantic Markdown only. |
...
- **NEEDS VERIFICATION** — Whether `contracts/` or `schemas/contracts/v1/` is the live machine-schema authority for Habitat in the current repo. Default per ADR-0001 is `schemas/contracts/v1/`; mounted-repo inspection or a drift entry should resolve this.
...
- **NEEDS VERIFICATION** — Whether `data/triplets/` is plural or singular in the current repo. Directory Rules §18 currently uses `triplets/`; a one-line ADR is recommended to freeze the form.
...
- `docs/adr/ADR-0001-schema-home.md` — Authoritative schema-home rule.
...
- `docs/standards/PROV.md` — Provenance profile referenced by Habitat EvidenceBundles.
...
- `docs/registers/DRIFT_REGISTER.md` — Where to file path drift discovered against this doc.
...
**Last updated:** 2026-05-17 &nbsp;·&nbsp; **Doc class:** standard / domain canonical-paths reference &nbsp;·&nbsp; **Status:** draft
