...
Evidence basis
- Primary: `KFM_Domains_Culmination_Atlas_v1_1.pdf` — §6 Habitat (sections A–N including J. API/contract/schema surfaces), §20.3 Master API Surface Table, §24.1 Master Source-Role Anti-Collapse Register, §24.3 Master Decision Outcome Envelope Reference (outcome classes + outcome × surface mapping), §24.4 Master Cross-Lane Relation Atlas, §24.5 Master Sensitivity / Rights Tier Reference (tier scheme + per-domain matrix + tier transitions), §24.6 Master Pipeline Gate Reference, §24.13 Atlas Section ↔ Dossier ↔ Responsibility Root Crosswalk.
- Primary: `directory-rules.md` (mounted, inspected directly) — §4 Placement Protocol Step 3 (domain as segment), §6.1 `docs/` layout (confirms `docs/domains/habitat/` is the canonical doc home), §6.3 `contracts/`, §6.4 `schemas/` and ADR-0001 schema-home rule, §6.5 `policy/`, §7.1 `apps/governed-api/` as the trust membrane, §9 `data/` lifecycle invariant, §12 Domain Placement Law (with full habitat lane pattern enumerated), §13 anti-patterns (style-hiding is not redaction; watcher-as-non-publisher).
- Primary: `kfm_encyclopedia.pdf` — §7.4 Habitat mission/boundary, object families, spatial/temporal model, map layers, user actions, analytics, knowledge systems, AI behavior table, J. API/contract/schema possibilities row, K. Tests and validators, L. Feature backlog, M. Risks and mitigations.
- Primary: `KFM_Whole_UI_Governed_AI_Expansion_Report.pdf` — `apps/governed-api/` route file structure (PROPOSED), `apps/explorer-web/` governed client, response validators, Evidence Drawer + Focus finite-outcome rendering.
...
- Primary: `KFM_Unified_Implementation_Architecture_Build_Manual.pdf` — §3.5 schema/contract split with ADR-0001 reference, §6.3 Habitat lane scope/sensitivity/objects/pipelines/gates, the cross-lane note that habitat outputs joined to occurrence records may need generalization or denial.
- Secondary: prior conversation — Flora API_CONTRACTS.md drafted under the same prompt pattern; used as a structural mirror for symmetric Habitat treatment, with Habitat-specific content drawn from the KFM corpus.

Major project constraints captured
- Domain Placement Law (DIRRULES §12): `habitat` is a segment under each responsibility root, never a root folder. The directory-placement table is enumerated literally from the lane pattern (§16).
- Trust membrane (DIRRULES §7.1; KFM-IDX-API-001): public clients and normal UI surfaces consume the governed API, never canonical/internal stores; co-location does not relax the membrane.
- Lifecycle invariant: `RAW → WORK/QUARANTINE → PROCESSED → CATALOG/TRIPLET → PUBLISHED`; promotion is a governed state transition, not a file move.
- Finite outcomes for caller-facing surfaces (`ANSWER` / `ABSTAIN` / `DENY` / `ERROR`); validator-internal surfaces additionally use `PASS` / `FAIL` / `HOLD`.
- Source-role anti-collapse (Atlas §24.1): `observed` / `regulatory` / `modeled` / `aggregate` / `administrative` / `candidate` / `synthetic` must never be conflated. Habitat-acute: `SuitabilityModel` ≠ Regulatory critical habitat; NLCD ≠ regulatory; aggregate ≠ per-place.
- Sensitivity tier scheme T0–T4 (Atlas §24.5); upgrades require both a transform receipt and a review record; downgrades to T4 always permitted via `CorrectionNotice` alone.
- Cite-or-abstain default for AI; uncited authoritative claims → `DENY`; no chain-of-thought persisted as truth (GAI doctrine).
- Style-only hiding is not redaction; denials happen at API and tile-artifact level (Master MapLibre sensitive-geometry-deny pattern; DIRRULES §13.5).
- KFM is never an alert authority; emergency-alert framing → `DENY` (DOM-HAZ + Atlas §24.5).
- Watcher-as-non-publisher invariant (DIRRULES §13.5; KFM-IDX-PLN-004).
- Separation of duties when materiality justifies it (Atlas §24.3.2 review-queue forbidden behavior).
- Schema-home rule (ADR-0001): `schemas/contracts/v1/...` is canonical; `contracts/` retains semantic Markdown only.
- KFM-specific casing preserved exactly: `EvidenceBundle`, `EvidenceRef`, `HabitatPatch`, `LandCoverObservation`, `EcologicalSystem`, `HabitatQualityScore`, `SuitabilityModel`, `ConnectivityEdge`, `Corridor`, `RestorationOpportunity`, `StewardshipZone`, `ModelRunReceipt`, `UncertaintySurface`, `RedactionReceipt`, `AggregationReceipt`, `RuntimeResponseEnvelope`, `AIReceipt`, `ReleaseManifest`, `RollbackCard`, `PolicyDecision`, `CorrectionNotice`, `ReviewRecord`, `SourceDescriptor`, `ValidationReport`, `CatalogMatrix`.

What was CONFIRMED vs INFERRED vs PROPOSED vs UNKNOWN / NEEDS VERIFICATION
- CONFIRMED (doctrine): trust-membrane principle (§2); finite-outcome envelope grammar shape and the master outcome enum (§4); source-role anti-collapse rules and the seven canonical source-role classes (§11); sensitivity tier scheme T0–T4 and the transform/review-record motion rules (§12); cross-lane ownership boundaries (§13); lifecycle invariant and gate ordering (§14); validator/test families list (§15); Domain Placement Law and ADR-0001 schema-home rule (§16); Habitat ownership boundary (owns HabitatPatch, LandCoverObservation, EcologicalSystem, etc.; does not own occurrence/plant taxonomy); style-hiding-is-not-redaction; KFM-not-alert-authority; watcher-as-non-publisher.
- INFERRED: Habitat-specific reason codes (modeled_as_critical_denied, regulatory_as_observed_denied, aggregate_as_perplace_denied, candidate_as_published_denied, synthetic_as_observed_denied, cross_lane_sensitivity_join, cross_lane_occurrence_leak, etc.) — derived from the master anti-collapse rule, cross-lane rule, and Habitat-acute risk patterns named in [ENCY §24.1.2]. Labeled PROPOSED in the doc.
- PROPOSED: HabitatDecisionEnvelope as a per-domain extension of the master DecisionEnvelope (Atlas marks it PROPOSED at §6.J); illustrative envelope JSON; route paths (`GET /api/v1/domains/habitat/features/{id}`, etc.); reason-code register; the seven surface inventory rows; the five review queues; thin-slice AOI; mock fixture identifiers; the proposed schema filenames; all specific file paths under contracts/, schemas/, policy/, tests/, fixtures/, pipelines/, pipeline_specs/, data/, release/.
- UNKNOWN: exact route paths in the governed API; mounted-repo presence of any habitat lane file; final reason-code enumeration; live source rights status; geoprivacy transform parameters; watcher state placement; Habitat MapLibre overlay registry shape; Habitat × Fauna thin-slice AOI selection; CI badge target URL; owner of the Habitat lane.
- NEEDS VERIFICATION: rights, cadence, and source-role attribution for USFWS ECOS, KDWP review context, NLCD, NWI, GAP/LANDFIRE, NatureServe, GBIF/iNaturalist/iDigBio, PAD-US (Atlas flags all of these NEEDS VERIFICATION at [DOM-HAB §D]); model-card requirements for suitability products; cross-cutting AI schema home (`schemas/contracts/v1/ai/`) vs domain-bound.

Repository evidence checked (mounted or indexed); what could not be checked
- Mounted (read directly): `/mnt/project/directory-rules.md` — §4 Step 3, §6.1 docs/ layout, §6.3 contracts/, §6.4 schemas/ (ADR-0001), §6.5 policy/, §7.1 apps/, §9 data/, §12 Domain Placement Law (full habitat lane pattern enumerated), §13 anti-patterns.
- Indexed (project_knowledge_search): KFM_Domains_Culmination_Atlas_v1_1.pdf, kfm_encyclopedia.pdf, KFM_Whole_UI_Governed_AI_Expansion_Report.pdf, Master_MapLibre_Components-Functions-Features.pdf, KFM_Pass_20_Part_2_Idea_Index_Category_Atlas_and_Expansion_Dossier.md, KFM_Unified_Implementation_Architecture_Build_Manual.pdf, New_Ideas_5-8/5-10/5-15-26.pdf, DomainDriven_Design_Reference.pdf.
- Not checked (not available in this session): actual `apps/governed-api/` route source; actual `schemas/contracts/v1/domains/habitat/` schema files; actual `contracts/domains/habitat/` semantic contracts; CI workflow files; ADR-0001 text; mounted source-activation decisions; existing peer documents (`docs/domains/habitat/README.md`, `MAP_UI_CONTRACTS.md`, `SOURCES.md`, `SENSITIVITY.md`, `sublanes/ecoregions.md`, `REASON_CODES.md`, `docs/architecture/governed-api.md`, `docs/architecture/contract-schema-policy-split.md`, `docs/doctrine/trust-membrane.md`, `docs/doctrine/lifecycle-law.md`, `docs/runbooks/habitat/SOURCE_REFRESH_RUNBOOK.md`) — all referenced as PROPOSED.

External sources consulted
- None. No external trigger fired: every claim in the document is either project doctrine (Atlas v1.1, Encyclopedia, Build Manual, Whole-UI Report, MapLibre Master, Pass 20 idea index, Directory Rules) or clearly labeled PROPOSED / NEEDS VERIFICATION pending repo evidence or ADR. External standards (W3C PROV, OGC API, STAC, etc.) are referenced only via existing KFM standards docs (`docs/standards/PROV.md`), not researched directly.
...
- All eleven items in §17 (Open questions and verification backlog) remain open; each is paired with a concrete piece of evidence that would settle it.
...
Major additions or restructures and why (revision-from-prior-draft pass)
- Fixed nested HTML-comment bug in the KFM Meta Block. The prior draft contained `<!-- [KFM_META_BLOCK_V2]` with an embedded `<!-- TODO confirm -->` inside the same comment — HTML comments cannot nest, which would corrupt rendering. Reworked to a single outer comment and a hash-style `# TODO confirm` for the inline annotation.
- Fixed two Mermaid syntax bugs in §14: (1) the node id `hold` collided with the edge label `hold` on the same diagram (`relgate -- hold --> hold`); renamed the node to `heldstate`. (2) Arrow labels with unquoted parentheses (`pub -. rollback (RollbackCard) .-> relgate`) can confuse Mermaid's parser; quoted the labels and renamed `rel` to `relgate` to avoid any confusion with the schema-side relation language.
- Fixed §2 Mermaid arrow label by quoting `"governed reads via release/policy"` to avoid forward-slash parser interactions.
- Fixed doctrinal slip in §5 example envelope. The prior draft had `HabitatPatch` inlining `quality_score: 0.62`, but `HabitatQualityScore` is a separate canonical object family per [DOM-HAB §E]. Replaced with `habitat_quality_score_ref`, added an explicit `uncertainty_surface_ref`, and added `model_run_receipt_ref` (since the example carries `source_role: modeled`). Added a callout explaining the reference boundary.
- Normalized §3 Status column to consistent `PROPOSED — <note>` form and replaced ASCII slashes between outcomes with middle-dots for readability.
- Promoted §8.2 from a bulleted list to a table for visual rhythm and parity with §11/§15.
- Added §9 "Required attachments" sub-list and §10 "Queues" sub-table to give those previously-thin sections proportional weight.
- Standardized "Regulatory critical habitat" (the KFM ubiquitous-language term per Atlas §6.C) as the proper capitalized term; left the lowercase `regulatory` for the `source_role` enum value to preserve KFM's distinction between term-of-art and enum-value.
- Added §13 row for Agriculture (Atlas §24.4 names habitat-agriculture context as a relation type).
- Added §15 row for separation-of-duties (rounds out the negative-test list).
- Reorganized §18 into labeled subsections (lane peers, cross-lane peers, architecture/doctrine, governance, implementation roots) so a reader can scan by purpose; this preserves all prior links and adds three (REASON_CODES.md, trust-membrane.md, lifecycle-law.md, runbooks/habitat/SOURCE_REFRESH_RUNBOOK.md, tests/domains/habitat/, fixtures/domains/habitat/).
- Added Q-11 to §17 (drawer route shape — domain-bound vs cross-cutting) because §7 says `GET /api/v1/evidence/{evidence_ref}` (cross-cutting) while §3 lists the drawer as a Habitat-named surface. That tension warranted explicit listing rather than silent paper-over.
- Added §16 rows for `pipelines/domains/habitat/`, `pipeline_specs/habitat/`, `data/registry/sources/habitat/`, and `release/candidates/habitat/` because [DIRRULES §12]'s habitat-lane pattern enumerates them and the prior draft had partial coverage.
...
- Internal links to peer docs — reachable relative paths inside `docs/domains/habitat/` but not confirmed to exist; flagged with `(PROPOSED)` in the link list.

Anchor or link breakage risk (revisions only)
- This is a revision of an in-conversation draft, not a previously-merged doc; no external anchors are known to depend on it. Section anchors (`#1-purpose-and-scope` through `#18-related-docs`) and the top anchor (`#habitat--api-contracts`) are preserved from the prior draft, so internal back-to-top and quick-jump links continue to resolve.

Polish elements rendered as placeholders due to thin evidence
- Status / Owners / Build / CI badges — placeholder values; specific badge endpoints (`status`, `policy_label`, `lifecycle`, `schemas`, `CI`) labeled as orientation rather than live links.
- Owner names left as `<habitat-steward>` with a `TODO confirm` marker (now in a hash-style inline note rather than a nested HTML comment).
...
- Reason-code register inside the collapsible `<details>` block is a PROPOSED extension; no canonical reason-code list exists in the indexed corpus yet.
