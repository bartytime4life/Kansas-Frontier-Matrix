...
version: v1
status: draft
owners: TODO — Habitat domain steward; Docs steward
created: 2026-05-17
updated: 2026-05-17
policy_label: public
related:
...
[![Last reviewed](https://img.shields.io/badge/last%20reviewed-2026--05--17-lightgrey)](#)
[![CI: TODO](https://img.shields.io/badge/CI-TODO-lightgrey)](#)

**Status:** Draft  ·  **Owners:** TODO — Habitat domain steward; Docs steward  ·  **Updated:** 2026-05-17
...
> The KFM-canonical object families remain **`HabitatPatch`**, **`LandCoverObservation`**, **`EcologicalSystem`**, and the Flora-owned **`VegetationCommunity`**. This sublane is a **documentation-layer grouping** over those object families, not a replacement vocabulary.
...
| Concern | KFM object family (CONFIRMED) | Sublane treatment |
|---|---|---|
| Habitat areas as typed polygons or cells | `HabitatPatch` | Identity, vintage, source role, public-safe geometry |
| Land cover observations (e.g., NLCD class assignments) | `LandCoverObservation` | Source-role distinction; classifier version; effective time |
| Ecological system classification (NatureServe / GAP / LANDFIRE) | `EcologicalSystem` | Crosswalk, model-vs-observation labels, vintage |
| Generalized public-safe derivatives | (derived layers) | Generalization, sensitivity transforms, release manifests |
| Vintage / classifier-version labeling | All of the above | Surface model/observation distinction in every view |

### 2.2 What this sublane explicitly does **not** cover

- **Habitat suitability modeling.** Owned by the Habitat lane via `SuitabilityModel`, `HabitatQualityScore`, `UncertaintySurface`, and `ModelRunReceipt`. *Proposed sublane home: `docs/domains/habitat/sublanes/suitability.md`.*
...
- **Restoration and stewardship.** Owned via `RestorationOpportunity`, `StewardshipZone`. *Proposed sublane home: `docs/domains/habitat/sublanes/restoration.md`.*
...
- **Plant communities as plant identity.** `VegetationCommunity` is **owned by Flora**, not Habitat; biotope context cites it but does not own it. [DOM-FLORA]
...
> [!NOTE]
> The Habitat lane already names its full object spine. This sublane re-groups three of those families for documentation clarity. It **MUST NOT** introduce parallel object families, schemas, contracts, or policy.
...
>
> 1. A short **ADR** establishing `sublanes/` as a permitted `docs/`-only sub-organizational tier inside domain folders (per Directory Rules §2.4 *if* it is judged to constitute a new structural convention; otherwise per the §17 routine-PR path). **NEEDS VERIFICATION.**
> 2. A per-root **`docs/domains/habitat/README.md`** entry naming the sublanes under that domain, what each covers, and that they create no parallel authority elsewhere.
...
- Become a root folder (`biotopes/` at repo root → **forbidden** by Directory Rules §12, Domain Placement Law).
...
| A floristic plant community | `VegetationCommunity` | **Flora** (not Habitat) | CONFIRMED ownership | Flora-owned per `[DOM-FLORA]`; cited here, not owned. |
...
| Modeled habitat suitability surface | `SuitabilityModel`, `HabitatQualityScore` | Habitat (suitability sublane) | CONFIRMED object families | **Out of scope** for this sublane. |
...
> [!NOTE]
> When this doc says **"biotope"**, read it as a *grouping noun for the three object families above*. When the underlying claim is operational (validation, publication, sensitivity), the canonical KFM term takes precedence in code, schemas, policy, and receipts. *External* usages (e.g., EUNIS-style habitat typologies, IUCN Habitats Classification) may be referenced as **EXTERNAL** crosswalk targets in future verification work, but are **not** authoritative inside KFM.
...
CONFIRMED Habitat source families per `[DOM-HAB]` and `[ENCY 7.4]`. Source-role distinctions (authority / observation / context / model) are preserved per CONFIRMED cross-domain rule that *source role cannot be inferred from convenience*. [DOM-HAB] [DOM-HF] [ENCY] [BLD-COMP §8.2]

| Source family | Typical role for biotopes | Rights / sensitivity | Freshness | Status |
|---|---|---|---|---|
| **NLCD** land cover | observation / context | rights NEEDS VERIFICATION; generally public | classifier-version specific (e.g., NLCD 2019, 2021, 2023) | `[DOM-HAB]` |
...
| **Remote sensing vegetation indices** (Landsat, Sentinel-2 derivatives) | observation / model | source-role bounded; AOD/QA gates apply | scene-time specific | `[DOM-HAB]` + `[New Ideas 5-8]` (PROPOSED watcher lane) |

> [!CAUTION]
> No biotope claim may be promoted to `PUBLISHED` from a source whose **role**, **rights**, or **vintage** is unresolved. *Cite-or-abstain* applies. Source activation flow (CONFIRMED): `SourceDescriptor` → review → `SourceActivationDecision` → connectors remain inactive until activation, fixtures, validators, and policy gates exist. [BLD-COMP §§8.1–8.2; IMPL-PIPE §13]
...
flowchart TB
  ...
  subgraph HABITAT["docs/domains/habitat — Habitat lane (CONFIRMED)"]
    ...
    SUIT["sublanes/suitability<br/>SuitabilityModel · HabitatQualityScore<br/>(PROPOSED sublane)"]:::prop
    CONN["sublanes/connectivity<br/>ConnectivityEdge · Corridor<br/>(PROPOSED sublane)"]:::prop
    REST["sublanes/restoration<br/>RestorationOpportunity · StewardshipZone<br/>(PROPOSED sublane)"]:::prop
  end

  FAUNA["Fauna lane<br/>Taxon · Occurrence Public/Restricted"]:::ext
  FLORA["Flora lane<br/>VegetationCommunity (Flora-owned)"]:::ext
  ...
  FLORA -- "VegetationCommunity referenced (not owned)" --> BIO
...
> [!NOTE]
> Boxes shown in amber are **PROPOSED**. The deny-by-default node enforces the CONFIRMED Habitat ↔ Fauna geoprivacy rule: "Habitat patches are admitted to 3D scenes only via generalized geometry; sensitive habitat denied" and "Occurrence records (public-safe only) feed habitat-quality model evaluation; restricted occurrences never cross." [DOM-HAB] [DOM-FAUNA]
...
- **Regulatory critical habitat, modeled habitat, species range, occurrence points, and landscape context MUST NOT be flattened.** A biotope view that erases the regulatory-vs-modeled distinction is a CONFIRMED anti-pattern.
- **Sensitive occurrence details deny by default.** When a biotope polygon would, by join, reveal a sensitive taxon's exact site — nest, den, roost, hibernaculum, spawning site, steward-controlled record — the join MUST fail closed unless a documented `Geoprivacy transform` and review state allow release. [DOM-FAUNA §§12–13]
...
|---|---|---|---|
| **RAW** | Capture immutable source payload or reference for NLCD/NatureServe/GAP/LANDFIRE/USFWS/NWI/KDWP/field surveys with source role, rights, sensitivity, citation, time, and hash. | `SourceDescriptor` exists; `SourceActivationDecision` recorded. | PROPOSED |
...
> [!NOTE]
> Watcher-as-non-publisher applies: any biotope watcher (e.g., a vegetation-anomaly observer over Landsat/Sentinel-2/MAIAC AOD per `[New Ideas 5-8]`) observes and records; it does **not** promote. PROPOSED.
...
| Biotopes | Restoration | Provides current-condition baselines for `RestorationOpportunity` framing. | Restoration framing is advisory, never an instruction to land managers. [DOM-HAB ↔ DOM-AG] |
...
| Habitat ↔ **Flora** — vegetation community and rare-plant context | Flora | Flora owns `VegetationCommunity` and rare-plant records; biotope cites, does not own; rare-plant exact location denied to public consumers. | `[DOM-FLORA]` |
...
| Habitat ↔ **Hazards** — fire, drought, flood, smoke, resilience stress context | Hazards | Context only; not regulatory. | `[DOM-HAB]` `[DOM-HAZ]` |
...
> [!CAUTION]
> No biotope route may bypass the **trust membrane**. Public clients MUST consume biotope content through governed APIs and released manifests — never directly from `data/processed/`, `data/catalog/`, or any canonical store. [DIRRULES §7] [ENCY]
...
- **PROPOSED — No-network fixtures.** All source connectors remain inactive until activation, fixtures, validators, and policy gates exist. [BLD-COMP §§8.1–8.2]
...
|---|---|---|
| Whether `docs/domains/<domain>/sublanes/` is a permitted `docs/`-only convention. | An accepted ADR, an updated Directory Rules §6.1 reference, or a `docs/domains/habitat/README.md` entry naming the sublanes. | **NEEDS VERIFICATION** |
...
- [`docs/domains/flora/README.md`](../../flora/README.md) — `VegetationCommunity` ownership and rare-plant controls (TODO — verify presence).
...
- [`docs/runbooks/fauna/SOURCE_REFRESH_RUNBOOK.md`](../../../runbooks/fauna/SOURCE_REFRESH_RUNBOOK.md) — Adjacent runbook pattern for sensitive sources.
...
- [ ] Habitat × Fauna sensitive joins fail closed unless a documented `Geoprivacy transform` + review state allow release.
...
**Related docs:** [Habitat README](../README.md) · [Directory Rules](../../../doctrine/directory-rules.md) · [Lifecycle Law](../../../doctrine/lifecycle-law.md) · [Trust Membrane](../../../doctrine/trust-membrane.md) · [Fauna README](../../fauna/README.md) · [Flora README](../../flora/README.md)

**Last updated:** 2026-05-17 · **Doc version:** v1 · **Status:** Draft · [⬆ Back to top](#habitat-sublane--biotopes)
