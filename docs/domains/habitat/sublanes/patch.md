...
tags: [kfm, habitat, sublane, patch, HabitatPatch]
notes:
...
> [!IMPORTANT]
> A `HabitatPatch` is a **delineated habitat unit**, not a raw land-cover tile and not a species record. It is *built from* land-cover and ecological-system evidence (and other context), it *supplies context* to Fauna, Flora, Agriculture, and the connectivity/suitability sublanes, and it is **admitted to public/3D surfaces only via generalized geometry — sensitive habitat is denied.** `[DOM-HAB §24.4.4] [MAP-MASTER]`
...
**Path (PROPOSED):** `docs/domains/habitat/sublanes/patch.md`
...
> [!NOTE]
> The `sublanes/` path segment was **not located in `directory-rules.md`** this session. A `docs/`-internal sub-tier is most likely a **§17 routine-PR** change rather than a §2.4 ADR trigger. Reconcile via the `sublanes/README.md` index or a drift entry in `docs/registers/DRIFT_REGISTER.md`. **NEEDS VERIFICATION.**
...
- Become a root folder (`patch/` at repo root → forbidden by Directory Rules §3 and §12).
- Create a parallel `schemas/`, `policy/`, `contracts/`, or `data/` home under "patch". Those live under the **Habitat** domain segment.
...
flowchart TB
  ...
  subgraph HABITAT["docs/domains/habitat — Habitat lane (CONFIRMED)"]
    ...
    subgraph PB["sublanes/patch — this doc (PROPOSED convention)"]
...
- [ ] No parallel schema/contract/policy home created under "patch"; files live under the **Habitat** domain segment.
...
**Related docs:** [Habitat README](../README.md) · [Sublane index](./README.md) · [Land Cover](./land_cover.md) · [Ecological Systems](./ecological_systems.md) · [Connectivity](./connectivity.md) · [Operating Contract](../../../doctrine/ai-build-operating-contract.md) · [Directory Rules](../../../doctrine/directory-rules.md)
