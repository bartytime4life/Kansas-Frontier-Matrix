...
version: v0.1
status: draft
owners: TBD — Habitat lane steward; Spatial Foundation reviewer
created: 2026-05-17
updated: 2026-05-17
policy_label: public
related:
...
notes:
  ...
  - "EPA Ecoregions is anchored as a habitat-bound source by New_Ideas_5-15-26 future-sources table."
  - "Multi-zoom render pattern anchored by Master MapLibre idea ML-E-057."
[/KFM_META_BLOCK_V2] -->
...
![sensitivity: low](https://img.shields.io/badge/sensitivity-low-green)
...
> [!IMPORTANT]
> **Truth posture.** Ecoregion polygons are a **regionalization context**. They classify *places*, not *species presence* or *habitat patches*. Joins to fauna occurrence, rare-plant records, or sensitive habitat **fail closed** until a documented geoprivacy transform and review state allow release. [DOM-HAB] [DOM-HF] [DOM-FAUNA]
...
- Hierarchical biophysical region polygons (commonly Level I → Level IV, source-dependent).
...
- CRS handling: keep authoritative source CRS (e.g., EPSG:5070 / EPSG:4326) for provenance; reproject to EPSG:3857 only for tiling. [`ML-E-062`, MapLibre Master]
- Public-safe PMTiles with explicit attribute include-lists. [`ML-E-061`, MapLibre Master]
- Multi-zoom render contract: broad regions at low zoom, finer levels at high zoom. [`ML-E-057`, MapLibre Master]
...
| Plant occurrence and rare-plant records | **Flora** — `PlantTaxon`, `FloraOccurrence`, `RarePlantRecord` | Flora owns plant taxonomy and sensitive records. |
...
| Regulatory critical habitat (USFWS ECOS) | **Habitat — critical-habitat sublane** _(PROPOSED sibling)_ | Source-role separation; `regulatory ≠ model ≠ context`. |
...
| **SourceDescriptor** | Cross-cutting governance object pinning identity, role, rights, cadence, and access for each ecoregion source. | `[BLD-COMP]` `[ENCY]` |
...
> All source rights, current terms, and access posture are `NEEDS VERIFICATION` until validated against the mounted repo `data/registry/sources/habitat/` register and a current `SourceDescriptor`. Source role is **never** inferred from convenience. [`[BLD-COMP §8]`, `[IMPL-PIPE §13]`]

| Source family | Role | Granularity | Rights / sensitivity | Freshness | Status |
|---|---|---|---|---|---|
| **EPA Ecoregions** (Omernik framework; CONUS Level III/IV — EXTERNAL) | `authority` — biophysical regionalization | Levels I–IV; state-level Level IV available | `NEEDS VERIFICATION` — current terms; redistribution license must be pinned | Multi-year cadence; version-pinned | `PROPOSED` per `New_Ideas_5-15-26.pdf` future-sources table |
...
| **NatureServe — Terrestrial Ecological Systems** | `authority` / `context` — ecological-systems crosswalk | System-level | `NEEDS VERIFICATION`; controlled biodiversity source posture | Source-vintage specific | `PROPOSED` per `[DOM-HAB] §D` |
...
> [!CAUTION]
> **Source-role anti-collapse.** EPA Ecoregions and USFS Bailey are *parallel* `authority` frameworks, not a single canonical region scheme. Picking one as KFM's default is an **ADR-class decision**, not a sublane decision. Until that ADR lands, both frameworks must be kept addressable side-by-side with framework-level `SourceDescriptor` separation.
...
| **EcoregionCrosswalk** | A reviewed mapping between two frameworks (e.g., EPA → Bailey) or between framework levels. | `PROPOSED`: `source_framework + target_framework + version + reviewer_id`. | `crosswalk_valid_time` distinct from input snapshot times. |
...
| **ModelRunReceipt** _(reused)_ | If derived ecoregion classes are produced (e.g., a refined model), a `ModelRunReceipt` is required. | Per Habitat lane. | Per Habitat lane. |
...
|---|---|---|---|
| Habitat / Ecoregions | **Fauna** | Range-context join; ecoregion attribution attached to species range polygons. | Sensitive occurrence joins **fail closed** unless a documented geoprivacy transform and review state allow release. [`[DOM-FAUNA §§12-13]`] |
| Habitat / Ecoregions | **Flora** | Vegetation-community and rare-plant context. | Rare-plant exact location remains denied; ecoregion attribution allowed only at framework-level granularity. [`[DOM-FLORA]`] |
...
flowchart LR
  subgraph EXT["External authorities (EXTERNAL)"]
    ...
    NS["NatureServe<br/>Terrestrial Ecological Systems"]
  end
...
|---|---|---|
| **Ecoregion overlay (multi-zoom)** | Low zoom shows broad ecoregions; high zoom reveals Level IV detail (and may reveal adjacent PLSS lines from Spatial Foundation). | `[ML-E-057]` MapLibre Master |
...
| **Uncertainty / generalization view** | Renders the `GeneralizationTransform` receipt where simplification was applied for tiling. | `[ML-E-062]`, Spatial Foundation |
...
> [!TIP]
> **PMTiles attribute discipline.** Per `ML-E-061`, ecoregion PMTiles must trim attributes to an **explicit include-list** declared in the `LayerManifest`. Default-deny on attributes; default-deny on raw geometry where generalization is required.
...
| **WORK / QUARANTINE** | Normalize schema, validate geometry (run deterministic `make-valid` before any simplification per `ML-E-033/034`), normalize attribute names, evaluate rights and policy. Quarantine failures with a recorded reason. | Validation and policy gate pass, or quarantine reason recorded. | `PROPOSED` |
...
| Ecoregion + sensitive occurrence join | `DENY` by default | Documented geoprivacy transform + review record. [`[DOM-FAUNA §§12-13]`] |
| Ecoregion + rare-plant record join | `DENY` by default | Flora steward review + transform. [`[DOM-FLORA]`] |
| Ecoregion + archaeology join | `DENY` by default | Cultural-sovereignty review path. [`[DOM-ARCH]`] |
...
> [!CAUTION]
> **Publication gate.** Unclear rights, unresolved source role, missing evidence, unresolved sensitivity, or absent release state **blocks public promotion**. There is no "publish first, gate later." [`[ENCY]`, `[DIRRULES]`, `[DOM-HAB §I]`]
...
> All endpoints are `PROPOSED`. Exact route names, DTO shapes, and schema paths are **`UNKNOWN`** until verified against a mounted `apps/governed-api/` and the schemas tree. Schema home defaults to `schemas/contracts/v1/...` per ADR-0001. [`[DIRRULES] §6.4`]
...
| Correction submission | `POST /api/v1/corrections` _(cross-cutting)_ | `CorrectionNoticeCandidate` | `ACCEPTED / DENY / ERROR` | `PROPOSED` |

> [!NOTE]
> **Trust membrane.** Public surfaces consume **governed APIs**, never `data/processed/`, `data/catalog/`, or raw stores directly. The MapLibre shell receives `LayerManifest` pointers; it does not parse RAW source files. [`[DIRRULES] §7.1`]
...
- **Geometry validity** — deterministic `make-valid` before simplification or tiling; self-intersection, dangling ring, and bowtie checks. [`[ML-E-033/034]`]
...
> [!IMPORTANT]
> **Structural status — `PROPOSED`.** The `docs/domains/habitat/sublanes/` subfolder is not present in established Directory Rules `[DIRRULES]`, which canonicalizes `docs/domains/<domain>/` as the doc home for a domain but does not name a `sublanes/` subdivision. This file proposes the convention; an ADR should ratify or reject it before further sublane files land.
...
| Sibling sublane charters (future) | `docs/domains/habitat/sublanes/{patches,suitability,connectivity,restoration,stewardship,critical-habitat}.md` | Each is its own sublane; PROPOSED. |
...
|---|---|
| Domain folder at root (`ecoregions/` at repo root) | **Refused.** Sublane lives inside `docs/domains/habitat/`. |
...
|---|---|---|
| Verify whether `docs/domains/habitat/sublanes/` already exists or has an ADR. | Mounted repo + ADR index. | `NEEDS VERIFICATION` |
...
| Confirm `EcoregionCrosswalk` reviewer protocol. | Habitat lane steward review pattern; review record schema. | `PROPOSED` |
...
- `docs/domains/habitat/sublanes/patches.md` _(PROPOSED sibling)_
- `docs/domains/habitat/sublanes/suitability.md` _(PROPOSED sibling)_
- `docs/domains/habitat/sublanes/connectivity.md` _(PROPOSED sibling)_
- `docs/domains/habitat/sublanes/critical-habitat.md` _(PROPOSED sibling)_
...
<details>
<summary><strong>B. Anchored MapLibre Master ideas referenced</strong></summary>

| ID | Idea title |
|---|---|
| `ML-E-057` | Low zoom should show broad ecoregions while high zoom reveals detailed Level IV and PLSS lines. |
| `ML-E-061` | Habitat protection PMTiles should trim attributes to explicit include fields. |
| `ML-E-062` | CRS should stay 5070/4326 for provenance and reproject to 3857 only for tiling. |
| `ML-E-064` | Tile properties should stay flat, typed, and filterable. |
| `ML-E-066` | Style versioning must align with layer release versions. |
| `ML-061-132` | Vector-tile attributes should be whitelisted to prevent attribute leakage. |
| `ML-061-033 / 034` | Geometry repair (deterministic `make-valid`) before simplification or tiling. |
| `ML-064-019` | Sovereignty / sensitivity labels must stay synchronized across STAC / DCAT / PROV. |
...
| Label | Meaning in this sublane |
|---|---|
| `authority` | The source defines the regionalization (e.g., EPA, USFS). |
| `observation` | Field- or sensor-based input that informs ecoregion attribution but does not redefine it. |
| `context` | Adjacent geographic / thematic data joined to ecoregions for explanation, never to constitute their truth. |
| `model` | Derived or refined classification produced inside KFM; always labeled and accompanied by `ModelRunReceipt`. |
...
**Related docs:** [Habitat lane README](../README.md) · [Sublane index](./README.md) · [Spatial Foundation](../../spatial-foundation/README.md) · [Fauna](../../fauna/README.md) · [Flora](../../flora/README.md) · [PROV standard](../../../standards/PROV.md) · [PMTiles standard](../../../standards/PMTILES.md)

**Last updated:** 2026-05-17 · **Status:** `draft` · **Maturity:** `CONFIRMED doctrine / PROPOSED implementation` · **Repo state:** `UNKNOWN` (no mounted repo this session)
