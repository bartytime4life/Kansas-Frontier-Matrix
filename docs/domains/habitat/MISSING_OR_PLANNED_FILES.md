...
[![Last Updated: 2026-05-17](https://img.shields.io/badge/updated-2026--05--17-informational)](#footer)
...
In this session **every habitat lane path is PROPOSED**. The habitat lane appears in ENCY §12 Phase 2 of the programming-possibilities backlog as **PROPOSED**, and the DOM-HAB verification backlog lists four open habitat items as **NEEDS VERIFICATION**: official critical habitat source descriptors, sensitive occurrence policy and geoprivacy transforms, model-card requirements for suitability products, and habitat MapLibre overlay registry / Focus behavior.
...
| `schemas/` | `schemas/contracts/v1/domains/habitat/` | Machine-checkable JSON Schemas for each habitat object family (ADR-0001 home) | **PROPOSED** |
...
| `connectors/` | `connectors/usfws/`, `connectors/usgs/` (NLCD), `connectors/nrcs/`, `connectors/kansas/` (KDWP), `connectors/gbif/` | Source-specific fetchers (no domain segment under connectors per §7.3) | **PROPOSED** |
| `data/` | `data/<phase>/habitat/` | Lifecycle artifacts; phases per Directory Rules §4 Step 2 | **PROPOSED** |
...
| `docs/domains/habitat/SOURCE_FAMILIES.md` | Source-role reference (USFWS ECOS, NLCD, NWI, GAP/LANDFIRE, NatureServe, GBIF/iNaturalist/iDigBio, PAD-US, KDWP) with rights/sensitivity/freshness columns | High | PROPOSED | NEEDS VERIFICATION of source rights & cadence (DOM-HAB §D). |
| `docs/domains/habitat/SENSITIVITY_AND_GEOPRIVACY.md` | Habitat-specific sensitivity guide: modeled vs. critical habitat distinction, occurrence-linked exposure risk, generalization / redaction rules, transform-receipt expectations | High | PROPOSED | NEEDS VERIFICATION of geoprivacy transforms (DOM-HAB §N). |
...
| `docs/runbooks/habitat/SOURCE_REFRESH_RUNBOOK.md` | Operational runbook mirroring the fauna pattern (`docs/runbooks/fauna/SOURCE_REFRESH_RUNBOOK.md`) | Medium | PROPOSED | Runbook subfolder convention is ADR-pending (see [Open file-home conflicts](#open-file-home-conflicts-and-adr-pending-decisions)). |

### `contracts/domains/habitat/`

`contracts/` files are `.md` describing **object meaning**. Executable validation belongs in `schemas/`, `policy/`, and `tests/` per Directory Rules §6.3.
...
| `contracts/domains/habitat/habitat_quality_score.md` | `HabitatQualityScore` | PROPOSED | Score basis, support, uncertainty fields. |
...
| `contracts/domains/habitat/uncertainty_surface.md` | `UncertaintySurface` | PROPOSED | Raster/polygon uncertainty representation. |

### `schemas/contracts/v1/domains/habitat/`

Default schema home per **ADR-0001** (`schemas/contracts/v1/...`). Any duplicate definition under `contracts/<domain>/*.schema.json` is **CONFLICTED / lineage** and must migrate per Directory Rules §13.1.
...
| `schemas/contracts/v1/domains/habitat/model_run_receipt.schema.json` | `ModelRunReceipt` | PROPOSED | Aligns with cross-cutting `RunReceipt` schema. |
...
> [!TIP]
> Schema-home reconciliation is one of the highest-priority Pass-20 items. Before adding any new habitat schema, verify against `schemas/contracts/v1/...` and add a drift entry if a legacy `contracts/<domain>/*.schema.json` form is found.

[Back to top](#habitat-domain--missing-or-planned-files)

### `policy/domains/habitat/`

Habitat policy must distinguish **regulatory critical habitat** from **modeled habitat**, fail closed on sensitive occurrence joins, and carry transform-receipt expectations for any public-safe derivative.

| Path (PROPOSED) | Decision surface | Status | Notes |
|---|---|---|---|
| `policy/domains/habitat/source_role_authority.rego` | Source-role authority (authority / observation / context / model) | PROPOSED | DOM-HAB §D source families. |
| `policy/domains/habitat/critical_habitat_vs_modeled.rego` | Modeled-as-critical denial | PROPOSED | DOM-HAB §K modeled-as-critical denial tests. |
| `policy/domains/habitat/occurrence_geoprivacy.rego` | Sensitive occurrence joins (Habitat × Fauna) | PROPOSED | Deny-by-default; pairs with `policy/domains/fauna/...`. |
...
Cross-domain validators (e.g., a habitat × fauna × hydrology validator) belong **without** a domain segment per Directory Rules §12 ("multi-domain and cross-cutting files").
...
`pipeline_specs/` declares **what** runs; `pipelines/` is **how** it runs (Directory Rules §7.4).
...
Per Directory Rules §7.3, connectors are **source-specific**, **not domain-specific**, and write only to `data/raw/<domain>/...` or `data/quarantine/...`. Habitat-relevant connectors are:
...
> [!WARNING]
> Connectors must not publish. Any connector writing to `data/processed/`, `data/catalog/`, `data/published/`, or `release/` is a §13.5 anti-pattern. Habitat connectors land in `data/raw/habitat/<source_id>/<run_id>/` or `data/quarantine/...`.

### `data/<phase>/habitat/`

Lifecycle phases per Directory Rules §4 Step 2. Receipts, proofs, registry, and rollback are emitted **alongside** lifecycle directories, not as substitutes.
...
| `data/triplets/habitat/...` | TRIPLET | PROPOSED |
...
| `release/candidates/habitat/<release_id>/promotion_decision.json` | PromotionDecision (ANSWER / ABSTAIN / DENY / ERROR) | PROPOSED |
...
| `control_plane/contradiction_register.yaml` | Any habitat-related drift (e.g., legacy `contracts/<domain>/*.schema.json`) | PROPOSED |
...
> [!CAUTION]
> The thin slice must not load live sensitive source connectors. It is a **fixture-first** proof of the governed flow, not a production lane. Activation of live sensitive sources requires source-rights, cadence, and policy verification first.
...
| Cesium / 3D habitat overlays | After 2D Habitat overlay registry, LayerManifest, and EvidenceDrawerPayload are CONFIRMED | Cesium is an alternate renderer, not an alternate truth path (Directory Rules §11). |
...
| Parallel schema home under `contracts/domains/habitat/*.schema.json` | ADR-0001 sets `schemas/contracts/v1/...` as canonical; any duplicate is CONFLICTED. |
| Habitat-specific "live alert" surfaces | Habitat is not a life-safety domain; alert content belongs to Hazards (and even there, the no-emergency-alert boundary applies). |
...
| **Schema-home reconciliation** | Any legacy `contracts/<domain>/*.schema.json` form requires drift entry and migration under ADR-0001. | All habitat schemas | NEEDS VERIFICATION. |
| **Runbook subfolder convention** | Habitat runbooks under `docs/runbooks/habitat/` mirror the existing `docs/runbooks/fauna/...` form, but the broader runbook subfolder convention (flat-prefix vs. domain-subfolder) is ADR-pending. | `docs/runbooks/habitat/SOURCE_REFRESH_RUNBOOK.md` | ADR-PENDING (per project memory). |
| **Validator exit-code contract** | Repo-wide exit-code contract for validators (e.g., `0/1/2/3` for PASS/FAIL/ERROR/ABSTAIN) affects every habitat validator under `tools/validators/domains/habitat/`. | All habitat validators | ADR-PENDING (per project memory). |
| **`PROV.md` vs. `PROVENANCE.md` naming** | Naming discrepancy between `docs/standards/PROV.md` and corpus references to `PROVENANCE.md` is ADR-pending; resolution affects how habitat docs cite provenance doctrine. | Habitat docs that link provenance doctrine | ADR-PENDING (per project memory). |
...
> [!NOTE]
> Items HAB-V1 through HAB-V4 are the four habitat verification items recorded in DOM-HAB §N as NEEDS VERIFICATION. HAB-V5–V7 are this-tracker-specific items.
...
- **Model vs. observation labels must remain visible** through every pipeline stage and every map product (DOM-HAB §D).
- **Habitat × Fauna joins** must preserve fauna geoprivacy. The Habitat lane never publishes exact sensitive fauna occurrence as a habitat-derived product.
...
└── domains/
    └── habitat/
        ...
        ├── SOURCE_FAMILIES.md                 # PROPOSED
        ├── SENSITIVITY_AND_GEOPRIVACY.md      # PROPOSED
        └── MODEL_VS_OBSERVATION.md            # PROPOSED

docs/runbooks/
└── habitat/
    └── SOURCE_REFRESH_RUNBOOK.md              # PROPOSED (ADR-pending subfolder convention)
...
- `docs/doctrine/trust-membrane.md` *(public path through governed API)*
...
- `docs/standards/PROV.md` *(provenance profile; naming discrepancy with `PROVENANCE.md` is ADR-pending)*
...
**Last updated:** 2026-05-17 · **Doc owner:** `<habitat-domain-steward>` · **Status:** `draft` · **Authority:** tracker (non-canonical for object meaning, schemas, policy, or release decisions).
