...
notes:
  ...
  - "TERMINOLOGY: 'sublane' is not established KFM doctrine. 'lane' is defined; 'sub-lane' exists only in the Focus Mode cross-root sense (Directory Rules §6.7). See OPEN-ROADS-01."
...
> [!NOTE]
> **Term status.** `Road Segment`, `Rail Segment`, `Depot`, `Siding`, `Yard`, `Crossing`, `Bridge`, `Ferry`, `River Crossing`, `Freight Corridor`, `Route Event`, `Operator Status`, `Access Restriction`, `Network Edge`, `Movement Story Node`, `Historic Route`, `Historic RouteClaim`, `CorridorRoute`, `RouteMembership`, and `TradeRouteCorridor` are **CONFIRMED** terms from `[DOM-ROADS]`. `StatusEvent` and `OperatorAssignment` are **PROPOSED** rail/road-specific realizations of the dossier's `Route Event` and `Operator Status`. Sibling filenames are PROPOSED — the parent dossier should ratify `trade-routes.md` vs `historic-routes.md` before either is treated as canonical.
...
The block below is **illustrative**, not a contract. The canonical shape lives in `schemas/contracts/v1/...` per ADR-0001 (schema-home rule); the slug under that root (`domains/roads-rail-trade/` per Directory Rules §12 vs. `transport/` per Atlas §24.13) is itself an open question — see [§13 OPEN-ROADS-03](#13-open-questions-and-verification-backlog). Do not use this shape to validate anything; it exists to make field intent legible while real schemas are authored.
...
> [!NOTE]
> **Public clients use governed APIs, not canonical stores.** `[DIRRULES]` §7.1; `[ENCY]` anti-pattern: "Public client reads RAW / WORK / QUARANTINE". Map shells, dashboards, and AI surfaces consume `apps/governed-api/` responses — they do not read from `data/processed/`, `data/catalog/`, or `data/published/` directly.
...
| 5 | Public generalization receipt tests. | High — every public-safe geometry transform emits a RedactionReceipt. | PROPOSED `[DOM-ROADS]` `[ENCY]` |
...
> [!IMPORTANT]
> **The map renderer is not the truth path.** Per `[MAP-MASTER]` / `[DIRRULES]` §11, the browser renderer consumes the same EvidenceBundle and DecisionEnvelope as any other client — it is an alternate renderer, not an alternate truth path. A roads layer whose styling implies a designation that no EvidenceBundle supports is a trust-membrane bug, not a styling choice.

> [!NOTE]
> **Renderer note (v1.3).** `packages/maplibre-runtime/` is the **sole governed browser-side renderer adapter**; the v1.2 Cesium dual-renderer posture is **retired** (doctrine-target, pending acceptance of the *MapLibre as Sole Browser-Side Renderer; Retire Cesium Dependency* ADR — Directory Rules §18.e OPEN-DR-10). The freeze rule is in effect: no new `cesium*` code, schemas, policies, or tests. Earlier drafts of this dossier referenced "Cesium where present"; that language is removed. `[DIRRULES]` `[MAP-MASTER]`
...
|---|---|---|---|
| **OPEN-ROADS-01** | Is `docs/domains/<domain>/sublanes/` a recognized organizational pattern — and is "sublane" the right term, given it collides with the Focus Mode "sub-lane" (§6.7)? Or should sublane content stay as flat files under `docs/domains/<domain>/`? | Updated Directory Rules §6.1, parent dossier README, or a small ADR. | CONFLICTED — needs ADR or per-root README. `[DIRRULES]` |
...
| **OPEN-ROADS-03** | Schema-home slug for this domain: `schemas/contracts/v1/domains/roads-rail-trade/` (Directory Rules §12 pattern) vs. `schemas/contracts/v1/transport/` (Atlas §24.13 responsibility-root crosswalk)? | ADR aligning Atlas §24.13 and Directory Rules §12; mounted-repo inspection; `DRIFT_REGISTER.md` entry. | CONFLICTED — slug variance between two project docs. `[DIRRULES §12]` `[ENCY §24.13]` |
...
| Item | Status |
|---|---|
| Whether any of `contracts/domains/roads-rail-trade/`, `schemas/contracts/v1/domains/roads-rail-trade/`, `policy/domains/roads-rail-trade/`, `tests/domains/roads-rail-trade/`, `pipelines/domains/roads-rail-trade/`, or `data/<phase>/roads-rail-trade/` exist in the mounted repo. | UNKNOWN — no mounted repo this session. |
...
<sub><strong>Last updated:</strong> 2026-06-07 · <strong>Doc version:</strong> v0.2 (draft) · <strong>Status:</strong> standard doc; sublane structure PROPOSED · <strong>CONTRACT_VERSION:</strong> 3.0.0 · <strong>Owners:</strong> Roads/Rail/Trade Routes steward — `TBD` · [↑ back to top](#roads-sublane--roads-rail-and-trade-routes-domain)</sub>
