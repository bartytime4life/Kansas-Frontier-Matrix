...
notes: [
  "All repo-state claims are PROPOSED or NEEDS VERIFICATION; no mounted repo was inspected in this session.",
...
![last updated: 2026-05-19](https://img.shields.io/badge/last%20updated-2026--05--19-success)

**Status:** draft · **Owners:** `TODO Roads/Rail/Trade domain steward + Docs steward + Release authority` · **Last updated:** 2026-05-19
...
- Object families, ubiquitous language, pipeline shape, and cross-lane relations → see [`./README.md`](./README.md) *(PROPOSED neighbor doc)* and Atlas Chapter 13.
...
|---|---|---|---|---|
| **Modern roads layer** | Current Kansas road network (centerlines, classification, functional class). | `Road Segment`, `Network Node`, `Crossing` | Public (legal-class evidence only) | PROPOSED |
| **Rail alignment layer** | Active and historic rail corridors and operator assignments. | `Rail Segment`, `Depot`, `Siding`, `Yard`, `OperatorAssignment` | Public; **Critical transport facilities → review** | PROPOSED |
| **Facility / crossing view** | Bridges, ferries, river crossings, grade crossings. | `Bridge`, `Ferry`, `River Crossing`, `Crossing`, `TransportFacility` | Mixed; **infrastructure-critical → review** | PROPOSED |
| **Restriction / status timeline** | Time-aware access restrictions, work zones, closures. | `RestrictionEvent`, `StatusEvent`, `Route Event` | Public; **WZDx = observation, not history** | PROPOSED |
| **Freight-corridor context** | National Highway Freight Network and freight corridor designations. | `Freight Corridor`, `Network Edge` | Public; **observation/context role** | PROPOSED |
| **Historic route claim view** | Wagon, military, mail, emigrant, stage, cattle, and trade-route claims. | `Historic RouteClaim`, `CorridorRoute` | **Steward review + uncertainty labeling**; precision generalized | PROPOSED |
| **Generalized trade-route corridor** | Indigenous trade and mobility corridors as **generalized polygons**, not lines. | `TradeRouteCorridor` | **Indigenous / cultural → steward review; generalized public geometry** | PROPOSED |
...
> [!WARNING]
> **Source-role collapse is the highest-priority anti-pattern for this domain.** Every Roads/Rail/Trade source family must be admitted with an explicit role that **cannot be upgraded** by promotion. Treating an observational feed (e.g., OpenStreetMap edits, WZDx work-zone events) as an `authority` claim is a release-gate failure (reason: `ROLE_COLLAPSE` / `ROLE_DOWNCAST_FORBIDDEN`).

CONFIRMED source families (Atlas §13.D), with their **PROPOSED canonical roles** for release-time admission:

| Source family | Canonical role(s) | Rights / sensitivity | Freshness cadence |
|---|---|---|---|
| Census TIGER/Line roads | `authority` (administrative geometry) / `context` | Public; **role as source descriptor declares** | Annual vintage |
| FHWA HPMS | `authority` (federal functional class) / `observation` | NEEDS VERIFICATION (terms) | Annual |
| FHWA National Highway Freight Network | `authority` (freight designation) / `context` | NEEDS VERIFICATION (terms) | Periodic |
| WZDx feeds | **`observation` only** (work-zone event stream) | NEEDS VERIFICATION (terms); **never a historical record by itself** | Near real-time |
| KDOT / KanPlan / KanDrive / Kansas GIS | `authority` (state DOT) / `observation` (live conditions) | NEEDS VERIFICATION (terms) | Source-vintage or cadence specific |
| County / state bridge and restriction data | `authority` (bridge/inspection records) | NEEDS VERIFICATION; **infrastructure-critical joins fail closed** | Source-vintage |
| GNIS names | `authority` for **naming only** — **not** for legal status, geometry, or designation | Public | Periodic |
| OpenStreetMap | **`observation` only** — never `authority` for legal class, designation, or restriction | Public (per OSM ODbL — verify) | Continuous |
...
CONFIRMED doctrine (Atlas §13.I): *"Indigenous trade and mobility corridors, oral history, treaty, cultural, and interpretive evidence default to steward review and generalized public geometry. Critical transport facilities require review."*

| Material | Default posture | Required reviewers (at material release) |
|---|---|---|
| Indigenous trade and mobility corridors | **Steward review + generalized geometry**; precise lines denied | Domain steward + sensitivity reviewer + **rights-holder representative** |
...
| Historic route claims (wagon, military, mail, emigrant, stage, cattle, trade) | **Steward review for overprecision**; uncertainty labels required | Domain steward + sensitivity reviewer |
| Critical transport facilities (bridges, sensitive crossings, key freight nodes) | **Restricted detail**; review before public exposure | Domain steward + sensitivity reviewer |
| Modern road centerlines and classifications (TIGER/Line, KDOT) | **Public** if rights resolved | Domain steward |
| Operator assignments, rail operator details | **Public-safe roll-up**; operator-sensitive specifics restricted | Domain steward |
| WZDx work-zone observations | **Public** as observation; never republished as historical record | Domain steward |
...
> [!CAUTION]
> **KFM is never an alert authority.** Releases must not present restriction, closure, work-zone, or freight-corridor data as an emergency-routing source. Hazards owns hazard-event truth (Atlas §12 / §24.4.10). Roads/Rail/Trade publishes context, not instruction.
...
The diagram below shows where a Roads/Rail/Trade release decision sits in the broader lifecycle. Boxes labeled *PROPOSED* are doctrine-driven placement targets pending repo verification.

```mermaid
flowchart TD
  SRC[Source families<br/>TIGER · HPMS · KDOT · WZDx · GNIS · OSM · …]
  RAW[(data/raw/roads-rail-trade/<br/><i>PROPOSED</i>)]
  WORK[(data/work + quarantine<br/><i>PROPOSED</i>)]
  PROC[(data/processed/roads-rail-trade<br/><i>PROPOSED</i>)]
  CAT[(data/catalog/domain/roads-rail-trade<br/><i>PROPOSED</i>)]
  ...
  classDef proposed fill:#fff8c5,stroke:#9a6700,color:#000;
  classDef canonical fill:#dafbe1,stroke:#1a7f37,color:#000;
  classDef gate fill:#ffd8d8,stroke:#cf222e,color:#000;
  class RAW,WORK,PROC,CAT,CAND,PUB proposed;
  class MAN,SIG,PROMO,ROLL,CORR,API canonical;
  class GATE gate;
```
...
> **CONFIRMED doctrine / PROPOSED implementation.** Roads/Rail/Trade publication requires complete closure across the gates below. Missing any required artifact means the transition **fails closed** and the prior state is preserved (Atlas §13.M; D-10).
...
> [!NOTE]
> **Watcher-as-non-publisher invariant** *(CONFIRMED doctrine, applied to this domain by analogy with the fauna runbook).* Roads/Rail/Trade source-drift detectors, KDOT/WZDx ingest watchers, and OSM diff watchers emit **candidate** records — they do **not** publish, mutate canonical truth, or expose `RAW`/`WORK`/`QUARANTINE` payloads to public surfaces. Watcher output enters the lifecycle on the WORK side and requires explicit promotion through G1–G8.
...
|---|---|---|---|---|---|---|---|---|
| `rel-roads-modern-network-v1-template` | *(template)* Kansas modern roads network — public layer | `DRAFT` | `YYYY-MM-DDTHH:MM:SSZ` | `sha256:…` | `release/manifests/rel-roads-modern-network-v1-template.json` | `data/proofs/evidence_bundle/<bundle_id>.json` | `null` *(initial)* | NEEDS VERIFICATION — rights confirmation for TIGER/Line + KDOT |
| `rel-roads-rail-alignment-v1-template` | *(template)* Rail alignment & depot layer | `DRAFT` | `YYYY-MM-DDTHH:MM:SSZ` | `sha256:…` | `release/manifests/rel-roads-rail-alignment-v1-template.json` | `data/proofs/evidence_bundle/<bundle_id>.json` | `null` *(initial)* | NEEDS VERIFICATION — operator sensitivity review |
| `rel-roads-historic-claim-v1-template` | *(template)* Historic route claims — generalized public view | `DRAFT` | `YYYY-MM-DDTHH:MM:SSZ` | `sha256:…` | `release/manifests/rel-roads-historic-claim-v1-template.json` | `data/proofs/evidence_bundle/<bundle_id>.json` | `null` *(initial)* | NEEDS VERIFICATION — overprecision denial test + uncertainty labels |
| `rel-roads-corridor-trade-v1-template` | *(template)* Indigenous trade & mobility corridors — generalized polygons | `DRAFT` | `YYYY-MM-DDTHH:MM:SSZ` | `sha256:…` | `release/manifests/rel-roads-corridor-trade-v1-template.json` | `data/proofs/evidence_bundle/<bundle_id>.json` | `null` *(initial)* | NEEDS VERIFICATION — **rights-holder representative review required**; `RedactionReceipt` required |
| `rel-roads-graph-connectivity-v1-template` | *(template)* Transport graph — derived connectivity projection | `DRAFT` | `YYYY-MM-DDTHH:MM:SSZ` | `sha256:…` | `release/manifests/rel-roads-graph-connectivity-v1-template.json` | `data/proofs/evidence_bundle/<bundle_id>.json` | `null` *(initial)* | NEEDS VERIFICATION — graph-projection rollback test (Atlas §13.K) required; **graph is derivative, not canonical** |
...
spec_hash:            sha256:<hex>          # JCS+SHA-256 of the ReleaseManifest body
...
CONFIRMED doctrine (Atlas §24.7.1, §24.7.2; D-9): release-time separation of duties is **maturity-dependent** and **enforced through tooling** as the public trust surface expands. The roles below are the minimum set Roads/Rail/Trade releases will use; named individuals are deliberately not fabricated.
...
> [!NOTE]
> **Author ≠ release authority** is the single most-cited release-time separation in the doctrine (Atlas §24.7.2). For Roads/Rail/Trade lanes that touch Indigenous corridors, the additional **rights-holder representative** role is non-optional regardless of materiality.
...
|---|---|---|
| **OSM as authority for legal class or designation** | Source-role collapse; community editorial state cited as canonical. | OSM is `observation`. Use TIGER/Line, FHWA, or KDOT for `authority`. (Atlas §13.K validator *"OSM/GNIS legal-status denial"*.) |
| **GNIS name match as proof of designation** | Naming authority used as legal-status authority. | GNIS is `authority` for names **only**. |
| **WZDx feed cited as historical record** | Live observation stream republished as the historical truth of restriction or closure. | WZDx is `observation`. Historical restriction history requires a distinct authority source and source-role admission. |
...
| **Historic route claim published without uncertainty label** | Overprecision presented as evidence-grade. | Apply uncertainty profile; PROPOSED `RouteUncertaintyProfile` (Atlas §13.N verification backlog). |
...
| 3 | **Indigenous / cultural corridor policy.** Atlas §13.N verification item *"Verify Indigenous/cultural corridor policy."* | Rights-holder representative role at release time depends on this policy being explicit. | Policy bundle under `policy/domains/roads-rail-trade/` *(PROPOSED)*; rights-holder consultation log. |
...
| 5 | **`RouteUncertaintyProfile` implementation.** Atlas §13.N verification item. | G4 overprecision check depends on this profile. | Schema + validator + fixtures under `schemas/contracts/v1/domains/roads-rail-trade/` *(PROPOSED)*. |
...
| 7 | **`release/` tree presence in the mounted repo.** Directory Rules §9.2 prescribes the layout; mounted-repo presence is NEEDS VERIFICATION. | Every PROPOSED path in this index resolves through `release/`. | `git ls-tree`-equivalent inspection. |
...
> [!NOTE]
> Targets marked **PROPOSED** or **NEEDS VERIFICATION** are doctrine-anchored placement targets per Directory Rules §6.1 and §12. Their presence in the mounted repo is not confirmed in this session.

- [`./README.md`](./README.md) — Roads/Rail/Trade domain README *(PROPOSED neighbor doc)*
- [`./SOURCE_INDEX.md`](./SOURCE_INDEX.md) — Roads/Rail/Trade source registry index *(PROPOSED neighbor doc)*
- [`./CATALOG_INDEX.md`](./CATALOG_INDEX.md) — Roads/Rail/Trade catalog / STAC closure index *(PROPOSED neighbor doc)*
- [`../../doctrine/directory-rules.md`](../../doctrine/directory-rules.md) — Directory Rules v1.1 *(CONFIRMED authored)*
- [`../../adr/`](../../adr/) — ADR index *(NEEDS VERIFICATION)*; relevant: ADR-0001 (schema home), Open Questions #1, #2, #9
...
- `release/README.md` — Release root README *(PROPOSED per Directory Rules §9.2; NEEDS VERIFICATION)*
...
| `ReleaseManifest` | Signed, hashable JSON object listing every dataset, bundle, and tile archive in a release; consumers bind by `spec_hash`. CONFIRMED doctrine, Atlas Pass-23/32 card KFM-P7-PROG-0003. |
...
| `EvidenceRef` | A reference whose target spec_hash and expected bundle digest let consumers verify resolution. Atlas Pass-32 card KFM-P26-PROG-0005. |
...
| `DecisionEnvelope` | Finite-outcome envelope: `ANSWER` / `ABSTAIN` / `DENY` / `ERROR`. Master MapLibre Components glossary. |
...
| `spec_hash` | RFC 8785 JSON Canonicalization Scheme (JCS) + SHA-256 of a spec or contract; deterministic identity. Pass-10 §6.1 / C1-02. |
| `cosign / DSSE / Rekor` | Keyless signing, DSSE envelope, and transparency-log inclusion used to bind a `ReleaseManifest` to its signer identity. Pass-10 §6.1 / C1-03, Atlas card KFM-P27-PROG-0014. |
...
**Doc id:** `kfm://doc/docs-domains-roads-rail-trade-release-index` · **Atlas anchor:** `[DOM-ROADS]` Chapter 13 · **Doctrine anchors:** Directory Rules §6.1, §9.2, §12, §13.2 · Atlas §13, §24.7, §24.8 · Encyclopedia Appendix E

**Last reviewed:** 2026-05-19 *(creation)* · **Next review trigger:** first real `ReleaseManifest` lands under `release/manifests/` for any Roads/Rail/Trade `release_id`, **or** Open Question #1 (`release_state` enum) is resolved by ADR.
