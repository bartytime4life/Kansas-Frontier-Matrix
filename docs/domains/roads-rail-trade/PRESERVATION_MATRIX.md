...
> Domain-scoped operational matrix that consolidates what must be preserved — and what may not be silently lost — for KFM Roads/Rail object families across lifecycle, sensitivity, historic-integrity, evidence-chain, and cross-lane dimensions.
...
policy_label: public
related:
  - docs/domains/roads-rail-trade/README.md            # NEEDS VERIFICATION — adjacent index, PROPOSED
  - docs/doctrine/lifecycle-law.md                     # NEEDS VERIFICATION — per directory-rules §6.1
  - docs/doctrine/trust-membrane.md                    # NEEDS VERIFICATION — per directory-rules §6.1
  - docs/doctrine/directory-rules.md                   # CONFIRMED in corpus
  - docs/standards/PROV.md                             # CONFIRMED authored; see OPEN-DR-01
  - docs/atlases/                                      # PROPOSED — Atlas v1.1, Ch. 13 / Ch. 24.5
  - schemas/contracts/v1/transport/                    # PROPOSED schema responsibility root (encyclopedia §5.1)
  - policy/sensitivity/                                # PROPOSED — pattern follows fauna/flora/archaeology
tags: [kfm, domain, roads-rail-trade, preservation, retention, sensitivity, evidence, historic-integrity]
notes:
  - "Framing as a 'Preservation Matrix' is PROPOSED: the term is not in KFM corpus; underlying axes are CONFIRMED doctrine."
  - "Per-object tier baselines in §5 extend Atlas v1.1 Ch. 24.5 by analogy; entries are PROPOSED pending steward review."
  - "Retention SLAs across lifecycle tiers are an unresolved KFM gap (Pass-10 §8.4, Expansion Agenda 10.1 priority H)."
[/KFM_META_BLOCK_V2] -->
...
![sensitivity](https://img.shields.io/badge/baseline-T1%E2%80%93T2-orange)
![truth_posture](https://img.shields.io/badge/truth_posture-cite--or--abstain-9f1239)
![last_updated](https://img.shields.io/badge/last_updated-2026--05--19-informational)

**Status:** draft &nbsp;·&nbsp; **Owners:** _TODO — assign Roads/Rail/Trade Routes domain steward(s)_ &nbsp;·&nbsp; **Last updated:** 2026-05-19

> [!IMPORTANT]
> This document is **operational doctrine for the Roads/Rail/Trade Routes domain**. It does not override the Atlas v1.1 Ch. 13 chapter (DOM-ROADS), the Master Sensitivity / Rights Tier Reference (Atlas v1.1 Ch. 24.5), or `directory-rules.md`. Where this matrix appears to disagree with those sources, the upstream doctrine wins and the conflict belongs in `docs/registers/DRIFT_REGISTER.md`.
...
In the KFM Roads/Rail/Trade Routes lane, *preservation* is not a single property — it is a small constellation of duties that the system owes to its sources, its stewards, and the public. Each duty has a different failure mode if it is dropped silently. This matrix groups those duties so a reviewer can ask, for any object family, **what must persist, in what form, at what stage, under what review state**.
...
> [!NOTE]
> **The framing as a single "Preservation Matrix" is PROPOSED.** The matrix axes are KFM-confirmed doctrines, but consolidating them into one domain-scoped matrix is an authoring decision made here. The matrix is meant to make multi-axis preservation duties reviewable in one place; it does **not** introduce new duties.
...
The Roads/Rail/Trade Routes lane owns: **Road Segment, Historic Route, Rail Segment, Depot, Siding, Yard, Crossing, Bridge, Ferry, River Crossing, Freight Corridor, Route Event, Operator Status, Access Restriction, Network Edge, Movement Story Node**, together with the lane-internal projections **CorridorRoute**, **RouteMembership**, **Network Node**, **TransportFacility**, **RestrictionEvent**, **Historic RouteClaim**, and **TradeRouteCorridor**. [DOM-ROADS]
...
- Not a substitute for Atlas v1.1 Ch. 24.5 (Master Sensitivity / Rights Tier Reference) — the T0–T4 scheme is **PROPOSED** there (ADR-S-05) and is applied here by analogy.
- Not a retention-SLA register. Quantitative retention windows for each lifecycle tier remain an **open KFM gap** (Pass-10 §8.4; Expansion Agenda 10.1, priority H). This matrix defines **what must be preserved**, not **how long**, except where doctrine sets the floor (e.g., RAW under source identity).
...
PROPOSED framing: five orthogonal preservation duties intersect at every Roads/Rail object family. The duties are independent — a Road Segment may be lifecycle-preserved (RAW retained) yet historically over-precise; a Historic Route may be culturally generalized yet still evidence-traceable. The matrix makes each axis explicit.

```mermaid
flowchart LR
    A[Roads/Rail<br/>object family]:::obj
    A --> L[Lifecycle retention<br/>RAW → PUBLISHED]:::axis
    A --> S[Sensitivity-tier<br/>preservation T0–T4]:::axis
    A --> H[Historic-integrity<br/>preservation]:::axis
    A --> E[Evidence-chain<br/>preservation]:::axis
    A --> X[Cross-lane<br/>preservation constraints]:::axis
...
CONFIRMED doctrine: **RAW preserves admitted source material under source identity**; PROCESSED and downstream stages emit normalized derivatives, not replacements. Temporal vacuuming and archival partition pruning are governed retention decisions and must never be treated as silent deletion of evidence (KFM-P9-PROG-0011, KFM-P9-PROG-0018). [UIA-BUILD]
...
| **Operator Status** | Source payload + observed time + hash | Quarantine for source-role drift | `EvidenceRef`; operator-history preserved | `EvidenceBundle`; operator timeline | `ReleaseManifest`; rollback target | Operator-history lineage |
...
CONFIRMED baseline: the Roads/Rail/Trade lane carries a default sensitivity range of **T1–T2** at the lane level. [ENCY §5.1]

PROPOSED per-object tier baselines, applied by analogy to the Master Sensitivity / Rights Tier Reference (Atlas v1.1 Ch. 24.5; ADR-S-05 PROPOSED). These entries do not yet appear in the Atlas per-domain tier matrix and are **PROPOSED pending steward review**.

| Object class | Default tier (PROPOSED) | Allowed transform toward more public | Required gates | Downgrade-to-T4 trigger | Citation basis |
|---|---|---|---|---|---|
| Road Segment — modern, public-roadway | **T0** | None required for public-roadway geometry from authoritative sources. | Standard `ReleaseManifest` + `ReviewRecord` per release maturity. | Rights-change detection; source revocation. | [DOM-ROADS] [ENCY] |
| Rail Segment — modern, public alignment | **T0–T1** | Generalization where source rights/precision require it. | `RedactionReceipt` if generalized. | Operator deny; rights revocation. | [DOM-ROADS] [ENCY] |
| Freight Corridor / TradeRouteCorridor — modern designation | **T1** | Generalized layer for public; freight-context as cite-only. | `AggregationReceipt` or `RedactionReceipt`; `ReviewRecord`. | Critical-corridor flag; agreement revocation. | [DOM-ROADS] [ENCY] |
| Historic Route / Historic RouteClaim — non-sensitive | **T1** | Generalized geometry with uncertainty surfaced. | `RedactionReceipt` + `ReviewRecord`; `RouteUncertaintyProfile` (PROPOSED). | Overprecision detected; new cultural context surfaced. | [DOM-ROADS] [DOM-ARCH] [ENCY] |
...
| **Crossing near archaeological site** | **T4** (defer to Archaeology) | Deny exact-coord exposure; corridor cited as context only, never used to reconstruct site location. | Steward review + `RedactionReceipt` + `PolicyDecision`. | Cross-lane join attempted that could re-identify site. | [DOM-ROADS] [DOM-ARCH] [ENCY] |
| **Critical Bridge / Ferry / Crossing — vulnerability or condition detail** | **T4** (defer to Settlements / Infrastructure) | T3 to named authorities only; never T0 / T1. | Steward review + named-party agreement. | Any public surface attempting to publish vulnerability detail. | [DOM-ROADS] [DOM-SETTLE] [ENCY] |
...
| Depot / Siding / Yard / TransportFacility — *cited identity only* | (Defer to **Settlements**) | Settlements owns the facility identity; Roads/Rail preserves the citation. | Cross-lane cite preserved per §8. | Settlements deny propagates. | [DOM-SETTLE] [DOM-ROADS] |

### 5.1 Allowed tier motion (CONFIRMED doctrine, Atlas v1.1 §24.5.3)

```mermaid
flowchart LR
    T4["T4<br/>Denied"]:::t4 -->|PolicyDecision + ReviewRecord<br/>+ agreement| T3["T3<br/>Restricted"]:::t3
    ...
    T1 -->|ReleaseManifest + ReviewRecord<br/>+ release authority| T0["T0<br/>Open"]:::t0
...
Historic alignment claims are a recurring source of preservation risk in this lane — they are easy to over-render, easy to over-precision, and frequently entangled with cultural sensitivity. The following rules are CONFIRMED in DOM-ROADS doctrine; their per-validator realization is PROPOSED.
...
| **`RouteUncertaintyProfile` (NEEDS VERIFICATION)** | A profile object that pairs claim geometry with uncertainty bounds and review state. | Uncertainty becomes a UI styling choice rather than a structural record. | PROPOSED, NEEDS VERIFICATION [DOM-ROADS §N] |

> [!WARNING]
> **A confident historic line on a map is a louder claim than a paragraph of qualifications next to it.** Preservation of historic integrity is partly a rendering discipline; the matrix only ensures that the underlying record carries the uncertainty the renderer must surface.
...
flowchart TB
    subgraph SRC["Sources (CONFIRMED roles)"]
      ...
      A3[WZDx feeds]:::src
      A4[KDOT / KanPlan /<br/>KanDrive / Kansas GIS]:::src
      A5[County/state bridge<br/>and restriction data]:::src
      A6[GNIS names]:::src
      A7[OpenStreetMap]:::src
      A8[Historic maps,<br/>archives, oral history]:::src
    end

    SRC --> SD[SourceDescriptor<br/>+ payload hash]:::ev
    SD --> RR[RunReceipt]:::ev
    SD --> ER[EvidenceRef]:::ev
    ER --> EB[EvidenceBundle]:::ev
    EB --> PR[PromotionReceipt]:::ev
    PR --> RM[ReleaseManifest /<br/>MapReleaseManifest]:::rel
...
| `CorrectionNotice` | Required for any post-publication change; tier downgrade is always permitted via correction. | CONFIRMED doctrine [ENCY §24.5.3] |
| `AIReceipt` / `RuntimeResponseEnvelope` | Bounded AI request/response accountability; AI never root truth. | CONFIRMED doctrine / PROPOSED implementation [UIA-BUILD §8] |
...
```mermaid
flowchart LR
    R[Roads / Rail /<br/>Trade Routes]:::own
    R -- "depots, crossings,<br/>facilities, dependencies" --> S[Settlements /<br/>Infrastructure]:::adj
    R -- "bridge / ferry /<br/>ford / river crossing" --> H[Hydrology]:::adj
    R -- "closure, detour,<br/>flood/fire/smoke exposure" --> Z[Hazards]:::adj
    R -- "historic routes,<br/>Indigenous corridors,<br/>forts, missions" --> A[Archaeology /<br/>Cultural Heritage]:::adj
    R -- "Frontier Matrix:<br/>access observations<br/>bound matrix cells" --> F[Frontier Matrix]:::adj
...
| **Roads/Rail ↔ Hydrology** | Bridges, ferries, fords, and river crossings preserve the hydrology cite as crossing context. Crossing identity remains Roads/Rail-owned; water context remains Hydrology-owned. | [DOM-ROADS §F] |
| **Roads/Rail ↔ Hazards** | Closure, detour, and flood/fire/smoke exposure observations cite the hazard event; **KFM is never an alert authority**. Hazards-T4-forever boundary preserved through Roads/Rail surfaces. | [DOM-ROADS §F] [DOM-HAZ] |
| **Roads/Rail ↔ Archaeology / Cultural Heritage** | Historic corridor reconstructions cite archaeology / cultural heritage **as context only**. Exact archaeological coordinates are **denied** at the Roads/Rail surface — corridor projections may not be used to derive site location. Sovereignty review propagates. | [DOM-ROADS §F] [DOM-ARCH] [ENCY §24.4.11] |
| **Roads/Rail ↔ Frontier Matrix** | Access observations bound the access cells of the Frontier Matrix; matrix-cell publication remains the Frontier Matrix's release surface, not Roads/Rail's. | [ENCY §24.4.11] |
...
> [!NOTE]
> **Reviewer separation-of-duties** (ADR-S-09 PROPOSED) is currently unresolved. Until the threshold is set, this matrix records the requirement but does not enforce it; treat as `NEEDS VERIFICATION`. [DIRRULES]
...
> [!WARNING]
> The following failure modes are **preservation failures** even when they leave the system technically functional. Each maps to an axis above; recognizing them in review is part of release maturity.

| Failure mode | What gets lost | Axis | Citation basis |
|---|---|---|---|
| **Silent vacuuming** | RAW payload or temporal history is compacted away without an archive manifest; later queries silently return a different answer. | Lifecycle | KFM-P9-PROG-0011, KFM-P9-PROG-0018 [UIA-BUILD] |
...
PROPOSED validators per DOM-ROADS §K; the **preservation-specific** validators below extend that list. None are CONFIRMED implemented in the repo — all are `NEEDS VERIFICATION` until a mounted repo, schema, and test surface confirm them.
...
- PROPOSED: **RAW-payload-under-source-identity preservation test** — every promoted Road / Rail / Historic-Route record must resolve back to a RAW `SourceDescriptor` and payload hash.
...
- PROPOSED: **AIReceipt presence test** — every Roads/Rail Focus Mode answer carries an `AIReceipt` and resolves to an `EvidenceBundle` (target rate: 100%, per Atlas v1.1 §24.11.4).

> [!NOTE]
> Validator placement is governed by `directory-rules.md`. Cross-lane validators (e.g., the archaeological-coord denial) belong under `tools/validators/<topic>/`, **not** under a domain-segmented validator folder. [DIRRULES §12]
...
| OQ-PRES-02 | **Which Roads/Rail object families must never be vacuumed without external archival proof?** | An explicit never-vacuum register tied to the retention SLA above. | OPEN — KFM-P9-PROG-0018 open question |
...
| Implement `RouteUncertaintyProfile`. | As above. | NEEDS VERIFICATION |
...
| `schemas/contracts/v1/transport/` | Schema responsibility root for Roads/Rail; per encyclopedia §5.1. | PROPOSED |
| `policy/sensitivity/<roads-rail-trade or transport>/` | Sensitivity policy lane for Roads/Rail (analogous to `policy/sensitivity/fauna/`). | PROPOSED |
...
| `data/{raw,work,quarantine,processed}/roads-rail-trade/` | Lifecycle lanes. | PROPOSED per `directory-rules.md` §12 |
| Naming: `roads-rail-trade` (docs) vs `transport` (schema responsibility root). | Whether to unify or keep as documented divergence. | NEEDS VERIFICATION — flag to `docs/registers/DRIFT_REGISTER.md` |
...
> Placeholders below reflect prior-session-authored artifacts and Directory Rules-defined homes. Mounted-repo presence is `NEEDS VERIFICATION`.
...
| Tag | Source |
|---|---|
| `[DOM-ROADS]` | KFM Roads, Rail, and Trade Routes Architecture Plan (Atlas v1.0 Ch. 13; Atlas v1.1 retains verbatim). |
...
<sub>Last updated: 2026-05-19 &nbsp;·&nbsp; Status: **draft** &nbsp;·&nbsp; Version: **v1** &nbsp;·&nbsp; [↑ Back to top](#contents)</sub>
