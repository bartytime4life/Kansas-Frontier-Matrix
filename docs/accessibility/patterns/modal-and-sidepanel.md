...
| Layer registry (declarative layers + gating) | `web/` (path TBD; not confirmed in repo) | Frontend | Intended: schema-validated; prevents ad-hoc layer leakage |
...
- Overlays create predictable accessibility failure modes:
  ...
  - Escape/backdrop click behavior conflicts with destructive actions
...
~~~mermaid
flowchart LR
  Trigger[Trigger element] -->|open| Overlay[Overlay container]
  Overlay -->|trap focus| Focus[Focus stays inside overlay]
  Overlay -->|close| Return[Return focus to trigger]
  Overlay --> Suppress[Background suppressed]
  Overlay --> Scroll[Optional: scroll lock]
~~~
...
User->>UI: Activate trigger
UI->>Page: Suppress background
UI->>UI: Move focus to initial target
loop while modal open
  User->>UI: Tab or Shift+Tab
  UI->>UI: Keep focus within overlay
end
User->>UI: Close (Escape or Close button)
...
- Predictable close behavior: Escape and close button work as expected; destructive flows require explicit confirmation.
...
| Layer registry | `web/` (path TBD; not confirmed in repo) | Schema-validated; breaking changes require versioning |
...
- Dismissal:
  ...
  - Escape closes the dialog unless it would create data loss or unsafe dismissal (if disabled, provide a clear on-screen explanation)
  - Backdrop click is optional; if enabled, it must behave the same as Escape (non-destructive only)
