...
- Indigenous and vulnerable communities are treated with **CARE** and sovereignty-aware framing  
...
|-----------------------|-----------------------------------------------------------------------------|----------------------------|
| **Contrast Clarity**  | AQI colors maintain ≥ 4.5:1 contrast; legends/panels use high-contrast text.| WCAG 1.4.3                 |
...
- Risk description text avoids alarmist wording and focuses on recommended actions (“limit outdoor activity”) rather than blame.
...
- Be validated by `color-contrast.yml`.
...
- Pollutants, interval, and uncertainty  
- Named health framework (e.g., “US EPA AQI”)  

Optional Indigenous/justice extensions:

- `environmental-justice-zone`: `"Yes/No"`  
- `tribal-land-mask-applied`: `true/false`  
...
| Arrow keys           | Navigate data points on keyboard-accessible charts | “Nov 2: AQI 68 — Moderate, PM2.5 primary pollutant.” |
| `Enter` / `Space`    | Toggle pollutant filters or detail views        | “County-level AQI details expanded.”              |
| `Esc`                | Close modals / details panels                   | “Detail view closed. Returning to AQI summary.”   |
| `aria-live="polite"` | Announce AQI updates                            | “AQI updated: 58 — Moderate, PM2.5.”              |

Patterns MUST be tested with NVDA, VoiceOver, and TalkBack and documented in audits.
...
|-----------------------|-----------------------------------------------|----------------------------------------|
| **axe-core**          | ARIA structure, contrast, landmarks            | `a11y_air_quality.json`                |
| **Lighthouse CI**     | Keyboard, performance, visual contrast         | `lighthouse_air_quality.json`          |
| **jest-axe**          | Component-level AQI widgets                    | `a11y_air_quality_components.json`     |
| **faircare-air-audit**| Tone, framing, consent & EJ checks             | `air_quality_audit.json`               |

Validation must confirm:

- No **color-only** coding of risk categories (text + icon + pattern recommended).  
- Risk language is bounded, factual, and non-stigmatizing.  
- Uncertainty, intervals, and provenance visible in the UI.
...
|-----------------------|--------------------------------------------------------------------------------------|
| **Collective Benefit**| AQI data used for public-good decisions (health advisories, EJ monitoring).         |
| **Authority to Control** | Agencies and communities can govern how local AQI data is aggregated or shown.  |
| **Responsibility**    | All visualizations include methodology, time windows, and uncertainty context.      |
| **Ethics**            | Messaging avoids blaming communities; clearly differentiates modeled vs observed data.|
...
|--------:|------------|-----------------------|---------------------------------------------------------------------------------------------------|
| v11.2.3 | 2025-11-29 | Accessibility Council | Upgraded to v11.2.3; telemetry v2; emoji directory layout; clarified uncertainty & EJ/sensitivity treatment. |
