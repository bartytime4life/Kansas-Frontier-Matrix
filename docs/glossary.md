...
Aligned with **MCP v6.3**, **FAIR+CARE**, **Platinum README v7.1**, **STAC/DCAT**, and **CIDOC CRM** to ensure consistent terminology across all documentation, APIs, and pipelines.

<img alt="Docs · MCP_v6.3" src="https://img.shields.io/badge/Docs-MCP_v6.3-blue" />
...
- Cross-references to related components or docs  
- Links to external standards where appropriate  

When adding a term, ensure:

- Clear, concise definition  
- At least one **Related Component** (directory, module, or doc)  
- Reference to an internal doc or external standard where relevant  
...
| **Focus Mode** | AI-assisted, entity-centric view that synchronizes map, timeline, and narrative insights with explainability and governance safeguards. | Web/UI, API, AI |
...
| **Telemetry** | Build/validation metrics exported to unified JSON telemetry. | `../releases/*/focus-telemetry.json` |
...
| **LangGraph** | DAG-based orchestration engine for ETL + AI workflows. | `src/agents/`, `src/pipelines/` |
| **MCP Server** | Tool endpoint exposed to agent frameworks (e.g., Neo4j, STAC, GDAL). | `docs/architecture/api-architecture.md` |
...
| **Supply Chain Security** | SLSA, SBOMs, and signatures for provenance. | `docs/security/supply-chain.md` |
...
| **Governance Ledger** | Append-only records of approvals, audits, and ethical decisions. | `docs/reports/audit/*` |
...
| **Transformers** | Foundation models for NLP tasks (e.g., summarization, classification). | `src/ai/models/*` |
| **Embedding Model** | Vector representations for semantic search and entity linking. | `src/ai/models/embeddings/` |
| **SHAP** | Explainability method for feature contribution analysis. | `src/ai/explainability/` |
...
| **Story Node** | Narrative unit linking text, geospatial footprint, and time; aligned with STAC/DCAT/CIDOC. | `docs/architecture/data-architecture.md` |
| **Focus Transformer** | Specialized transformer model powering Focus Mode narratives. | `src/ai/models/focus_transformer_v2/` |
...
| **COG** | Cloud-Optimized GeoTIFF for tiled web access. | ETL rasters, `data/processed/` |
...
| **Hydrography** | Rivers, lakes, basins networks (e.g., NHD). | Hydrology analyses |
| **DCAT JSON-LD** | Dataset metadata representation for linked data. | Catalog export and integration |
...
| **GeoSPARQL 1.1** | OGC           | Spatial reasoning in graph layers        |
| **SPDX 2.3**   | Linux Foundation | Software bill of materials                |
| **CycloneDX**  | OWASP            | SBOM alternative for security tooling     |

---

## 🧩 Controlled Vocabulary (Selected)

Use these controlled values in metadata to avoid drift.

| Field        | Allowed Values                               | Notes                           |
|--------------|----------------------------------------------|----------------------------------|
| `care.tag`   | `public` · `restricted` · `sensitive`        | CARE governance labels          |
| `stac:stage` | `raw` · `work` · `staging` · `processed` · `archive` | KFM lifecycle states     |
| `license`    | `CC-BY-4.0` · `MIT` · `ODbL-1.0` · `Public-Domain` | As permitted per asset    |
| `crs`        | `EPSG:4326`                                  | Default published CRS           |
| `a11y.level` | `AA` · `AAA`                                 | Target WCAG conformance         |
