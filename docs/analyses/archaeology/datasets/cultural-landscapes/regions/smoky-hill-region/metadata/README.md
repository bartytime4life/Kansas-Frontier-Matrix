---
title: "🧾 KFM v11.2.3 — Smoky Hill Cultural Drainage Region Metadata (Diamond⁹ Ω / Crown∞Ω Ultimate Certified)"
path: "docs/analyses/archaeology/datasets/cultural-landscapes/regions/smoky-hill-region/metadata/README.md"
version: "v11.2.3"
...
intent: "cultural-landscape-region-smoky-hill-metadata"
...
# 🧾 **KFM — Smoky Hill Cultural Drainage Region · Metadata**  
`docs/analyses/archaeology/datasets/cultural-landscapes/regions/smoky-hill-region/metadata/README.md`

**Purpose:**  
Define and govern the **region-specific metadata** for the **Smoky Hill Cultural Drainage Region**.  
This directory holds **DCAT/JSON-LD, contextual markdown, and optional helper metadata** that:

- Describe the Smoky Hill drainage corridor as a governed cultural landscape dataset  
...
This `metadata/` directory provides the **Smoky Hill–specific view** of:
...
- Ensures that all Smoky Hill metadata remains **graph-safe** and **governed**.
...
~~~text
docs/analyses/archaeology/datasets/cultural-landscapes/regions/smoky-hill-region/metadata/
├── 📄 README.md                                        # This file (region metadata contract)
│
├── 🧾 dcat-smoky-hill-region-v1.jsonld                 # DCAT/JSON-LD metadata record (canonical per-region)
├── 🧾 extras-smoky-hill-region-metadata.v1.json        # Optional derived/summary metadata (non-authoritative)
│
└── 📖 context-smoky-hill-region-v1.md                  # Human-readable contextual narrative (for Story Nodes/Focus Mode)
~~~

**Directory contract:**

- `dcat-smoky-hill-region-v1.jsonld` is the **canonical region-level metadata record** for Smoky Hill (version 1).  
- The global metadata registry entry for this region must reference this file as its authoritative source.  
- `context-smoky-hill-region-v1.md` provides narrative/contextual content and is **not a schema authority**.  
...
- `kfm:region_slug`  
  - `"smoky-hill-region"`.  
- `kfm:region_kind`  
  - `"drainage"` (and optionally `"eco-cultural"` if modeled).

### Descriptive Content

- `dct:description`  
  - Concise description of the region’s purpose and context (Smoky Hill drainage corridor as a multi-phase cultural landscape).  
- `dcat:keyword`  
...
- `kfm:stac_item_ids`  
  - Optional list of Item IDs under `regions/stac/items/` (e.g., `item-smoky-hill-region-v1`).  
- `kfm:provenance_ref`  
  - Reference(s) to canonical PROV-O logs, e.g.:  
    `docs/analyses/archaeology/datasets/cultural-landscapes/provenance/smoky-hill-region-v1.json`.  
...
The Smoky Hill metadata must explicitly encode CARE and sovereignty information:
...
- `care:notes`  
  - Explanation of generalization strategy, drainage-corridor considerations, review process, and any suppressed areas.  
...
- CARE metadata here must **match**:
  ...
  - Region entries in the region provenance index registry.  
...
Region metadata must encode the Smoky Hill region’s **temporal scope** and cultural-phase alignment:
...
- Refer to the region’s spatial extent via:
  - Hydrological unit names, drainage basin descriptors, valley corridor descriptions, etc.  
- Use KFM ontology references where available:
  - `kfm:hydrological_unit_ref`, `kfm:eco_region_ref`, `kfm:geomorphic_unit_ref`.  
...
- Describe the Smoky Hill region as a **cultural drainage corridor**.  
- Summarize key environmental, geomorphological, and cultural factors:
  - River valley structure, terrace systems, movement and interaction corridors.  
...
- `kfm:region_slug` must match `smoky-hill-region`.  
- `kfm:stac_collection_id` and `kfm:stac_item_ids` must refer to existing STAC artifacts for Smoky Hill.  
...
From `dcat-smoky-hill-region-v1.jsonld`, graph loaders derive:
...
- Discover the Smoky Hill region as a spatial/narrative anchor.  
...
- Filter overlays by region kind (`"drainage"`), cultural phase, or sensitivity.  
...
When updating the Smoky Hill region:
...
3. **Edit Region Metadata JSON-LD**  
   - Update `dcat-smoky-hill-region-v1.jsonld`, or bump to `v2` as needed.  
   - Maintain links to STAC and provenance.

4. **Update Context Markdown**  
   - Adjust `context-smoky-hill-region-v1.md` (or bump version) to reflect new understanding or narratives.

5. **Run Local Validation**  
   - Use repo tooling (e.g., `make validate-region-smoky-hill-metadata`) to run all checks.
...
|----------|------------|---------------------------------------------|-------------------------------------------------------------------------|
| v11.2.3  | 2025-12-02 | Cultural Landscape WG · FAIR+CARE Council   | Established Smoky Hill region metadata layout, DCAT/JSON-LD contract, CARE/sovereignty rules, and CI linkage to STAC and provenance; aligned with global region metadata registry and KFM-MDP v11.2.2. |
...
[⬅ Back to Smoky Hill Cultural Drainage Region](../README.md) · [⬅ Back to Cultural Landscape Regions](../..//README.md) · [⬅ Back to Global Region Metadata Registry](../../metadata/README.md)
