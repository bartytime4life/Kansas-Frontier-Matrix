<!-- [KFM_META_BLOCK_V2]
doc_id: kfm://doc/9b62f0d2-7b2e-4f69-8b0b-7a14cfe8d3fe
title: KFM CLI
type: standard
version: v1
status: draft
...
related:
  - (not confirmed in repo) docs/cli/README.md
tags:
  - kfm
  - cli
  - governance
  - promotion-contract
notes:
  - Contract-first CLI surface; source of truth is `kfm --help`.
  - Default-deny and fail-closed behavior required.
[/KFM_META_BLOCK_V2] -->
...
`policy: fail-closed` · `audit: run receipts` · `zones: RAW → WORK/QUARANTINE → PROCESSED → CATALOG/TRIPLET → PUBLISHED` · `outputs: table|json`
...
> **Contract-first rule:** the authoritative command list is whatever `kfm --help` prints in *your* build.  
> If this document conflicts with reality, fix one of them—either update the docs or align the code.
...
- **Data lifecycle operations:** move artifacts through the truth path zones  
  `RAW → WORK/QUARANTINE → PROCESSED → CATALOG/TRIPLET → PUBLISHED`
- **Promotion gates:** run Promotion Contract checks (identity, license, sensitivity, catalogs, receipts, policy tests) **before** anything reaches runtime surfaces.
- **Rebuildable projections:** rebuild search/graph/tiles/DB projections from canonical truth (object store + catalogs + audit).
- **Evidence inspection:** resolve `EvidenceRef` → policy-filtered `EvidenceBundle` used by UI and Focus Mode.
- **Governed runs:** emit receipts / audit references for pipeline runs and Focus Mode runs.
...
- A backdoor to bypass the **trust membrane** (no direct database/object-store access from client mode).
- A tool that leaks existence of restricted resources (no “it exists but you can’t see it” inference surface).
- A replacement for policy, catalog, or provenance systems (it orchestrates them; it does not redefine them).
...
### Normative language
This doc uses the keywords **MUST**, **SHOULD**, and **MAY** to indicate requirement level.
...
### Promotion Contract
Promotion to runtime **MUST** fail-closed unless all required gates pass.
...
- **Gate G (optional posture):** SBOM, perf smoke, a11y smoke

### Trust membrane
Clients—including this CLI when acting as a client—**do not** bypass the governed boundary.  
The policy/provenance boundary must be enforceable in CI and at runtime.
...
### CLI shape
```text
kfm [GLOBAL FLAGS] <command> [subcommand] [args]
```
...
- `--output <path>` (write machine output to a file; still emit human summary unless `--quiet`)
- `--quiet` / `--verbose`
- `--no-color`
- `--audit` (print `audit_ref` and/or `run_id` prominently when present)
...
**PR-first, promotion-gated** is the safe default.
...
2. Open a PR including:
   ...
   - fixtures + expected outputs (for CI)
3. CI runs:
...
Projections are rebuildable; reindexing **MUST NOT** mutate canonical truth.

```bash
# Rebuild everything
kfm index rebuild

# Rebuild for a specific promoted dataset version
...
- `status`: `ok|error`
- `audit_ref` and/or `run_id` (when applicable)
- `policy_label` (policy-safe)
- `dataset_version_id` (when applicable)
- `digests` (when applicable)
- `errors` (stable error model with `error_code`, policy-safe `message`, and optional `audit_ref`)
...
```json
{
  ...
  "errors": [
    {
      "error_code": "GATE_D_CATALOG_INVALID",
      "message": "Catalog cross-link validation failed.",
...
### Table output
When `--format table` is used, output **SHOULD**:

- be readable in a narrow terminal,
- include gate-by-gate summaries for validation/promotion commands,
- avoid printing restricted details when policy denies.
...
- If catalogs/receipts cannot be validated, promotion **MUST** fail.
...
- align deny/not-found pathways as appropriate to prevent inference

### Audit artifacts are sensitive
...
### Minimum test gates for new commands
New commands **MUST** ship with:
