...
* "docs/pipelines/validation-observability/README.[md@v10.4.x](mailto:md@v10.4.x)"
...
Every pipeline must pass all required layers before promotion to a **blue/green** release.
...
A corresponding provenance/telemetry layout:
...
**Failure behavior:** Hard block ingest. No data passes into `data/raw/**` or staging if Frictionless fails.
...
* Required checks (per dataset/domain):

  * Type coercion & non-null constraints
  * Unique IDs / natural keys
  * Bounding boxes within Kansas (and subregions if specified)
  * Temporal ordering & validity (no impossible intervals)
...
**Failure behavior:** Transform aborts; error details are serialized to `provenance/validation/pandera/<run_id>.json`.

---

## 5. Layer 3 — Great Expectations Publish Gate
...
* Run frequency & on-time execution
* Missed or delayed runs
* Duration anomalies (time-series baseline)
* Runtime error signatures (stack traces, error cause tags)
* Energy usage & carbon estimates (ISO 50001 / 14064-aligned)

These metrics flow into telemetry as described in `telemetry_schema`.

---

## 7. Lineage Layer — OpenLineage & UI

All pipelines emit **OpenLineage events**:

* Job name
* `run_id`
* Input dataset URIs (e.g., STAC assets or tables)
* Output dataset URIs
* Schema fingerprints / checksums
* `commit_sha` and pipeline version

A lineage UI (Marquez or equivalent) MUST allow:

* Full **graph of lineage** across datasets, runs, models, and Story Nodes
* Dataset impact analysis for changes & rollbacks
* Run-level metadata (timing, environment, parameters)
...
**PROV-O nodes include:**

* `prov:Activity` → pipeline job / run
* `prov:Entity` → datasets, artifacts, models
* `prov:Agent` → pipeline owner / automation
* `prov:wasGeneratedBy` → run_id
* `prov:qualifiedUsage` → validation reports, telemetry references

Story Nodes and Focus Mode v3 reference validation and telemetry evidence to support **narrative transparency** (e.g., “this overlay uses data that passed schema + drift validation on 2025-11-18”).
...
1. Run **Frictionless** validation on ingest data changed in PR.
2. Run **pandera** schema checks on modified transforms.
3. Run **GE checkpoints** on publish-ready data.
...
* Telemetry & cron summaries are referenced via `telemetry_ref` and `telemetry_schema`.
...
### 10.2 Great Expectations Checkpoint
...
Every production pipeline must include functions equivalent to:

* `validate_ingest()` → Frictionless (external boundary)
* `validate_transform()` → pandera (internal transformations)
* `validate_publish()` → GE Checkpoint (publish gate)
* `record_lineage()` → OpenLineage / PROV-O
* `record_telemetry()` → metrics & energy/carbon logging
* `write_provenance()` → JSON reports under `provenance/**`

**No dataset** is allowed to bypass these gates on its path to blue/green production.

---

## 12. Governance Compliance

This framework satisfies and operationalizes:

* **MCP-DL v6.3** — Documentation-first, fully-specified validation logic.
* **FAIR principles** — Findable, Accessible, Interoperable, Reusable metadata for validation results.
* **CARE principles** —

  * Collective Benefit: validated data reduces harm from poor quality.
  * Authority to Control: Indigenous-partnered rules for sensitive gates.
  * Responsibility: explicit monitoring & remediation for failures.
  * Ethics: transparent, auditable, and non-extractive data usage.
* **Diamond⁹ Ω / Crown∞Ω certification** — reliability, tracing, sustainability.

---

## 13. Versioning & Backwards Compatibility

All validation schemas (Frictionless, pandera, GE) MUST include:

* `schema_version`
* `last_modified`
* `breaking_change = true|false`

Before applying schema updates, pipelines MUST:

* Run compatibility checks against representative historical data.
* Log any “breaking” change into governance notes and, where relevant, STAC/DCAT metadata.
...
* Temporal anomaly detection on ETL run durations & resource usage
* Focus Mode v3 narratives with validation & telemetry evidence overlays
* AI assistants to propose contract/expectation updates based on repeated failure patterns
