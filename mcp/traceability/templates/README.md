...
```flowchart LR; A["⚙️ ETL / Pipelines"] --> B["📚 Catalogs - STAC • DCAT • PROV"]; B --> C["🕸️ Graph - Neo4j"]; C --> D["🔌 API Boundary - governed access"]; D --> E["🗺️ UI - map + timeline + layers"]; E --> F["📖 Story Nodes - evidence manifest"]; F --> G["🧠 Focus Mode - AI - citations + ledger"];
