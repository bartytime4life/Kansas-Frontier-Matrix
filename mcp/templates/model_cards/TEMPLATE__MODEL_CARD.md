<!--
📄 FILE: mcp/dev_prov/templates/TEMPLATE_model_card.md
🎯 PURPOSE: A KFM-native Model Card + DevProV (development provenance) template.
✅ COPY/PASTE: Duplicate this file into mcp/dev_prov/model_cards/{{MODEL_ID}}.md (or equivalent) and fill in all required fields.
-->

---
kfm:
  project: "Kansas Frontier Matrix (KFM) 🌾🧭"
  template: "TEMPLATE_model_card"
  template_version: "1.0.0"
  evidence_first: true
  provenance_first: true
  human_in_the_loop: true
model:
  id: "{{MODEL_ID}}"                    # e.g., kfm-focusmode-llm-v1
  name: "{{MODEL_NAME}}"                # e.g., Focus Mode Answerer
  version: "{{MODEL_VERSION}}"          # e.g., 1.2.3
  stage: "{{STAGE}}"                    # research | staging | production | deprecated
  type: "{{MODEL_TYPE}}"                # LLM | embedding | classifier | detector | correction | simulator | vision | hybrid
  modalities: ["{{MODALITY}}"]          # text | image | audio | video | geo | timeseries | multimodal
  provider: "{{PROVIDER}}"              # openai | local | hf | custom | other
  base_model: "{{BASE_MODEL}}"          # if applicable
  fine_tuned: {{FINE_TUNED}}            # true | false
  retrieval_augmented: {{RAG_ENABLED}}  # true | false
owners:
  product_owner: "{{NAME_OR_TEAM}}"
  tech_owner: "{{NAME_OR_TEAM}}"
  security_contact: "{{NAME_OR_TEAM}}"
  data_steward: "{{NAME_OR_TEAM}}"
release:
  created_utc: "{{YYYY-MM-DDThh:mm:ssZ}}"
  repo_commit: "{{GIT_SHA}}"
  build_id: "{{CI_BUILD_ID}}"
  artifact_registry: "{{OCI_REGISTRY_OR_OTHER}}"
  artifact_digest: "{{SHA256_DIGEST}}"  # required for production
  signatures:
    cosign: "{{COSIGN_BUNDLE_OR_REF}}"  # recommended
  sbom:
    spdx: "{{SBOM_REF}}"                # recommended
licenses:
  model_weights: "{{LICENSE}}"
  code: "{{LICENSE}}"
  data: "{{LICENSE_OR_MIXED}}"
---
