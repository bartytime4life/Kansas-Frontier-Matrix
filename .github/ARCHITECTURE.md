...
- **Telemetry snapshots** (energy, performance, compliance)  
...
| `stac-dcat-bridge.yml` | Ensures STAC↔DCAT parity; publishes diffs | Schedule / Release / PR | `reports/self-validation/bridge/parity_report.json` |
| `faircare-validate.yml` | CARE consent flags, provenance, data contracts | PR / Push | `reports/fair/faircare_summary.json` |
| `docs-lint.yml` | Markdown headings, anchors, links, fenced blocks | PR / Push | `reports/self-validation/docs/lint_summary.json` |
| `codeql.yml` | Static security analysis (SARIF) | PR / Schedule | `reports/security/codeql/*.sarif` |
| `trivy.yml` | CVE scan on images & lockfiles (fail CRITICAL) | PR / Push | `reports/security/trivy/*.json` |
| `build-and-deploy.yml` | Builds & deploys the web client/docs | After validations | `docs/reports/telemetry/build_metrics.json` |
| `telemetry-export.yml` | Aggregates CI/CD + sustainability metrics | Post-build | `releases/v10.2.0/focus-telemetry.json` |
| `ai-model-audit.yml` | Bias/drift, SHAP export, model card checks | Schedule / Model Commit | `reports/audit/ai_model_faircare.json` |

> 💡 **Tip:** All artifacts are hashed and referenced by `sbom.spdx.json` and `manifest.zip` for **SLSA provenance**.
...
Outputs are **append-only**, signed, and cross-linked to commits for verifiable lineage.
...
| Secrets | GitHub Encrypted Secrets / OIDC | Continuous | Rotation; no plaintext in repo |
...
**Disclosure Policy:** `.github/SECURITY.md` defines reporting channels, patch SLAs, and backport strategy.
...
|---|---|---|
| FAIR | STAC/DCAT validation, catalog integrity, DOIs | `stac-validate.yml`, `stac-dcat-bridge.yml` |
...
| ISO 50001 / 14064 | Energy & carbon management | Telemetry (build energy / CO₂e) |
| SLSA / SPDX | Supply chain integrity & licensing | `sbom.spdx.json`, signed artifacts |
...
| Metric | Description |
|---|---|
| `workflow_duration_sec` | Per-workflow runtime |
| `build_energy_wh` | Build energy usage (ISO 50001) |
...
| `docs_lint_pass` | Markdown compliance rate |
| `stac_parity` | STAC↔DCAT alignment status |
...
3. **Executes FAIR+CARE Audit** with reviewer gates where required  
4. **Exports Telemetry** for sustainability and ethics reporting

All logs are **append-only**, signed, and linked to commit SHAs for verifiable provenance.
...
|---|---|---|---|
| v10.2.2 | 2025-11-12 | FAIR+CARE Council & DevSecOps | Upgraded to v10.2: STAC↔DCAT bridge parity reports, stricter CVE gates, energy/CO₂ telemetry, immutable ledger sync. |
| v10.0.0 | 2025-11-09 | A. Barta | Introduced AI audit pipeline, ISO 50001 telemetry, SLSA/SPDX provenance alignment, and governance ledger updates. |
| v9.7.0 | 2025-11-05 | A. Barta | Added security posture, governance mapping, and CI/CD artifacts. |
