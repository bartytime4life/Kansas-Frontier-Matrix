...
doc_uuid: "urn:kfm:doc:github:apps-readme:v1.0.2"
semantic_document_id: "kfm-github-apps-readme-v1.0.2"
event_source_id: "ledger:kfm:doc:github:apps-readme:v1.0.2"
...
# GitHub Apps — Repository Automation
...
- Standardize how we request/review app permissions (least privilege) and how CI workflows authenticate **without committing secrets**.
- Keep automation aligned with KFM’s contract-first pipeline rules and canonical subsystem homes (see `docs/MASTER_GUIDE_v12.md`).

### Scope

| In Scope | Out of Scope |
|---|---|
| Secret-free GitHub App manifests, permission justifications, mapping apps → workflows/jobs, rotation/runbook pointers, telemetry pointers, decommissioning guidance | Private keys, webhook secrets, installation tokens, and other credential *values* (must live in secret storage); GitHub Actions workflow definitions themselves; generating new governance policy text |
...
- Glossary link: `docs/glossary.md` *(not confirmed in repo; use the repo’s canonical glossary path if different)*
- Terms used in this doc:
  ...
  - **OIDC**: preferred for external cloud auth (not a replacement for GitHub App auth for GitHub API calls).
...
| App inventory | `.github/apps/inventory.md` | Repo maintainers | Optional but recommended (single source of truth for installed apps) |
...
| Ingestion architecture | `docs/architecture/KFM_INGEST_ARCHITECTURE.md` *(if present)* | Data engineering | ETL IO patterns; provenance expectations |
...
| Universal template | `docs/templates/TEMPLATE__KFM_UNIVERSAL_DOC.md` | KFM maintainers | Governing structure for this doc |
| Story Node template | `docs/templates/TEMPLATE__STORY_NODE_V3.md` | Story owners | Narrative + Focus Mode surfacing |
| API contract extension template | `docs/templates/TEMPLATE__API_CONTRACT_EXTENSION.md` | API owners | Contract changes for REST/GraphQL |

### Definition of done (for this document)

- [ ] Front-matter complete + valid (path/version/last_updated)
- [ ] All references resolve or are explicitly labeled “not confirmed in repo” (as applicable)
- [ ] Folder contract documented and consistent across apps
- [ ] Every app has required files and maps permissions → workflows/jobs
- [ ] No secrets committed (private keys/webhook secrets/installation tokens) anywhere in `.github/apps/**`
- [ ] Workflows depending on an app are listed and reviewed
- [ ] Validation steps below are repeatable (doc lint, link checks, secret scan)
- [ ] Governance + CARE/sovereignty considerations explicitly stated (especially for publish workflows)
...
| Schemas | `schemas/` *(if present)* | Schema sources for STAC/DCAT/PROV/story/ui/telemetry |
| Tools | `tools/` *(if present)* | Validators, secret/PII scan runners, publishing helpers |
| Tests | `tests/` *(if present)* | Unit + integration tests |
...
| Story Nodes | `docs/reports/story_nodes/` *(pattern; draft/published split if defined)* | Templates + governed narratives + assets |
...
| MCP | `mcp/` | Run logs + experiments + artifacts |
| Releases | `releases/` *(if used)* | Versioned packaged artifacts |
...
└── 📁 apps/
    ├── 📄 README.md
    ├── 📄 inventory.md                  # optional but recommended
    └── 📁 <app_slug>/
        ├── 📄 permissions.md
        ├── 📄 manifest.json
        ├── 📄 workflows.md
...
- `permissions.md` (required)
  ...
  - Map each permission to workflow job(s) and the GitHub operations it enables.
...
- `manifest.json` (required)
  ...
  - If this repo is public, avoid embedding internal-only callback/webhook endpoints.
...
- `<app_slug>` should be stable, kebab-case, and safe to expose publicly (example: `kfm-catalog-publisher`).
- Prefer 1 app per distinct automation role over “one app to rule them all” (enables least privilege).

### Inventory contract (recommended)

If present, `.github/apps/inventory.md` should be a single table that answers:

- Which apps exist (and whether they are installed)?
- Where are they installed (org vs repo) and for which repositories?
- Which workflows/jobs depend on them?
- Who owns them and who approves permission changes?

Suggested columns (minimum):

| app_slug | app_name | install_scope | repos_in_scope | workflows | owner | status | notes |
|---|---|---|---|---|---|---|---|
...
- KFM is an open-source **geospatial + historical** “living atlas” that publishes governed evidence (STAC/DCAT/PROV), a graph, contracted APIs, and a map/narrative UI.
- CI/CD keeps the canonical pipeline synchronized and reviewable: **ETL → STAC/DCAT/PROV → Graph → APIs → UI → Story Nodes → Focus Mode**.
- GitHub Apps are preferred over long-lived tokens for GitHub API automation (scoped permissions + auditable installations).
...
- Prefer **OIDC** when authenticating to external cloud resources (object storage / compute).

  *(OIDC does not replace GitHub App auth for GitHub API calls.)*
...
### GitHub App lifecycle (operational model)

1. **Request (in-repo)**
   - Add `.github/apps/<app_slug>/` with required docs.
   - Describe required permissions and map them to workflows/jobs.
   - Trigger security review for any write/admin scope.
2. **Provision (GitHub UI)**
   - Create the app from the committed `manifest.json`.
   - Install at the required scope (org/repo).
3. **Operate (CI/CD)**
   - Store generated credentials in secret storage.
   - Workflows request short-lived installation tokens at runtime.
4. **Rotate (routine / incident)**
   - Rotate private keys and/or webhook secrets per `rotation.md`.
   - Verify workflows continue to pass (and do not leak secrets).
5. **Retire**
   - Remove workflow dependencies first.
   - Uninstall the app and archive docs (retain history for auditability).

### Assumptions

- This repository may be public; anything committed under `.github/apps/**` must be safe to disclose.
- Secrets are managed via GitHub Secrets/Environments or an external secret manager (never stored in git).
- CI gates should “validate if present; fail if invalid; skip if not applicable” where appropriate.
...
- Permission increases require explicit security review and justification (least privilege).
- Treat callback URLs/webhook URLs as potentially sensitive when this repo is public.
...
- Add `.github/apps/inventory.md` listing every installed app, installation scope, and workflows/jobs that depend on it.
- Standardize `rotation.md` per app (key rotation cadence + incident response pointers).
- Add a lightweight validator (CI) to enforce the per-app folder contract (presence + link checks + JSON parse + secret scan).
...
| Permission justification | Markdown | `.github/apps/<app_slug>/permissions.md` | Must map permissions → workflow jobs; least privilege |
...
| Output | Format | Path | Contract / notes |
|---|---|---|---|
...
- Never include: private keys, webhook secrets, installation tokens, or credential values.
...
- Links to workflows and governance references resolve (or are explicitly labeled “not confirmed in repo”).
...
|---|---|---|
| GitHub Actions workflows | Trigger, validate, run pipelines, publish artifacts | GitHub events + workflow inputs |
| GitHub App integration | Scoped GitHub API access for automation | App manifest + installation scope |
| Secret storage | Store private keys/webhook secrets | GitHub Secrets/Environments or external manager |
| Validators / gates | Secret scan, schema validation, link checks, tests | Composite actions + `tools/` (if present) |
| KFM pipelines | Produce catalogs/graph/API/UI/story artifacts | `src/pipelines/**` + run logs |
...
- Story Nodes surfaced in the UI should retain evidence affordances (citations that resolve to datasets/records, and map highlighting where applicable).
- Story metadata should reference key entities (places, people, events, dataset IDs) to enable context-aware surfacing without direct UI → graph coupling.

### Provenance-linked narrative rule

- If a GitHub App–authenticated workflow writes any narrative-facing artifact (for example `docs/reports/story_nodes/**`), it must enforce:
...
Minimum checks (align with KFM “v12-ready” expectations where applicable):

- [ ] Markdown protocol validation (front-matter + required sections)
- [ ] Link/reference checks (no orphan pointers)
- [ ] Secret scan (no credentials in repo)
- [ ] PII scan / sensitive-location leakage checks *(as applicable)*
- [ ] JSON schema validation *(when relevant)*:
  - STAC/DCAT/PROV
  - story node schemas (if present)
  - telemetry schemas (if present)
  - UI layer registry schemas (if present)
- [ ] Graph integrity tests *(when relevant)* (constraints, expected labels/edges)
- [ ] API contract tests *(when relevant)* (OpenAPI/GraphQL schema + tests)
- [ ] UI quality gates *(when relevant)*: accessibility (a11y) checks + “no hidden data leakage” checks
- [ ] Classification / sensitivity propagation checks *(as applicable)*
...
| Promotion blocked | CI gates | `docs/telemetry/` *(if present)* |
...
- Do not use automated tooling to infer sensitive locations or generate new governance policy text.
...
| v1.0.1 | 2025-12-28 | Align to Universal template + Master Guide invariants; add Data/Metadata, STAC/DCAT/PROV alignment, architecture + Story/Focus integration guidance | TBD |
| v1.0.2 | 2025-12-29 | Tighten per-app folder + inventory contract; align CI gates with Master Guide “v12-ready” checks; clarify lifecycle + security notes; strengthen Story/Focus integration wording | TBD |
...
- Ingestion architecture (if present): `docs/architecture/KFM_INGEST_ARCHITECTURE.md`
- Redesign blueprint (if adopted): `docs/architecture/KFM_REDESIGN_BLUEPRINT_v13.md`
- Universal template: `docs/templates/TEMPLATE__KFM_UNIVERSAL_DOC.md`
- Story Node template: `docs/templates/TEMPLATE__STORY_NODE_V3.md`
- API contract extension template: `docs/templates/TEMPLATE__API_CONTRACT_EXTENSION.md`
- Glossary (recommended): `docs/glossary.md` *(not confirmed in repo)*
