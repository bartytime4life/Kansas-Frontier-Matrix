...
- Glossary link: `docs/glossary.md` *(expected canonical location; if missing, add or replace with repo’s glossary path)*
...
| Universal template | `docs/templates/TEMPLATE__KFM_UNIVERSAL_DOC.md` | KFM maintainers | This doc’s governing structure |
...
- KFM uses CI/CD to keep **data → catalogs → graph → APIs → UI → Story Nodes → Focus Mode** synchronized and reviewable.
...
- If a GitHub App–authenticated workflow writes any narrative-facing artifact (e.g., `docs/reports/story_nodes/**`), it must enforce:
