...
## Where this fits
This README lives in `.github/ISSUE_TEMPLATE/` and documents **how we expect issues to be filed** so they are:
- **actionable** (reproducible),
- **governed** (policy-safe),
- **traceable** (evidence-first),
- **time-aware** (explicit dates/timezones).

It is intentionally **safe for public repos**.
...
└─ ISSUE_TEMPLATE/
   ├─ README.md          # this file
   ├─ *.yml / *.md       # issue templates (names vary)
   └─ config.yml         # optional GitHub issue forms config (if used)
```

> TIP  
> If you add/remove templates, update the table in **[Which template should I use](#which-template-should-i-use)** and the checklist in **[Maintainers: keep templates aligned](#maintainers-keep-templates-aligned)**.
...
## Acceptable inputs
You can include:
- **repro steps**, **minimal failing tests**, or **small redacted data slices** (bbox/time/filter) that demonstrate the issue.
- **logs**, **error codes**, **stack traces**, **screenshots**, **screen recordings** *(redacted when needed)*.
- **dataset identities** and **version IDs** (preferred), **run receipts/run_id**, **artifact digests/checksums**.
- **explicit dates** + **timezone** (or UTC), and (when relevant) both:
  - **event time** (when something happened in the world), and
  - **transaction time** (when we ingested/updated it).
...
- precise coordinates / access instructions for sensitive sites,
- personal data (addresses, phone numbers, emails, unique IDs),
- secrets (tokens, API keys, cookies),
- details of vulnerabilities or policy bypasses (see [Security](#security)).

If unsure: **assume restricted** and **generalize**. See [Sensitive data and location safety](#sensitive-data-and-location-safety).
...
> NOTE  
> Template names shown above are **human-facing categories**. The actual template filenames may differ. Keep this README aligned with what’s present in this directory.
...
### Maintainers: keep templates aligned
When templates change, update this README so it stays accurate.

- [ ] Template picker categories match what’s actually in this directory
- [ ] Every template enforces an **evidence pack** (required fields where possible)
- [ ] Security guidance routes vuln reports away from public issues
- [ ] Sensitive location guidance remains clear and conservative
- [ ] Labels recommended here match labels available in the repo

> TIP  
> If you’re using GitHub Issue Forms (`*.yml`), prefer required fields for:
> - repro steps
> - “when” (explicit timestamp + timezone)
> - dataset_id / dataset_version_id / run_id (when relevant)
> - policy label + license status (when relevant)
...
[Back to top](#issue-templates)<!-- [KFM_META_BLOCK_V2]
doc_id: kfm://doc/b49f150f-953e-426a-bae7-2f507cff8f57
title: Issue templates README
type: standard
version: v1
