...
| Owners | `@bartytime4life` *(owner mapping against `./CODEOWNERS` needs verification before merge)* |
| Intended path | `.github/SECURITY.md` |
| Repo fit | GitHub-facing vulnerability-reporting and coordinated-disclosure entrypoint for repo-wide security issues |
| Referenced repo surfaces | [`../README.md`](../README.md) · [`../CONTRIBUTING.md`](../CONTRIBUTING.md) · [`./README.md`](./README.md) · [`./CODEOWNERS`](./CODEOWNERS) · `../SECURITY.md` *(all live-path existence needs verification)* |
| Canonical disclosure path | `.github/SECURITY.md` *(if a root `../SECURITY.md` exists, reduce it to a pointer or keep it word-for-word aligned after live repo verification)* |
...
![Owners badge](https://img.shields.io/badge/owners-bartytime4life-lightgrey)
![Surface badge](https://img.shields.io/badge/surface-.github%2FSECURITY.md-blue)
![Reporting badge](https://img.shields.io/badge/reporting-private--first-red)
![Posture badge](https://img.shields.io/badge/posture-fail--closed-0a7d5a)
![Trust badge](https://img.shields.io/badge/trust-membrane-informational)

> [!IMPORTANT]
> Keep one canonical disclosure-policy path. If the repo also keeps a root `../SECURITY.md`, either reduce that file to a short pointer or keep it text-aligned with this file after live repo verification.

> [!NOTE]
> This draft is intentionally strict about uncertainty. Live GitHub private-reporting settings, monitored inboxes, acknowledgement windows, branch protections, required checks, `CODEOWNERS` coverage, and root-level file layout were not directly verified in this drafting pass and must be confirmed before publication.

## Scope

Use this file for KFM’s public-facing security entrypoint: how to report vulnerabilities privately, what kinds of issues count as security problems, how maintainers should triage them, and how coordinated disclosure should work for repository-wide security-affecting issues.
...
> [!WARNING]
> Treat any unverified root-level `SECURITY.md`, private-advisory setting, workflow name, escalation alias, or reviewer mapping as a merge-time verification item, not as a settled repo fact.
...
| Supply-chain, workflow, or automation failure | credential leaks, over-permissioned GitHub Actions, weak release identity, missing attestations, unsafe automation token usage, review-bypass workflow logic | GitHub automation is part of KFM’s governed delivery path. |
...
KFM security is broader than generic repository hardening. The following principles are load-bearing:
...
| Short-lived automation identity | GitHub App-style control surfaces, constrained job permissions, and short-lived identity are preferred over long-lived personal tokens. |
| Policy in CI and runtime | Policy is not just documentation; it should gate PRs, releases, publication, and response shaping. |
| Trust-visible surfaces | Evidence, freshness, review state, rights posture, and partial/stale/generalized states should stay visible where users act on them. |
| Negative outcomes are valid | `hold`, `deny`, `abstain`, `generalized`, `superseded`, `withdrawn`, and similar states are acceptable trust-preserving results. |
...
| GitHub private advisory / private vulnerability-reporting setting | `NEEDS VERIFICATION` |
...
Exact supported versions, release inventory, and live GitHub security settings were not directly verified for this draft. Until the live repo surfaces those details, use the matrix below.
...
7. Update documentation, runbooks, and release/correction evidence.
...
Above: a high-level disclosure path showing private intake, safe reproduction, containment, fix verification, release-or-correction evidence updates, and coordinated disclosure.
...
- [ ] Review ownership still aligns with [`./CODEOWNERS`](./CODEOWNERS) *(verify live file and coverage)*
...
- [ ] Recheck live branch protection, required status checks, and reviewer/CODEOWNERS enforcement expectations referenced by this policy.
