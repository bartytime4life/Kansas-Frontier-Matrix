...
class CapturedPayload:
    ...
    def __post_init__(self) -> None:
        normalized = tuple(bytes(value) for value in self.chunks)
        ...
        except (TransportValueError, TypeError) as exc:
            raise TransportInputError(str(exc)) from exc

    def __repr__(self) -> str:
        return (
            "CapturedPayload("
            f"digest={self.digest!r}, byte_length={self.byte_length}, "
...
class RetrievalResult:
    ...
    def __post_init__(self) -> None:
        ...
        if numbers != expected and numbers != (0,):
            raise TransportInputError(
                "attempts must be contiguous and 1-based, except "
                "pre-transport cancellation"
            )
        if self.attempts[-1].category is not self.category:
            raise TransportInputError(
                "final category must match the last attempt"
            )

        success = self.category in {
    ...
    def __repr__(self) -> str:
        return (
            ...
            f"source_head={self.source_head!r}, "
            ...
            f"{self.repository_mutation_allowed})"
        )
