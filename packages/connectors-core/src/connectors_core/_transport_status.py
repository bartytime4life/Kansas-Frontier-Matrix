...
def evaluate_status(
    ...
    """Evaluate response routing and metadata before payload inspection."""

    final_url = response.final_url or request.url
    try:
        profile.validate_request(
            TransportRequest(request.method, final_url)
        )
        final_identity = url_identity(final_url)
        request_identity = url_identity(request.url)
    except (ConnectorPrimitiveError, TransportValueError, TypeError, ValueError):
        return (
            failure_evaluation(
    ...
    if final_identity != request_identity:
        return (
            failure_evaluation(
                TransportCategory.UNSAFE_METADATA,
                "REDIRECT_BLOCKED",
    ...
    if status == 304:
        return _evaluate_not_modified(request, response, observed_at), None
    if 300 <= status <= 399:
        return (
            failure_evaluation(
                TransportCategory.UNSAFE_METADATA,
