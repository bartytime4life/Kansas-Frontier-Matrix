...
const artifactDigest = candidateFixture.assets.raster_tiff.content_sha256;
const candidateId = candidateFixture.candidate_id;
const baseRequest: TerrainElevationSampleRequest = {
  ...
  candidateId,
  artifactId: candidateFixture.assets.raster_tiff.filename,
  artifactDigest,
  verticalDatum: candidateFixture.vertical_reference.datum_name,
  ...
  sourceNodataValue: inspectionSample.fixture_encoding.source_nodata_value,
  sample: { column: sourcePixel.column, row: sourcePixel.row },
};

describe("fixture-only Terrarium elevation sampling", () => {
  it("decodes the hash-bound public-safe inspection fixture without asserting provenance verification", () => {
    const result = sampleTerrariumElevationNearestCell(baseRequest);
