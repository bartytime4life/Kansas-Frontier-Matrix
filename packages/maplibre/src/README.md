...
truth_posture: CONFIRMED accepted package-owned seam, exact maplibre-gl 6.9.0 dependency and lock closure, exported renderer-neutral port, deterministic null runtime, bounded MapLibreAdapter lifecycle/camera and rendered-feature selection projection, Vite-specific self-contained worker setup, hosted bounded browser smoke, finite KFM errors, package-local positive and fail-closed tests, no raw renderer types in the public seam, and retired CDN/global performance acquisition / HOLD governed performance fixtures, CSP enforcement, PMTiles, terrain, accessibility, visual parity, release, deployment, and publication evidence / CONFLICTED stale packages/maplibre-runtime references and trust-shaped candidate builders under artifacts/perf / NEEDS VERIFICATION owners, license/provenance, broader browser probes, performance authority, correction, and rollback
...
notes:
  ...
  - "MapLibreAdapter owns inline-only style construction, camera synchronization, finite failures, teardown, and optional bounded rendered-feature selection projection; live source transport, evidence resolution, plugins, protocols, and external styles remain out of scope."
...
| `package.json` | **CONFIRMED admitted on current main** | It declares exact `maplibre-gl@6.9.0`, focused tests, the Vite-adapter export, and no publication authority. |
...
- Exact `maplibre-gl@6.9.0`, the initial package-owned `MapLibreAdapter`, and its lock closure are present on current `main`.
...
- [ ] Exact `maplibre-gl@6.9.0` is pinned; human dependency review and the transitive style/runtime closure remain pending.
...
| ML-SRC-007 | Pin MapLibre GL JS version. | PROPOSED ADMISSION — exact 6.9.0 |
