...
```text
data/                                                # Catalog + provenance + promotion manifests (served + gated)
├─ catalog/
│  └─ dcat/
│     └─ datasets/                                  # Dataset-level DCAT records (JSON-LD)
│        └─ <dataset_slug>.jsonld                    # One DCAT dataset record per dataset_slug
│
├─ stac/                                             # STAC discovery outputs (Collections + Items)
│  ├─ collections/
│  │  └─ <dataset_id>/collection.json                # STAC Collection for a dataset_id
│  └─ items/
│     └─ <dataset_id>/                               # Items grouped by dataset_id
│        └─ <item_id>.json                           # One STAC Item per item_id (geometry/time/assets)
│
├─ prov/                                             # PROV lineage bundles (per dataset version)
│  └─ <dataset_id>/
│     └─ <dataset_version_id>.jsonld                 # PROV bundle linking inputs → outputs for this version
│
└─ manifests/                                        # Promotion manifests (what was checked + what is being shipped)
   └─ <dataset_id>/
      └─ <dataset_version_id>.promotion.json         # Promotion manifest for this dataset version (gates + receipts + checksums)
```
...
```text
packages/catalog/                                 # Catalog tooling: build/validate profiles + link-check + reports
├─ src/
│  ├─ index.ts                                    # Public exports (validators, link-checkers, types)
│  │
│  ├─ profiles/                                   # Standard profiles (KFM constraints for each catalog family)
│  │  ├─ dcat.ts                                  # DCAT profile rules/helpers (required fields, vocab, conventions)
│  │  ├─ stac.ts                                  # STAC profile rules/helpers (Collection/Item constraints)
│  │  └─ prov.ts                                  # PROV profile rules/helpers (lineage requirements)
│  │
│  ├─ validate/                                   # Validation entrypoints (return typed results)
│  │  ├─ validateDcat.ts                           # Validate DCAT artifacts against profile + schema
│  │  ├─ validateStac.ts                           # Validate STAC artifacts against profile + schema
│  │  ├─ validateProv.ts                           # Validate PROV bundles against profile + schema
│  │  └─ validateTriplet.ts                        # Validate DCAT+STAC+PROV together (cross-links + completeness)
│  │
│  ├─ links/                                      # Cross-link checking (DCAT↔STAC↔PROV↔EvidenceRefs)
│  │  ├─ linkChecker.ts                            # Link graph builder + broken-link detection + reason codes
│  │  └─ resolvers.ts                              # Resolver adapters (filesystem/object store/http) for link targets
│  │
│  ├─ evidenceref/                                # EvidenceRef/CitationRef utilities (parse/format/canonicalize)
│  │  ├─ parse.ts                                  # Parse refs into typed structures (strict, fail-closed)
│  │  └─ format.ts                                 # Format/canonicalize refs (stable serialization)
│  │
│  └─ report/                                     # Reporting helpers (machine + human)
│     ├─ reportTypes.ts                            # Types for validation reports (findings, severities, counts)
│     └─ renderHumanSummary.ts                     # Render human-readable summary (markdown/text)
│
└─ test/
   ├─ fixtures/                                   # Deterministic fixtures for tests
   │  ├─ valid/                                    # Must pass validation
   │  └─ invalid/                                  # Must fail (negative tests)
   └─ validateTriplet.test.ts                      # Cross-catalog triplet validation regression test
```
