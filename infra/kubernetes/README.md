...
It does **not** exist to redefine KFM doctrine, public contract law, source admission rules, policy semantics, or canonical truth handling. Those remain upstream responsibilities that Kubernetes must consume, not invent.
...
| OpenAPI / JSON Schema | No | Keep in `../../contracts/` |
...
> [!WARNING]
> Kubernetes convenience must not become KFM authority. If a manifest starts deciding publication truth, evidence legality, policy meaning, or public contract shape, the design has crossed the wrong boundary.
...
7. Confirm rollback, restore, and correction paths before broadening the workload set.
8. Only then decide whether the change belongs here, in [`../gitops/`](../gitops/README.md), or in [`../terraform/`](../terraform/README.md).
...
- the work is local-first development or thin-slice bootstrap on one host  
  → use [`../local/`](../local/README.md) or [`../compose/`](../compose/README.md)
...
```mermaid
flowchart LR
    A[contracts/ + policy/ + tests/] --> B[CI validation]
...
- `contracts/ + policy/ + tests/` stay upstream of runtime wiring.
...
|---|---|---|
| Public API truth | `../../contracts/` + governed-api implementation | ad hoc manifest annotations |
...
- [ ] the change does not silently redefine publication, evidence, or policy semantics
...
Only cluster-admission mechanics belong here. Business policy meaning, rights logic, evidence legality, and publication law belong upstream in shared policy and contract surfaces.
...
- Would a new contributor know where to update the adjacent contract, policy, or runbook material?
