<!-- [KFM_META_BLOCK_V2]
doc_id: kfm://doc/tools-validators-source-descriptor-underscore-readme
title: tools/validators/source_descriptor README
type: README
version: v0.1
status: draft
...
related:
  ...
  - ../../../tests/
notes:
  - "This README replaces an empty placeholder at tools/validators/source_descriptor/README.md. It does not confirm executable SourceDescriptor validators, registry wiring, schema bindings, fixtures, policy bundles, receipt emission, runtime behavior, or CI behavior."
  - "docs/sources/SOURCE_DESCRIPTOR_STANDARD.md names tools/validators/source_descriptor/ as the PROPOSED long-lived validator home. The already-authored tools/validators/source-descriptor/README.md documents the hyphenated route and named this underscore route as drift before this file existed."
  - "Until an accepted ADR or migration note settles spelling, this underscore README is a compatibility/canonical-candidate routing lane and must not create a second independent validator authority."
...
![scope](https://img.shields.io/badge/scope-source__descriptor--validator-informational)
![path](https://img.shields.io/badge/path-underscore--candidate-blueviolet)
![posture](https://img.shields.io/badge/posture-fail--closed-critical)
![truth](https://img.shields.io/badge/truth-cite--or--abstain-success)

> **One-line purpose.** `tools/validators/source_descriptor/` is the underscore SourceDescriptor validator routing lane named by the Source Descriptor Standard; it should validate SourceDescriptor admission posture or route to the existing hyphenated lane without becoming a second independent validator authority.

---

## Purpose

`tools/validators/source_descriptor/` exists to document the underscore SourceDescriptor validator path named by source-admission doctrine and to prevent silent divergence from the already-authored hyphenated path.
...
KFM currently has both path shapes in play:

```text
tools/validators/source_descriptor/   # underscore path named by Source Descriptor Standard
tools/validators/source-descriptor/   # hyphenated README already authored as a routing lane
```

Use this posture until an ADR or migration note resolves the spelling:

- Treat `tools/validators/source_descriptor/` as the **canonical-candidate / compatibility** path because the Source Descriptor Standard names it as the proposed validator home.
- Treat `tools/validators/source-descriptor/` as the **confirmed existing hyphenated documentation lane**.
- Do not create separate executable validators with divergent behavior in both places.
- Prefer one implementation entrypoint and one registry id; use the other path only as a compatibility README or shim if needed.
...
|---|---|---|
| `tools/validators/source_descriptor/README.md` | **CONFIRMED README** | This README replaces the previous empty placeholder. |
| `tools/validators/source-descriptor/README.md` | **CONFIRMED README / executable behavior NEEDS VERIFICATION** | Hyphenated SourceDescriptor routing lane exists and documents the same validation family. |
| `docs/sources/SOURCE_DESCRIPTOR_STANDARD.md` | **CONFIRMED standard / implementation NEEDS VERIFICATION** | Names `tools/validators/source_descriptor/` as the proposed validator home and defines SourceDescriptor admission posture. |
...
| `tools/validators/source/README.md` | **CONFIRMED parent README / executable behavior NEEDS VERIFICATION** | Broad source validator parent route; this path is SourceDescriptor-specific. |
...
|---|---|---|
| One validator authority | Hyphenated and underscored paths must not diverge into competing implementations. | Two paths define separate behavior, registry ids, or outcome vocabularies without ADR. |
...
- source id, descriptor version, source type, source role, authority rank, publisher/steward, rights, sensitivity default, cadence, access, citation, review state, release state, or lifecycle posture is missing or malformed;
...
- hyphenated and underscored validator routes disagree about schema path, fixture path, output vocabulary, or registry id.
...
| Responsibility | Home |
|---|---|
| SourceDescriptor underscore validator routing | `tools/validators/source_descriptor/` |
| SourceDescriptor hyphenated compatibility/routing lane | `tools/validators/source-descriptor/` |
| Broad source validator parent | `tools/validators/source/` |
...
- **CONFIRMED:** this README exists at the requested underscore path.
- **PROPOSED:** validator code may live here when it is the chosen SourceDescriptor implementation path or a shim to the chosen implementation, and when it delegates meaning, schemas, registry records, policy decisions, evidence, receipts, lifecycle data, release records, and public runtime authority to owning roots.
...
Good fits for `tools/validators/source_descriptor/` include:

- this README;
- compatibility notes for underscore versus hyphenated SourceDescriptor validator paths;
- a single SourceDescriptor validator implementation or shim, if accepted and tested;
...
- finite outcome vocabulary and reason-code mapping shared with the chosen SourceDescriptor validator path;
...
|---|---|
| Duplicate validator implementation that diverges from `source-descriptor/` | choose one path via ADR/migration and use the other as shim or docs |
...
| `SOURCE_DESCRIPTOR_VALIDATOR_PATH_DRIFT` | Validator path naming is unresolved or routes disagree. |
...
If the project chooses the hyphenated `tools/validators/source-descriptor/` spelling instead, keep this folder as a compatibility README or shim until references are migrated. Do not maintain two divergent validators.
...
- [x] It replaces the empty placeholder at `tools/validators/source_descriptor/README.md`.
- [x] It records the underscore path as the Source Descriptor Standard's proposed validator path while noting the existing hyphenated route.
...
- [ ] Tests prove positive, negative, deny, restrict, hold, abstain, schema-invalid, path-drift, role-collapse, rights-unresolved, sensitivity-unresolved, stale-cadence, missing-citation, registry-missing, release-missing, and public-surface-blocked cases.
...
| Date | Change | Status |
|---|---|---|
| 2026-07-08 | Replaced empty placeholder with underscore SourceDescriptor validator compatibility README. | **CONFIRMED README / implementation NEEDS VERIFICATION** |
