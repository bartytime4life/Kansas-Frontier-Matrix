...
related: [../README.md, ../../contracts/README.md, ../../schemas/README.md, ../../schemas/contracts/README.md, ../../schemas/tests/README.md, ../../tests/README.md, ../../tests/contracts/README.md, ../attest/README.md, ../../policy/README.md]
tags: [kfm, tools, validators, fail-closed, verification, contracts]
notes: [Retains the existing repo-local document identifier from the supplied draft. Mounted repo inventory beyond PDF-visible evidence remains needs-verification, so file-level examples are kept conservative.]
[/KFM_META_BLOCK_V2] -->
...
Fail-closed validation helpers for trust-bearing artifacts, contract-first checks, and declared asset linkage.

> **Status:** experimental  
> **Owners:** @bartytime4life  
> ![status: experimental](https://img.shields.io/badge/status-experimental-orange) ![doc: draft](https://img.shields.io/badge/doc-draft-lightgrey) ![policy: public--safe](https://img.shields.io/badge/policy-public--safe-brightgreen)  
> **Quick jump:** [Scope](#scope) · [Repo fit](#repo-fit) · [Quickstart](#quickstart) · [Usage](#usage) · [Diagram](#diagram) · [Reference tables](#reference-tables) · [Task list](#task-list) · [FAQ](#faq) · [Appendix](#appendix)

> [!IMPORTANT]
> This README is strongest on **lane role**, **validator posture**, and the **documented starter slice**. Current-session implementation evidence is narrower than the doctrine: mounted repo contents beyond PDF-visible evidence were not directly inspected here. Treat exact file inventory, workflow wiring, and command runtime details beyond the supplied draft as **NEEDS VERIFICATION**.

## Scope

This lane exists for reusable helpers whose primary job is to **check / report / fail** against trust-bearing artifacts.

Use `tools/validators/` for validators that:
...
- require proof or bundle references when configured
- verify declared asset SHA-256 and byte length when local release bytes are available
- emit stable, reviewable output for humans and CI

The strongest first landed slice should remain **contract-first** or **release-manifest-first**. This family stays narrow, deterministic, and read-only by default.

### Operating posture

- **Fail closed:** blocking conditions must remain visible and return non-zero status.
- **Deterministic:** same inputs should produce the same class of outcome and the same report shape.
...
| --- | --- | --- |
| [`tools/validators/README.md`](./README.md) | this file | lane README for validator logic |
| [`../README.md`](../README.md) | tools hub | parent lane for shared tooling |
| [`../attest/README.md`](../attest/README.md) | attestation/signing lane | adjacent lane; signatures are related but not the same as validation |
...
| [`../../tests/README.md`](../../tests/README.md) | test hub | downstream execution lane for unit/integration/e2e checks |
| [`../../tests/contracts/README.md`](../../tests/contracts/README.md) | contract tests | downstream fixture execution and contract proof |
| [`../../policy/README.md`](../../policy/README.md) | policy hub | downstream reason/obligation and decision consistency pressure |
...
- build outputs / staged release assets
- `policy/`
...
- governed UI trust surfaces
- map-adjacent trust controls such as `ui/controls/`
...
- readers over declared contract, fixture, or release-manifest surfaces when authority is explicit
...
- sign artifacts
- decide organizational approval policy
...
- signing / attestation mechanics → [`../attest/README.md`](../attest/README.md)
...
A validator in this lane should be easy to answer three questions about:
...
flowchart LR
    ...
    C[release manifests + local assets] --> V
    D[policy/ reason & obligation pressure] --> V
...
| Outcome | Meaning |
| --- | --- |
| `ANSWER` | Declared checks passed |
| `DENY` | A trust requirement failed |
| `ABSTAIN` | Reserved for higher-level callers when inputs are intentionally partial |
...
| Placeholder-state handling | Distinguish between meaningful contract content and scaffold-state files |
...
| Runtime envelope grammar | an envelope admits an outcome outside `ANSWER \| ABSTAIN \| DENY \| ERROR` |
| Citation / evidence structure | an answer-shaped runtime envelope lacks required citation structure |
| Linkage reconstruction | a release or proof linkage check cannot reconstruct the expected chain |
| Required references | a required `bundle_ref` or `proof_ref` is missing when strict validation was requested |
...
| **NEEDS VERIFICATION** | A review flag for inventory, commands, workflow wiring, or file presence that should be checked before hardening claims |
...
> [!NOTE]
> Cosign verification guidance for signed blobs expects the artifact, verification bundle, and signer identity context. That is why a validator here may require `bundle_ref` and related references while still remaining separate from signature execution details.
