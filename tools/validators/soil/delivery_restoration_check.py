...
def main(argv=None):
 ...
 m=load_json(root/'delivery_restoration_manifest.json'); r=load_json(root/'delivery_restoration_receipt.json')
 ok=m.get('object_type')=='SoilDeliveryRestorationManifest' and r.get('receipt_type')=='DeliveryRestorationReceipt' and r.get('signatures')
 print(json.dumps({'delivery_restoration_valid':bool(ok and m.get('restoration_state')!='blocked'),'restoration_id':rid,'recommissioning_id':m.get('recommissioning_id'),'delivery_id':m.get('delivery_id'),'routing_id':m.get('routing_id'),'prior_release_id':m.get('prior_release_id'),'active_release_id':m.get('active_release_id'),'restoration_state':m.get('restoration_state'),'failure_reasons':[] if ok else ['invalid']})); return 0 if ok and m.get('restoration_state')!='blocked' else 1
