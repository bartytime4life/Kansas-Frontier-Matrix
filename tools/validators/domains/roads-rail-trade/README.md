...
related:
  ...
  - ../../../../release/
notes:
  - "This README replaces a greenfield stub and now confirms one bounded fixture-only executable: validate_corridor_route.py."
...
![authority](https://img.shields.io/badge/authority-index--plus--bounded--runner-lightgrey)
...
|---|---|---|
| `tools/validators/domains/roads-rail-trade/README.md` | **CONFIRMED** | This README replaces the previous greenfield stub and now indexes one bounded runner. |
...
| `validate_corridor_route.py` | **CONFIRMED bounded executable** | Fixture-only no-network PASS/ABSTAIN/DENY/ERROR checks against the paired CorridorRoute schema; no source, policy, review, release, publication, legal-designation, or live-routing authority. |
| CorridorRoute fixtures and focused tests | **CONFIRMED synthetic/focused** | Two schema-valid fixtures exercise PASS and ABSTAIN; eight exact-negative fixtures exercise DENY; focused tests cover deterministic hash and anti-collapse behavior. |
| Broader executables, schemas, policy bundles, and CI wiring | **NEEDS VERIFICATION** | No broader Roads/Rail/Trade implementation, policy execution, runtime behavior, or CI integration is claimed here. |
...
No child README lanes were confirmed during this edit. The current CorridorRoute validator is a bounded root-lane executable rather than a new child authority.
...
| Tests and fixtures | `tests/validators/domains/roads-rail-trade/`, `tests/domains/roads-rail-trade/`, `tests/schemas/`, `fixtures/domains/roads-rail-trade/`, or accepted conventions |

Safe interpretation:

- **CONFIRMED:** this README and `validate_corridor_route.py` exist in the proposed slice.
- **PROPOSED:** the CorridorRoute schema profile and validator behavior remain bounded, fixture-only implementation pending review; future validator code may live below this folder when it checks declared Roads/Rail/Trade invariants and delegates meaning, source roles, policy, evidence, and release authority to owning roots.
- **NEEDS VERIFICATION:** schema registry admission, policy bundle digests, source descriptors, live evidence, report destinations, runtime behavior, slug resolution, broad CI wiring, release/correction integration, and other executables.
...
| `PASS` | Bounded configured profile passed; no higher authority is granted. |
| `DENY` | Candidate violates a declared schema, integrity, boundary, or public-safety rule. |
...
The current fixtures exercise one `PASS`, one `ABSTAIN`, and eight exact `DENY` outcomes. A green run proves only the declared bounded profile. It does not prove source admission, historical accuracy, rights clearance, policy approval, review, release, publication, legal designation, public access, or current routing suitability.

Suggested future broader test surface:
...
> [!NOTE]
> This is a proposed broader interface, not proof that `run_roads_rail_trade_domain_validators.py` or the future test path exists.
...
- [x] CorridorRoute validator reads the paired semantic contract/schema profile rather than defining route meaning or policy authority locally.
- [x] CorridorRoute, segment, membership, embedded geometry, graph projection, live routing, and publication authority remain distinct in the bounded profile.
- [x] Source role, route membership references, geometry lineage, date uncertainty, and temporal scope remain visible.
- [ ] Crossing profiles prove they do not become hydrology truth, infrastructure truth, legal-access truth, live-routing truth, or publication approval.
- [x] The current validator never presents KFM as live closure, routing, safe-passage, legal-access, right-of-way, bridge-safety, rail-operating, emergency-route, or regulatory authority.
- [ ] Cross-domain joins preserve ownership, source role, sensitivity, and EvidenceBundle support.
- [ ] Map, tile, search, graph, export, Focus Mode, and AI surfaces prove they do not reveal restricted details or reverse-engineerable derivatives.
- [x] The implementation receipt fixture stays in the existing fixture lane; no release/proof authority is created.
- [x] Validator output is not described as truth, policy, release, publication, route authority, or Directory Rules approval.
- [x] Current tests use synthetic fixtures and require no network.
- [x] Current executable claims are backed by repository paths and focused test results.
- [ ] Broad CI and repository-wide regression status are verified on the exact pull-request head.
...
| Review state | Draft parent index with one bounded CorridorRoute executable; broader validator maturity remains unverified. |
| Next smallest safe change | Verify exact-head hosted checks, then separately address schema-registry admission, source descriptors, policy integration, broader CI wiring, release linkage, correction/rollback behavior, and additional object-family profiles. |
