...
SCHEMA_ROOT = REPO_ROOT / "schemas/contracts/v1/domains/roads-rail-trade"
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(
        description="Validate Roads/Rail/Trade JSON Schema documents against Draft 2020-12."
    )
    parser.add_argument(
        "schemas",
        nargs="*",
        help=(
            ...
            "under the canonical Roads/Rail/Trade schema root."
        ),
    )
    ...
    if not paths:
        print(f"No Roads/Rail/Trade schemas found under {SCHEMA_ROOT}", file=sys.stderr)
        return 2
