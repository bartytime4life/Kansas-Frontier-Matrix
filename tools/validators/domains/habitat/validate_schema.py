...
SCHEMA_ROOT = REPO_ROOT / "schemas/contracts/v1/domains/habitat"
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(
        description="Validate Habitat JSON Schema documents against Draft 2020-12."
    )
    parser.add_argument(
        "schemas",
        nargs="*",
        help=(
            ...
            "under the canonical Habitat schema root."
        ),
    )
    ...
    if not paths:
        print(f"No Habitat schemas found under {SCHEMA_ROOT}", file=sys.stderr)
        return 2
