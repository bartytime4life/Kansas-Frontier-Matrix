...
| `validate_source_descriptor.py` | Validates candidates against the shared canonical SourceDescriptor schema (`schemas/contracts/v1/source/source_descriptor.schema.json`), plus one domain-specific semantic rule: an optional `domain_scope` (or legacy `domain`) must include `"habitat"` when declared. Adds no independent source-admission, rights, sensitivity, or policy semantics beyond that membership check. | `tests/validators/domains/habitat/test_habitat_source_descriptor_validator_entrypoint.py`; `fixtures/domains/habitat/source_descriptor/` | none yet configured |
| `validate_habitat_patch.py` | Mostly structural: checks the still-empty `habitat_patch.schema.json` scaffold (root must be a JSON object; no duplicate keys or non-finite numbers), plus reference-string hygiene (sorted, unique, grammar-bounded, no internal-lifecycle prefixes) on the optional `connectivity_edge_refs`/`corridor_refs` fields — reusing the shared CatalogMatrix closure validator's ref-hygiene rule unchanged, without resolving to a real ConnectivityEdge/Corridor (both are themselves empty scaffolds). Asserts no other HabitatPatch identity, source-role, geometry, evidence, or release semantics, which remain `NEEDS VERIFICATION` pending a domain-steward schema expansion. | `tests/validators/domains/habitat/test_habitat_patch_validator_entrypoint.py`; `fixtures/domains/habitat/patch/` | none yet configured |
| `validate_suitability_model.py` | Mostly structural: checks the still-empty `suitability_model.schema.json` scaffold (root must be a JSON object; no duplicate keys or non-finite numbers), plus one well-grounded semantic rule — an optional `model_card_ref` must resolve to a file that independently passes the real governance `tools/validators/governance/validate_model_card_envelope.py` validator. Asserts no other SuitabilityModel identity, source-role, uncertainty, or release semantics, which remain `NEEDS VERIFICATION` pending a domain-steward schema expansion. | `tests/validators/domains/habitat/test_suitability_model_validator_entrypoint.py`; `fixtures/domains/habitat/suitability_model/` | none yet configured |
