...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(
        description="Validate Atmosphere JSON Schema documents against Draft 2020-12."
    )
    parser.add_argument(
        "schemas",
        nargs="*",
        help=(
            ...
            "under the canonical Atmosphere schema root."
        ),
    )
    ...
    if not paths:
        print(f"No Atmosphere schemas found under {SCHEMA_ROOT}", file=sys.stderr)
        return 2
