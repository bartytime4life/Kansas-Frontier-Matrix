#!/usr/bin/env python3
"""Validate Settlements/Infrastructure catalog closure through the shared local profile.
...
def run_shared(args: Sequence[str]) -> int:
    if not SHARED_VALIDATOR.is_file():
        print(
            f"SETTLEMENTS_INFRASTRUCTURE_CATALOG_VALIDATOR_MISSING path={SHARED_VALIDATOR}",
            file=sys.stderr,
        )
        return 2
