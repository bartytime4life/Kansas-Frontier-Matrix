...
SCHEMA_ROOT = REPO_ROOT / "schemas/contracts/v1/domains/settlements-infrastructure"
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(
        description="Validate Settlements/Infrastructure JSON Schema documents against Draft 2020-12."
    )
    parser.add_argument(
        "schemas",
        nargs="*",
        help=(
            ...
            "under the canonical Settlements/Infrastructure schema root."
        ),
    )
    ...
    if not paths:
        print(f"No Settlements/Infrastructure schemas found under {SCHEMA_ROOT}", file=sys.stderr)
        return 2
