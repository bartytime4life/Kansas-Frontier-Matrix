...
No network access or source, evidence, policy, promotion, release,
publication, or public-use authority is created.
"""
...
def _profile_findings(profile: dict[str, Any]) -> list[Finding]:
    ...
    if findings:
        return findings
    if _is_zero_digest(profile.get("spec_hash")):
        findings.append(Finding("DIGEST_PLACEHOLDER", "/spec_hash"))
    if profile.get("spec_hash") != _sha256(_canonical_without(profile, "spec_hash")):
    ...
    if isinstance(rules, list):
        ...
        for index, rule in enumerate(rules):
            if not isinstance(rule, dict):
                continue
            for field in (
                "source_families", "source_roles", "spatial_support",
                "claim_kinds", "forbidden_claim_kinds",
            ):
                if not _sorted_unique_strings(rule.get(field)):
                    findings.append(Finding(
                        "PROFILE_ARRAY_NOT_CANONICAL", f"/support_types/{index}/{field}"
                    ))
    return findings
...
def validate_candidate(candidate: dict[str, Any], profile: dict[str, Any]) -> ValidationResult:
    ...
    findings.extend(_schema_findings(
        candidate, CANDIDATE_SCHEMA_PATH, "CANDIDATE_SCHEMA_INVALID"
    ))
    if any(f.code == "SCHEMA_UNAVAILABLE" for f in findings):
        return ValidationResult(tuple(sorted(set(findings))))
    ...
    bindings = (
        ("profile_id", "profile_id", "PROFILE_ID_MISMATCH"),
        ("profile_version", "profile_version", "PROFILE_VERSION_MISMATCH"),
        ("profile_spec_hash", "spec_hash", "PROFILE_HASH_BINDING_MISMATCH"),
    )
    for candidate_field, profile_field, code in bindings:
        if candidate.get(candidate_field) != profile.get(profile_field):
    ...
    if rule is None:
        ...
        for candidate_field, rule_field, code in checks:
            ...
            if not isinstance(allowed, list) or candidate.get(candidate_field) not in allowed:
                findings.append(Finding(code, f"/{candidate_field}"))
        if candidate.get("claim_kind") in rule.get("forbidden_claim_kinds", []):
    ...
    authority_fields = (
        "authority_created", "evidence_closure_claimed", "policy_evaluated",
        "promotion_authorized", "release_authorized", "publication_authorized",
    )
    if isinstance(governance, dict) and (
        any(governance.get(field) is not False for field in authority_fields)
...
def validate_file(candidate_path: Path, *, profile_path: Path = PROFILE_PATH) -> ValidationResult:
    ...
    if findings or profile is None:
        return ValidationResult(tuple(sorted(set(findings or [Finding("PROFILE_INVALID", "/")]))))
    candidate, findings = _read_object(candidate_path)
    if findings or candidate is None:
        return ValidationResult(tuple(sorted(set(findings or [Finding("ROOT_NOT_OBJECT", "/")]))))
