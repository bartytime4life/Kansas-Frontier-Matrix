...
SCHEMA_ROOT = REPO_ROOT / "schemas/contracts/v1/domains/flora"
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(
        description="Validate Flora JSON Schema documents against Draft 2020-12."
    )
    parser.add_argument(
        "schemas",
        nargs="*",
        help=(
            ...
            "under the canonical Flora schema root."
        ),
    )
    ...
    if not paths:
        print(f"No Flora schemas found under {SCHEMA_ROOT}", file=sys.stderr)
        return 2
