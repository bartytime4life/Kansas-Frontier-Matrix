...
type: README
version: v0.2
status: draft; EvidenceBundle alias, separated aquifer-pair, synthetic flow, and datum-bound water-level fixture validation executable; broader validator families held
...
related:
  ...
  - ../../../../release/
notes:
  - "Five local JSON Schema wrappers are present. EvidenceBundle, AquiferObservation, and AquiferContextLink have bounded workflow execution; DecisionEnvelope and RunReceipt remain inventory-only here."
  ...
  - "The bounded synthetic WaterLevelObservation fixture profile binds record identity to its generalized gauge, datum, source descriptor, and observation time; binds the generalized gauge identity to its county-level spatial support; requires concrete lowercase canonical fixture identifiers for its source, gauge, at most 32 unique canonically ordered evidence receipts, and datum references; caps synthetic source latency at 300 seconds and retrieval latency at 900 seconds from observation; and limits the generalized water-level value to two meaningful decimal places in minimal plain decimal notation without redundant fractional trailing zeros, while rejecting lexically signed negative zero in integer or decimal JSON form and distinguishing nonzero decimal underflow. Evidence receipts bind the water-level family, governed gauge, observation time, datum, and source descriptor, while timestamp-shaped segments are reserved case-insensitively and admitted only as calendar-valid exact ASCII-digit YYYYMMDDTHHMMSSZ tokens. It creates no live-source, release, flood-warning, or publication authority."
[/KFM_META_BLOCK_V2] -->
...
![scope](https://img.shields.io/badge/scope-bounded--schema--shape-blue)
...
> **One-line purpose.** `tools/validators/domains/hydrology/` is the per-domain Hydrology validator index with bounded executable EvidenceBundle alias and separated `AquiferObservation` / `AquiferContextLink` shape checks; broader watershed, HUC, reach, gauge, observation, NFHL, identity, time, policy, proof, release, correction, rollback, and public-surface validation remains proposed or held.
...
| `validate_public_safe_water_level_fixture.py` | **CONFIRMED bounded executable** | Validates a frozen synthetic fixture-only WaterLevelObservation profile whose record identity is bound to its generalized gauge, datum, source descriptor, and observation time; the generalized gauge identity must match its county-level spatial support. Concrete lowercase canonical fixture identifiers are required for its source, gauge, at most 32 unique canonically ordered evidence receipts, and datum references. It caps synthetic source latency at 300 seconds, retrieval latency at 900 seconds from observation, and generalized water-level precision at two meaningful decimal places in minimal plain decimal notation without redundant fractional trailing zeros, while rejecting lexically signed negative zero in integer or decimal JSON form and distinguishing nonzero decimal underflow. Evidence receipts must identify the water-level family, the same governed gauge, observation time, datum, and source descriptor, while timestamp-shaped segments are reserved case-insensitively and admitted only as calendar-valid exact ASCII-digit `YYYYMMDDTHHMMSSZ` tokens. It validates no real gauge or datum authority. |
...
- `gauge-observation/` for gauge/site identity, flow/water-level/water-quality observations, units, timestamps, and source-role posture;
  - `groundwater-aquifer/` for broader groundwater wells, endpoint resolution, well/privacy boundaries, public-safe geometry, policy, and evidence beyond the bounded aquifer pair;
...
- **CONFIRMED:** this README and five local JSON Schema wrappers exist;
  EvidenceBundle, AquiferObservation, and AquiferContextLink have bounded
  shape/polarity workflow coverage.
...
- **NEEDS VERIFICATION:** every broader executable contract, endpoint/evidence
  resolver, policy bundle digest, source descriptor, report destination,
  receipt, runtime behavior, and CI lane beyond the bounded three-family checks.
...
- collapses Watershed, HUCUnit, HydroFeature, ReachIdentity, GaugeSite, GroundwaterWell, FlowObservation, WaterLevelObservation, WaterQualityObservation, AquiferObservation, AquiferContextLink, Hydrograph, NFHLZone, UpstreamTrace, WaterUseLink, DroughtLink, or IrrigationLink into another role;
- puts a measurement into `AquiferContextLink`, copies Geology geometry into
  Hydrology, or treats an unresolved link as proof of aquifer membership;
...
The tests invoke the three bounded wrappers, enforce valid/invalid fixture
polarity, check aquifer type separation and optional-link behavior, and install
socket, DNS, and URL denial guards. They establish neither endpoint or
