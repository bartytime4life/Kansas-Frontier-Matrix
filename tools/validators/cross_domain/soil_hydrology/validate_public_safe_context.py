#!/usr/bin/env python3
"""Validate fixture-only Soil-Hydrology public-safe context candidates."""
...
CASES_PATH = REPO_ROOT / "fixtures/contracts/v1/joins/soil_hydrology_public_safe_context/cases.json"
RELATION_PROFILE_REF = "kfm:relation-profile:soil-hydrology-public-safe-context:v1"
FIXTURE_PREFIX = "kfm:fixture:"
SCOPE = "soil-hydrology-public-safe-context-fixture-only-v1"
...
def validate_document(candidate: object) -> ValidationResult:
    ...
    request = _mapping(candidate.get("request")); endpoints = _mapping(candidate.get("endpoints"))
    left, right = _mapping(endpoints.get("left")), _mapping(endpoints.get("right")); decision = _mapping(candidate.get("decision"))
    ...
    if right.get("domain") != "hydrology": add("ENDPOINT_DOMAIN_MISMATCH", "/endpoints/right/domain")
    ...
    if decision.get("validator_outcome") == "ALLOW":
        ...
        hydrology_ref = right.get("object_ref")
        if isinstance(hydrology_ref, str) and any(token in hydrology_ref for token in ("flood-warning", "emergency-alert", "causal-runoff")):
            add("OPERATIONAL_OR_CAUSAL_CLAIM_DENIED", "/endpoints/right/object_ref")
...
def _set_pointer(candidate: dict[str, Any], pointer: str, value: Any) -> None:
    parts = [p.replace("~1", "/").replace("~0", "~") for p in pointer.lstrip("/").split("/")]; parent: Any = candidate
...
def fixture_cases(path: Path = CASES_PATH):
    ...
    base = GENERIC.seal(GENERIC.derive_outputs(matrix["base"])); out = []
    for raw in matrix["cases"]:
        ...
        out.append((raw["name"], candidate, validate_document(candidate), raw["expected"]))
    return out
