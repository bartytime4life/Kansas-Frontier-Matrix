"""Validate PROPOSED generic ValidationReport instances without network or mutation.

This validator intentionally enforces only the current paired Draft 2020-12
schema and bounded JSON-input safety. A PASS records shape conformance only; it
does not establish evidence truth, proof closure, policy approval, review,
release readiness, correction completion, or publication state.
"""
...
SCHEMA_PATH = REPO_ROOT / "schemas/contracts/v1/data/validation_report.schema.json"
FIXTURES_ROOT = REPO_ROOT / "fixtures/data/validation_report"
...
def run_fixtures(root: Path = FIXTURES_ROOT) -> int:
    ...
    if not valid or not invalid:
        print("VALIDATION_REPORT_FIXTURES_ERROR nonempty valid and invalid lanes required")
        return 2
    failures: list[str] = []
    for path in valid:
    ...
    if failures:
        for failure in failures:
            print(f"VALIDATION_REPORT_FIXTURE_POLARITY_FAIL {failure}")
        return 1
    print(
        "VALIDATION_REPORT_FIXTURES_VALID "
...
def main(argv: Sequence[str] | None = None) -> int:
    parser = argparse.ArgumentParser(
        description="Validate generic ValidationReport instances against the current proposed schema."
    )
    ...
    if args.fixtures:
        if args.reports:
            parser.error("--fixtures cannot be combined with report paths")
        return run_fixtures()
    if not args.reports:
        parser.error("at least one report path is required unless --fixtures is used")
    failed = False
    for path in sorted(args.reports):
        ...
        if findings:
            ...
            for code in findings:
                print(f"VALIDATION_REPORT_INVALID file={path.name} code={code}")
        else:
            print(f"VALIDATION_REPORT_VALID file={path.name}")
