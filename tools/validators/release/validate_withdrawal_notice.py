"""Validate PROPOSED WithdrawalNotice instances without network or mutation.

This validator intentionally enforces only the current paired Draft 2020-12
schema and bounded JSON-input safety. A PASS does not prove withdrawal
completion, downstream invalidation, policy/review closure, release authority,
...
SCHEMA_PATH = REPO_ROOT / "schemas/contracts/v1/release/withdrawal_notice.schema.json"
FIXTURES_ROOT = REPO_ROOT / "fixtures/release/withdrawal_notice"
...
def validate_document(document: Mapping[str, object]) -> tuple[str, ...]:
    """Return stable, non-value-bearing findings for the current schema only."""
...
def run_fixtures(root: Path = FIXTURES_ROOT) -> int:
    ...
    if not valid or not invalid:
        print("WITHDRAWAL_NOTICE_FIXTURES_ERROR nonempty valid and invalid lanes required")
        return 2
    failures: list[str] = []
    for path in valid:
    ...
    if failures:
        for failure in failures:
            print(f"WITHDRAWAL_NOTICE_FIXTURE_POLARITY_FAIL {failure}")
        return 1
    print(
        "WITHDRAWAL_NOTICE_FIXTURES_VALID "
...
def main(argv: Sequence[str] | None = None) -> int:
    parser = argparse.ArgumentParser(
        description="Validate WithdrawalNotice instances against the current proposed schema."
    )
    ...
    for path in sorted(args.notices):
        ...
        if findings:
            ...
            for code in findings:
                print(f"WITHDRAWAL_NOTICE_INVALID file={path.name} code={code}")
        else:
            print(f"WITHDRAWAL_NOTICE_VALID file={path.name}")
