#!/usr/bin/env python3
"""Validate fixture-only StacZarrAssetMetadataProfileCandidate records.

A PASS proves bounded declared metadata shape, rank consistency, evidence-reference
ordering, non-authority controls, and deterministic identity only. It does not
access a Zarr store, resolve a STAC asset, verify bytes, mutate a catalog,
evaluate policy, promote, release, or publish.
"""
...
SCHEMA = ROOT / "schemas/contracts/v1/data/stac_zarr_asset_metadata_profile.schema.json"
CASES = ROOT / "fixtures/contracts/v1/data/stac_zarr_asset_metadata_profile/cases.json"
IDENTITY_PREFIX = "kfm:stac-zarr-asset-metadata:"
...
def _semantic_findings(candidate: Mapping[str, Any]) -> list[Finding]:
    ...
    if not _canonical_strings(stac["roles"]) or not _canonical_strings(provenance["metadata_evidence_refs"]):
        findings.append(Finding("REFERENCES_NOT_CANONICAL", "/"))

    shape = zarr["shape"]
    chunks = zarr["chunk_shape"]
    names = zarr["dimension_names"]
    if not (len(shape) == len(chunks) == len(names)):
        findings.append(Finding("RANK_MISMATCH", "/zarr"))
    elif any(chunk > size for chunk, size in zip(chunks, shape, strict=True)):
        findings.append(Finding("CHUNK_EXCEEDS_SHAPE", "/zarr/chunk_shape"))

    if "data" not in stac["roles"]:
        findings.append(Finding("DATA_ASSET_ROLE_REQUIRED", "/stac/roles"))

    codec_names = [codec["name"] for codec in zarr["codecs"]]
    if len(codec_names) != len(set(codec_names)):
        findings.append(Finding("CODEC_NAME_DUPLICATE", "/zarr/codecs"))
