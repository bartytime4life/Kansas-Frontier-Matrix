#!/usr/bin/env python3
"""Validate the inactive, fixture-only ClassificationRelease profile.

PASS proves bounded local shape and invariants only. It creates no source,
evidence, policy, review, lifecycle, promotion, release, or public authority.
"""
...
SCHEMA = ROOT / "schemas/contracts/v1/common/classification_release.schema.json"
CASES = ROOT / "fixtures/contracts/v1/common/classification_release/cases.json"
MAX_FILE_BYTES = 1_048_576
MAX_SCHEMA_FINDINGS = 100
SCOPE = "classification-release-fixture-only-v1"
FALSE_EFFECTS = {
    "source_activated": False, "evidence_resolved": False,
    "policy_evaluated": False, "promoted": False,
...
ERROR_CODES = {
    ...
    "CLASSIFICATION_RELEASE_ID_MISMATCH", "FIXTURE_MANIFEST_INVALID",
}
...
def identity_subject(candidate: Mapping[str, Any]) -> dict[str, Any]:
    ...
    subject.pop("classification_release_id", None)
...
def expected_release_id(candidate: Mapping[str, Any]) -> str:
    return "classification-release:" + canonical_spec_hash(candidate).removeprefix("sha256:")[:24]


def assign_identity(candidate: dict[str, Any]) -> dict[str, Any]:
    result = copy.deepcopy(candidate)
    result["spec_hash"] = canonical_spec_hash(result)
    result["classification_release_id"] = expected_release_id(result)
    return result
...
def _canonical(values: Any) -> bool:
    return isinstance(values, list) and all(isinstance(v, str) for v in values) and values == sorted(set(values))


def _semantic_findings(candidate: Mapping[str, Any]) -> list[Finding]:
    findings: list[Finding] = []
    if candidate.get("source_role") != "CLASSIFICATION":
        findings.append(Finding("SOURCE_ROLE_COLLAPSE", "/source_role"))
    if candidate.get("support_type") != "DERIVED_CLASSIFICATION":
        findings.append(Finding("SUPPORT_TYPE_COLLAPSE", "/support_type"))

    for key in (
        "source_snapshot_refs", "spatial_artifact_refs",
        "aggregate_statistic_refs", "content_digests", "evidence_refs",
        "class_codes", "limitations",
    ):
    ...
    for key in ("corrects", "supersedes", "superseded_by", "conflict_refs"):
    ...
    if isinstance(digests, list) and any(v == "sha256:" + "0" * 64 for v in digests):
    ...
    cutoff, valid_from, valid_to, released, retrieved = (
        _dt(times.get(name)) for name in (
            "source_data_cutoff_at", "source_valid_from", "source_valid_to",
            "source_released_at", "retrieved_at",
        )
    )
    corrected, superseded = _dt(times.get("corrected_at")), _dt(times.get("superseded_at"))
    if cutoff and released and cutoff > released:
        findings.append(Finding("DATA_CUTOFF_AFTER_RELEASE", "/times/source_data_cutoff_at"))
    if valid_from and valid_to and valid_from > valid_to:
        findings.append(Finding("TEMPORAL_ORDER_INVALID", "/times/source_valid_to"))
    ...
    if corrected and released and corrected < released:
        findings.append(Finding("CORRECTION_BEFORE_RELEASE", "/times/corrected_at"))
    if superseded and released and superseded < released:
        findings.append(Finding("SUPERSESSION_BEFORE_RELEASE", "/times/superseded_at"))

    space = candidate.get("space") if isinstance(candidate.get("space"), Mapping) else {}
    if space.get("scale") == "POINT":
        findings.append(Finding("POINT_SCALE_CLASSIFICATION_DENIED", "/space/scale"))
    unresolved = space.get("geometry_role") == "UNRESOLVED"
    if unresolved and any(space.get(k) is not None for k in ("geography_ref", "geometry_digest")):
        findings.append(Finding("UNRESOLVED_GEOMETRY_OVERCLAIM", "/space"))
    if not unresolved and (space.get("geography_ref") is None or space.get("geometry_digest") is None):
    ...
    elif state == "CORRECTED" and (not corrects or not corrected):
        findings.append(Finding("CORRECTION_LINEAGE_INCOMPLETE", "/lineage"))
    elif state == "SUPERSEDED" and (not superseded_by or not superseded):
        findings.append(Finding("SUPERSESSION_LINEAGE_INCOMPLETE", "/lineage"))
    elif state == "CONFLICTED":
        if len(conflicts) < 2 or not unresolved:
    ...
    try:
        ...
        expected_id = expected_release_id(candidate)
    except RuntimeError:
        findings.append(Finding("HASHING_UNAVAILABLE", "/spec_hash"))
    else:
        if candidate.get("spec_hash") != expected_hash:
            findings.append(Finding("SPEC_HASH_MISMATCH", "/spec_hash"))
        if candidate.get("classification_release_id") != expected_id:
            findings.append(Finding("CLASSIFICATION_RELEASE_ID_MISMATCH", "/classification_release_id"))
    return findings
...
def validate_payload(candidate: Mapping[str, Any]) -> ValidationResult:
    ...
    return ValidationResult(
        "ERROR" if any(f.code in ERROR_CODES for f in ordered) else "DENY",
        ordered,
    )
...
def _set_pointer(candidate: dict[str, Any], pointer: str, value: Any) -> None:
    parts = [p.replace("~1", "/").replace("~0", "~") for p in pointer.removeprefix("/").split("/") if p]
...
def _load_fixture_document() -> dict[str, Any]:
    ...
    if document is None or findings or not isinstance(document.get("bases"), dict) or not isinstance(document.get("cases"), list):
...
def materialize_case(document: Mapping[str, Any], case: Mapping[str, Any]) -> dict[str, Any]:
    ...
    if mode == "MISMATCH_SPEC_HASH":
        ...
        candidate["classification_release_id"] = "classification-release:" + "0" * 24
    elif mode != "RECOMPUTE":
...
def _serialize(result: ValidationResult, *, path: Path | None = None, case: str | None = None) -> str:
    payload: dict[str, Any] = {
        ...
        "scope": SCOPE,
        "authority": {
            "network_fetch": False, "source_activation": False,
            "evidence_resolution": False, "policy_evaluation": False,
            "review_approval": False, "lifecycle_write": False,
            "promotion": False, "release": False, "publication": False,
            "public_use": False,
        },
    }
    if path is not None:
...
def replay_fixtures() -> int:
    ...
    for raw, candidate in cases:
        ...
        actual = [f.code for f in result.findings]
...
def main(argv: Sequence[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Validate one fixture-only ClassificationRelease candidate.")
