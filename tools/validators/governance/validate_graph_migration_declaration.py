#!/usr/bin/env python3
"""Validate fixture-only GraphMigrationDeclaration records.

A PASS proves bounded shape, semantics, and deterministic identity only. It
does not access a graph, execute a query or migration, authenticate review,
evaluate policy, mutate evidence, promote data, release, or publish.
"""
...
SCHEMA = ROOT / "schemas/contracts/v1/governance/graph_migration_declaration.schema.json"
CASES = ROOT / "fixtures/contracts/v1/governance/graph_migration_declaration/cases.json"
IDENTITY_PREFIX = "kfm:graph-migration-declaration:"
MAX_BYTES = 2 * 1024 * 1024
MAX_SCHEMA_FINDINGS = 50
REQUIRED_INVARIANTS = {
    "DETERMINISTIC_IDENTITY", "DIRECTIONALITY_CARDINALITY", "ENDPOINT_INTEGRITY",
    "EVIDENCE_CLOSURE", "IDENTITY_LINEAGE", "NO_AUTHORITY_INFLATION",
    "PROVENANCE", "RECOVERY", "SENSITIVITY_RIGHTS", "TEMPORAL_SPATIAL_SCOPE",
}
...
def identity_subject(value: Mapping[str, Any]) -> dict[str, Any]:
    ...
    subject.pop("declaration_id", None)
...
def materialize_case(corpus: Mapping[str, Any], case: Mapping[str, Any]) -> dict[str, Any]:
    ...
    spec_hash, declaration_id = canonical_identity(candidate)
...
def _semantic_findings(candidate: Mapping[str, Any]) -> list[Finding]:
    ...
    reference_paths = (
        ("/migration/reason_refs", candidate["migration"]["reason_refs"]),
        ("/scope/affected_surfaces", candidate["scope"]["affected_surfaces"]),
        ("/scope/node_classes", candidate["scope"]["node_classes"]),
        ("/scope/relationship_types", candidate["scope"]["relationship_types"]),
        ("/scope/projections", candidate["scope"]["projections"]),
        ("/blast_radius/affected_components", candidate["blast_radius"]["affected_components"]),
        ("/telemetry/metric_names", candidate["telemetry"]["metric_names"]),
        ("/telemetry/precheck_refs", candidate["telemetry"]["precheck_refs"]),
        ("/telemetry/postcheck_refs", candidate["telemetry"]["postcheck_refs"]),
        ("/governance/authority_refs", candidate["governance"]["authority_refs"]),
        ("/governance/evidence_refs", candidate["governance"]["evidence_refs"]),
        ("/governance/policy_refs", candidate["governance"]["policy_refs"]),
        ("/non_effects", candidate["non_effects"]),
    )
    if any(not _canonical_strings(value) for _, value in reference_paths):
        findings.append(Finding("REFERENCES_NOT_CANONICAL", "/"))

    invariants = candidate["invariants"]
    invariant_names = [item["name"] for item in invariants]
    if set(invariant_names) != REQUIRED_INVARIANTS or invariant_names != sorted(invariant_names):
        findings.append(Finding("INVARIANT_COVERAGE_INVALID", "/invariants"))
    ...
    try:
        ...
        if candidate.get("declaration_id") != expected_id:
            findings.append(Finding("DECLARATION_ID_MISMATCH", "/declaration_id"))
    return findings
