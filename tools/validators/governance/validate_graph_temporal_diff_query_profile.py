#!/usr/bin/env python3
"""Validate fixture-only graph temporal-diff query profile candidates.

A PASS proves bounded shape, conservative read-only template checks, fixture
polarity, and deterministic identity only. It does not parse Cypher fully,
connect to a graph, execute a query or migration, evaluate policy, approve
review, release, or publish.
"""
...
SCHEMA = ROOT / "schemas/contracts/v1/governance/graph_temporal_diff_query_profile.schema.json"
CASES = ROOT / "fixtures/contracts/v1/governance/graph_temporal_diff_query_profile/cases.json"
IDENTITY_PREFIX = "kfm:graph-temporal-diff-query-profile:"
MAX_BYTES = 2 * 1024 * 1024
MAX_SCHEMA_FINDINGS = 50
QUERY_SCOPES = {
    "LABEL_SCOPED_NODE_DIFF": "LABEL_SCOPED",
    "MIGRATION_WINDOW_NODE_DIFF": "MIGRATION_WINDOW",
    "RELATIONSHIP_SCOPED_DIFF": "RELATIONSHIP_SCOPED",
}
BASE_PARAMETERS = {"after_snapshot_ref", "before_snapshot_ref", "migration_window_id"}
EXTRA_PARAMETERS = {
    "LABEL_SCOPED_NODE_DIFF": {"label"},
    "MIGRATION_WINDOW_NODE_DIFF": set(),
    "RELATIONSHIP_SCOPED_DIFF": {"relationship_type"},
}
FORBIDDEN_CYPHER = re.compile(
    r"(?:\b(?:CREATE|MERGE|DELETE|DETACH|SET|REMOVE|DROP|CALL|FOREACH|USE)\b|"
    r"\bLOAD\s+CSV\b|\bAPOC\b|\bDBMS\b|https?://|file://|;|`)",
    re.IGNORECASE,
)
...
def _semantic_findings(candidate: Mapping[str, Any]) -> list[Finding]:
    ...
    reference_paths = (
        governance["authority_refs"],
        governance["evidence_refs"],
        governance["policy_refs"],
        candidate["non_effects"],
    )
    if any(not _canonical_strings(value) for value in reference_paths):
        findings.append(Finding("REFERENCES_NOT_CANONICAL", "/"))

    window = candidate["snapshot_window"]
    try:
        ordered = _parse_time(window["valid_time_start"]) < _parse_time(window["valid_time_end"])
    except (TypeError, ValueError):
        ordered = False
    if not ordered or window["before_snapshot_ref"] == window["after_snapshot_ref"]:
        findings.append(Finding("SNAPSHOT_WINDOW_INVALID", "/snapshot_window"))

    queries = candidate["queries"]
    query_ids = [query["query_id"] for query in queries]
    if query_ids != sorted(QUERY_SCOPES) or len(set(query_ids)) != len(query_ids):
        findings.append(Finding("QUERY_SET_INVALID", "/queries"))

    for index, query in enumerate(queries):
        query_id = query["query_id"]
        template = query["cypher_template"]
        if QUERY_SCOPES.get(query_id) != query["scope_kind"]:
            findings.append(Finding("QUERY_SCOPE_INVALID", f"/queries/{index}/scope_kind"))

        parameter_names = [item["name"] for item in query["parameters"]]
        expected_parameters = BASE_PARAMETERS | EXTRA_PARAMETERS.get(query_id, set())
        template_parameters = set(PARAMETER.findall(template))
        if (
            parameter_names != sorted(set(parameter_names))
            or set(parameter_names) != expected_parameters
            or template_parameters != expected_parameters
        ):
            findings.append(Finding("QUERY_PARAMETER_CLOSURE_INVALID", f"/queries/{index}/parameters"))

        normalized = template.strip()
        if (
            FORBIDDEN_CYPHER.search(normalized)
            or not normalized.upper().startswith("MATCH ")
            or " RETURN " not in normalized.upper()
            or " ORDER BY " not in normalized.upper()
        ):
            findings.append(Finding("QUERY_NOT_READ_ONLY", f"/queries/{index}/cypher_template"))
        if not _canonical_strings(query["result_columns"]):
            findings.append(Finding("RESULT_COLUMNS_NOT_CANONICAL", f"/queries/{index}/result_columns"))
