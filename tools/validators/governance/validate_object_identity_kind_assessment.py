"""Validate fixture-only object identity-kind assessment candidates.

The validator checks closed shape, deterministic content identity, one of four
finite identity semantics, an opaque object-family register binding, review
declarations, and UTC timestamps. It does not create identifiers, mutate an
object, schema, or register, resolve evidence, decide policy or review, or grant
...
SCHEMA_PATH = REPO_ROOT / "schemas/contracts/v1/governance/object_identity_kind_assessment.schema.json"
FIXTURE_PATH = REPO_ROOT / "fixtures/contracts/v1/governance/object_identity_kind_assessment/cases.json"
MAX_FILE_BYTES = 1_048_576
ABSTAIN_CODES = {
    "IDENTITY_KIND_UNRESOLVED",
    "OBJECT_FAMILY_NOT_REGISTERED",
    "REGISTER_BINDING_UNRESOLVED",
    "REVIEW_PENDING",
    "REVIEW_UNKNOWN",
}
EXPECTED_SEMANTICS = {
    "ENTITY": ("DURABLE_IDENTITY", True, True, False, False),
    "VALUE_OBJECT": ("STRUCTURAL_VALUE", False, False, True, False),
    "DERIVED_ARTIFACT": ("CONTENT_ADDRESS", False, False, False, True),
    "UNRESOLVED": ("UNRESOLVED", False, False, False, False),
}
...
def _register_findings(binding: Mapping[str, object]) -> set[Finding]:
    ...
    if state == "RESOLVED":
        if ref is None or digest is None:
            findings.add(Finding("REGISTER_BINDING_REQUIRED", "/register_binding"))
    else:
        if ref is not None or digest is not None:
            findings.add(Finding("REGISTER_BINDING_PROHIBITED", "/register_binding"))
        if state == "UNRESOLVED":
            findings.add(Finding("REGISTER_BINDING_UNRESOLVED", "/register_binding/state"))
        elif state == "NOT_REGISTERED":
            findings.add(Finding("OBJECT_FAMILY_NOT_REGISTERED", "/register_binding/state"))
    return findings


def _declaration_findings(declaration: Mapping[str, object]) -> set[Finding]:
    findings: set[Finding] = set()
    kind = declaration.get("identity_kind")
    observed = (
        declaration.get("identity_basis"),
        declaration.get("durable_identity_required"),
        declaration.get("lifecycle_continuity_required"),
        declaration.get("attribute_equality_defines_equivalence"),
        declaration.get("content_digest_defines_equivalence"),
    )
    assert isinstance(kind, str)
    if observed != EXPECTED_SEMANTICS[kind]:
        findings.add(Finding("IDENTITY_SEMANTICS_INCOHERENT", "/declaration"))
    if kind == "UNRESOLVED":
        findings.add(Finding("IDENTITY_KIND_UNRESOLVED", "/declaration/identity_kind"))

    refs = declaration.get("review_record_refs")
    if not _canonical_strings(refs):
        findings.add(Finding("ARRAY_NOT_CANONICAL", "/declaration/review_record_refs"))
    ...
    elif review_state == "COMPLETE_FOR_DECLARED_SCOPE":
        if not refs:
            findings.add(Finding("REVIEW_RECORD_REQUIRED", "/declaration/review_record_refs"))
        if declaration.get("rationale_summary") is None:
...
def _semantic_findings(candidate: Mapping[str, object]) -> list[Finding]:
    ...
    binding = candidate["register_binding"]
    declaration = candidate["declaration"]
    assert isinstance(binding, Mapping) and isinstance(declaration, Mapping)
    findings.update(_register_findings(binding))
    findings.update(_declaration_findings(declaration))
...
def main(argv: Sequence[str] | None = None) -> int:
    parser = argparse.ArgumentParser(
        description="Validate fixture-only object identity-kind assessments."
    )
