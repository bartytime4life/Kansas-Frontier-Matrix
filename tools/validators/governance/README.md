...
snapshot, 424 workflows passed all twenty rules with an empty implementation-
waiver baseline. That result is historical evidence, not a claim about the
current tree. Run `make workflow-security` at the exact head before reporting a
current result. The tool reads local bytes only; a pass is static source
evidence, not a hosted result, ruleset guarantee, vulnerability-absence claim,
approval, release, deployment, or publication authority. See the current
[workflow inventory](../../../.github/workflows/README.md) for the pinned file
count and its explicit verification boundary.
