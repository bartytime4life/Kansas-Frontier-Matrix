#!/usr/bin/env python3
"""Validate fixture-only KFM domain context-map assessment candidates.

A PASS proves only that a proposed DDD relationship label is locally coherent
with the current domain-lane and cross-domain-seam projections. It does not
write either register, authorize a join or mutation, lower sensitivity, decide
policy or review, or grant promotion, release, deployment, or publication.
"""
...
def _schema_findings(candidate: object) -> list[Finding]:
    try:
        ...
        errors = sorted(validator.iter_errors(candidate),
                        key=lambda error: (list(error.absolute_path), str(error.validator)))
    except (OSError, UnicodeError, json.JSONDecodeError, ValueError, RecursionError):
        return [Finding("SCHEMA_UNAVAILABLE", "/")]
    return [Finding("SCHEMA_INVALID", "/" + "/".join(str(x) for x in error.absolute_path))
...
def _load_projections(repo_root: Path) -> tuple[set[str] | None, dict[str, tuple[str, ...]] | None, Finding | None]:
    ...
    except (OSError, UnicodeError, yaml.YAMLError, json.JSONDecodeError, DuplicateKeyError,
...
def validate_candidate(candidate: object, *, repo_root: Path = REPO_ROOT) -> ValidationResult:
    ...
    if system_error:
        ...
        outcome = "DENY"
    return ValidationResult(outcome, tuple(findings), "REVIE_REQUIRED" if outcome == "PASS" else None)
...
def validate_fixture_manifest() -> list[dict[str, object]]:
    ...
    for entry in manifest["cases"]:
        ...
        results.append({"name": entry["name"], "ok": observed == expected,
...
def _serialize(result: ValidationResult) -> str:
    return json.dumps({
        ...
        "authority": {"writes_register": False, "authorizes_join": False,
                      "authorizes_mutation": False, "authorizes_release": False,
                      "publishes": False},
        "codes": result.codes,
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    result = (ValidationResult("ERROR", tuple(load_findings), None)
              if candidate is None else validate_candidate(candidate))
