#!/usr/bin/env python3
"""Validate fixture-only non-container provenance predicate candidates.

A PASS proves bounded shape, declared binding consistency, fixture polarity,
and deterministic identity only. It does not read artifacts, verify signatures
or transparency, evaluate OPA policy, admit evidence, approve review, release,
or publish.
"""
...
SCHEMA = ROOT / "schemas/contracts/v1/evidence/non_container_provenance_predicate_profile.schema.json"
CASES = ROOT / "fixtures/contracts/v1/evidence/non_container_provenance_predicate_profile/cases.json"
IDENTITY_PREFIX = "kfm:non-container-provenance-predicate:"
MAX_BYTES = 2 * 1024 * 1024
MAX_SCHEMA_FINDINGS = 50
INPUT_ROLES = {"CONFIG_INPUT", "SOURCE_INPUT"}
OUTPUT_ROLES = {"DATA_OUTPUT", "METADATA_OUTPUT", "PROOF_CANDIDATE"}
...
def materialize_case(corpus: Mapping[str, Any], case: Mapping[str, Any]) -> dict[str, Any]:
    ...
    spec_hash, predicate_id = canonical_identity(candidate)
...
def _bindings_valid(bindings: list[Mapping[str, Any]], allowed_roles: set[str]) -> bool:
    ids = [item["artifact_id"] for item in bindings]
    uris = [item["artifact_uri"] for item in bindings]
    return (
        ids == sorted(ids)
        and len(ids) == len(set(ids))
        and len(uris) == len(set(uris))
        and all(item["role"] in allowed_roles for item in bindings)
    )
...
def _semantic_findings(candidate: Mapping[str, Any]) -> list[Finding]:
    ...
    inputs = candidate["declared_inputs"]
    outputs = candidate["declared_outputs"]
    if not _bindings_valid(inputs, INPUT_ROLES):
        findings.append(Finding("INPUT_BINDINGS_INVALID", "/declared_inputs"))
    if not _bindings_valid(outputs, OUTPUT_ROLES):
        findings.append(Finding("OUTPUT_BINDINGS_INVALID", "/declared_outputs"))

    input_ids = {item["artifact_id"] for item in inputs}
    output_ids = {item["artifact_id"] for item in outputs}
    input_digests = {item["digest"] for item in inputs}
    output_digests = {item["digest"] for item in outputs}
    if input_ids & output_ids or input_digests & output_digests:
        findings.append(Finding("ARTIFACT_ROLE_COLLAPSE", "/"))

    run = candidate["run_details"]
    try:
        timing_valid = _parse_time(run["started_on"]) < _parse_time(run["finished_on"])
    except (TypeError, ValueError):
        timing_valid = False
    if not timing_valid:
        findings.append(Finding("RUN_TIMING_INVALID", "/run_details"))
    ...
    try:
        ...
        if candidate.get("predicate_id") != expected_id:
            findings.append(Finding("PREDICATE_ID_MISMATCH", "/predicate_id"))
    return findings
