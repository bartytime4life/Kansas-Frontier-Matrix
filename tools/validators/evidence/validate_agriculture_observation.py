#!/usr/bin/env python3
"""Validate fixture-only AgricultureObservation records."""
...
SCHEMA = ROOT / "schemas/contracts/v1/evidence/agriculture_observation.schema.json"
FIXTURES = ROOT / "fixtures/contracts/v1/evidence/agriculture_observation/cases.json"
PREFIX = "kfm:agriculture-observation:"
...
FAMILY_UNITS = {
    "FARM_COUNT": "COUNT",
    "LAND_IN_FARMS": "ACRES",
    "HARVESTED_CROPLAND": "ACRES",
    "IMPROVED_ACRES": "ACRES",
    "MARKET_VALUE_SALES": "USD",
    "CROP_PRODUCTION": "SOURCE_DEFINED",
    "LIVESTOCK_INVENTORY": "HEAD",
}
...
def _read(path: Path) -> tuple[dict[str, Any] | None, tuple[Finding, ...]]:
    try:
        if path.is_symlink():
            return None, (Finding("AGRICULTURE_INPUT_SYMLINK_DENIED", "/"),)
        if not path.is_file():
            return None, (Finding("AGRICULTURE_INPUT_NOT_FILE", "/"),)
        if path.stat().st_size > MAX_BYTES:
            return None, (Finding("AGRICULTURE_INPUT_TOO_LARGE", "/"),)
        value = json.loads(
        ...
        return None, (Finding("AGRICULTURE_JSON_DUPLICATE_KEY", "/"),)
    except NonFiniteNumberError:
        return None, (Finding("AGRICULTURE_JSON_NONFINITE_NUMBER", "/"),)
    except (OSError, UnicodeError, json.JSONDecodeError, RecursionError):
        return None, (Finding("AGRICULTURE_JSON_INVALID", "/"),)
    if not isinstance(value, dict):
        return None, (Finding("AGRICULTURE_JSON_ROOT_INVALID", "/"),)
    return value, ()
...
def _schema_findings(value: Mapping[str, Any]) -> tuple[Finding, ...]:
    ...
    findings = {
        Finding("AGRICULTURE_SCHEMA_INVALID", _pointer(error.absolute_path))
...
def _semantic_findings(value: Mapping[str, Any]) -> tuple[Finding, ...]:
    ...
    if start != date(year, 1, 1):
        findings.add(
            Finding("AGRICULTURE_PERIOD_START_INVALID", "/temporal/reference_period_start")
        )
    if end != date(year, 12, 31):
        findings.add(
            Finding("AGRICULTURE_PERIOD_END_INVALID", "/temporal/reference_period_end")
        )
    released_at = _instant(temporal["source_released_at"])
    retrieved_at = _instant(temporal["retrieved_at"])
    if released_at.date() < end:
        findings.add(
            Finding("AGRICULTURE_RELEASE_PRECEDES_PERIOD", "/temporal/source_released_at")
        )
    if retrieved_at < released_at:
        findings.add(
            Finding("AGRICULTURE_RETRIEVAL_PRECEDES_RELEASE", "/temporal/retrieved_at")
        )
    ...
    if corrected_instant is not None and corrected_instant < released_at:
        findings.add(
            Finding("AGRICULTURE_CORRECTION_PRECEDES_RELEASE", "/temporal/corrected_at")
        )
    ...
    if state == "OBSERVED":
        if not value_present:
            findings.add(Finding("AGRICULTURE_OBSERVED_VALUE_REQUIRED", "/measure/value"))
        if missing_reason != "NOT_APPLICABLE":
            findings.add(
                Finding(
                    "AGRICULTURE_OBSERVED_MISSING_REASON_INVALID",
                    "/measure/missing_reason",
                )
            )
    elif state == "SUPPRESSED":
        if value_present:
            findings.add(Finding("AGRICULTURE_SUPPRESSED_VALUE_DENIED", "/measure/value"))
        if missing_reason != "SOURCE_SUPPRESSED":
            findings.add(
                Finding("AGRICULTURE_SUPPRESSION_REASON_REQUIRED", "/measure/missing_reason")
            )
    elif state == "MISSING":
        if value_present:
            findings.add(Finding("AGRICULTURE_MISSING_VALUE_DENIED", "/measure/value"))
        if missing_reason not in MISSING_REASONS:
            findings.add(
                Finding("AGRICULTURE_MISSING_REASON_INVALID", "/measure/missing_reason")
            )
    ...
    if state == "SUPPRESSED":
        if suppression["status"] != "SOURCE_SUPPRESSED" or any(
            ...
            findings.add(Finding("AGRICULTURE_SUPPRESSION_BLOCK_REQUIRED", "/suppression"))
    elif suppression["status"] != "NOT_APPLICABLE" or any(
        item is not None for item in suppression_fields
    ):
        findings.add(Finding("AGRICULTURE_SUPPRESSION_BLOCK_DENIED", "/suppression"))

    family = measure["measure_family"]
    if measure["unit"] != FAMILY_UNITS[family]:
        findings.add(Finding("AGRICULTURE_UNIT_MISMATCH", "/measure/unit"))
    if measure["unit"] == "SOURCE_DEFINED":
        if measure["source_unit_ref"] is None:
            findings.add(
                Finding("AGRICULTURE_SOURCE_UNIT_REQUIRED", "/measure/source_unit_ref")
            )
    elif measure["source_unit_ref"] is not None:
        findings.add(Finding("AGRICULTURE_SOURCE_UNIT_DENIED", "/measure/source_unit_ref"))

    price_basis = measure["price_basis"]
    price_year = measure["price_year"]
    if family == "MARKET_VALUE_SALES":
        if price_basis == "NOT_APPLICABLE":
            findings.add(Finding("AGRICULTURE_PRICE_BASIS_REQUIRED", "/measure/price_basis"))
        if price_basis == "CONSTANT_DOLLARS" and price_year is None:
            findings.add(Finding("AGRICULTURE_PRICE_YEAR_REQUIRED", "/measure/price_year"))
        if price_basis == "CURRENT_DOLLARS" and price_year is not None:
            findings.add(Finding("AGRICULTURE_PRICE_YEAR_DENIED", "/measure/price_year"))
    else:
        if price_basis != "NOT_APPLICABLE":
            findings.add(Finding("AGRICULTURE_PRICE_BASIS_DENIED", "/measure/price_basis"))
        if price_year is not None:
            findings.add(Finding("AGRICULTURE_PRICE_YEAR_DENIED", "/measure/price_year"))

    classification_fields = (
        measure["classification_kind"],
        measure["classification_scheme_ref"],
        measure["classification_code"],
    )
    if measure["classification_scope"] == "SOURCE_CLASSIFIED":
        if any(item is None for item in classification_fields):
            findings.add(Finding("AGRICULTURE_CLASSIFICATION_REQUIRED", "/measure"))
    elif any(item is not None for item in classification_fields):
        findings.add(Finding("AGRICULTURE_CLASSIFICATION_DENIED", "/measure"))

    evidence_refs = source["evidence_refs"]
    if evidence_refs != sorted(evidence_refs):
        findings.add(Finding("AGRICULTURE_EVIDENCE_ORDER_INVALID", "/source/evidence_refs"))
    ...
    if correction_state == "ORIGINAL":
        if corrected_at is not None or any(item is not None for item in lineage_refs):
            findings.add(Finding("AGRICULTURE_ORIGINAL_LINEAGE_INVALID", "/lineage"))
    elif corrected_at is None or any(item is None for item in lineage_refs):
        findings.add(Finding("AGRICULTURE_CORRECTED_LINEAGE_INCOMPLETE", "/lineage"))

    limits = disclosure["interpretation_limits"]
    if limits != sorted(limits):
        findings.add(
            Finding("AGRICULTURE_LIMIT_ORDER_INVALID", "/disclosure/interpretation_limits")
        )
    if set(limits) != REQUIRED_LIMITS:
        findings.add(
            Finding("AGRICULTURE_REQUIRED_LIMIT_MISMATCH", "/disclosure/interpretation_limits")
        )

    try:
        digest, identifier = canonical_identity(value)
    except (CanonicalizationFailure, TypeError, ValueError, RecursionError):
        findings.add(Finding("AGRICULTURE_IDENTITY_ERROR", "/spec_hash"))
    else:
        if value["spec_hash"] != digest:
            findings.add(Finding("AGRICULTURE_SPEC_HASH_MISMATCH", "/spec_hash"))
        if value["observation_id"] != identifier:
            findings.add(
                Finding("AGRICULTURE_OBSERVATION_ID_MISMATCH", "/observation_id")
            )
    return tuple(sorted(findings))
    ...
    return json.dumps(
        {
            ...
            "non_effects": [
                ...
                "no_classification_resolution",
                "no_farm_or_producer_identification",
