<!-- [KFM_META_BLOCK_V2]
doc_id: kfm://doc/tools-validators-hydro-readme
title: tools/validators/hydro README
type: README
version: v0.2
status: draft
owner: TODO-tooling-qa-owner-plus-hydrology-steward-plus-schema-steward-plus-policy-steward-plus-fixture-steward-plus-evidence-steward-plus-release-steward
created: 2026-07-08
updated: 2026-07-08
policy_label: repository-facing; hydro-validator-routing-index; hydrology; hydro-alias; huc; reach; gauge; nfhl; source-role-aware; not-flood-warning; release-gated; non-authoritative
owning_root: tools/
responsibility: scaffolded shorthand Hydrology validator routing index for tools/validators/hydro; routes schema, policy, fixture, evidence, proof, receipt, lifecycle-data, test, and release responsibilities to their owning roots while preserving the richer per-domain Hydrology validator authority at tools/validators/domains/hydrology
...
related:
  ...
  - ../../../release/
notes:
  - "This README replaces the previous scaffold at tools/validators/hydro/README.md. It does not confirm executable validator code."
  - "tools/validators/domains/hydrology/ remains the richer per-domain Hydrology validator index. This hydro path is a shorthand/routing lane unless an ADR or validator registry promotes it."
  - "Child routing READMEs currently exist for fixtures/, policy/, and schemas/. They route to fixtures/, policy/, and schemas/ roots instead of storing authority locally."
...
![scope](https://img.shields.io/badge/scope-hydro--validator--routing-informational)
...
> **One-line purpose.** `tools/validators/hydro/` is the scaffolded shorthand Hydrology validator routing lane. The richer per-domain Hydrology validator index remains `tools/validators/domains/hydrology/`; schemas, policy, fixtures, proofs, receipts, data, and release decisions stay in their owning roots.

---

## Purpose

`tools/validators/hydro/` exists to keep the planned shorthand Hydrology validator surface navigable without turning it into a parallel domain, schema, policy, evidence, fixture, release, data, or public-output authority.

The durable KFM question for this routing lane is:

> Do Hydrology validator entry points and helper lanes preserve watershed/reach/site identity, source-role posture, observed-vs-regulatory-vs-modeled separation, HUC/COMID/reach lineage, units, time/freshness posture, not-flood-warning boundaries, neighboring-domain ownership, evidence closure, review state, policy decisions, release readiness, correction paths, rollback support, and public-surface denial before Hydrology candidates reach catalog, proof, release, map, API, graph, Focus Mode, export, search, tile, screenshot, embedding, or AI surfaces?
...
|---|---|---|
| `tools/validators/hydro/README.md` | **CONFIRMED README** | Replaces the previous proposed scaffold with routing documentation. |
...
| `tools/validators/hydro/fixtures/README.md` | **CONFIRMED README / fixture files NEEDS VERIFICATION** | Validator-local fixture guidance; canonical fixtures route to accepted `fixtures/` homes. |
| `tools/validators/hydro/policy/README.md` | **CONFIRMED README / policy bundles NEEDS VERIFICATION** | Validator-local policy routing; real policy authority stays under `policy/`. |
| `tools/validators/hydro/schemas/README.md` | **CONFIRMED README / schema files NEEDS VERIFICATION** | Validator-local schema routing; real machine shape stays under `schemas/`. |
| Cross-domain Atmosphere/Hydrology lane | **CONFIRMED README / executable NEEDS VERIFICATION** | `tools/validators/atmosphere_hydrology/` covers overlap where atmosphere and hydrology interact. |
| Cross-domain join lanes | **CONFIRMED README siblings / executable NEEDS VERIFICATION** | `tools/validators/cross-domain-joins/` and `tools/validators/cross-lane/` preserve neighboring-domain boundaries. |
...
| Path | Role | Boundary |
|---|---|---|
| `tools/validators/hydro/` | Shorthand/routing Hydrology validator lane. | Useful as a compatibility or convenience entry point, but not canonical Hydrology meaning. |
...
| `tools/validators/hydro/fixtures/` | Validator-local fixture guidance. | Routes sample data to accepted `fixtures/` roots; does not store real hydrology data or proofs. |
| `tools/validators/hydro/policy/` | Validator-local policy binding guidance. | Routes allow/deny/restrict/abstain authority to `policy/`; does not define policy. |
| `tools/validators/hydro/schemas/` | Validator-local schema binding guidance. | Routes machine shape to `schemas/`; does not define schemas. |
...
If new Hydrology validation work is broad and domain-scoped, prefer `tools/validators/domains/hydrology/`. If a legacy caller, planned inventory, or CLI shortcut uses `hydro`, keep this path as a routing adapter and document the compatibility reason.
...
|---|---|---|
| [`fixtures/`](fixtures/README.md) | Documents validator fixture expectations and routes reusable sample data to accepted `fixtures/` roots. | **CONFIRMED README / fixture files NEEDS VERIFICATION** |
| [`policy/`](policy/README.md) | Documents policy lookup, binding, digest, and reporting expectations while routing decisions to `policy/`. | **CONFIRMED README / policy bundles NEEDS VERIFICATION** |
| [`schemas/`](schemas/README.md) | Documents schema lookup, binding, id/version/digest, and reporting expectations while routing machine shape to `schemas/`. | **CONFIRMED README / schema files NEEDS VERIFICATION** |
...
- `nfhl-flood-context/` for NFHL/FEMA/regulatory flood context without observed-inundation, forecast, insurance, emergency-warning, or regulatory-determination collapse;
...
- **CONFIRMED:** this README exists and child README lanes exist for `fixtures/`, `policy/`, and `schemas/`.
- **PROPOSED:** validator code may live here only as a shorthand adapter or runner that delegates meaning, schema, policy, evidence, fixtures, receipts, and release authority to owning roots.
...
Good fits for `tools/validators/hydro/` include:

- this routing README;
- lightweight validator entry points that delegate to `tools/validators/domains/hydrology/` or shared validator utilities;
- child README lanes for schema, policy, fixture, or binding guidance that clearly route authority elsewhere;
...
- compatibility notes for planned or legacy `hydro` naming;
...
| Do not put in `tools/validators/hydro/` | Correct home |
|---|---|
...
Hydrology validators under this shorthand lane must fail closed, deny, abstain, or route to steward review when a candidate:
...
| Outcome | Meaning |
|---|---|
| `HYDRO_ROUTE_READY` | Candidate can be routed to configured Hydrology validators. |
| `HYDRO_VALIDATORS_PASS` | Configured shorthand Hydrology validators passed. |
| `HYDRO_VALIDATORS_FAIL` | One or more configured shorthand Hydrology validators failed. |
...
| `ROUTE_TO_CROSS_DOMAIN_JOIN_VALIDATOR` | Candidate should be checked by shared cross-domain join validators. |
...
| `FIXTURE_HOME_UNVERIFIED` | Test fixture path, manifest, or fixture class is unverified. |
...
├── schemas/
│   └── README.md
├── validate_hydro.py                    # PROPOSED; not confirmed
└── registry_notes.md                    # PROPOSED; documentation only
```

If `validate_hydro.py` is added, it should act as an adapter or runner that delegates to accepted Hydrology, schema, policy, fixture, evidence, and release validators. It should not redefine domain meaning, copy schemas, copy policy, store fixtures, write lifecycle data, approve release, or publish public outputs.
...
- [x] It replaces the previous scaffold at `tools/validators/hydro/README.md`.
- [x] It marks `hydro` as a shorthand/routing lane, not canonical Hydrology authority.
- [x] It points the richer Hydrology validator boundary to `tools/validators/domains/hydrology/`.
- [x] It links child routing READMEs for `fixtures/`, `policy/`, and `schemas/`.
...
- [ ] Validator registry or CLI references to `hydro` are searched and classified.
...
| Date | Change | Status |
|---|---|---|
| 2026-07-08 | Replaced proposed scaffold with Hydrology shorthand validator routing README. | **CONFIRMED README / implementation NEEDS VERIFICATION** |
