...
related: [../README.md, ../.github/README.md, ../.github/CODEOWNERS, ../.github/workflows/README.md, ../scripts/README.md, ../contracts/README.md, ../schemas/README.md, ../policy/README.md, ../tests/README.md, ./attest/README.md, ./catalog/README.md, ./ci/README.md, ./diff/README.md, ./docs/README.md, ./probes/README.md, ./validators/README.md]
tags: [kfm, tools, validation, reproducibility, ci]
notes: [Current public main now shows seven child lanes under /tools/; each visible child lane is still README-first on public main. doc_id, created date, and policy_label remain NEEDS VERIFICATION.]
[/KFM_META_BLOCK_V2] -->

# tools

Verification, validation, diff, probe, catalog, attestation, docs, and CI support tooling surface for Kansas Frontier Matrix.
...
> **Evidence posture:** doctrine-grounded · public-`main` repo-grounded for visible tree state · deeper executable inventory, branch protections, and local checkout parity remain bounded  
> **Current public snapshot:** `tools/` currently exposes `attest/`, `catalog/`, `ci/`, `diff/`, `docs/`, `probes/`, `validators/`, and `README.md`; each visible child lane is presently **README-first** in public view  
...
| --- | --- | --- |
| Public `main` repo root visibly includes `contracts/`, `data/`, `docs/`, `infra/`, `policy/`, `schemas/`, `scripts/`, `tests/`, and `tools/` among other root lanes | **CONFIRMED** | Grounds repo-fit and relative-link structure |
| Public `tools/` now visibly contains `attest/`, `catalog/`, `ci/`, `diff/`, `docs/`, `probes/`, `validators/`, and `README.md` | **CONFIRMED** | Replaces the older README-only subtree claim |
| Each visible child lane currently exposes `README.md` only in public view | **CONFIRMED** | Keeps executable inventory claims conservative |
| `.github/CODEOWNERS` assigns `/tools/` to `@bartytime4life` | **CONFIRMED** | Grounds the owners line and review expectation |
| `.github/workflows/README.md` says `.github/workflows/` is currently README-first on public `main` | **CONFIRMED** | Keeps checked-in workflow claims bounded |
| Public Actions history is visible and references recent workflow filenames, but current checked-in workflow YAML inventory is not proven from the public tree | **CONFIRMED historical signal / bounded current-state use** | Allows historical signal without overstating present tree contents |
| Child lane READMEs frame those lanes as validators, probes, diffs, catalog QA, attestation helpers, docs tooling, and CI helpers | **CONFIRMED** | Grounds the family map and placement guidance |
| Attached KFM doctrine still prioritizes contract lattice, policy registries, evidence bundles, runtime envelopes, and a hydrology-first proof slice before broad surface expansion | **CONFIRMED doctrine** | Grounds task ordering, lane purpose, and definition of done |
...
2. Recheck owners and workflow neighbors.
...
```bash
rg -n "tools/(attest|catalog|ci|diff|docs|probes|validators)" README.md .github docs scripts tests policy contracts schemas -S 2>/dev/null
```
...
### Wire helpers into scripts or CI without hiding them
...
2. let `tools/` own reusable inspection, validation, comparison, proof, or support evidence
3. keep workflow YAML thin by calling stable tool entrypoints instead of embedding large shell blobs
...
```mermaid
flowchart LR
    subgraph Adjacent[Adjacent governed surfaces]
      C[contracts/]
      S[schemas/]
      P[policy/]
      T[tests/]
...
Because KFM treats scripts as transition or orchestration machinery, while `tools/` is the reusable helper surface those scripts and workflows can call. If a helper’s main job is validate, compare, summarize, attest, or fail clearly, it should not be buried inside lifecycle choreography.
...
2. /tools/ currently contains seven visible child lanes plus README.md.
3. Each visible child lane currently exposes README.md only in public view.
4. /tools/ is owned by @bartytime4life in .github/CODEOWNERS.
5. .github/workflows/ currently exposes README.md only in the public tree.
6. Public Actions history is visible, but that history is not the same thing as current checked-in workflow inventory.
```
...
# Where does tooling wire into scripts, docs, policy, contracts, and schemas?
rg -n "tools/(attest|catalog|ci|diff|docs|probes|validators)" .github docs scripts policy tests contracts schemas -S 2>/dev/null
