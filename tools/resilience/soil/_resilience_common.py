...
def validate_base_public_url(v): u=urlparse(str(v or '')); return (u.scheme=='https' and u.netloc and '@' not in u.netloc) or (u.scheme=='http' and u.hostname in {'localhost','127.0.0.1'} and '@' not in u.netloc)
def scan_text_for_forbidden_terms(t): s=str(t).lower(); return any(x.lower() in s for x in FORBIDDEN_TERMS)
scan_payload_for_forbidden_terms=lambda p: scan_text_for_forbidden_terms(json.dumps(p,sort_keys=True))
def scan_payload_for_contact_or_secret_terms(p): s=json.dumps(p,sort_keys=True).lower(); return any(x in s for x in [*map(str.lower,CONTACT_TERMS),'secret','token','authorization','cookie'])
...
load_current_delivery_incident_closure=lambda r:_load(r,'incident_closure/soil/current_delivery_incident_closure.json')
load_delivery_incident_closure_manifest=lambda r,i:_load(r,f'incident_closure/soil/cycles/{i}/delivery_incident_closure_manifest.json')
load_delivery_incident_closure_receipt=lambda r,i:_load(r,f'incident_closure/soil/cycles/{i}/delivery_incident_closure_receipt.json')
load_recovery_closure_verification_registry=lambda r,i:_load(r,f'incident_closure/soil/cycles/{i}/recovery_closure_verification_registry.json')
load_safe_mode_exit_recommendation_packet=lambda r,i:_load(r,f'incident_closure/soil/cycles/{i}/safe_mode_exit_recommendation_packet.json')
load_resilience_control_update_plan=lambda r,i:_load(r,f'incident_closure/soil/cycles/{i}/resilience_control_update_plan.json')
