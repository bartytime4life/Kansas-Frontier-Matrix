...
title: scripts/release — Governed release runbook
type: standard
version: v1
status: draft
...
**Status:** draft • **Owners:** TBD • **Last updated:** 2026-02-22
...
**Legend:** ✅ Confirmed • 🧪 Proposed • ❓ Unknown (verify in repo)
...
- ✅ **Package**: deterministic, versioned bundles (data + catalogs + receipts).
- ✅ **Verify**: strict, *fail-closed* gates for identity, licensing, policy, and provenance.
- ✅ **Optionally attest/sign**: SBOM + supply-chain provenance for promoted artifacts.
- ✅ **Publish**: move a release into a runtime surface (e.g., `releases/`, OCI registry, static hosting).

### What this directory is not

Release scripts **must not** be the place where we “do the real work” of ingestion/transforms.
...
> If you don’t yet have a single entrypoint script in this folder, start by adding one and implementing the **gates** section below.
...
2. Search CI workflows for references to `scripts/release` and “publish/promote/release”.
...
1) **Fail-closed promotion**
   - If evidence, signatures, catalogs, or policies fail → **nothing ships**.
...
5) **Policy-aware by default**
   ...
   - Never “leak by behavior” (e.g., distinguish 403 vs 404 in a way that reveals restricted existence).
...
- [ ] **Publish step is explicit:** publish requires an explicit command/flag (never “by accident”).
- [ ] **Artifacts are reversible:** you can roll back the publication without rewriting history (audit remains).

> **WARNING:** If you can’t check all of these boxes, treat the output as **preview-only** and do **not** publish.
...
KFM’s minimum credible promotion contract can be implemented as release-time gates.
...
| B. Licensing & rights | Legal/ethical publishability | Missing license, ambiguous rights, inconsistent STAC/DCAT license fields | Require SPDX/approved license values + rights holder + attribution; block unknown → quarantine |
...
| D. Catalog triplet validation | Evidence resolution + interoperability | STAC invalid, DCAT invalid JSON-LD, PROV missing, broken cross-links | Validate schemas + JSON-LD shapes; run link-check across DCAT⇄STAC⇄PROV + receipt pointers |
...
| F. Policy + contract tests | Trust membrane at API/UI boundary | OPA tests fail; evidence resolver can’t resolve refs; API schema drift | Run policy tests (fixtures); resolve at least one EvidenceRef in CI; validate API contracts/schemas |
...
scripts/release/
├─ README.md
├─ kfm-release            # entrypoint (sh/py/node) — supports subcommands; prints --help
├─ lib/                   # shared helpers (canonicalize, hashing, logging)
├─ gates/                 # gate implementations (validate catalogs, license checks, policy tests)
└─ templates/             # manifest + receipt templates (JSON/YAML)
```
...
└─ <dataset_version_id>/
   ...
   │  └─ *.jsonl
   └─ artifacts/                      # pointers or copies (repo-specific)
      ├─ *.parquet
      ├─ *.pmtiles
      ├─ *.tif
      └─ ...
...
> well-known artifact types + OCI referrers (SBOMs and attestations “travel with” the release).
...
Because this repo’s exact entrypoint may differ, start by discovering what exists:
...
- PRs run **verify-only** (never publish):
  - Gate A–F must pass for touched datasets/components.
...
- Tags/releases (or an approved workflow dispatch) can publish:
  - Gate A–G enforced; require approval if policy label is restricted.
...
- Restore prior “latest” pointer (if you maintain one).
...
### “Gate A failed: dataset_version_id changed”
...
### “Gate B failed: license missing/unknown”
...
### “Gate D failed: cross-link not resolvable”
...
- `kfm-release verify` — run gates A–F (and optionally G) for a dataset version
- `kfm-release build` — package a release bundle (no publish)
- `kfm-release publish` — publish a previously-built bundle to a destination
- `kfm-release attest` — generate SBOM/attestations/signatures (optional, but recommended)
- `kfm-release doctor` — environment check (tooling versions, required binaries)
