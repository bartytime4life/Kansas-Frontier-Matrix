...
- Filters by formats/scales/series (if provided, case-insensitive and normalized)
- Downloads rasters (streamed) to a target directory with SHA256 sidecars
- Writes a README index and a machine-readable manifest for CI
...
----------------------
{
  ...
  "query": {
    "min_year": 1870,
    "max_year": 1950,
    "formats": ["GeoTIFF"],                        # match downloads[].format (case-insensitive)
    "series": ["US Topo", "Historic Topographic"], # optional, substring match (case-insensitive)
    "scales": [24000, 62500],                      # optional (ints/strings ok; normalized)
...
try:
    # Prefer urllib3 Retry (requests re-exports in many versions, but this is future-proof)
...
def build_session(
    *,
    ...
    sess.headers.update({
        ...
        "User-Agent": user_agent or "kfm-topoview/1.1",
    })
    # default timeout shim
    orig = sess.request
...
def _as_float_list(v: Any, fallback: List[float]) -> List[float]:
    try:
...
def _fetch_page(sess: requests.Session, api: str, params: Dict[str, Any]) -> Json:
    ...
    if "application/json" in ctype:
        obj = r.json()
        # Some errors are returned with HTTP 200 {error: ...}
...
def _norm_scale(x: Any) -> Union[int, str]:
    try:
...
def iter_products(
    ...
    *,
    ...
    """
    Iterate products from TopoView with paging. Conservative query: bbox & year range.
    Local filters: formats/series/scales (case-insensitive).
    """
    base_params = {
    ...
    fetched = 0
    cap = min(limit or cfg.max_results, cfg.max_results)

    wanted_formats = {_norm_str(f) for f in (cfg.formats or ["GeoTIFF"])}
    wanted_series = [s.lower() for s in (cfg.series or [])]
    wanted_scales = None
    if cfg.scales:
        wanted_scales = {_norm_scale(s) for s in cfg.scales}
    ...
    while fetched < cap:
        ...
        for pdt in prods:
            pid = str(pdt.get("productId") or pdt.get("id") or "")
            ...
            downloads = pdt.get("downloads", []) or []
            dl_candidates = []
            for d in downloads:
                fmt = _norm_str(d.get("format"))
                if fmt in wanted_formats:
                    dl_candidates.append((fmt, d.get("url")))

            if not dl_candidates:
                continue

            # Optional series filter (substring)
            if wanted_series:
                series_str = _norm_str(pdt.get("series"))
                if not any(s in series_str for s in wanted_series):
                    continue

            # Optional scale filter (normalize)
            if wanted_scales is not None:
                pscale_n = _norm_scale(pdt.get("scale"))
                if pscale_n not in wanted_scales:
                    continue

            # Prefer geotiff if present
            url = None
            preferred_order = ["geotiff"] + [fmt for fmt in wanted_formats if fmt != "geotiff"]
            for pf in preferred_order:
                url = next((u for fmt, u in dl_candidates if fmt == pf and isinstance(u, str) and u), url)
            if not url:
                continue

            bbox = pdt.get("bbox")
            bbox_f = _as_float_list(bbox, _as_float_list(cfg.bbox, cfg.bbox))
            yield Product(
                name=pdt.get("name") or "usgs_topo",
                year=pdt.get("year"),
                scale=pdt.get("scale"),
                download_url=url,
                fmt=next((fmt for fmt, u in dl_candidates if u == url), "GeoTIFF"),
                bbox=bbox_f,
...
def download_file(
    ...
    *,
    overwrite: bool,
    show_progress: bool,
    per_file_chunk: int = 1 << 20,
) -> Tuple[bool, Optional[str], bool]:
    """
    Returns (ok, sha256, was_skipped_existing)
    """
    dst.parent.mkdir(parents=True, exist_ok=True)
    if dst.exists() and not overwrite:
        sha = sha256_file(dst)
        sidecar_sha(dst, sha)
        return True, sha, True

    with sess.get(url, stream=True) as r:
        r.raise_for_status()
        total = int(r.headers.get("Content-Length") or 0)
        bar = tqdm(total=total, unit="B", unit_scale=True, desc=dst.name) if (HAS_TQDM and show_progress) else None
        try:
            with dst.open("wb") as f:
                for chunk in r.iter_content(chunk_size=per_file_chunk):
                    if chunk:
                        f.write(chunk)
                        if bar:
...
def stac_item_for_product(p: Product, cfg: SourceConfig, local_path: Path) -> Json:
    west, south, east, north = _as_float_list(p.bbox, _as_float_list(cfg.bbox, [-180.0, -90.0, 180.0, 90.0]))
    dt_year = p.year if isinstance(p.year, int) else None
    iso = f"{dt_year:04d}-01-01T00:00:00Z" if dt_year else None
    # Ensure unique ID: prefer product_id if present
    ...
    item = {
        ...
        "links": [
            ...
            *([{"rel": "about", "href": cfg.viewer, "title": "TopoView viewer"}] if cfg.viewer else [])
        ],
        "license": "US-PD",
        "providers": [
...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument("--max", type=int, default=None, help="Cap results for testing (<= query.max_results)")
    ...
    p.add_argument("--sleep", type=float, default=0.0, help="Sleep seconds between downloads (politeness/backoff)")
...
def main(argv: Optional[Sequence[str]] = None) -> int:
    ...
    try:
        ...
        print(f"[FAIL] TopoView enumeration failed: {e}", file=sys.stderr)
        return 2
    ...
    for rec in iterator:
        ...
        year = rec.year if rec.year is not None else "NA"
        scale = _norm_scale(rec.scale)
        out_name = f"{name}_{year}_{scale}.tif"
        out_file = cfg.out_dir / out_name

        try:
            ok, sha, was_skipped_existing = download_file(
                sess, rec.download_url, out_file,
                overwrite=bool(args.overwrite),
                show_progress=False  # per-file progress disabled; we use the overall bar
            )
            if ok:
                if was_skipped_existing and not args.overwrite:
                    skipped += 1
                else:
                    downloaded += 1
                lines.append(f"{rec.download_url}\t{out_file.name}\t{rec.year}\t{scale}")
                # STAC item
                if args.stac_dir:
    ...
    try:
        ...
        print(f"[WARN] provenance write skipped: {e}", file=sys.stderr)
