#!/usr/bin/env python3
"""Validate and plan one deterministic backfill window without network or writes."""
...
def _load_input(path: Path) -> tuple[dict[str, Any] | None, list[dict[str, str]], bool]:
    try:
        if path.is_symlink():
...
def _schema_findings(path: Path, payload: dict[str, Any]) -> list[dict[str, str]]:
...
def _parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Plan one bounded deterministic backfill window.")
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    if payload is not None:
        ...
        if not findings:
            try:
                plan = plan_backfill_window(payload)
    ...
    outcome = "ANSWER" if not findings else ("ERROR" if operational_error else "DENY")
    result = {
        "ok": not findings,
        "outcome": outcome,
        "plan": plan if not findings else None,
        "findings": sorted(findings, key=lambda item: (item["path"], item["code"])),
        "scope": "planning-only-backfill-window",
        "authority": {
            "network_fetch": False,
