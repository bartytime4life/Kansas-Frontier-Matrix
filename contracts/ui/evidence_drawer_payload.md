...
`SUPPORTED` is valid only for `ANSWER`, `UPSTREAM_ERROR` is valid only for `ERROR`, and `POLICY_DENIED`, `RIGHTS_UNRESOLVED`, and `SENSITIVE_DETAIL_RESTRICTED` are valid only for `DENY`. An abstention cannot mask an upstream failure or policy denial. A malformed or internally contradictory payload becomes the app-local `INVALID_PAYLOAD` error state and no input values are reflected.
