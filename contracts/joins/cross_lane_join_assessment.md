...
- The unresolved-alias projection is required to make that conservative check. Missing, unreadable, malformed, or structurally invalid projection data produces `VALIDATOR_SYSTEM_ERROR`; it never degrades to an empty alias inventory or an `ALLOW` candidate.
