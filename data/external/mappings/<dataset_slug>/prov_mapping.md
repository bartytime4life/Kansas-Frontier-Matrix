...
> **Purpose:** Define how this external dataset’s lineage is captured as a **PROV activity bundle** (W3C PROV-style), so the project can answer: **“How was this data produced?”** 🧾✅

**Quick links 🔗**
- 📄 STAC mapping: `./stac_mapping.md`
- 📄 DCAT mapping: `./dcat_mapping.md`
- 🧬 Output PROV bundle: `../../../prov/<dataset_slug>.prov.json`
...
- [🗂️ Directory & file layout](#️-directory--file-layout)
- [🧬 PROV modeling rules](#-prov-modeling-rules)
- [🔑 Namespaces & ID strategy](#-namespaces--id-strategy)
...
- [🧾 Example PROV-JSON snippet](#-example-prov-json-snippet)
...
- Lineage is **transparent** (inputs → steps → outputs) 🧵  
- Work is **reproducible** (run id / git commit / parameters are recorded) 🔁  
- Metadata stays **in sync** across STAC, DCAT, and PROV 🔗  
- CI can validate provenance completeness ✅

> **Non‑negotiable for KFM:** PROV must link the full chain **raw inputs → intermediate work → processed outputs**, and identify the **pipeline run/config** (e.g., run id + commit hash). 🧬🧾
...
│   ├── 📄 <dataset_slug>.prov.json
│   └── 📁 <dataset_slug>/                   # (optional) per-run bundles
│       └── 📄 <run_id>.prov.json
└── 📁 external/
    ├── 📁 raw/                              # 📥 raw inputs (read-only)
    │   └── 📁 <dataset_slug>/
    ├── 📁 work/                             # 🧪 intermediate artifacts
    │   └── 📁 <dataset_slug>/
    ├── 📁 processed/                        # 📦 final published outputs
    │   └── 📁 <dataset_slug>/
    └── 📁 mappings/                         # 🧭 mapping docs (STAC/DCAT/PROV)
        └── 📁 <dataset_slug>/
            ├── 📄 prov_mapping.md           # 👈 this file
            ├── 📄 stac_mapping.md           # (optional) sibling doc
            └── 📄 dcat_mapping.md           # (optional) sibling doc
```
...
### 🔗 Core relations we use
At minimum, model:

- `prov:used` — Activity used an Entity (inputs)  
- `prov:wasGeneratedBy` — Entity generated by Activity (outputs)  
- `prov:wasAssociatedWith` — Activity associated with Agent (who/what ran it)  
- `prov:wasDerivedFrom` — Entity derived from another Entity (lineage, versioning)  

Recommended:
- `prov:wasAttributedTo` — crediting/ownership for Entities  
- `prov:actedOnBehalfOf` — automation acting for a maintainer/org  
- `prov:wasRevisionOf` *(or `prov:wasDerivedFrom`)* — dataset version lineage

### 🧾 Required attributes (KFM minimum)
...
## 🔑 Namespaces & ID strategy

### Prefixes (recommended)
Use predictable namespaces so IDs are human-auditable:

- `prov:` — PROV vocab
- `kfm:` — project namespace
- `ext:` — external provider namespace
- `file:` — file locations (repo paths)

Example `prefix` map (PROV‑JSON):
```json
{
...
> ✅ Tip: If you embed a checksum in the entity ID (`...#sha256=<hash>`), keep it **also** as an attribute for easy validation.

---

## 🧱 Entity mapping

Fill out the table below for this dataset. Keep it boring and complete. 🧾🧱

### Entity catalog

| Entity class | Entity ID (pattern) | `prov:type` | Required attributes | Notes |
|---|---|---:|---|---|
| External distribution / landing page | `ext:distribution/<dataset_slug>` | `prov:Entity` | `prov:location`, `prov:label`, `dcterms:license` | Source “thing” even if it’s not a file |
...
| Published dataset “concept” | `kfm:entity:dataset/<dataset_slug>#v=<dataset_version>` | `prov:Entity` | `prov:label`, `kfm:dataset_version`, `dcterms:license` | Useful for version lineage |

> 🧠 Large/remote assets: If the data lives outside git (S3, etc.), the Entity should still exist and carry a **stable URL + hash**.
...
| Package / publish | `kfm:activity:publish/<dataset_slug>/<run_id>` | processed outputs + STAC/DCAT entities | PROV bundle entity + any release artifacts | `kfm:release_tag` *(if used)* | Publication = “boundary moment” |
...
> ✅ If the pipeline is fully automated, you can omit the person Agent **unless** a human made a decision or intervened (then model it explicitly).

---

## 🔗 Cross-links to STAC & DCAT

KFM expects **cross-references** so catalogs, graph, and narrative layers stay aligned. 🔗🧭

### STAC ↔ Data ↔ PROV
- STAC Items **must link to the actual processed assets** in `data/.../processed/...`
- STAC should carry **source attribution + license**
- STAC should include a **provenance reference** pointing to this dataset’s PROV bundle (field name per `KFM_STAC_PROFILE.md`)
...
When `<dataset_slug>` is updated or reprocessed, ensure lineage between versions is explicit. 🔁🧬
...
- Link the new version to the prior version:
  - DCAT: `prov:wasRevisionOf` (new → old)
...
> 🏷️ If the repo uses release tags for data bundles, include `kfm:release_tag` in the publish activity and/or output entities.
...
- [ ] STAC/DCAT cross-links exist (STAC/DCAT → data, and provenance references)
- [ ] Version updates include revision/derivation links (v2 → v1)

---

## 🧾 Example PROV-JSON snippet

<details>
<summary><strong>📄 Minimal skeleton (copy + customize)</strong></summary>
...
### Recommended generation workflow 🛠️
1. Pipeline produces/updates:
   - raw → work → processed assets
...
### “Red flag” rule 🚩
If something is “published” but lacks a PROV bundle, treat it as **incomplete** until provenance is added.
