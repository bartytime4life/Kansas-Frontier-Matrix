...
- [ ] 🗂️ **Catalog metadata** exists (STAC/DCAT) and is complete enough for discovery + reuse.
- [ ] 🧭 **Spatial reference** is unambiguous (CRS/datum) and transformations are documented.
- [ ] 🕰️ **Temporal semantics** are unambiguous (timezone/granularity; no “mystery dates”).
- [ ] 🧹 Core data-quality issues are addressed (missing/duplicate/implausible/outdated/wrong/ambiguous).
- [ ] 🗄️ DB load validates constraints; API surfaces expected outputs; UI consumes API only.
...
- [ ] **License present** and explicit (no “unknown” / “assume open”).  
...
### 4) 🕰️ Temporal QA (if dataset has dates/times)

Time-oriented data is especially prone to quality issues (“dirty data”) and problems may not show up until visualization/analysis starts.  [oai_citation:9‡Visualization of Time-Oriented Data.pdf](sediment://file_000000001468722f929b8752236e5a72)

#### 4.1 Time semantics
- [ ] Identify time fields (`event_date`, `start_date`, `end_date`, etc).
- [ ] Confirm timezone handling (explicit timezone or clearly “local time”).
...
### 7) 🗄️ Database + API Integration QA
...
### 8) ⚡ Performance & Scale Notes (as needed)
...
### 10) 🗺️ UI Map-Readiness (if this becomes a layer)
