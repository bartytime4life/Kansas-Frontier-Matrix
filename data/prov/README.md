---
title: "KFM data/prov/ — Provenance Bundles (README)"
...
# KFM data/prov/ — Provenance Bundles
...
- Make provenance “first-class” so that audits and Focus Mode can trace every displayed claim or asset back to evidence and lineage.

### Scope

| In Scope | Out of Scope |
|---|---|
| Folder conventions and expectations for provenance artifacts under `data/prov/` | Full normative PROV profile spec (see `docs/standards/KFM_PROV_PROFILE.md` — placeholder/empty per v13 blueprint) |
...
| Domain staging | `data/<domain>/raw/` → `work/` → `processed/` | Source snapshots → intermediates → curated outputs |
...
└── 📁 prov/
    ...
    └── 📁 <domain>/
        └── 📁 <dataset_id>/
            └── 📁 runs/
                ├── 📄 <run_id>.prov.jsonld      # recommended (serialization may vary)
                └── 📄 <run_id>.manifest.json    # optional run manifest (if adopted)
~~~
...
- KFM’s canonical pipeline ordering is preserved: **ETL → STAC/DCAT/PROV → Graph → APIs → UI → Story Nodes → Focus Mode**.
...
- Provenance should be sufficient to support:
  ...
  - Focus Mode’s “provenance-linked context only” behavior.

### Constraints / invariants

- **No direct UI→Graph access**: UI consumes provenance-linked context via contracted APIs only.
...
- Add a minimal “vertical slice” domain example that produces a real PROV bundle.
...
- `prov:wasDerivedFrom`: output entities → input entities
- `prov:wasGeneratedBy`: output entities → activity (run)
- `prov:used`: activity (run) → input entities
- `prov:wasAssociatedWith`: activity (run) → agent (pipeline identity)
