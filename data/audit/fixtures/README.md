...
│
├─ audit_record/                                   # AuditRecord fixture examples (valid/invalid)
│  ├─ audit_record.minimal.valid.json              # Minimal valid AuditRecord (must pass schema)
│  └─ audit_record.missing_required_fields.invalid.json # Invalid: missing required fields (must fail)
│
├─ gate/                                           # Gate decision fixtures (publish/promotion allow/deny)
│  ├─ gate.publish.allow.minimal.json              # Allow decision example (minimal publish gate pass)
│  └─ gate.publish.deny.policy_restricted.json     # Deny decision when policy label is restricted (fail-closed)
│
└─ redaction/                                      # Redaction transform fixtures (expected policy outcomes)
   └─ redaction.coordinates.coarsen.json           # Coordinate coarsening example (precision/generalization)
```
