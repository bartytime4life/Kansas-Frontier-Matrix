...
title: data/triplet/ — Singular Triplet Compatibility Pointer
...
owners: NEEDS VERIFICATION — data, graph, catalog, migration, and docs stewards
updated: 2026-07-24
...
current_path: data/triplet/README.md
...
evidence_snapshot:
  ...
  prior_blob: 8ccabe28c3153ebf56d1828294e5810e420be446
...
# `data/triplet/` — Singular Triplet Compatibility Pointer

> **One-line purpose.** Preserve the singular historical path as a pointer to canonical `data/triplets/` and prevent a parallel graph or relationship-projection authority.
...
Preserve the singular historical path as a pointer to canonical `data/triplets/` and prevent a parallel graph or relationship-projection authority.
...
**Compatibility — transitional; SINGULAR PATH responsibility.**
...
| Path | `data/triplet/` |
| Version | `v0.2.0` |
| Prior blob | `8ccabe28c3153ebf56d1828294e5810e420be446` |
...
- this README, bounded inventory, redirects/deprecation notes, and migration mappings to `data/triplets/`;
...
|---|---|
| triplet payloads, graph deltas, graph exports, catalog records, proofs, receipts, releases, or public graph artifacts | canonical `data/triplets/` and the appropriate supporting lanes |
| sensitive or claim-bearing relationship data | governed canonical lanes after evidence and policy checks |
...
Inventory and consumer evidence for the singular path and its `habitat/` compatibility child.
...
A migration and stale-reference surface only. New payload writes are denied.
...
Accountable ownership remains **NEEDS VERIFICATION**. Changes should include data, graph, catalog, migration, and docs stewards as applicable. Payload, source activation, rights/sensitivity, migration, public-serving, correction, or rollback changes require corresponding specialist and independent review. CODEOWNERS routing is not approval evidence.
...
Canonical home: [`../triplets/`](../triplets/README.md). Retirement requires payload and consumer inventory, accepted migration/deprecation decision, reference updates, stale-path tests, preserved IDs/evidence, public-path checks, and rollback.
