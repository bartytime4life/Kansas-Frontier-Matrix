---
title: "🕸️ KFM `data/graph/` — Graph Import Artifacts for Neo4j"
...
# 🕸️ KFM `data/graph/` — Graph Import Artifacts for Neo4j
...
- 🚫 **No “mystery nodes”**: every node/edge must carry **evidence pointers** (STAC/DCAT) and, when applicable, **lineage pointers** (PROV).
...
- ♻️ **Deterministic builds**: artifacts here should be generated by deterministic pipelines (no hand-edited drift).
...
|---|---|---|
| 🛰️ STAC | Assets and spatial records | `data/stac/` |
| 🗂️ DCAT | Dataset discoverability + publishing metadata | `data/catalog/dcat/` |
| 🧬 PROV | Lineage, reproducibility, audit trails | `data/prov/` |
| 🕸️ Graph import CSV | Bulk import artifacts | `data/graph/csv/` |
...
| 🗺️ UI | Map + exploration interface | `web/` |
...
- [⏱ Real-time, simulation, and Focus Mode integration](#real-time-simulation-and-focus-mode-integration)
...
|---|---|
| 📄 Importable CSV exports | 🗃️ Full Neo4j store files / db dumps |
| 🧾 Small fixtures for tests | 🔐 Secrets, tokens, `.env`, credentials |
| 🧩 Constraints, indexes, migrations | 📥 Raw source snapshots |
| 🔗 Evidence + lineage pointers | 🧑‍🎨 UI code or UI → Neo4j direct access |
...
- **Claim node**: an explicit interpretation statement with evidence + confidence, rather than a “hard fact”.
...
- ♻️ **Reproducibility:** “Can we rebuild and compare outputs?”
...
> [!NOTE]
> This supports “code + data lineage in one graph,” enabling queries like “Which code version produced this dataset and who reviewed it?”
...
Rules:
- ✅ Stable across rebuilds for the “same conceptual thing”
...
| `name` | ✅ | Display name |
| `description` | ⚠️ | Short summary, not full text |
| `classification` | ✅ | `public/internal/confidential/restricted` |
| `care_label` | ✅ | `n/a` or sensitivity tag |
| `valid_from` / `valid_to` | ⚠️ | ISO 8601 validity |
...
| `license_id` | ⚠️ | License id or SPDX (esp. for datasets) |
...
### Sensitive locations and CARE labels ❤️🧭
...
## ⏱ Real-time, simulation, and Focus Mode integration

### Real-time data is still governed 🛰️
...
- graph links should allow queries like “which model run produced this layer?”
