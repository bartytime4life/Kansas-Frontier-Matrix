...
This directory contains harmonized, validated, and ethically certified datasets representing Kansas vegetation, soil, and surface-cover classifications for **open publication, scientific reuse, and Focus Mode v2.1 analytics**.

[![Docs · MCP v6.3](https://img.shields.io/badge/Docs%20·%20MCP-v6.3-blue.svg)](../../../docs/README.md)
[![FAIR+CARE Certified](https://img.shields.io/badge/FAIR%2BCARE-Landcover%20Certified-gold.svg)](../../../docs/standards/faircare.md)
...
- **Focus v2.1** alignment (explainability overlays, citations, and CARE ethics flags).  
...
- Enable **AI/Focus v2.1** vegetation, NDVI, and land-use change analytics with explainability.
...
├── README.md
│
├── landcover_classifications_v10.2.2.parquet     # Harmonized NLCD/CLC classes (Kansas subset + crosswalk)
├── vegetation_index_ndvi_2025_v10.2.2.csv        # MODIS/VIIRS NDVI annual composites & stats
├── soil_moisture_surface_2025_v10.2.2.csv        # ESA CCI surface soil moisture (resampled & masked)
├── canopy_cover_trends_2000_2025_v10.2.2.csv     # Multi-year canopy cover & change metrics
├── landuse_change_matrix_2001_2021_v10.2.2.csv   # Crosswalk-based transitions (NLCD epochs)
...
```mermaid
flowchart TD
    STAGE["Staging Layer (data/work/staging/landcover/*)"] --> FAIR["FAIR+CARE Validation & Ethics Certification"]
    FAIR --> SCHEMA["Schema Alignment (JSON Schema · ISO 19115)"]
    SCHEMA --> CHECK["Checksum Verification (SHA-256) & Ledger Logging"]
    CHECK --> PUB["Publication (data/processed/landcover/*)"]
    PUB --> CATALOG["STAC/DCAT Catalog Sync + Streaming STAC + Focus v2.1 Enablement"]
```

### Validation & Governance Artifacts

- **Schema Validation:** `data/reports/validation/schema_validation_summary.json`  
- **Checksums:** `data/processed/metadata/checksums_landcover_v10.2.2.json`  
...
```json
{
  "id": "processed_landcover_classifications_v10.2.2",
  "domain": "landcover",
...
| **Ethics** | Sensitive ecological locations generalized as necessary. | `@kfm-ethics` |
...
| **Checksum Verification** | Integrity & reproducibility proof. | `checksums_landcover_v10.2.2.json` |
...
| Checksum Records | Permanent | Cross-verified each release |
| Metadata & Lineage | Permanent | ISO 19115 lineage protocols |
| Logs | 365 Days | Rotated annually per policy |
...
```text
Kansas Frontier Matrix (2025). Processed Landcover Data (v10.2.2).
Final FAIR+CARE-certified vegetation, soil, and surface-cover datasets integrating NLCD, MODIS/VIIRS, and Copernicus sources.
Checksum-verified, schema-aligned, and governance-certified for public research, policy, and Focus Mode v2.1 analytics.
```
...
|---|---|---|---|
| v10.2.2 | 2025-11-12 | `@kfm-land` | Align to v10.2: Streaming STAC sync, Focus v2.1 compatibility, telemetry/schema v2, and governance cert paths. |
| v10.0.0 | 2025-11-09 | `@kfm-land` | Streaming STAC enablement, Focus v2 alignment, telemetry/schema v2. |
