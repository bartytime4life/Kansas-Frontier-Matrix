...
telemetry_schema: "../../../../../schemas/telemetry/data-work-staging-spatial-validation-v10.json"
...
Ensures every geospatial dataset is schema-verified, ethics-certified, and promotion-ready for the processed spatial layer, with **telemetry v2** capture.
...
**v10 Enhancements**
- Telemetry v2 (energy/CO₂, validation coverage) attached to validation outputs.  
- Streaming STAC link checks incorporated into metadata validation.  
- Unified QA schema surfaced to Focus Mode v2 dashboards.
...
└── metadata.json                          # Validation metadata, telemetry, and governance linkage
```
...
```mermaid
flowchart TD
    "Spatial Data (staging/spatial/tmp/*)" --> "Geometry Integrity Validation (Topology + Extent)"
    "Geometry Integrity Validation (Topology + Extent)" --> "CRS Audit (EPSG:4326 Normalization Check)"
    "CRS Audit (EPSG:4326 Normalization Check)" --> "FAIR + CARE Ethics Review"
    "FAIR + CARE Ethics Review" --> "STAC/DCAT Metadata Compliance Audit"
    "STAC/DCAT Metadata Compliance Audit" --> "Checksum Verification + Governance Ledger Sync"
    "Checksum Verification + Governance Ledger Sync" --> "Certification → Processed Spatial Layer"
```
...
```json
{
  "id": "spatial_validation_landcover_v10.0.0",
...
| **Reusable** | Metadata includes lineage, checksum, telemetry, and schema verification. | `@kfm-design` |
...
| `metadata.json` | Validation metadata, telemetry, and ledger record. | JSON |
...
## 🧾 Internal Citation
```text
Kansas Frontier Matrix (2025). Spatial Validation Workspace (v10.0.0).
...
|---|---|---|---|
| v10.0.0 | 2025-11-09 | `@kfm-spatial` | Upgraded to v10: telemetry v2 attachment, Streaming STAC link checks, unified QA schema. |
| v9.7.0 | 2025-11-06 | `@kfm-spatial` | Telemetry schema added; governance fields expanded. |
