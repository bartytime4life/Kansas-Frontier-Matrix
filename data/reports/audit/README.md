...
telemetry_schema: "../../../schemas/telemetry/data-raw-audit-ledger-v10.json"
...
**Purpose:**  
Define the **audit, provenance, and integrity verification framework** for KFM data and model assets.  
Track dataset/model lineage, transformation history, checksum integrity, governance decisions, and **FAIR+CARE** outcomes for every asset in the repository, with **telemetry v2** bindings and **Streaming STAC** references.
...
```mermaid
flowchart TD
  "Ingest / Train / Update" --> "Schema & STAC/DCAT Validation"
  "Schema & STAC/DCAT Validation" --> "FAIR+CARE Audit"
  "FAIR+CARE Audit" --> "Checksums & SBOM Generation"
  "Checksums & SBOM Generation" --> "Governance Review & Sign-off"
  "Governance Review & Sign-off" --> "Ledger Update — data/raw/reports/audit"
  "Ledger Update — data/raw/reports/audit" --> "Release Manifest & Telemetry Sync"
  "Ledger Update — data/raw/reports/audit" --> "STAC / DCAT Back-Reference"
```

**Sequence**
1. **Event**: ETL job, model training, or dataset update completes.  
2. **Validation**: STAC/DCAT/schema checks verify metadata and structure.  
3. **FAIR+CARE**: Ethics & completeness audits run; results logged.  
4. **Integrity**: SHA-256 checksums computed; SBOM generated/updated.  
...
```json
{
  ...
  "ingest": {
    ...
    "completed_at": "2025-11-09T16:05:12Z",
...
- Release-time consolidation into `releases/v10.0.0/manifest.zip` (checksums + signatures)  
...
These linkages power **Focus Mode v2** tracebacks and enable external verification.
...
|---|---|---|---|
| v10.0.0 | 2025-11-09 | `@kfm-governance`| Upgraded to v10: telemetry v2 & Streaming STAC references; AI explainability schema expanded; integrity logs hardened. |
| v9.7.0  | 2025-11-06 | `@kfm-governance`| SBOM/manifest refs aligned; telemetry/schema paths added; badges hardened. |
| v9.5.1  | 2025-10-30 | `@kfm-data-lab`  | DCAT cross-refs and nightly integrity checks. |
| v9.3.2  | 2025-10-28 | `@kfm-data-lab`  | Baseline audit & governance workflows established. |
