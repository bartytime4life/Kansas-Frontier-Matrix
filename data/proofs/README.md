...
tags: [kfm, data, proofs, release-evidence, proof-pack, correction, rollback]
notes: [doc_id and exact created date need verification; policy_label needs repo-backed confirmation; owners reflect current public CODEOWNERS coverage for /data/.]
[/KFM_META_BLOCK_V2] -->
...
Governed **release evidence** surface for proof packs, release manifests, and rollback/correction trace for KFM promotion-ready releases.
...
> ![Status: Experimental](https://img.shields.io/badge/status-experimental-orange) ![Surface: Release Evidence](https://img.shields.io/badge/surface-release%20evidence-8250df) ![Proofs: Required](https://img.shields.io/badge/promotion-proof%20required-d73a49) ![Repo Tree: Public Main Verified](https://img.shields.io/badge/repo%20tree-public%20main%20verified-1f6feb) ![Truth Labels](https://img.shields.io/badge/truth-CONFIRMED%20%7C%20INFERRED%20%7C%20PROPOSED%20%7C%20UNKNOWN-2ea043)  
...
>
> It should make promotion, rollback, correction, and supersession inspectable. It should **not** become a second home for schemas, policy bundles, process-memory receipts, runtime code, or unpublished source data.
...
> `data/published/` preserves already release-backed outward scope.
...
| Sibling lifecycle docs | `data/receipts/README.md` and `data/published/README.md` are real public-main sibling surfaces, which makes the **receipts vs proofs vs published** boundary worth stating explicitly here |
...
| Validator pressure | `tools/validators/README.md` and `tools/validators/promotion_gate/README.md` now make release-facing validation and proof linkage more explicit |
| Doctrine | `ReleaseManifest`, `ReleaseProofPack`, `ProjectionBuildReceipt`, `EvidenceBundle`, `RuntimeResponseEnvelope`, and `CorrectionNotice` are named trust objects in current KFM manuals |
| Evidence tension | `data/specs/README.md` is directly retrievable on public `main`, but `data/README.md` still warns that `data/specs/` is unknown or not visible on the current public tree |
...
3. What checks passed, failed, abstained, or blocked promotion?
...
| Lateral | [`../catalog/README.md`](../catalog/README.md) | Catalog closure is a prerequisite for trustworthy release proof, and current catalog surfaces already expose `dcat/`, `stac/`, and `prov/` closure pressure | CONFIRMED |
...
| Adjacent validator | [`../../tools/validators/README.md`](../../tools/validators/README.md) | Validators may check linkage among receipts, proofs, catalogs, and manifests without becoming proof ownership | CONFIRMED |
| Adjacent validator | [`../../tools/validators/promotion_gate/README.md`](../../tools/validators/promotion_gate/README.md) | Promotion validation is the clearest release-facing consumer of this lane’s trust objects | INFERRED / PROPOSED alignment |
...
| Adjacent gatehouse | [`../../.github/actions/README.md`](../../.github/actions/README.md) | Public-main exposes placeholder proof-adjacent local action names such as `opa-gate/`, `provenance-guard/`, and `sbom-produce-and-sign/` | CONFIRMED adjacent seam / UNKNOWN live callers |
...
| Adjacent gatehouse | [`../../.github/workflows/README.md`](../../.github/workflows/README.md) | Current public workflow inventory is README-only, which is a useful boundary on what can and cannot be claimed about active proof emitters | CONFIRMED |
...
| Downstream | Governed APIs and trust-visible shells | Runtime surfaces should expose correction, freshness, and proof state without making storage layout itself the public contract | PROPOSED |
...
| `ReleaseProofPack` | This is the core publishability bundle: checks, signatures, attestations, SBOM refs, accessibility gate, rollback posture | Promotion / release |
...
- it is clearly **proof-shaped** rather than process-memory-shaped
- it is release-linked or correction-linked
- it preserves stable joins to decision, catalog, and lineage surfaces
- it does not quietly become a second schema or policy home
- it remains inspectable during rollback, correction, or release review without guesswork
...
| `.github/actions/` | Public inventory shows proof-adjacent local action names, but those directories are still placeholder-heavy | Proof-adjacent implementation seams exist without proving live callers or production action contracts |
...
| `data/README.md` vs `data/specs/README.md` | Parent `data/README.md` still warns that `data/specs/` is not visible on current public main, while the direct `data/specs/README.md` path is retrievable | Reconcile this documentation tension before teaching the specs lane as fully settled current-tree fact |
...
Where release-facing validators use finite outcomes, proof objects should remain joinable to at least:

| Outcome | Why it matters in proofs |
|---|---|
| `ALLOW` | explains why promotion advanced |
| `ABSTAIN` | explains why release stayed unresolved without contradiction |
| `DENY` | explains why release was blocked |
| `ERROR` | explains validator or runtime failure without ambiguity |
...
- [ ] `receipts/`, `proofs/`, `catalog/`, and `published/` boundaries remain explicit and cross-linked
...
- [ ] adjacent docs stay in sync: `data/README.md`, `data/receipts/README.md`, `data/catalog/README.md`, `data/published/README.md`, `contracts/README.md`, `policy/README.md`, `tests/README.md`, `tools/validators/README.md`, `tools/validators/promotion_gate/README.md`, `.github/README.md`, `.github/actions/README.md`, `.github/workflows/README.md`, and `.github/PULL_REQUEST_TEMPLATE.md`
...
Because the current public repo directly confirms the README path but does not publicly surface the deeper proof inventory in this directory. This README distinguishes doctrine-aligned starter shape from checked-in public-tree fact.

### What should this README do about `data/specs/README.md`?

Treat it carefully.

The direct file path is retrievable on public `main`, but the current parent `data/README.md` still teaches that `data/specs/` is unknown or not visible on the public tree. Keep that mismatch visible until the working branch reconciles it.
...
- Preserve the `# proofs` title only if the working branch and surrounding docs still expect it; otherwise prefer the explicit path-style heading used elsewhere.
