...
related: [data/README.md, data/specs/README.md, data/catalog/README.md, contracts/README.md, policy/README.md, tests/README.md, .github/README.md, .github/workflows/README.md]
tags: [kfm, proofs, release-evidence]
notes: [doc_id and exact created/updated dates need verification; owners reflect current public CODEOWNERS coverage for /data/]
[/KFM_META_BLOCK_V2] -->
...
> **Status:** experimental · **Doc state:** draft · **Owners:** `@bartytime4life` · **Path:** `data/proofs/README.md` · **Repo fit:** directory contract for release manifests, proof packs, attestations, and visible recovery evidence  
...
[!NOTE]
Current public-repo evidence confirms this README path exists and that the public `data/proofs/` listing currently shows `README.md`. Exact non-README subtree contents, proof emitters, signature tooling, and workflow wiring still need live-checkout verification before stronger claims are merged.

## Scope

`data/proofs/` is where KFM should keep the evidence that a release is **publishable**, not merely buildable or deployed.

In March 2026 doctrine, promotion changes trust state; missing or invalid `ReleaseManifest` / `ReleaseProofPack` should block promotion; and correction / rollback must remain visible rather than silently overwriting history.

### Current verified snapshot

| Layer | What this README treats as settled |
|---|---|
| Current public repo | `data/proofs/` exists on public `main`, and its public directory listing currently shows `README.md`. |
| Adjacent repo docs | `data/README.md`, `data/specs/README.md`, `data/catalog/README.md`, `contracts/README.md`, `policy/README.md`, `tests/README.md`, `.github/README.md`, and `.github/workflows/README.md` exist as neighboring contract, policy, verification, and gatehouse surfaces. |
...
**Path status:** **CONFIRMED** directory README on current public `main`; deeper subtree inventory remains **UNKNOWN** unless directly surfaced.

### Upstream, lateral, and downstream anchors

| Direction | Surface | Why it matters | Status in this README |
|---|---|---|---|
| Upstream | [`../README.md`](../README.md) | Governs the wider `data/` lifecycle and still describes `proofs/` as a release-evidence lane within the `data/` structure. | CONFIRMED |
| Lateral | [`../specs/README.md`](../specs/README.md) | Declares spec inputs, `spec_hash`, vocab/fixture expectations, and promotion-facing specification discipline that should feed release proof. | CONFIRMED |
| Lateral | [`../catalog/README.md`](../catalog/README.md) | Catalog closure is a prerequisite for release proof, and the current public catalog surface already exposes `dcat/`, `stac/`, and `prov/` as neighboring closure lanes. | CONFIRMED |
...
| Upstream | [`../../policy/README.md`](../../policy/README.md) | Policy decisions, deny-by-default logic, review gates, and correction/withdrawal behavior should remain executable and testable. | CONFIRMED surface / mounted bundles UNKNOWN |
| Upstream | [`../../tests/README.md`](../../tests/README.md) | Release proof is incomplete unless tests can explain why the release is trustworthy and why negative paths fail closed. | CONFIRMED |
| Adjacent gatehouse | [`../../.github/README.md`](../../.github/README.md) | Repo-wide review, CI/CD, and delivery controls should emit or validate proof objects rather than bypass them. | CONFIRMED |
...
| Downstream | Correction lineage lookup | Public surfaces should preserve superseded, withdrawn, and correction-pending state through governed lookup rather than silent replacement. | PROPOSED |
...
| Candidate vs release proof split | INFERRED / PROPOSED | The manuals distinguish candidate proof before deployment from release proof at promotion. |
| Correction and drill subtrees | PROPOSED starter shape | Useful to keep operational evidence reviewable, but not yet confirmed as the repo’s mounted public layout. |
...
| `data/catalog/` | Current public catalog surface exists and exposes `dcat/`, `stac/`, and `prov/`. | Release proof should join to real closure lanes rather than treating closure as a hypothetical. |
| `data/README.md` | Parent `data/` README still uses continuity-era “starter skeleton” wording for several children, including `proofs/`. | If this README becomes the stronger local contract, sync the parent README in the same PR to reduce drift. |
...
grep -RIn "ReleaseProofPack\|ReleaseManifest\|CorrectionNotice\|ProjectionBuildReceipt\|rollback\|postdeploy" . \
...
- [ ] Adjacent docs stay in sync: `data/README.md`, `data/catalog/README.md`, `contracts/README.md`, `policy/README.md`, `tests/README.md`, and `.github/workflows/README.md`.
- [ ] Parent `data/README.md` evidence posture is synchronized if `data/proofs/` is treated as a confirmed public directory rather than only a starter skeleton.
...
|---|---|
| CONFIRMED current public repo | `data/proofs/README.md` exists; current public `data/proofs/` listing shows `README.md`; adjacent README surfaces exist; `.github/workflows/` is currently a README-only public inventory surface. |
...
- If the parent `data/README.md` still describes `data/proofs/` only as a starter skeleton, retire that mismatch when the live tree is verified.
