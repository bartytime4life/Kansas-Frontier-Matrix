...
title: data/published/api_payloads/fauna README
...
related:
  ...
  - ../../../../policy/README.md
notes:
  - "Directory README for released Fauna API payload carriers under data/published/api_payloads/."
...
> Published API-payload lane for released, public-safe Fauna payload carriers. Files here should be immutable, release-linked API payload snapshots or packages consumed through governed API or approved released-artifact paths.
...
> [!IMPORTANT]
> **Status:** `draft`  
> **Owners:** `TODO(owner): data steward` · `TODO(owner): fauna domain steward` · `TODO(owner): API steward` · `TODO(owner): publication steward` · `TODO(owner): release steward`  
> **Path:** `data/published/api_payloads/fauna/README.md`  
> **Truth posture:** CONFIRMED target path and Fauna docs from current repo evidence / PROPOSED child layout and naming / NEEDS VERIFICATION for emitted payloads, schemas, validators, release manifests, CI checks, and governed API routes.
...
`data/published/api_payloads/fauna/` is the published carrier lane for release-approved Fauna API payload snapshots and packages.
...
- explicit about source role, evidence support, time scope, uncertainty, caveats, and correction status.
...
|---|---|---|
| `data/raw/fauna/` | Source capture lane. | Not public-readable. |
| `data/work/fauna/` | Working candidate lane. | Not public-readable. |
| `data/quarantine/fauna/` | Held-material lane. | Not public-readable. |
| `data/processed/fauna/` | Validated candidate lane. | Upstream of release. |
| `data/catalog/domain/fauna/` | Catalog lane. | Discovery and lineage, not release authority. |
| `data/proofs/fauna/` | Proof-support lane. | Support, not published payload. |
...
| Payload index | `indexes/fauna-api-payload-index.json` | Points to release-approved payloads only. |
...
|---|---|
| Source payloads and source-system exports | `data/raw/fauna/` |
| Working or held candidates | `data/work/fauna/` or `data/quarantine/fauna/` |
...
Before a Fauna API payload is placed here, verify:
...
```text
fauna.published.api_payload.<payload_family>.<scope>.<release_id>.<short_hash>.json
```
...
- [ ] Source role, evidence support, time scope, uncertainty, and public-safe posture are preserved.
...
- [ ] Fauna API payload contracts and schemas exist under approved homes.
...
Published Fauna API payloads should be compact, citable, public-safe, audience-aware, and reversible. If evidence, validation, policy, release, correction, or rollback support is incomplete, keep the payload upstream instead of placing it here.
