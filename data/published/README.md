...
tags: [kfm, data, published, release-backed, public-safe, steward-facing]
notes: [public-main inspection confirms `data/published/` exists and currently shows `README.md` only; `doc_id`, `created`, and `policy_label` still need repo-side verification before merge]
[/KFM_META_BLOCK_V2] -->
...
> ![Status: Experimental](https://img.shields.io/badge/status-experimental-orange) ![Surface: Published](https://img.shields.io/badge/surface-published-6f42c1) ![Publication: State First](https://img.shields.io/badge/publication-state--first-0a7d5a) ![Trust: Release Backed](https://img.shields.io/badge/trust-release--backed-5b4bdb) ![Public Main: README Only](https://img.shields.io/badge/public__main-published%2FREADME--only-lightgrey)  
...
>
> In KFM, publication is a **governed trust state first**. This directory is the optional materialized surface for scope that is already:
>
> - release-backed
> - catalog-linked
> - policy-shaped
> - correction-aware
> - fit to reference from governed APIs
...
>
> `data/published/` is the materialized outward edge.  
> It should point back to process memory, release evidence, and catalog closure rather than replace any of them.

> [!NOTE]
> This README is intentionally **public-tree-grounded**.
>
> Current public `main` confirms that `data/published/` exists and that `README.md` is its only visible child. The broader `data/` tree already includes `catalog/`, `processed/`, `proofs/`, `published/`, `quarantine/`, `raw/`, `receipts/`, `registry/`, and `work/`. Deeper `published/` conventions remain **UNKNOWN / NEEDS VERIFICATION** until the checked-out branch is inspected directly.
...
- what still needs explicit verification before it is documented as branch fact
...
| Bundle-specific manifests or pointer files already checked in under `data/published/` | **UNKNOWN / NEEDS VERIFICATION** | Do not imply filenames or contents not directly seen in the public tree |
...
| Shared control | [`../../contracts/README.md`](../../contracts/README.md) | **CONFIRMED** | Publication-facing trust objects and machine-readable shapes should stay explicit and reviewable |
...
| Shared control | [`../../.github/workflows/README.md`](../../.github/workflows/README.md) | **CONFIRMED README surface / UNKNOWN workflow depth** | Workflow documentation exists, but current public `main` shows the workflow lane as README-only |
...
| Adjacent validator | [`../../tools/validators/README.md`](../../tools/validators/README.md) | **CONFIRMED** | Validators may check linkage and release-facing conditions without becoming publication ownership |
| Adjacent validator | [`../../tools/validators/promotion_gate/README.md`](../../tools/validators/promotion_gate/README.md) | **INFERRED / PROPOSED alignment** | Promotion is the clearest release-facing membrane immediately upstream of this lane |
...
| `tests/` currently shows multiple verification families | Repo-level verification families exist even though published-surface coverage still needs direct inspection |
...
- [ ] replace placeholder metadata values in the KFM meta block before merge (`doc_id`, `created`, `policy_label`)
