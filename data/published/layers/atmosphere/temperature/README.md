<!-- [KFM_META_BLOCK_V2]
doc_id: kfm://data/published/layers/atmosphere/temperature/readme
title: data/published/layers/atmosphere/temperature README
...
path: data/published/layers/atmosphere/temperature/README.md
related:
  - ../../README.md
  - ../README.md
...
notes:
  - "Directory README for released Atmosphere temperature layer carriers. It replaces an empty file."
  ...
  - "Temperature layer carriers must preserve source roles, observed/estimated/forecast or modeled method, metric family, units, valid time, freshness/stale-state posture, caveats, correction path, and rollback support."
[/KFM_META_BLOCK_V2] -->

<a id="top"></a>

# `data/published/layers/atmosphere/temperature/`

> Published layer sublane for released **Atmosphere temperature map carriers**.
...
![Layer: temperature](https://img.shields.io/badge/layer-temperature-blue)
...
> **Status:** `draft`  
> **Path:** `data/published/layers/atmosphere/temperature/README.md`  
> **Truth posture:** CONFIRMED target path, Atmosphere published README, and sibling Atmosphere layer README files from current repo evidence / PROPOSED layout and naming / NEEDS VERIFICATION for emitted temperature layer files, schemas, validators, release manifests, method records, and governed routes.

> [!WARNING]
> This directory is not release authority, official forecast authority, regulatory determination, raw observation storage, catalog authority, or proof authority. A temperature layer belongs here only after release authority, EvidenceBundle support, validation, policy state, correction path, rollback support, and integrity references exist.

---

## 1. Scope

This lane is for released public-safe Atmosphere temperature layer carriers used by governed map, API, report, export, or UI surfaces.

Examples of valid carrier families, once released:

- public-safe temperature context map layers;
- temperature layer sidecars and manifests;
- public index files that point only to released temperature layers; and
- retired or superseded temperature carriers with correction or rollback references.
...
- [ ] source roles, observed/estimated/forecast or modeled method, metric family, units, and valid-time range are clear;
...
│   └── <release_id>/
├── indexes/
│   └── atmosphere-temperature-layer-index.json
└── retired/
    └── <release_id>/
```
...
```text
atmosphere.published.layer.temperature.<scope>.<release_id>.<short_hash>.<ext>
```
...
Atmosphere temperature layers are public-facing weather and climate-context carriers, not root truth, release authority, official forecasts, regulatory determinations, or raw observations. Keep them citable, source-role-aware, metric-aware, unit-aware, method-aware, valid-time-aware, stale-state-aware, release-linked, correction-ready, and reversible. If release support is incomplete, keep the artifact upstream.
