<!-- [KFM_META_BLOCK_V2]
doc_id: kfm://data/published/layers/atmosphere/ozone/readme
title: data/published/layers/atmosphere/ozone README
...
path: data/published/layers/atmosphere/ozone/README.md
related:
  - ../../README.md
  - ../README.md
...
notes:
  - "Directory README for released Atmosphere ozone layer carriers. It replaces an empty file."
  ...
  - "Ozone layer carriers must preserve pollutant basis, source roles, observation or modeled method, time scope, freshness/stale-state posture, caveats, correction path, and rollback support."
[/KFM_META_BLOCK_V2] -->
...
> Published layer sublane for released **Atmosphere ozone map carriers**.
...
![Layer: ozone](https://img.shields.io/badge/layer-ozone-blue)
...
> **Status:** `draft`  
> **Path:** `data/published/layers/atmosphere/ozone/README.md`  
> **Truth posture:** CONFIRMED target path, Atmosphere published README, and sibling Atmosphere layer README files from current repo evidence / PROPOSED layout and naming / NEEDS VERIFICATION for emitted ozone layer files, schemas, validators, release manifests, ozone method records, and governed routes.

> [!WARNING]
> This directory is not release authority, official AQI authority, medical guidance, emergency guidance, regulatory determination, raw observation feed, catalog authority, or proof authority. An ozone layer belongs here only after release authority, EvidenceBundle support, validation, policy state, correction path, rollback support, and integrity references exist.

---

## 1. Scope

This lane is for released public-safe Atmosphere ozone layer carriers used by governed map, API, report, export, or UI surfaces.

Examples of valid carrier families, once released:

- public-safe ozone context map layers;
- ozone layer sidecars and manifests;
- public index files that point only to released ozone layers; and
- retired or superseded ozone carriers with correction or rollback references.
...
- [ ] source roles, pollutant basis, observation or modeled method, and time range are clear;
...
│   └── <release_id>/
├── indexes/
│   └── atmosphere-ozone-layer-index.json
└── retired/
    └── <release_id>/
```
...
```text
atmosphere.published.layer.ozone.<scope>.<release_id>.<short_hash>.<ext>
```
...
Atmosphere ozone layers are public-facing air-quality context carriers, not root truth, release authority, official AQI authority, medical guidance, emergency guidance, regulatory determinations, or raw observations. Keep them citable, source-role-aware, pollutant-aware, method-aware, time-aware, stale-state-aware, release-linked, correction-ready, and reversible. If release support is incomplete, keep the artifact upstream.
