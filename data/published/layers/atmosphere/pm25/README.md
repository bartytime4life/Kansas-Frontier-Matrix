<!-- [KFM_META_BLOCK_V2]
doc_id: kfm://data/published/layers/atmosphere/pm25/readme
title: data/published/layers/atmosphere/pm25 README
...
path: data/published/layers/atmosphere/pm25/README.md
related:
  - ../../README.md
  - ../README.md
...
notes:
  - "Directory README for released Atmosphere PM2.5 layer carriers. It replaces an empty file."
  ...
  - "PM2.5 layer carriers must preserve pollutant basis, source roles, observation or modeled method, time scope, freshness/stale-state posture, caveats, correction path, and rollback support."
[/KFM_META_BLOCK_V2] -->
...
> Published layer sublane for released **Atmosphere PM2.5 map carriers**.
...
> **Status:** `draft`  
> **Path:** `data/published/layers/atmosphere/pm25/README.md`  
> **Truth posture:** CONFIRMED target path, Atmosphere published README, and sibling Atmosphere layer README files from current repo evidence / PROPOSED layout and naming / NEEDS VERIFICATION for emitted PM2.5 layer files, schemas, validators, release manifests, PM2.5 method records, and governed routes.

> [!WARNING]
> This directory is not release authority, official AQI authority, medical guidance, emergency guidance, regulatory determination, raw observation feed, catalog authority, or proof authority. A PM2.5 layer belongs here only after release authority, EvidenceBundle support, validation, policy state, correction path, rollback support, and integrity references exist.

---

## 1. Scope

This lane is for released public-safe Atmosphere PM2.5 layer carriers used by governed map, API, report, export, or UI surfaces.

Examples of valid carrier families, once released:

- public-safe PM2.5 context map layers;
- PM2.5 layer sidecars and manifests;
- public index files that point only to released PM2.5 layers; and
- retired or superseded PM2.5 carriers with correction or rollback references.
...
| `data/published/layers/atmosphere/ozone/` | Sibling ozone layer lane. |
...
│   └── <release_id>/
├── indexes/
│   └── atmosphere-pm25-layer-index.json
└── retired/
    └── <release_id>/
```
...
```text
atmosphere.published.layer.pm25.<scope>.<release_id>.<short_hash>.<ext>
```
...
Atmosphere PM2.5 layers are public-facing air-quality context carriers, not root truth, release authority, official AQI authority, medical guidance, emergency guidance, regulatory determinations, or raw observations. Keep them citable, source-role-aware, pollutant-aware, averaging-period-aware, method-aware, time-aware, stale-state-aware, release-linked, correction-ready, and reversible. If release support is incomplete, keep the artifact upstream.
