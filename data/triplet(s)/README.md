...
title: data/triplet(s)/ — Literal-Path Compatibility Pointer
...
owners: NEEDS VERIFICATION — data, graph, migration, and docs stewards
updated: 2026-07-24
...
current_path: data/triplet(s)/README.md
...
evidence_snapshot:
  ...
  prior_blob: 13a114c1eb7559c09ec9b51a41285dbc1f61fbce
...
# `data/triplet(s)/` — Literal-Path Compatibility Pointer

> **One-line purpose.** Preserve the literal parentheses path as a documented pointer to canonical `data/triplets/` while denying all new graph, triplet, catalog, proof, receipt, release, and public payloads.
...
Preserve the literal parentheses path as a documented pointer to canonical `data/triplets/` while denying all new graph, triplet, catalog, proof, receipt, release, and public payloads.
...
**Compatibility — transitional; LITERAL PATH responsibility.**
...
| Path | `data/triplet(s)/` |
| Version | `v0.2.0` |
| Prior blob | `13a114c1eb7559c09ec9b51a41285dbc1f61fbce` |
...
- this README, bounded inventory, redirects/deprecation notes, and migration mappings;
...
|---|---|
| all lifecycle, trust, schema, policy, code, test, and public payloads | their canonical responsibility roots |
| sensitive joins or claim-bearing graph relationships | canonical governed lanes after evidence/policy review |
...
Recursive inventory, inbound-reference search, migration decision, and rollback evidence.
...
A compatibility status and redirect/tombstone plan only.
...
Accountable ownership remains **NEEDS VERIFICATION**. Changes should include data, graph, migration, and docs stewards as applicable. Payload, source activation, rights/sensitivity, migration, public-serving, correction, or rollback changes require corresponding specialist and independent review. CODEOWNERS routing is not approval evidence.
...
The canonical lane is [`../triplets/`](../triplets/README.md). Missing inventory or consumer evidence keeps this path compatibility-only and blocks deletion.
