...
This folder contains **static chart renders** (images/SVGs) shipped with the web app—used in:
- 🧭 **Story Nodes / Focus Mode** (narrative + map context)
- 🧩 **UI panels** (e.g., feature detail panels, onboarding, docs)
- 🧪 **Reference visuals** for tests, snapshots, and QA

> KFM rule of thumb: **If it shows up in the UI, it must be traceable to cataloged sources.** ✅
...
- “Mystery charts” with unknown data sources
...
```json
{
  "title": "Kansas Drought Index (1895–2025)",
...
**Minimum required fields** (don’t skip these):
- `title`, `chart_kind`
- `data_sources[]` (at least one)
- `pipeline.generator` (or a clear “manual” explanation)
...
Use **kebab-case** and encode the “shape” of the asset in the filename.
...
Tips:
- Use `ks-` prefix when it’s statewide
...
- [ ] Labels don’t overlap; legends don’t cover data
...
General:
- Keep `export/` assets as the “final form” (optimized, production-ready).
- Keep edits in `raw/` when source files are small enough to version cleanly.

---

## 🧠 How charts are used in the app

Typical integration points:
- `web/components/` → reusable chart components
- `web/story_nodes/` → Markdown narratives that embed chart assets
- Feature details (popovers/panels) → charts that summarize a clicked dataset
