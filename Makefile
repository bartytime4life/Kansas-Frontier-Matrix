...
help:
	...
	@echo "  arch-sites            Run archaeology sites pipeline (fetch→unpack→process→stac→validate→render)"
	@echo "  arch-sites-validate   Validate archaeology sites STAC + render viewer config"
	@echo "  arch-sites-render     Render viewer config for archaeology sites (includes thumbnails)"
	@echo ""
	@echo "Data:"
...
site-config: | $(WEB)
	@if [ "$(HAVE_KGT)" = "yes" ]; then \
	  if [ -f "src/kansas_geo_timeline/templates/app.config.json.j2" ]; then \
	    ...
	    echo "[site-config] No Jinja template; rendering directly from STAC items"; \
	    kgt render-config --stac "$(STAC)/items" --output "$(WEB)/app.config.json" --pretty; \
	  fi; \
	else \
...
# -------- Convenience: Archaeology Sites --------
arch-sites:
	@bash scripts/collections/archaeology_sites.sh fetch || true
	@bash scripts/collections/archaeology_sites.sh unpack || true
	@bash scripts/collections/archaeology_sites.sh process || true
	@bash scripts/collections/archaeology_sites.sh stac || true
	@bash scripts/collections/archaeology_sites.sh validate || true
	@bash scripts/collections/archaeology_sites.sh render || true
	@echo "✓ archaeology sites pipeline complete"

arch-sites-validate:
	@bash scripts/collections/archaeology_sites.sh validate || true
	@bash scripts/collections/archaeology_sites.sh render  || true
	@echo "✓ archaeology sites validated + rendered"

arch-sites-render:
	@bash scripts/collections/archaeology_sites.sh render
	@echo "✓ archaeology sites rendered"
