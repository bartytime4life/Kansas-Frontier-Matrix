...
tags: [kfm, catalog, publication, published, release, compatibility-root, redirect, data-published, release-plane, non-authoritative, drift-fence]
notes:
  - "Root-level catalog/publication/ is treated as a compatibility/redirect fence, not canonical publication authority."
  - "Released artifacts belong under data/published/ after governed release."
  - "Release decisions, release manifests, rollback cards, corrections, and signatures belong under release/, not under catalog/publication/."
  - "Do not add publication artifacts, release records, receipts, proofs, source registry rows, or catalog records here without an ADR/migration note."
  - "Specific current contents, producers, migration status, release/publication schema maturity, and CI enforcement remain NEEDS VERIFICATION."
[/KFM_META_BLOCK_V2] -->
...
**Compatibility / redirect fence for legacy or accidental root-level publication placement. Released artifacts belong under `data/published/`, while release decisions and release-state records belong under `release/`.**
...
This README does not prove that any publication material currently exists here, that a migration has been completed, that publication schemas are implemented, or that CI currently blocks writes to this path.
...
`catalog/publication/` has **no canonical publication, release, or catalog authority**. It may hold only README guidance, migration notes, drift logs, or temporary redirect markers while publication material is moved into its proper lifecycle home.
...
Do not cite or depend on root-level publication files as canonical published artifacts or release records. First confirm source, provenance, rights, sensitivity, schema validity, lifecycle state, receipts, proofs, release state, rollback path, and correction path.
...
|---|---|
| Released artifacts, published map/docs/data bundles | `data/published/` |
...
9. Leave root-level `catalog/publication/` as a redirect/fence unless an ADR explicitly says otherwise.
...
- CI or review checks flag root-level `catalog/publication/` writes;
...
The previous README was empty. This replacement adds a publication redirect and anti-parallel-authority contract without claiming publication files, release decision files, migration work, CI enforcement, producer workflows, publication schema maturity, or ADR disposition are implemented.

</details>

## Status summary

`catalog/publication/` is a root-level compatibility redirect and publication drift fence. It is not the canonical publication, release decision, or catalog home.
