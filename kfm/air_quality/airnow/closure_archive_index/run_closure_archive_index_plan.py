...
from .constants import REQUIRED_L27,FORBIDDEN_RECEIPT_TRUE
...
def run_closure_archive_index_plan(manifest_path,out_dir,created_at):
 ...
 e.extend(validate_manifest(m)); d=m.get('layer27_inputs',{})
 for k in sorted(REQUIRED_L27):
  p=d.get(k)
  if not p: e.append(f"MISSING_REQUIRED_INPUT:layer27_inputs:{k}"); continue
  if not validate_path_safe(p): e.append(f"UNSAFE_PATH:layer27_inputs:{k}"); continue
  if not Path(p).exists(): e.append(f"MISSING_INPUT_FILE:layer27_inputs:{k}")
 r=make_receipt(m.get('manifest_id'),created_at,e)
 if e: _wj(out/'closure_archive_index_receipt.json',r); return r
 ar=loadj(d['preservation_closure_audit_receipt_json'])
 if ar.get('object_type')!='AirNowPreservationClosureAuditReceipt': e.append('INVALID_LAYER27_AUDIT_RECEIPT_OBJECT_TYPE')
 for f in FORBIDDEN_RECEIPT_TRUE:
  if ar.get(f) is True: e.append(f'L27_RECEIPT_FORBIDDEN:{f}')
 if e: r=make_receipt(m.get('manifest_id'),created_at,e); _wj(out/'closure_archive_index_receipt.json',r); return r
 inv=[]
 for k,v in sorted(d.items()):
  p=Path(v); inv.append({"record_id":sid('layer28inv',[m.get('manifest_id'),k]),"input_key":k,"path":v,"sha256":sha256_path(p),"byte_size":p.stat().st_size,"created_at":created_at})
 ...
 for z in hc: h.append({"hash_id":sid('hashchain',[z]),"sha256":z,"source_record_id":"layer27_hash_chain"})
 ...
 (out/'closure_archive_index_report.md').write_text('# AirNow Layer 28 Closure Archive Index Report\n\nReferences: docs.airnowapi.org, docs.airnowapi.org/webservices, docs.airnowapi.org/faq, docs.airnowapi.org/docs/AirNowAPIFactSheet.pdf, docs.airnowapi.org/docs/HourlyAQObsFactSheet.pdf, docs.airnowapi.org/docs/HourlyDataFactSheet.pdf, docs.airnowapi.org/docs/DailyDataFactSheet.pdf, www.airnowapi.org/docs/MonitoringSiteFactSheet.pdf, airnow.gov/about-the-data, docs.airnowapi.org/docs/DataUseGuidelines.pdf, epa.gov/outdoor-air-quality-data/download-daily-data.\n',encoding='utf-8')
