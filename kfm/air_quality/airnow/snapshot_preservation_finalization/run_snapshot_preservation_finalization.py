...
def _deny(m,out,created_at,errs):
 ...
 wj(out/'snapshot_preservation_finalization_receipt.json',r);return r
...
def run_snapshot_preservation_finalization(manifest_path,out_dir,created_at):
 ...
 assess=loadj(inp['snapshot_preservation_review_assessment_matrix']); sat=loadj(inp['snapshot_preservation_item_satisfaction_matrix']); val=loadj(inp['snapshot_preservation_review_validation_results']); pol=loadj(inp['snapshot_preservation_review_policy_attestation']); cav=loadj(inp['snapshot_preservation_review_caveat_register']); ready=loadj(inp['snapshot_preservation_review_readiness_summary'])
 ...
 wj(out/'snapshot_preservation_acceptance_consolidation.json',{"object_type":"AirNowSnapshotPreservationAcceptanceConsolidation","schema_version":"v1","records":ac,"created_at":created_at}); wjl(out/'snapshot_preservation_acceptance_consolidation.jsonl',sorted(ac,key=lambda x:json.dumps(x,sort_keys=True)))
 wj(out/'snapshot_preservation_blocker_consolidation.json',{"object_type":"AirNowSnapshotPreservationBlockerConsolidation","schema_version":"v1","records":bl,"created_at":created_at}); wjl(out/'snapshot_preservation_blocker_consolidation.jsonl',sorted(bl,key=lambda x:json.dumps(x,sort_keys=True)))
 nonexec={"object_type":"AirNowSnapshotPreservationNonExecutionAttestation","schema_version":"v1","commands_executed":False,"finalization_actions_executed":False,"preservation_actions_executed":False,"created_at":created_at}
 ...
 wj(out/'snapshot_preservation_readiness_final_summary.json',{"object_type":"AirNowSnapshotPreservationReadinessFinalSummary","schema_version":"v1","readiness_status":ready.get('readiness_status','UNKNOWN'),"candidate_outcome":outcome,"created_at":created_at})
