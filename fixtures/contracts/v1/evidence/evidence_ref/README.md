...
related: [valid/README.md, valid/valid_1.json, invalid/README.md, invalid/invalid_1.json, invalid/invalid_1.expected_error.txt, ../../../../../schemas/contracts/v1/evidence/evidence_ref.schema.json, ../../../../../contracts/evidence/evidence_ref.md, ../../../../../tools/validators/validate_evidence_ref.py, ../../../../../tests/schemas/test_common_contracts.py, ../../../../../tests/schemas/test_evidence_ref_validator.py, ../../../../../docs/doctrine/directory-rules.md]
...
notes: ["This README replaces a blank file at `fixtures/contracts/v1/evidence/evidence_ref/README.md`.", "This directory is the schema-declared fixture root for `evidence_ref`.", "Fixtures are schema test inputs only; schema shape, contract meaning, validator implementation, and policy rules stay in their owning roots.", "The current fixture family has two valid cases and three invalid cases.", "The schema-declared validator exists, is aggregate-wired, and has focused valid/missing-ref CLI polarity tests; it does not resolve evidence or establish bundle closure."]
[/KFM_META_BLOCK_V2] -->
...
|---|---|---|---|
| [`valid/`](valid/README.md) | `valid_1.json`, `valid_2.json` | Minimal measurement ref and dataset ref with optional `bundle_ref`. | CONFIRMED |
| [`invalid/`](invalid/README.md) | `invalid_1.json` through `invalid_3.json` | Missing required `ref`, extra property, and unsupported `kind`. | CONFIRMED |
| [`invalid/`](invalid/README.md) | matching `*.expected_error.txt` sidecars | Required, additional-properties, and enum/pattern matchers. | CONFIRMED |
...
| Schema test harness | `tests/schemas/test_common_contracts.py` | CONFIRMED |
| Dedicated validator wrapper | `tools/validators/validate_evidence_ref.py` | CONFIRMED / SHAPE ONLY |
| Focused CLI polarity test | `tests/schemas/test_evidence_ref_validator.py` | CONFIRMED |
...
| Valid lane | CONFIRMED | `valid/README.md` documents two valid fixtures. |
| Invalid lane | CONFIRMED | `invalid/README.md` documents three invalid fixtures and their expected-error sidecars. |
...
| Declared validator wrapper | CONFIRMED / SHAPE ONLY | The wrapper targets the declared schema and fixture root and is aggregate-wired. |
| Focused validator test | CONFIRMED | Valid and missing-`ref` CLI exit polarity is covered under `tests/schemas/`. |
...
| [`../../../../../tools/validators/validate_evidence_ref.py`](../../../../../tools/validators/validate_evidence_ref.py) | CONFIRMED | Dedicated wrapper delegates to the shared JSON Schema runner. | Does not resolve refs or establish bundle closure. |
| [`../../../../../tests/schemas/test_common_contracts.py`](../../../../../tests/schemas/test_common_contracts.py) | CONFIRMED / TEST-COVERED | Fixture discovery and valid/invalid fixture behavior. | Shape behavior only; not resolver or policy proof. |
| [`../../../../../tests/schemas/test_evidence_ref_validator.py`](../../../../../tests/schemas/test_evidence_ref_validator.py) | CONFIRMED | Tests valid and missing-`ref` CLI exit polarity. | Bounded shape behavior only. |
