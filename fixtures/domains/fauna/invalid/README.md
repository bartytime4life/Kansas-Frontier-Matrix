...
Status: draft fixture lane with five accepted bounded negative fixtures.
...
| `encoded_location_clue.json` | Location alias and free-form URL/coordinate-shaped clues fail closed without containing a real place, taxon, URL destination, or usable coordinate. |
...
- small synthetic invalid `*.json` examples;
...
- negative cases for identity, source role, evidence, time, sensitivity, rights, geoprivacy, or release boundaries; and
...
- connector, pipeline, validator, package, or schema implementation code; or
...
- Use out-of-range or non-numeric placeholders when a negative case must resemble a coordinate; never use a real or plausible protected location.
