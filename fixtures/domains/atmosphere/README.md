...
Status: draft / fixture index / bounded synthetic precipitation, knowledge-character, low-cost-sensor calibration, and observed-versus-modeled profiles executable.
...
| `observed_modeled_separation/` | Closed AirObservation and ForecastContext profile inputs with explicit source/evidence posture, time, units, model-run identity, DERIVED_FROM lineage, uncertainty, abstention, and anti-collapse denials. | CONFIRMED bounded executable input with three positive/abstaining and twelve exact-negative fixtures; no live source, model, policy, release, or life-safety authority. |
...
- Payload inventory: the bounded `public_safe_precipitation/` pair, `knowledge_character/` anti-collapse corpus, `low_cost_sensor_calibration/` corpus, and `observed_modeled_separation/` corpus are CONFIRMED; broader payload inventory remains NEEDS VERIFICATION.
- Tests and validators: the standard-library precipitation, knowledge-character, low-cost-sensor calibration, and observed-versus-modeled fixture suites are CONFIRMED executable; broader Atmosphere behavior remains NEEDS VERIFICATION.
